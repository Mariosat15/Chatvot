...
| **R26** | Admin-app finalization pays no Game Master earnings | Critical | **ALREADY OCCURRED** | X1 or X5 - **CLOSED 5 Sep 2026**, not retroactive; **historical compensation leave-it 18 Sep 2026** (test DB until owner says real production) |
...
| **R59** | **Every dialog that asked to be wide rendered at 32rem** - an unprefixed `max-w-*` never displaces the primitive's `sm:max-w-lg` | Medium | **ALREADY OCCURRED, 31 dialogs** | **CLOSED 18 Sep 2026** — admin `DialogContent` gained a `size` prop (ported from the main app); remaining admin + player call sites converted; games surface kept `DIALOG_WIDTH_*` tokens |
| **R60** | **A native `<select>` on a translucent background renders white on white** - the *browser* paints the list, taking the background from the element and letting options inherit `color`. **Two instances found** | Low | **ALREADY OCCURRED, 2 controls; both CLOSED** | **Genre picker CLOSED 9 Sep 2026**; **MessagingSection Transfer-to CLOSED 18 Sep 2026** (`bg-gray-800`; 15 pre-existing lint warnings cleared so the file can commit). Exception list emptied and pinned; probe re-aimed at reintroducing the translucent class |
...
| **R96** | **A player who only plays games can earn five badges out of 128.** `RARITY_MIN_REQUIREMENTS` in `badge-evaluation.service.ts` refuses any badge below a **trades** floor and a **completed-competitions** floor (5/0 common, 25/1 rare, 50/3 epic, 100/5 legendary), and `TRADE_EXEMPT_TYPES` lets only a handful past. `completedCompetitionsWithTrades` counts **trading** competitions by construction. So the gate is not merely trading-flavoured, it is arithmetically closed: **122 of 128 badges are unreachable without trading**, and the catalogue contains **zero** game badges to begin with | Medium | **LATENT in effect and LIVE in mechanism** - no games-only player has completed enough to test it, so nobody has yet been refused, but every branch is in production today and refuses correctly. **Nothing to backfill**: no badge was wrongly awarded or withheld, and the remedy is a gate change plus new content, not a data repair | **R96a CLOSED 16 Sep 2026; R96b AUTHORING CLOSED 17 Sep 2026 (content still owner).** Gate classifies by condition TYPE (registry-derived `CROSS_GAME_CONDITION_TYPES` / `TRADE_EXEMPT_TYPES`): trading floors on trading-typed conditions; cross-game types zero trade floors and count `completedCompetitions`; `game_*` conditions read `UserGameStats`; unknown fail closed. Both evaluators byte-identical (R100). **Authoring surface built** (registry, Games category, `gameTypes` form/AI/wizard/balance, add-only wizard, ladder CRUD). **Catalogue still holds zero game-specific badge rows** until owner naming/thresholds |
| **R99** | **The badge catalogue in the repository is not the one that gets installed.** `lib/constants/badges.ts` holds 128 badges; **`data/defaults/badges.json` holds 134**, is written by the admin app, is read by `whitelabel-defaults-reader.ts`, and is **preferred over the constants on a fresh seed** - so the file a developer reads is not the file an installation gets, and the two disagree by six badges and by their tuning (115 of the 134 declare `minTrades > 0`). Worse, `badge-config-seed.service.ts`'s sync used to update `condition` on existing rows when the source differed, so **a seed run silently overwrote values an operator tuned in the database** *(**overwrite CLOSED 16 Sep 2026.** Sync is now add-only in both apps; intentional reset remains `resetBadgeAndXPConfigs` / `seed-badges-xp`. Catalogue divergence (128 vs 134) is **documented and pinned**, not merged — aligning them is a separate pass. `__tests__/services/badge-catalogue-r99.test.ts`, 5 probes.)* | Medium | **Divergence LIVE as documentation debt; overwrite CLOSED.** Nothing backfilled | **OVERWRITE CLOSED 16 Sep 2026; catalogue alignment still OPEN** (JSON remains install-canonical; constants remain the missing-id insert source). Owner may later choose to regenerate `BADGES` from JSON |
...
| **R101** | **Ninety-nine admin API route files have no authorization check of any kind, and fifty-eight of them write.** *(**R101a–R101ad CLOSED 18 Sep 2026.** Inventory after R101ad: **0 no-check / 0 hand-verified / 0 helper debt / 331 section-granted** (+ 10 public-by-design + 8 helper-by-design carve-outs). R101ad section-granted `gamemaster/fix-purchases` and carved the remaining eight intentional helpers into `HELPER_BY_DESIGN`. A document treating R101 as still open on helper debt is stale; one quoting **9 helper** or **30 helper** or **41 helper** or **78 / 58 / 55 / 51 / 44** still-unguarded is quoting a figure the next closure moved - **say which**.)* Counted over all admin API route files with comments stripped: see the detail section for the full a–ad closure history | **Critical** | **CLOSED for authorization debt.** Public-by-design and helper-by-design carve-outs remain intentional and pinned; absence of evidence of past unguarded calls is not reassurance | **R101a–R101ad CLOSED 18 Sep 2026.** Inventory: **0 / 0 / 0 debt / 331**. **ELEVENTH instance of this class** — every previous instance fixed the routes it happened to be looking at; the directory inventory (R101d) is what makes the rest countable |
...
| R21 | Dashboard mega-action split | Medium | **CLOSED 18 Sep 2026.** Extract + trade-fetch gate + hide trading chrome; games-only players skip TradeHistory / positions / forex | Fixed in its own commit |
...
| R35 | **A provider's `replayUrl` can leak a live contest's content.** Section 8 requires a `replayUrl` on every result and defined nothing about it - not what it serves, who may open it, whether it authenticates, or how long it lives. A contest's boards are **identical for every player** by design, because that is what makes the ranking a fair comparison. So a replay that shows *the puzzle* rather than *the player's own attempt* lets a player who has finished read the content of a contest still being played | Medium | **CLOSED 24 Sep 2026** (contract + ChartVolt Games). Latent for third parties until they comply; on our side `GET /replay/:providerRoundId?t=…` now serves an attempt summary only, after the round is terminal (ranked: also after `expiresAt`). Requirements HTML **v1.19** / A14 / `01` s5.0. **Nothing was backfilled** — no production provider ever existed | Spec: player's own attempt, token-scoped, not before terminal (+ ranked window). ChartVolt Games implements `serveReplay` + `verifyReplayToken`. Third-party compliance remains their duty under the issued contract |
...
### R1 - Money paths refactored without tests first - CLOSED (residual 18 Sep 2026)
...
Re-verified 1 September 2026, with two corrections still worth carrying — and the first
was closed later:

- The finalize-time safeguard that caps the pool to `currentParticipants x entryFee`
  **did not mask this**. It only fired when the pool is too *high*, so an under-counted
  pool was under-distributed with no correction and no log line. **CLOSED 18 September
  2026 (R1 residual):** `lib/services/settlement/prize-pool-integrity.ts` (mirrored)
  raises an under-counted pool to `participantCount × entryFee` as well as capping an
  over-count, logs loudly both ways, and persists the corrected value. Wired into both
  apps' `competition-end.actions.ts` and both copies of `provider-settlement.service.ts`.
  `__tests__/services/prize-pool-integrity.test.ts` + under-count case in
  `provider-settlement.test.ts`. Free contests (`entryFee` 0) are left alone. The
  safeguard remains a **safety net**, not a licence for writers to skip the `$inc` —
  entry still owns the increment in the same transaction that takes the fee.
...
earnings per contest. **No backfill was written.** **Historical compensation CLOSED as
leave-it, 18 Sep 2026** (same decision as R43): the owner says the live database is **for
testing only** - the site is live but **not real users** - so do not write a compensate
script, and do not reopen this as outstanding, until they explicitly say the platform is
**real on production**. A script that credits wallets from inferred history remains an
unreviewed money writer either way.
...
script was written.** **Historical compensation CLOSED as leave-it, 18 Sep 2026**: the owner
says the live database is **for testing only** - live but **not real users** - so leave the
balances as they are until they say the platform is **real on production**. The query to find
affected contests remains the deliverable *if* that day comes; crediting wallets from inferred
history is still an unreviewed money writer and must not be built until then.
...
exemption goes red instead of silently re-permitting the defect. Same device as the former
`app/api/dashboard/competitions/route.ts` comparator exception (**closed 18 Sep 2026**, `13`
s5.1c).
...
### R93 - The exchange rate no operator can reach - **CLOSED 18 September 2026**

**What it was.** `CreditConversionSettings.eurToCreditsRate` is the number the whole platform
prices money against. Exactly one component can edit it —
`CreditConversionSection.tsx` — and it **was imported by nothing**. Found by X6.5 A6's
stale-navigation audit: a path naming no screen is either wrong or unreachable.

**It is not the same defect as R110.** R110 was an unmounted *provider* (values never arrive).
R93 was an unmounted *editor* (values arrive and cannot be changed).

**Harm.** Nothing miscomputed; **nothing backfilled**. Cost was reachability only.

**Fix.** Owner scope answered: mount under **Settings → Currency**, reusing the existing
`currency` grant — the API already calls `guardSection("currency")`, so inventing a new
`ADMIN_SECTIONS` id would have been a second grant decision for no gain. Stacked below
`CurrencySettingsSection` in `AdminDashboard.tsx`. Copy on Currency, Fees, and the AI
knowledge base updated. Pinned by `__tests__/admin/credit-conversion-mount.test.ts`
(import + positional render inside the currency case). **Never verified by eye.**
...
it rather than quietly re-permitting the defect. Same device as the former
`app/api/dashboard/competitions/route.ts` comparator exception (**closed 18 Sep 2026**, `13`
s5.1c — canary flipped rather than deleted).
...
**What it is now.** `closePosition` refuses `cancelled` — and since **18 September 2026**
also `completed` and `finalizing` (X6.5 leftover from `12` s3.2a). Closing after the
leaderboard snapshot, or while ranking is mid-flight, moved capital settlement already
used, with a success toast and no error. Pinned by
`__tests__/services/close-position-terminal-status.test.ts`. The dead branch in
...
**AMENDED 18 September 2026 — player twin CLOSED.** `lib/services/games/player-game-performance.service.ts`
(main app) + `components/dashboard/PlayerGamePerformancePanel.tsx` on the dashboard
Performance tab, fetched from `getComprehensiveDashboardData`. Games render **above** a
labelled "Trading performance" block; empty games list returns null so pure traders are
unchanged. Trading is never gated away. `__tests__/dashboard/player-game-performance-r64.test.ts`.
Never verified by eye (sign-in). The wider dashboard (GettingStartedCard, mega-action, public
arena board) remains trading-shaped — separate from this Performance-tab twin.
...
#### There were TWO instances; BOTH are closed as of 18 Sep 2026
...
picker in `MessagingSection.tsx`** carried `bg-white/5 text-white` on a native select whose
options are employee names, so **every name in it was white on white too**. Found by grepping for
...
**It sat as a named exception until 18 Sep 2026**, and the reason was mechanical rather than a
judgement about priority. The fix is one token - `bg-white/5` to `bg-gray-800` - but that file
carried about fifteen pre-existing lint warnings and this repository's pre-commit hook lints
staged files at `--max-warnings=0`. Touching it therefore required clearing that debt in the
same commit. **CLOSED 18 Sep 2026**: the background is opaque, the 15 warnings are gone, the
`KNOWN_UNFIXED` exception list is empty and pinned, and the probe that used to delist the
exception now reintroduces `bg-white/5` on the Transfer-to select itself. A document saying the
MessagingSection picker is still an exception is correct as history and stale as a present fact
— **say which**.

#### Severity, stated in both directions

**Low, and both controls closed.** It was an operator-facing legibility fault: no money moved, no
...
likely to be stale on any title touched before 9 Sep 2026, and that field is the grouping key
tasks 21-24 will join on. The Transfer-to half was the same fault one screen along.
...
### R59 - A width class that is present, correct-looking and inert - **CLOSED platform-wide, 18 September 2026** (games surface 9 Sep)
...
**The other 29 admin dialogs were not swept on 9 Sep**, recorded rather than quietly scoped —
each is a screen getting visibly wider, and doing it in the same commit as the reported fix
would have destroyed the only evidence the reported fix was safe.

**CLOSED for the remainder on 18 Sep 2026 (owner decision to sweep).** Admin
`components/ui/dialog.tsx` gained a `size` prop (`sm | default | lg | xl | full`), all
variants `sm:`-prefixed, ported from the main app's mechanism. Unprefixed `max-w-*` call
sites across admin and the two player dialogs (`IndicatorSelector`, `TradingArsenalSection`)
were converted to `size=`. Games surface keeps `DIALOG_WIDTH_*`. Guard extended:
`__tests__/admin/dialog-widths.test.ts` now asserts the platform-wide absence of inert
unprefixed max-w on admin `DialogContent`. **Never verified by eye.**
...
### R101 - Ninety-nine admin routes with no authorization, and fifty-eight of them write - **R101a–R101ad CLOSED 18 Sep 2026**
...
**R101e CLOSED 16 September 2026.** The whole of `visitors/` - **4 route files, 6 handlers** -
grant `visitors` from `VisitorAnalyticsSection`. Went first among the remaining no-check writers
because `block` POST/DELETE and `clear` DELETE create and destroy records anonymously. 190 tests
in the privileged suite, 4 new probes each red on exactly one failure. Inventory after:
**51 no-check / 3 hand-verified / 189 helper / 94 section-granted**.

**R101f CLOSED 16 September 2026.** The whole of `landing-pages/` - **7 route files, 10 handlers** -
grant `landing-pages` from `LandingPagesSection`. Went next because `analytics/clear` DELETE
zeroes every visit counter and deletes every visit record anonymously, and create / update /
delete / AI write content. 214 tests in the privileged suite, 4 new probes each red on exactly
one failure. Inventory after:
**44 no-check / 3 hand-verified / 189 helper / 101 section-granted**.

**R101g CLOSED 16 September 2026.** The whole of `market-data/` - **6 route files, 11 handlers** -
grant `market-data` from `MarketDataSection`. Went next because `cleanup`, `seed-history` and
`gap-fill` rewrite candle history anonymously, and settings / stats / download-history went with
them so the folder cannot grow a seventh unguarded file. Privileged suite grew; 4 new probes
under `=== R101g probes ===` each red on exactly one failure. Inventory after:
**38 no-check / 3 hand-verified / 189 helper / 107 section-granted**.

**R101h CLOSED 16 September 2026.** The whole of `symbols/` - **3 route files, 7 handlers** -
grant `symbols` from `SymbolsSection`. Went next because sync and the create/update/delete
writers rewrite the live symbol catalogue anonymously, and the three GETs went with them so
the folder cannot grow a fourth unguarded file. Privileged suite grew; 4 new probes under
...
**R101j CLOSED 16 September 2026.** The whole of `pages/` - **5 route files, 8 handlers** -
grant `site-pages` from `SitePagesSection`. CMS writers and AI generators rewrite public
...
**R101n CLOSED 16 September 2026.** The last **3 hand-verified-token** route files
(**5 handlers**) - grants from calling screens: `messaging` (`employees/availability`,
MessagingSection), `trading-risk` (`trading-risk-settings`, TradingRiskSection; and
`trigger-margin-check`, which has no UI caller and shares the risk grant by domain).
5 new probes each red on exactly one failure. Inventory after:
**0 no-check / 0 hand-verified / 180 helper / 157 section-granted**.
The hand-verified frozen list was **flipped empty**, not deleted.

**R101o CLOSED 16 September 2026.** First helper-but-no-grant money slice: **11 files,
15 handlers**. Grants from calling screens: `financial` (`admin-funds`, `vat`,
`vendor-payments`, `atlas/refund`, `atlas/refund/clawback`, `atlas/refunds/pending` —
FinancialDashboard and TransactionDetailDialog) and `pending-withdrawals`
(`withdrawals/` including approve/reject/complete — PendingWithdrawalsSection). Audit
logging on Atlas refund/clawback now uses the section-granted admin rather than a second
`getAdminSession()` that could silently skip the log. Withdrawals PUT refuses **before**
opening a Mongo session. **Deferred in this slice:** `admin-bank-accounts` (dual callers
on company-settings and pending-withdrawals — grant decision, not just a guard). 6 new
probes each red on exactly one failure. Inventory after:
**0 no-check / 0 hand-verified / 169 helper / 168 section-granted**.

**R101p CLOSED 16 September 2026.** Second money helper slice: **10 files, 15 handlers**.
Grants from calling screens: `financial` (`financial-dashboard`, `financial-analytics`,
`chargebacks` list), `users` (`chargebacks/lookup` — ChargebackCreateDialog),
`failed-deposits` (`deposits/failed`, `deposits/[id]/manual-complete`), `fees`
...
**R101s CLOSED 16 September 2026.** Employees helpers + reset: **5 files, 13 handlers**.
`EmployeesSection` → `employees` on `[id]`, `role-templates`, `upgrade-super-admin` and
`POST /employees`; `GET /employees` is dual-caller with `UsersSection` /
`TransferCustomerDialog` → `guardAnySection(["employees", "users"])`.
`DatabaseSection` → `database` on `admin/reset-all-employees`. `availability` stays
...
**R101v CLOSED 16 September 2026.** Tutorials helpers: **7 files, 9 handlers**.
`TutorialsSection` → `guardSection("tutorials")`. The two `tutorials/videos/*` asset
streamers stay **public-by-design**; closed-folder leak checks now exclude
`PUBLIC_BY_DESIGN` so those routes do not count as regressions. 6 new probes each red on
...
**R101x CLOSED 16 September 2026.** Announcements helpers: **5 files, 8 handlers**.
`SystemAnnouncementsSection` → `guardSection("system-announcements")` on the whole tree
(list/create, `[id]` GET/DELETE, ai-generate, templates GET/POST, templates/`[id]` DELETE).
**`system-announcements` was a menu id only until this slice** — added to `ADMIN_SECTIONS`
(add-only) so the grant can be issued; same shape as `vendors` / `journey-map`. Writers
attribute `createdBy` / `createdByEmail` from `guard.admin.id` / `email` and no longer call
`getAdminSession` after auth (R101b shape). 6 new probes each red on exactly one failure —
closed-folder / inventory canaries aimed at single-handler `ai-generate`. Inventory after:
**0 no-check / 0 hand-verified / 84 helper / 253 section-granted**.

**R101y CLOSED 16 September 2026.** Settings cluster helpers: **17 files, 27 handlers**
across seven top-level folders (`settings/`, `challenge-settings/`, `company-settings/`,
`hero-settings/`, `kyc-settings/`, `redis-settings/`, `mdb-cluster-settings/`).
`CurrencySettingsSection` → `guardSection("currency")` on `/api/settings` (AppSettingsProvider
is unmounted, so the generic `settings` section is never the right grant).
`CompetitionCreatorForm` → `guardSection("competitions")` on `settings/trading-risk`.
`CompanyDetailsSection` / hero → `guardAnySection(["company", "hero-page"])` on
`company-settings`. Hero / branding → `guardAnySection(["hero-page", "branding"])` on
`hero-settings` (+ upload). KYC → `kyc-settings`, Redis → `redis`, MDB → `mdb-cluster`.
**`mdb-cluster` was a menu id only until this slice** — added to `ADMIN_SECTIONS` (add-only)
so the grant can be issued. The bare `settings/` tree is not walked as one section in
`CLOSED_FOLDERS` (trading-risk breaks a single-section walk); root `settings/route.ts` is
asserted in the R101y describe. 9 new probes each red on exactly one failure. Inventory after:
**0 no-check / 0 hand-verified / 67 helper / 270 section-granted**.

**R101aa CLOSED 16 September 2026.** Ops / money / customer helpers: **26 files, 45 handlers**
across ten folders (`customer-assignments/`, `customer-audit/`, `database/backups/`,
`email-templates/`, `incidents/`, `lockouts/`, `payment-providers/`, `platform-financials/`,
`reconciliation/`, `transactions/`). Single-caller → `guardSection` from the owning screen;
dual-caller list/transfer, reconciliation and transactions → `guardAnySection`.
`platform-financials/backfill` replaced a hand-rolled JWT that fell back to a hard-coded
secret (R101n shape). 9 new probes each red on exactly one failure; inventory closed-folder
canary aimed at single-handler `payment-providers/regenerate-env`. Inventory after:
**0 no-check / 0 hand-verified / 41 helper / 296 section-granted**.

**R101ab CLOSED 16 September 2026.** Money leftovers / ops health / employee self:
**11 files, 18 handlers** (`pending-payments/`, `payment-history/`, `live-ops/`,
`health-overview/`, `price-health/`, `kyc-history/`, `notifications/`, `employee/` profile +
password + notifications, `security/alerts/`). Single-caller → `guardSection` from the
owning screen (`payments`, `overview`, `price-health`, `kyc-history`, `notifications`,
`profile`). `security/alerts` is dual-caller overview|fraud via `guardAnySection`.
`health-overview` has no UI caller and is granted overview with live-ops. 11 probes each
red on exactly one failure (`tools/probe-r101ab-ops.ps1`). Inventory after:
**0 no-check / 0 hand-verified / 30 helper / 307 section-granted**.

**R101ac CLOSED 16 September 2026.** Clear helpers: **21 files, 29 handlers** (`ai-agent/`,
`audit-logs/`, `credentials/`, `environment/`, `cookie-consent/`, `images/` + dual
`images/upload`, `server-options/`, `admin/backfill-ranks/`, `admin/cleanup/run/`,
`admin/reset-all-users/`, `reset-all-data/`, `debug-fraud/`, `gamification/sync-user/`,
`finalize-challenges/`, `dev-scripts/`, `dev-zone/dependency-check/`). Single-caller →
`guardSection`; `images/upload` → `guardAnySection(["branding","landing-pages"])`.
`data-cleanup` and `data-maintenance` added to `ADMIN_SECTIONS` (add-only). `cookie-consent`
lost its hand-rolled JWT; environment / reset-all-data attribute from `guard.admin`;
cleanup/run keeps `super_admin` after the section grant. 11 probes each red on exactly one
failure (`tools/probe-r101ac-helpers.ps1`). Inventory after:
**0 no-check / 0 hand-verified / 9 helper / 328 section-granted**.

**R101ad CLOSED 18 September 2026.** Of the nine leftover helpers, **one was real debt**
and eight were intentional. `POST /api/gamemaster/fix-purchases` is admin repair under a
Game Master path — it lived on `verifyAdminAuth` alone so any admin-at-all token could run a
money-adjacent subscription backfill. Now `guardSection("gamemaster-management")` plus the
existing `super_admin` write gate (`verifyGameMasterAuth` would be the wrong helper). The
other eight are `HELPER_BY_DESIGN` in `tools/admin-routes/auth-inventory.ts`: `auth/logout`,
`verify-password`, `admin/events/poll` (admin-at-all — a section grant would lock employees
out of logout / password confirm / the event poll) and five Game Master portal routes under
`gamemaster/` (`verifyGameMasterAuth` — a different principal with no `ADMIN_SECTIONS` grant).
Distinct from `PUBLIC_BY_DESIGN`: these require a session. Partition math and
`routesOfClass` both include the carve-out. 6 probes each red on exactly one failure
(`tools/probe-r101ad-helpers.ps1`). Inventory after:
**0 no-check / 0 hand-verified / 0 helper debt / 331 section-granted**
(+ 10 public-by-design + 8 helper-by-design).

**R101 authorization debt is closed.** Carve-outs remain pinned; a new
`verifyAdminAuth`-only route that is not listed turns the empty debt list red.

---

### R96 - A games-only player can earn five badges out of 128 - **R96a CLOSED; R96b AUTHORING CLOSED 17 Sep 2026 (content still owner)**
...
- **R96a - the gate, code only.** *(**CLOSED 16 Sep 2026.**)* Classify by condition type
  via `CROSS_GAME_CONDITION_TYPES`: a trading minimum applies to a **trading-typed**
  condition; a cross-game badge zeros trade floors (including stored `minTrades`) and uses
  `completedCompetitions` so **any** game satisfies the contest floor; unknown types fail
  closed to trading. Unblocks the ~35-40 already-cross-game badges with **no data rewrite
  and no operator tuning overwritten**. Both evaluator copies stay byte-identical (R100).
  Pinned by `__tests__/services/badge-gate-r96a.test.ts` (9 tests), 7 probes in
  `tools/probe-r96a-badge-gate.ps1`. A document treating the gate as still trading-only is
  stale as a present fact though correct as history — **say which**.
- **R96b - the authoring.** *(**AUTHORING CLOSED 17 Sep 2026; catalogue content still
  owner.**)* Plumbing is built: `badge-condition-registry.ts` (scopes
  platform|trading|both|game), `game_*` conditions on `UserGameStats`, Games category on
  both badge models, admin form + AI generate/wizard/balance scoped by `gameTypes`,
  wizard apply **add-only** by default, ladder add/remove with neutral titles. Balance
  engine skips `minTrades` auto-fix on non-trading scopes. **What remains is content** —
  naming and thresholds for game-specific rows — not the authoring surface. Catalogue
  still holds **zero** game-specific badge rows until an operator (or AI with an owner
  brief) authors them. A document treating R96b as unbuilt is stale as a present fact
  though correct as history — **say which**.

**Two things had to happen first, and both are now closed as of 16 Sep 2026.** **R100** -
the admin copy of the evaluator was 221 lines behind and was brought to parity as a
**behaviour-preserving port in its own commit** (byte-identical; 6 tests / 6 probes), because
parity and a gate change in one diff destroy the only evidence the port is safe. And
**R101a** - `/api/badges` full CRUD and `trigger-badge-evaluation` are section-granted, so
the gate's own data is no longer world-writable. A document saying either is still open is
stale as a present fact though correct as history — **say which**.

**Harm, precisely.** **Latent in effect, live in mechanism** (before R96a): every branch was
in production and refused correctly, but no games-only player had yet completed enough to be
refused. **Nothing to backfill** - no badge was wrongly awarded or wrongly withheld, and the
remedy is a gate change plus new content, not a data repair.

---

### R102 - The wizard's level ladder was written where nothing reads - **CLOSED 17 Sep 2026**

**What it is.** `dbTools.writeXPConfig` in
`apps/admin/app/api/ai/gamification-wizard/route.ts` wrote into the `xpconfigs` collection
keyed on **`type`**, while `xp-config.model.ts` declares the discriminator as
**`configType`** and `POST /api/badges-xp/manage` reads through the model. So every level
ladder and every badge-XP table the wizard saved **landed in a document nothing else could
find**, and the screen that exists to show it kept reading the old one. The write succeeded,
the route returned success, and there was no error and no log line on either side.

**Two faults, and the second one hid the first.** `readXPConfig` returned
`levels: levels?.data`, but the `level_progression` document stores the array one level
deeper as `{ data: { levels: [...] } }`. So the wizard's own audit read `undefined` and
reported an **unconfigured ladder on every run** - including immediately after it had
written one. That is what made the key mismatch look like "the ladder step does not work
yet" rather than "the ladder step writes to the wrong place".

**Harm, precisely.** **Live from the moment the wizard's ladder step existed, and it is a
LOST WRITE rather than a wrong value.** No player's XP, level or badge was ever computed
from the orphaned document, because nothing read it - so no progression was wrong and
**nothing was backfilled**. What it cost is an operator's work: a ladder configured through
the wizard silently did not take effect, and the wizard then told them it was still
unconfigured.

**The fix.** `writeXPConfig` uses `configType` and wraps the ladder as
`{ levels: data }` when the config type is `level_progression`; `readXPConfig` reads
`levels?.data?.levels`. **Orphaned documents are left in place rather than migrated** -
they are keyed on a field nothing queries, so they are inert, and a script that rewrites
discriminators in a config collection is an unreviewed writer for no gain.

**The transferable rule.** **A collection reached by both the raw driver and a model has two
spellings of its own key, and only one of them is enforced.** The model validates
`configType`; the raw driver validates nothing, so it will accept `type` cheerfully for
ever. This is the third instance of the raw-driver boundary after the R42 fixture and the
analytics participation funnel, and the first where the disagreement was a **key** rather
than a value's type. Before writing to a collection with the raw driver, read the model's
discriminator rather than the field name in the document you happen to be looking at.

**A second finding came with the reset work and has no risk number, because nothing is
wrong with it.** `getBadgesFromDB()` and `getXPConfigFromDB()` **reseed the shipped
defaults when their collection reads empty** - correct behaviour for a fresh install, and
it means a plain `deleteMany({})` is **undone by the next read**. A "start from scratch"
that only deletes therefore appears to work and is reverted before an operator refreshes
the screen. That is why the rebuild path sets suppression flags
...
### R106 - Two maps and trading-only milestones - **CLOSED 17 Sep 2026 (code)**

**What it is.** `buildJourneyBlueprint` previously emitted **one map per catalogue scope**.
With trading + one provider game that is exactly two maps. Activity milestones carried only
trading `completeCondition`s. Operators expected the historic ~10-map sequence and a path a
games-only player could finish.

**Harm.** Generation-shape defect. No money moved. Stored designs stayed wrong until regenerate.

**The fix.** Fixed shells (`journey-map-shells.ts`, 10 historic mapIds), dual-path
`orCompleteConditions` evaluated OR in both progress services, gaming condition vocabulary,
master `journey_settings` toggle, editor Alternate Paths UI. `__tests__/services/journey-blueprint-dual-path.test.ts`, `tools/probe-journey-dual-path.ps1`.

**Owner regenerated 18 Sep 2026** with Generate Full Sequence (replace). Live-data half CLOSED.
A document saying "owner must regenerate" is correct as history and stale as a present fact —
**say which**.

---

### R107 - Journey editor kept map 1; badge ids looked like backend names - **CLOSED 17 Sep 2026**

**What it is.** Two owner-visible defects on the same day. (1) Journey Map Editor:
clicking map 3 only moved the selection ring; Current Map / Milestones / Zones still
showed map 1 because the click handler never called `fetchData` with that map's id.
(2) Player journey "Required Badges" printed raw blueprint ids (`trading_beat_top_trader_flag`)
because the modal looked up names only in `lib/constants/badges` (older catalogue ids).

**Harm.** Reporting / operator UX only. No money. Editing maps 2–10 was effectively blocked
from the Milestones and Zones tabs.

**The fix.** `selectAndLoadMap(order, mapId)` on card click; `handleEditorTabChange`
re-fetches when `mapConfig.mapId` disagrees with the selection; `getMapIdFromOrder`
matches API `order` before index. Badge names: `lib/utils/badge-display-name.ts`
...
### R109 - Ten themed maps were still identical - **CLOSED 17 Sep 2026 (code) + 18 Sep 2026 (live data)**

**What it is.** After R106 regenerate, all ten Sequence cards showed **750 XP and 12
milestones**, Current Map for Space Station still showed Treasure Map (Pirate Cove) art,
Milestones still started with `account_created` / early nodes, and zones were
Foundations / Ascent / Summit on every theme. R107 selection loading was not the defect —
generation content was flat.

**Harm.** Operators and players see ten copies of one chapter. No money moved. Latent for
payouts; live for the editor and player journey screens. Stored rows stayed flat until the
owner ran Generate Full Sequence (replace) on 18 Sep 2026.

**The fix.** Progressive `MAP_XP_BUDGETS` / `MAP_MILESTONE_COUNTS` and `THEME_ZONES` on
`journey-map-shells.ts` (mirrored); blueprint stamps `backgroundImage`, Map-1 onboarding
(KYC → deposit → first action) then dual-path, Map 2+ `map_completed` of the previous
mapId; both progress services evaluate `map_completed`; renderers use
`resolveMapBackgroundImage` before any pirate default. Tests extended in
`journey-blueprint-dual-path.test.ts` (9); schema + round-trip still green.

**Owner regenerated 18 Sep 2026.** Live-data half CLOSED.

---

### R110 - Admin AppSettingsProvider was mounted nowhere - **CLOSED 18 Sep 2026**

**What it is.** `apps/admin` shipped `AppSettingsContext.tsx` and nineteen components called
`useAppSettings()`, but **`AppSettingsProvider` was mounted in no layout**. Every caller
received `createContext` defaults, so a configured credit symbol (and the rest of the display
pack) never reached an admin screen. No error, no log line — the same silence shape as an
unmounted editor (R93) one layer out.

**How it was found.** X6.5 A1 built `TerminologyProvider` *because* chapter `14` said to reuse
`AppSettingsProvider` and a canary proved that provider was unmounted. The credit-symbol
silence stayed open as backlog until the dual-track engineering pass after R101ad.

**Harm.** LIVE and DISPLAY only — no money miscomputed, nothing to backfill. Operators editing
Settings → Currency saw one symbol while every other admin screen kept the default.

**The fix.** Mount `AppSettingsProvider` in `apps/admin/app/layout.tsx` wrapping
`TerminologyProvider`. Widen `GET /api/settings` to admin-at-all (`verifyAdminAuth`) so every
signed-in employee can load the display pack; `PUT` stays `guardSection("currency")`. The
terminology canary was **flipped, not deleted**. Terminology still uses its own provider —
credit symbol and noun overrides are different packs.

**Files:** `apps/admin/app/layout.tsx`, `apps/admin/app/api/settings/route.ts`,
`apps/admin/contexts/TerminologyContext.tsx`, `__tests__/admin/terminology-delivery.test.ts`,
...
> **AMENDED 20 September 2026 (owner decision), and the amendment is larger than the fix it
> sits under: every automatic outage response is now OPT-IN PER PROVIDER.** The owner's
> instruction on reading this closure was that the platform must never stop contest or
> challenge creation, refuse entry, or take a provider off sale by itself — that is an
> operator's decision, taken with a switch — with a **second** switch that grants the
> platform permission to act automatically, off unless somebody turns it on. Built as
> `GameProvider.autoOutageResponseEnabled` (**no schema default**, so "nobody has said" stays
> distinguishable from an explicit no) and one predicate, `systemMayActOnOutage`, which is
> `providerObservedDown` **and** the consent, with `PROVIDER_AUTO_OUTAGE_FILTER` as its query
> form. **R111 is not superseded by this**: the evidence half survives unchanged, so a
> consented provider still needs the stamp, and a document treating the consent flag as
> having replaced `providerObservedDown` is describing the deadlock with an extra switch in
> front of it. Three facts to carry. **The watching and the alerting are unconditional** —
> the kill switch still classifies, still moves `healthStatus`, still raises its critical
> alert, and `summary.withheld` counts the providers it declined to disable, which is the
> difference between no outage and an outage nobody let it act on. **The alert is claimed
> once per outage episode** via an atomic `outageAlertedAt` update keyed against
> `healthDownSince`, because a withheld provider stays `enabled: true` and is therefore
> re-examined every minute. And **resume is deliberately NOT gated on the consent**, the flag
> governing intervention rather than the undoing of one — gated, an operator switching the
> automation off mid-outage would strand every system-paused contest paused for ever, with
> nothing to log and nobody told. Details in `12` s4.1b and `07` s3.2/s3.3;
> `tools/probe-provider-auto-outage.ps1`, 12 probes, each red on exactly one named test.

---

### R112 - The incident screen was a fifth place to fix a contest, and it still spoke trading - **CLOSED 21 Sep 2026**

**What it is.** Four defects, one screen.

1. **The hub was trading-shaped.** Its create path listed contests from `GET /api/competitions`, which applies no filter, and then offered pause and emergency-cancel without `contestControlCopy`. Pausing a puzzle from that screen told the operator that trading was frozen and positions would be closed — the defect `12` s3.2a had already fixed on the contest panel and missed here.
2. **`challengeId` was a dead field.** The model had carried it since it was written. Nothing in the UI could attach a challenge, so a challenge incident could not be recorded, and a round could not be referenced by any field.
3. **Result adjustment was a dead end.** The resolve route returned `requiresManualReview: true` and did nothing, while `adjust-results` and `re-settle` already accepted an `incidentId`.
4. **The refund quoted fiat.** The player notification said *"You have been credited €X"* with the amount taken from `competition.entryFee`. A competition is denominated in credits.

**Harm.** The sentence was **live and player-visible**. The other three were missing capabilities, not wrong payments: no wallet was debited or credited by the wrong amount, and **nothing was backfilled**. The credit that moved was correct; only the words were not.

**The fix.** `apps/admin/lib/admin/incident-actions.ts` is the only definition of the action ids and their consequences, and it imports the contest-control copy and the round-resolution actions. `POST /api/incidents/[id]/act` requires the incidents grant and the section that owns the subject, decides applicability from the stored document, and appends `actionsTaken`. Destructive actions are withheld on the old panels with a refusal that names the hub. **Pause and resume stay in both places**, because they are reversible and have to be available while someone is watching — recorded in `12` s3.3 rather than treated as unfinished. The player sentence uses `formatVolts`. The ledger field `currency: "EUR"` was left, on purpose.

**Files:** both `incident.model.ts` copies; `incident-actions.ts`; `lib/services/incidents/`; `app/api/incidents/[id]/act` and `app/api/incidents/subjects`; `components/admin/incidents/`; `__tests__/admin/incident-hub.test.ts`; `tools/probe-incident-hub.ps1` (4 probes, each red on exactly one named test).

**Not verified by eye.**

---

### R113 - prize_pool_mismatch false-alerted refunded unscored contests - **CLOSED 21 Sep 2026**

**What it is.** Two reporting defects, one settlement path.

1. **The threshold monitor was blind to entry-fee refunds.** `checkPrizePoolMismatch` added finalLeaderboard `prizeAmount`s to PlatformTransaction rows of type `platform_fee` / `unclaimed_pool` / `retained_gm_fee`. Under `unscoredContestPolicy: "refund_entry_fees"`, settlement returns the net pot via WalletTransaction `competition_refund` and books only the platform fee — there is no `unclaimed_pool` row by design. Equation on the owner's Warrior's / Circuit Sniper contests: prizes 0 + fee 2 + refunds 9+9 = prizePool 20. The monitor saw 0 + 2 and raised a critical alert every minute.
2. **The admin no-winners notice always named the unclaimed pool.** `resolveNoWinnersNotice` had one sentence for every finished contest with `noWinners`, so an operator opening a correctly-refunded contest was told the pot had been booked as unclaimed funds.

**Harm.** **LIVE and REPORTING-ONLY.** Settlement and the wallet ledger were correct throughout — the alerts were false and the view sentence was wrong. **Nothing was backfilled**; false SecurityAlert rows already raised can be dismissed in the usual way.

**The fix.** The monitor sums completed `competition_refund` rows keyed by `competitionId` (declared `String` — match the id string, never an ObjectId). Accounted = prizes + booked platform rows + refunds. The view notice branches on `unscoredContestPolicy === "refund_entry_fees"`. Pinned by a behavioural test that seeds fee 2 + refunds 9+9 against pool 20 and asserts no `prize_pool_mismatch`, plus a probe that drops `+ refunded` from the equation. Notice test asserts "returned" and forbids "unclaimed pool" on the refund policy.

**Files:** `lib/services/game-providers/provider-threshold-monitors.service.ts`; `__tests__/services/provider-threshold-monitors.test.ts`; `tools/probe-provider-threshold-monitors.ps1`; `apps/admin/lib/admin/contest-result-presentation.ts`; `apps/admin/app/competitions/view/[id]/page.tsx`; `__tests__/admin/contest-result-presentation.test.ts`.

**Not verified by eye.** Existing false alerts stay until an operator dismisses them — the monitor will not re-raise once fingerprints are still within the suppression window after deploy, and after that the equation balances.

---

### R114 - prize_pool_mismatch false-alerted contests that paid a Game Master - **CLOSED 24 Sep 2026**

**What it is.** Sibling of R113 on the same equation. Settlement books `platform_fee` **net** of Game Master commission (`netPlatformFee = gross − gmEarnings` in `fees.service.ts`) and credits the GM via WalletTransaction `gamemaster_earning`. The threshold monitor summed prizes + PlatformTransaction (`platform_fee` / `unclaimed_pool` / `retained_gm_fee`) + `competition_refund` and never looked at `gamemaster_earning`. On the owner's "This is annw": prizes 18 + net fee 1.50 = 19.50 against prizePool 20, while Martha's +0.50 referral sat on the wallet ledger. The real equation is 18 + 1.50 + 0.50 = 20. `retained_gm_fee` already covers the inactive-subscription case inside PlatformTransaction — this hole is only the **paid** GM path.

**Harm.** **LIVE and REPORTING-ONLY.** Settlement, prizes, platform fee and GM credit were correct throughout. **Nothing was backfilled.** The existing false SecurityAlert can be dismissed in the usual way.

**The fix.** Accounted = prizes + booked platform rows + completed `gamemaster_earning` + refunds. Pinned by a behavioural test that seeds fee 1.50 + GM earning 0.50 + prize 18 against pool 20 (no alert), plus a control that omits the GM row and still alerts.
...
| R21 | Splitting the ~1,700-line dashboard action | **CLOSED 18 Sep 2026** — `lib/actions/dashboard/*` extract; `needsTradeHistory` gate; `showTradingChrome` UI |
