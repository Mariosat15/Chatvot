...
defect.** The sibling was the unmounted *provider* - `AppSettingsProvider` - so nineteen
components rendered with `createContext` defaults and a configured credit symbol never reached
them; **that half closed as R110 on 18 Sep 2026**. This is an unmounted *editor*: the value is
read correctly everywhere, and there is simply no way to change it. A document merging the two
is describing one absence twice. (The prose once labelled the provider half as R75 — wrong:
R75 is the admin build outage from a cross-app `@/` alias.)
