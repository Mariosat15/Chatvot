...
| **R109** | **Ten themed maps were still identical under the hood.** After R106, Sequence cards showed flat **750 XP / 12 milestones**, every canvas fell back to Pirate Cove art (`backgroundImage` never written), every start was `account_created`, and zones were Foundations/Ascent/Summit on every theme — so Space Station still began like Map 1 | **High** | **LIVE as GENERATION until regenerate.** Display + content; no money. **Nothing was backfilled** | **CLOSED 17 Sep 2026 (code).** Progressive budgets/counts, theme zones, Map-1 onboarding + `map_completed` gates, stamped art URLs, `resolveMapBackgroundImage`, progress evaluation for `map_completed`. Owner must Generate Full Sequence (replace) |
...
### R101 - Ninety-nine admin routes with no authorization, and fifty-eight of them write - **R101a–R101ac CLOSED 16 Sep 2026; helper-but-no-grant still open**
...
**R101 remains open** on the **9 helper-but-no-grant** routes — `auth/logout`,
`verify-password`, `admin/events/poll` (admin-at-all by design) and `gamemaster/` (deferred
— `verifyGameMasterAuth` by design).
...
**Harm.** Generation-shape defect. No money moved. Stored designs stay wrong until regenerate.
...
**Owner must regenerate** with Generate Full Sequence (replace). Code-complete is not data-complete.
...
### R109 - Ten themed maps were still identical - **CLOSED 17 Sep 2026**
...
payouts; live for the editor and player journey screens. **Nothing was backfilled** — live
rows stay flat until Generate Full Sequence (replace).
