...
5. Then the admin programme, per the owner's admin-first sequencing. **Most of X6 is now
   ...
   **game contest wizard was rebuilt on trading's chrome with a game-aware assistant**
   ...
   market refusal removed** (`12` s2.8a), and the **admin front page learned about
   competitions** (`12` s5.1b) - a row that read as an aggregate needing a game dimension,
   where the page in fact counted no contests at all, of any game. **What is left of X6 is
   the Game Master contest CONSTRUCTION half** - the permission half shipped 7 September and
   refuses a provider contest by name - plus two items inside `12` s5: the
...
`PriceHealthWidget.tsx`; the per-round provider cost, which has **no data source until X4**; the
participation funnel on the analytics screen itself; and `finalLeaderboard` rendered outside the
contest view screen, which is X6.5.
