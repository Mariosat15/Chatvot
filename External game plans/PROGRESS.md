...
5. Then the admin programme, per the owner's admin-first sequencing. **Most of X6 is now
   ...
   **game contest wizard was rebuilt on trading's chrome with a game-aware assistant**
   ...
   market refusal removed** (`12` s2.8a). **What is left of X6 is the Game Master contest
   CONSTRUCTION half** - the permission half shipped 7 September and refuses a provider
   contest by name - plus three items inside `12` s5: `AdminOverviewDashboard.tsx`, the
   hide-when-trading-off rows, and the per-round provider cost, which has no data source
   until X4 supplies a real contract.
...
no game dimension. The hide-when-trading-off rows of s5 are untouched. `finalLeaderboard` is
