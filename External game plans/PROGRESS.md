...
**Deviated from plan:** two of the owner's filter tabs cannot be answered. There is no friends
model anywhere in `database/models`, and publishing a player's country per row on a public board is
a disclosure decision. All three are drawn, `GLOBAL` lit and the other two as disabled labels with
the reason in a title attribute - **rendered as spans, so there is no control that appears to
work.** That **reverses a guard recorded in s4.1q** asserting the rail draws no dead scope tabs;
the test was **flipped rather than deleted**, keeping the original objection as the reason for
*how* they are drawn. Second time the owner has overridden a recorded guard here.
