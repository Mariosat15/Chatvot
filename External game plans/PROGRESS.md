...
| **19** - AI on the game content screen | **Half done** (19.1). Setting out to build it found that four of the six content fields the contract REQUIRES were being discarded on every sync (**R63**) - the storage half shipped 10 Sep, the AI panel has not. Owner decision the same day: **the AI never writes `rulesSummary` or `howToPlay`**, which are the provider's and are quoted back in prize disputes |
