...
| **Next action** | **First, an owner decision that is not technical: whether to compensate the players whose entry fees were kept by R43** (see the refunds row above). The affected contests are identifiable and the fix is not retroactive, so this is a real-money question waiting on a person, not on code. **Then technically: finish X4a** - pull and rebuild on the server, start `chartvolt-games`, register it through the admin screens, and drive one round end to end **by clicking** (`21` s4.1e has the six steps; **set the contest's playing time to one minute**, which is the dropdown's shortest option - step 3's advice to start with `circuit-perfect` is stale, since that title was retired 8 Sep 2026). **No DNS, nginx or certificate work is needed** since the play surface is proxied through the platform app (owner's choice, 6 Sep 2026). The game is **playable by a human in a browser**, **R34 is closed**, and the service is now **deployable** - PM2 entry, nginx block, `env.example` and a runbook in `deploy/README.md` (all 6 Sep 2026) - so nothing technical stands between the two halves. **Owner decided 6 Sep 2026 to deploy first and rehearse against the live site**, rather than complete the local rehearsal. Provider **health** - the last of X6's five admin destinations - **shipped 6 Sep 2026** (`12` s4.2b), so what remains of the player surface is the trading panels themselves and the per-game summary cards. **Commercially, in parallel: find and assess a provider using `08`** - X4 cannot start without one, and nothing in the programme is blocked on that search. **Engineering-wise the next slice is challenges** (X10 / E8): pick trading-or-game, then a title, then adapt the setup page to its shape, since the competition half of that shape shipped 8 Sep 2026 (`22` s8) and left the challenge half untouched - **and `ChallengeParticipant.score` still defaults to `0`, so the first provider challenge reproduces R50 exactly**. **AMENDED 13 Sep 2026 - that last sentence is closed and the slice is BUILT**, so it is correct as history and stale as a present fact; **say which**. A provider game can be challenged end to end, and the shape question was answered rather than solved: a challenge is `anytime` in every case, its window derived from acceptance, and it **never reserves a round out of that window** (R73). The engineering next step is therefore the same as the technical one above - **have the owner play one provider challenge and one provider contest through the browser** - plus, in code, the **per-title challenge defaults** the owner asked for on the same day, which are the only part of the three 13 Sep reports left unbuilt. **AMENDED 14 Sep 2026** - those defaults are built (`12` s4.2d) and so is the **opponent picker** (`13` s4.1aa), so both clauses are correct as history and stale as present facts; **say which**. The code next step is the rest of X10's opponent half: **open challenges** - conditional opponent fields in both model copies, an atomic claim on accept so a simultaneous second accepter is refused rather than both being debited, and a pass over the sites that assume two named players - then **per-game willingness**. **Count those sites before trusting any figure in a plan**. **AMENDED again later on 14 Sep 2026** - open challenges are built (`03` s2.4a) and so are the three gaps they left: the **lifecycle notifications and pop-ups** (`13` s11.1a), the **open seat's own expiry** (`03` s2.4b) and the **locked duration clocks** on the create dialog (`13` s4.1ab). So that clause is correct as history and stale as a present fact; **say which**. What remains of X10's opponent work is **per-game willingness** alone |
...
| **Who decides a game's play style?** | **The operator can, from 9 Sep 2026** (`22` s9, `12` s4.2c), which is the first step and for now the whole of **Task 10** in the owner's task document. The row above left the shape entirely in the provider's hands - right for a third party, **wrong for the one provider we run ourselves**, where the declaration is a TypeScript literal in `games-service/src/games/titles.ts` and making a title a race meant an edit, a build and a redeploy. A **Play style** control now sits on every row of the Games list, writing `provider_game.playModeOverride` through its own section-guarded route with its own audit line, and the resolved style is badged on the wizard's game picker because it changes more of the wizard than anything else about the title. **Nothing about the rules changed** - `22` s8.2 is unaltered and every gate still reads the same three forced values. **Four things drift easily.** It is a **second field, never an edit to `playMode`**: that one is in `providerOwnedFields`, so a control writing there saves, toasts and is reverted by the next sync with no error and nothing in a log - and because the safety lives in an allow-list in a *different file*, it is pinned by a test that runs a **real sync** and asserts the provider's own field **was** rewritten in the same pass, since asserting only that the override survived would pass against a sync that did nothing. **`head_to_head` beats the operator**, not the other way round - two people cannot play each other at different times, so an override there would be a declared, written, dead field, the fifth instance - and the control withholds itself with the reason rather than being greyed out. **Clearing uses `$unset`, never `""`**, because an empty string read literally would mask a provider's `scheduled` declaration behind an override nobody chose. And it is **out of the title-and-logo content editor deliberately** and on `NEVER_EDITABLE_CONTENT_FIELDS`: that dialog writes copy an operator can get wrong harmlessly, this decides when entry closes and how many attempts a player gets on a contest people have paid into. **37 tests, 19 probes.** **No title is `scheduled` yet and no override has been set in production**, so the scheduled path is still exercised only by tests. **Turn-based and heat-based stay BLOCKED** beside the per-round provider cost - a bracket needs opponents, and nothing in the catalogue and no signed provider works either way, so designing one now means inventing a protocol against no counterparty. **Since later on 9 Sep a title may support BOTH shapes and the operator picks per contest** (`22` s10), so this row's "a title has one shape" framing is correct as history and stale as a present fact - **say which**; see the row below |
| **Can one title be run BOTH ways?** | **Yes, from 9 Sep 2026** (`22` **s10**, task doc **11.1**), and **THIS REVERSES A DECISION** - `22` s8.3 and `play-shape.ts`'s own header said the shape is a property of the **title** and **never** comes from caller input, and the create service now takes it from its caller. That rule was correct for a world where a title had one shape, and the owner's example is what ended it: **a multiplayer racing game with a synchronised race AND an async time trial.** Recorded here rather than by editing s8, because a reversed direction is exactly the fact a new chat needs. **What survives is the safety, and all three properties are intact**: the pick is validated against a **stored** set (`resolveSupportedPlayModes`), so a race can be run staggered only if somebody declared this race has a legitimate time-trial form; the consequences are still stamped on at **write** time, so not one runtime gate reads a mode; and **no player-facing path supplies a shape at all**, which was always the absolute half. **The defect it closed is the reason it needed the model change**: the edit service resolved the shape from the **title**, so once a title has two, an ordinary rename would re-force a staggered contest to one attempt, `until_window_closes` and entry closing at the gun - **under people who had already paid to enter**, no error, nothing in a log. The contest therefore stores its own `Competition.playMode`, **frozen** in three places because one is a suggestion, and the flipped structural test asserts `resolvePlayShape` is now **absent** from both services. **Five more facts drift easily.** The supported set **unions the title's own style in**, or a title's declared style becomes unselectable while every contest already created on it was created as it. **`head_to_head` beats the operator here too**, so the picker's options are a **server** answer - a component deriving its own would offer a staggered chess match the create service is about to refuse, and a test asserts the two helpers **disagree** on that case. The picker is **withheld on `length < 2`**, which is what keeps the screen unchanged for the whole live catalogue, and `supportedPlayModes` has **no schema default** on the `playModeOverride` precedent - a default *is* a stored value. The play-style route **refuses a request carrying both fields**, so one audit entry cannot cover two decisions. And **two pre-existing probes were re-aimed** after reporting `PROBE DID NOT APPLY` - they had been matching `resolvePlayMode`'s original one-line body since the operator override replaced it, so two real guards had sat unprobed, which reads like a broken harness rather than a moved target. **49 tests, 37 probes all red on exactly one failure.** **Latent, nothing backfilled** - no title declares a second shape yet, so every contest resolves exactly as it did on 8 Sep. **Not verified by eye** |
| **What unit is a competition priced in?** | **Credits, and every screen says so from 9 Sep 2026** (task doc **1.1**) - and before that around forty render sites prefixed a credit amount with `settings.currency.symbol`, the **fiat** symbol configured for deposits and invoices, so a 50-credit entry fee read `EUR 50`. Two landing routes were worse, each carrying a private `formatCurrency` hard-coded to `$`, so a provider contest's prize pool advertised to signed-out visitors read `+$30.00`. **Every amount was correct and only its unit was a lie**, which is the whole reason forty sites survived: no error, no log line, and the figure reconciles perfectly against the ledger - so the guards are **structural rather than behavioural**, there being no wrong number to assert on. One mirrored formatter, `lib/utils/format-volts.ts`, **which never converts**, and that refusal is the finding rather than a simplification: the platform stores what a credit is worth **twice and the two defaults disagree by a factor of a hundred** - `AppSettings.credits.valueInEUR` at 1 credit = EUR 1 drives the player's "approximately EUR x" line, while `CreditConversionSettings.eurToCreditsRate` at 100 credits = EUR 1 is what deposits, withdrawals and the admin financial screens move money on. **That is a separate defect, recorded at the foot of the module and deliberately not fixed here - it was CLOSED as R74 on 14 Sep 2026, so that clause is correct as history and stale as a present fact, and say which**; this work removes the fiat equivalent from competition surfaces rather than correcting it, because a contest is denominated in credits and has no business quoting a second unit. **Do not summarise it as a display pass**: the same wrong unit reached players by **email** and operators in the **settlement reconciliation logs**. **There are two units on a trading contest and merging them is the tempting mistake** - a prize pool is credits, a trader's equity and P&L are simulated trading capital in the contest's quote currency - so the ranking panels write both and the guard **counts** `${currSymbol}` interpolations, a probe relabelling one metric having stayed green against a presence check while leaving the other right. **An absent amount is a dash, never a zero**, R45 and R50's rule one layer out. The unit stays configurable and `"Volts"` is only the default, so a rename reaches every screen and **not** the few server-composed strings, which have no React context - a boundary recorded rather than implied away, and strictly smaller than the euro sign it replaces. The assistant's no-fiat rule is **appended** to trading's prompt rather than written into it, because a test pins that prompt character for character as the only evidence the trading wizard still writes what it wrote; the guarantee is now "the historical string plus one shared rule" and **the composition itself is asserted**. **Two carve-outs are correct and must not be finished off:** deposit and withdrawal strings are genuinely fiat, so a control probe adding a euro deposit line **stays green on purpose**, and the admin analytics screen keeps its `creditsToEUR` figures, which are an operator converting deliberately. **One defect was introduced and caught before shipping, and the diagnostic is the point:** five admin challenge-view calls kept the `.toFixed(2)` / `.toLocaleString()` from the strings they replaced, so a **string** reached the formatter, hit the non-number guard and would have rendered the prize pool, entry fee and winner's prize as `-`. The typecheck saw only **two of the five** - that page's challenge object is loosely typed, so `?.toLocaleString()` widens to `any` - and every structural test here stayed green, because they ask whether a screen reads the fiat symbol and it does not. **Diffing the typecheck against a stashed baseline is what found it**, two new entries inside 225 being invisible in a count; closed with a repo-wide scan rather than a list of five files, plus the behavioural half. Separately **four typecheck errors disappeared**, which got the same suspicion as a rise and turned out to be real: `ContestStatsCards.tsx` read `settings.credits.decimals` unguarded on a context that can be `null`, so four latent dashboard crash paths went with the unit fix. **CORRECTED THE SAME DAY (task doc 1.2): it is the credit SYMBOL, not the credit name** - `AppSettings.credits.symbol` from Settings → Currency, defaulting to `⚡`, so an entry fee reads `50 ⚡` and not `50 Volts`. One field along on the right object, which typechecks and renders and is only wrong to somebody who opens the currency screen - so **any sentence above naming `credits.name` is correct as history and stale as a present fact**. Three things came with it: **singularisation was removed and its two tests inverted rather than deleted**, because a word has a plural and a glyph does not, and stripping a trailing `s` from a configured symbol is destructive; **both of those probes came back green on the FOURTH cause - the mutation changed no observable** - since re-adding the singulariser to `⚡` yields `⚡`, fixed by asserting a configured symbol that ends in one, the field being free text; and **the symbol goes after the number**, matching the wallet, with a test pinning it because the fiat convention is what a later edit corrects it to. A **codemod overreached into four files** where `creditName` was prose rather than a formatter input and was reverted, the tell being duplicate object keys - **a pattern-driven rename cannot tell an argument from a sentence.** And a **measurement error worth keeping**: a whole-file regex for the symbol default returned the euro sign, which reads as a live drift, because `currency:` precedes `credits:` in the model and the fiat field is correctly a euro sign - the test slices from `credits:` and was right. **70 tests, 27 probes red with a blast radius of one** |
| **What is one credit worth?** | **One number, derived, from 14 Sep 2026** (**R74**) - and until then two, disagreeing by a factor of a hundred, with the wrong one in front of the player. `AppSettings.credits.valueInEUR` defaulted to 1 credit = EUR 1 and drove the wallet's "approximately" line; `CreditConversionSettings.eurToCreditsRate` defaulted to 100 credits = EUR 1 and is what deposits and **withdrawals** actually move money on. So a player holding 1,000 credits read **EUR 1,000.00** above a button that would pay them **EUR 10**, and `/help` rendered both figures, from both models, on one page. **It is not a display bug and it is not a payment bug, and both halves matter**: no wallet was ever debited or credited wrongly - the withdrawal path was right throughout - so **nothing was backfilled**; what it cost is that the single most-read number on the platform was a hundredfold overstatement of what the platform would honour, which is a support and chargeback exposure rather than a ledger one. **The fix is a derivation, not a migration.** `lib/utils/credit-value.ts` (mirrored, byte-identical test) resolves the rate and returns `1 / rate`; both `/api/settings` routes and `/api/help-settings` serve a **derived** `valueInEUR`; the admin PUT **refuses to write that path**; and the currency screen shows the figure **read-only with a pointer to Credit Conversion**. **Five facts drift easily.** **The stored field is KEPT and read by nothing** - deleting it is a mirrored migration for a cosmetic gain, and an operator may have typed a deliberate value into it, so the comment in both model copies is corrected **in place** and says it is inert rather than being tidied into the present tense. **Every client fallback was a hard-coded `1`**, which is the wrong side of the disagreement, so a slow or failed settings fetch **reinstated the hundredfold figure briefly and silently** on the one screen where it matters - `DEFAULT_CREDIT_VALUE_IN_BASE_CURRENCY` exists for exactly that, because **a fallback is a stored value as far as the player reading it is concerned**. **`|| 1` became `?? DEFAULT_...`**, and the operator matters: a legitimately configured zero is not a missing value. **The admin control is withheld rather than deleted**, on the opposite reasoning to `shouldBlockEntry` - a number that simply vanishes from the screen sends an operator looking for it in the database, so the row stays and names where the decision now lives. And **the resolver refuses a non-positive or non-finite rate and falls back to the default**, because these arrive from `parseFloat` on an admin form and a `NaN` rate makes every derived figure `NaN` while nothing checks. **`format-volts.ts` still converts nothing, and that refusal does NOT depend on this defect** - a competition is denominated in credits and has no business quoting a second unit - so the assertion stays and its reasoning was rewritten rather than removed. **26 tests, 16 probes red on exactly one failure**, one of which had to be tightened from the bare identifier to **the call with its argument**, because both names appear on the import line - the "an import is not a use" trap again. **Never verified by eye**: the wallet line is behind sign-in |
| **Does the setup form match the contest's shape?** | **Yes on both screens from 10 Sep 2026** (task doc **12.1**), and until then **only on the wizard**. `ProviderContestEditor.tsx` did not import `play-shape.ts` at all, which is **R61**: it offered the attempts and round-start controls on a simultaneous contest while `applyEdit` forces both — reading `forcedAttemptsPolicy` **before** the operator's choice — so picking "Best of several" on a race saved as `single`, with a success toast and the screen still showing the choice until a reload. The dates were the quieter half, labelled "Contest starts" with no hint on a shape where the start is **also the moment entry closes**. **Latent, nothing backfilled** — no title declares `scheduled`, so no contest has ever been in the shape the editor got wrong — but do not file it as a labelling fix: the mechanism is a money-adjacent setting silently overridden. **The class is what to carry: a rule enforced by ONE screen is not a rule the product enforces.** Third instance of the sibling-screen shape after Edit-routing in `12` s2.2/s2.4, so the guard is deliberately not "the editor hides two controls" but the invariant **a control is withheld exactly when the shape forces its value** — a property of the shape module, so a third mode or a third screen is caught by it, and four of the eleven probes mutate `play-shape.ts` rather than a component. **Four facts drift easily.** **Four of the fields task 12 asks for are derived ON PURPOSE** — Registration Closes, Play Window Opens/Closes and the entry deadline — so a document treating them as missing is proposing to undo `12` s2.3's one contest clock, and the check-in / lobby / countdown trio is **blocked** like 10.2 rather than deferred, since it is a player-facing lobby for a shape nothing in the catalogue uses. **The screen is HANDED the resolved shape and must not derive it** — resolving needs the catalogue row, and `resolveContestPlayMode`, not `resolvePlayMode`, is the question, because once a title supports two shapes the title's answer is only its default. **Both attempts controls sit under ONE guard in a fragment**, because with the condition written twice, deleting the second leaves the first satisfying any backwards search — the fifth time one identifier appearing twice has defeated a structural test here. And the withheld sentence **moved into `PlayShapeRules.copy`** rather than being copied, which turned a pre-existing test red; it was **flipped, not deleted**, since the claim it made is unchanged and only the location moved. **13 tests, 11 probes red on exactly one failure.** **Not verified by eye** |
| **Who decides how a game RANKS?** | **The catalogue, and only the catalogue, from 10 Sep 2026** (task doc **13.1**) — and until then the *adapter* decided it for the one path that aggregates a player's attempts. `chartvolt-games/normalise.ts` held `TITLE_DIRECTIONS`, a hard-coded map of game code to score direction with **two entries**, defaulting anything else upward with a `console.warn`, and gate 11b of `result-ingestion.service.ts` passed its answer to `syncParticipantScore` — which uses the direction to pick the best of several attempts. So a third title shipped by our **own** games service, with nobody editing that platform file, would have had every player on a `best_of_n` contest scored on their **worst** attempt. That is **R62**. **Be precise in both directions, because it reads worse than it was and also better:** it is **not** a reversed leaderboard, since settlement and both boards have always resolved the direction from the catalogue, so the contest would have paid the correct order **of the wrong runs** — and it is not cosmetic either, because on a lower-is-better title the gap between a player's best and worst attempt is the entire point of offering several. **Latent, nothing backfilled** — the only two titles in the catalogue are the two the map listed — and say it precisely: the per-round scores are on `game_round`, so a wrongly-aggregated contest *could* have been recomputed and there simply is not one. **The failure is uniform across every player, which is why nothing looked broken**: no impossible board, no total that fails to add up, nothing thrown, and the old suite asserted that the guess for an unknown title **matched settlement's default exactly** — an assertion that was correct about the code and was pinning the very thing that hid it. **The file's own defence was accurate and did not apply:** `parseCallback` really is synchronous and really cannot await a catalogue read, but the mistake was in the **consumer**, which is async, holds `round.gameKey`, and had already read the row that declares the direction **forty-five lines earlier** for the range check. **Four facts drift easily.** The field **left `NormalisedRoundResult` altogether** rather than being read from the catalogue while still arriving on the payload — a supplier's opinion is an input and never a decision, and R32/R33 established that a duplicated ranking input lets two rows in **one** leaderboard disagree, which is incoherent rather than merely wrong. **`ProviderCatalogueGame.scoreDirection` is deliberately KEPT and a test pins it**, because sweeping it away "for consistency" leaves the platform no source at all, at which point every title ranks upward and a time trial pays the slowest player first. **The provider spec needed no change and checking cost one search** — `01` s3 and the requirements HTML ask for it on the **catalogue** and never on a result body, so the issued contract was right and the per-round copy was ours; **no version bump**. And **the schema task 13 sketches already existed**, almost all of it on `provider_game` since X2, so a document describing this as a new game model is describing something nobody built — `capabilities.attempts` was considered and **deliberately not added**, since `attemptsPolicy` is a contest setting and the one title-level constraint is already forced by `play-shape.ts`, which is the sixth declared-written-dead field avoided rather than created. **9 tests, 8 probes red on exactly one failure**, the eighth **behavioural** because the other seven stay green against a service that calls the resolver and discards its answer. **A second cluster is recorded and NOT fixed**: the global leaderboard, matchmaking, badges and the journey map all rank on `TradeHistory` alone, so a provider-only player ranks last — that is `05` cross-game scoring and risk **X13**, phases X7/X11, and a summary implying task 13 closed it is wrong. **Nothing here is player- or operator-visible**, so there was nothing to verify by eye |
| **Where a game's rules text lives** | **On the title, seeded from the provider, from 10 Sep 2026** (task doc **19.1**) - and until then **four of the six content fields the contract REQUIRES were discarded on every sync**. That is **R63**. `01` s3.1 marks `displayName`, `tagline`, `description`, `rulesSummary`, `howToPlay`, `thumbnailUrl` and `bannerUrl` all `Yes`, and has since the requirements document issued to providers reached version 1.1; `games-service`, our own reference provider, publishes every one of them; the platform stored two. `tagline` and `bannerUrl` had model fields and sat in **neither** sync allow-list. `rulesSummary` and `howToPlay` had no field at all and were **not declared on `ProviderCatalogueGame`**, which is where they were actually lost - an adapter cannot hand over a field the shape it returns has no room for, so they were dropped at parse. **Say the harm in both directions:** it was **live and happening on every sync**, which is rarer here than latent, and **nothing was destroyed** - the values are on the provider, so a re-sync populates them and **nothing was backfilled**. What it cost is that a game page had no rules text to show, which is why `13` s4.1h's **View Rules** button points at `/help/competitions`. **Why nothing caught it is the transferable part: the drift ran DOCUMENT to CODE**, a direction no guard looks - `01` and the requirements HTML were correct and agreed with each other, so the paired-document rule was satisfied and reported nothing, while every test touching the sync asserted the fields it already knew about. **And the model's own comment explained the omission as deliberate**, claiming these were fields "no provider ever supplies" and "in no contract at all", both false for two of the three - the **seventh** instance of an aside being a claim rather than a fact, and the first where the wrong sentence actively deterred the check; it is corrected **in place** rather than tidied into the present tense. All six now sit in **`firstSyncOnlyFields`**, and first-sync-only rather than provider-owned is the load-bearing choice: these are sentences a player reads, so an operator must be able to fix a provider's grammar, tone or language without the next scheduled sync silently reverting the edit - the opposite treatment to `scoreDirection` or `playMode`. **The guard READS THE SPECIFICATION**, parsing section 3.1's own table and requiring each `Yes` row to be stored by a real sync or listed in `NOT_STORED` **with a reason**, behaviourally rather than by matching the allow-list, because a name in an allow-list the schema then discards satisfies a structural check. **Two `Yes` rows are deliberately not stored and saying which beats claiming the contract is honoured**: `displayName` is covered by the sync suite, and `locales` is localisation, which is X11 and unbuilt - storing it now would be the sixth declared-written-dead field. **No change to the provider contract and no version bump**: the spec was right throughout and the code was wrong. 10 tests, 12 probes red on exactly one failure, and **probe 12 adds a seventh required field to the issued spec while changing no code at all**, which is the exact shape of the defect. **The assistant added later the same day may never write either of them** (task doc **19.2**, owner decision): `01` s3.1 calls the rules summary the first text support quotes back in a prize dispute, so generated wording there is invented wording in a money argument about a game the model has never seen. **The bar is enforced by the SHAPE, not by remembering** - `GameContentSuggestion` has no such field and the parser builds its result field by field rather than spreading the reply, so a model returning them produces an object with four keys; the prompt forbidding rules claims is belt to that braces, and it is written as **"do not state"** rather than "do not invent", because a model told not to invent will state the rules it has assumed and consider itself obedient |
| **How is a player who only plays games doing?** | **Answerable from 10 Sep 2026** (task doc **21.1**), and until then the answer was **"this client has no closed trades yet"**. That is **R64**. The admin's per-user **Performance** tab computed eleven figures - trades, wins, win rate, profit factor, P&L, trade ROI - all from `TradeHistory` and `startingCapital`, and then gated **the whole tab** on `totalTrades === 0`, so a games-only player's every ranked round, score and prize was invisible behind one true sentence presented as the answer to *how is this player doing*. `05` **s10** in its plainest form: **no performance figure may silently mean "trading only"**. A second defect sat inside the same gate - `netRoi` is prizes against entry fees, real wallet money and entirely game-agnostic, hidden by a *trade* count. **Live, but a REPORTING defect and never a payment one** - nothing is computed or stored near this screen, so **nothing was backfilled**; a summary calling it cosmetic is missing that it cost the ability to answer the support question the tab exists for, and one calling it a money defect is wrong. **TASK 21's PREMISE IS FALSE WHERE IT POINTS, and that had to be established first**: the only screen actually called Game Performance measures the round lifecycle and names no game, nothing in either app hardcodes Circuit-style metric labels, and the player's own result surfaces have rendered the reported breakdown generically since 7 Sep - so **tasks 22-24 were already satisfied there** and a fix aimed at the task's sentence would have changed the one thing that was right. Third instance after **R7** and **R31**: **correcting a claimed defect downward is the same duty as raising one.** **Task 22's declared performance schema is a deliberate deviation and must not be "finished off"** - a per-category table of metric names is the single failure mode the platform is built to avoid, and it is *also* a second source that can disagree with what the provider sends, so a metric declared and never sent is a permanent blank row while one sent and never declared is real data hidden; the rows rendered **are** the reported `scoreBreakdown`, labelled by the shared `humanizeMetric`, which satisfies task 22's actual requirement by construction rather than by configuration. Task 9's `category` earns its keep as the grouping and labelling key it already is, **never as a metric selector**, and a test forbids a game code in the service or the component at all. **The coupling was DELETED rather than detected**: the admin app must answer *which rounds produced a score* and cannot import `participant-score.service.ts`, which is unmirrored so there is exactly one ingestion door - so instead of a third copy plus a drift test, the status list moved into the mirrored, model-free `round-types.ts` and the ingestion path builds from it (`21` s4.1i's rule). **Four facts drift easily.** **`null` is not zero** - a stored nought is a real score on a points game, so absence renders `-`, the read-side form of R45 and R50. The **raw** score is shown and never negated, because a time trial's best is the *minimum* and is still the positive number the player achieved. A **`voided` round stores `rawScore: 0` deliberately**, so "has a number" is not the test - and on a lower-is-better title that zero sorts **first** and would be nominated as their best round. And the breakdown is **one round's, never an aggregate**, since nothing here knows what any key means. **No money on the screen**, which is RBAC rather than layout: it is granted by `users` while prizes stay behind `analytics` and `financial`. **33 tests, 22 probes red on exactly one failure**, and **probe 1 is the one that matters** because it restores the actual defect - a test merely asserting the panel renders the component is green against it, so the occurrences are counted and their position asserted against the gate. **Two probing lessons, both recurrences**: a probe came back green on the fourth cause, a mutation changing no observable, and then the fixture turned out **unable to express the case at all** - Mongoose's `minimize` default deletes an empty object before saving, so `scoreBreakdown: {}` cannot be stored through the model and had to be seeded with the raw driver. **The player's own dashboard has the same gap and is the natural next slice**, which is why the service is **not mirrored**: a file mirrored before anything imports it is **R42** exactly. **Never verified by eye** |
| **"I have to reload the page"** | **R67, closed 10 Sep 2026** (`13` **s1.1j**), and it is **two owner reports with one cause**. `app/(root)/competitions/[id]/page.tsx` returns the whole screen for a provider contest before the trading markup begins - which is right, and is argued for in the file - so **anything mounted above that markup is silently not mounted for a game**. `CompetitionStatusMonitor` was such a thing, and it is entirely game-agnostic, reading a status, a cancellation reason, a rank and a prize. The game lobby never had it, so `CompetitionEntryButton` went on deciding from `competition.status`, a prop frozen at render, and **a player who had already paid watched the countdown reach zero and had to reload before Play appeared.** Nothing errored and nothing was logged: the screen was not wrong when it was drawn, it had merely stopped being true. This is the trading-shaped-screen shape one layer out from the admin competitions list - **the machinery existed, worked, and was wired to one branch of two.** **Mounting it does NOT fix the second report**, and merging the two is the mistake a summary invites: the monitor fires on a status *change*, and a running contest sits at `active` throughout, so **both** lobbies - trading as well as game - showed the standings that existed when the page loaded. On the trading lobby that is arguably worse, since a rank there moves with the price rather than only when somebody finishes a round, and it was included **deliberately** because the owner's instruction was "all pages related to live data like scoring standings". **`LiveContestRefresher` re-reads the page rather than polling an endpoint, and that is a decision rather than the lazy option**: there is no player-facing JSON API that returns a competition's ranking, because `getCompetitionLeaderboard` is a **server action** and is where the whole rule lives - the direction resolved from the catalogue, the R45 eligibility gate, the tie handling - so an endpoint would be a second reader that can drift from it, the shape behind `referenceId`, `failedReason`, `challengeId` and the Game Master `||`, none of which `check:mirrors` can see. It costs a server render rather than a query, taken knowingly: a lobby is not a hot path, the refresh is visibility-gated, and React preserves client state across it. **It must NOT be mounted on the play screen and that negative is the load-bearing guard** - that page hosts the game in an iframe and owns a 20-second poll that updates state without re-rendering the frame, so a timer refreshing underneath a live round is a way to disturb an attempt somebody **paid** for, intermittently and unreproducibly; it is the obvious next step for anybody fixing that screen's own stale sidebar, so a test forbids it. **Running is read from the stored status, never computed from a clock** - a contest whose end time has passed is still `active` until a cron finalizes it, so a client deciding for itself freezes the board exactly while the last rounds are scored. **13 tests, 12 probes red on exactly the named test**, and one came back **green with the test at fault**: a bare `visibilitychange` match is satisfied by the *teardown* line, so the listener could be left unattached with everything passing - sixth instance of one identifier defeating a structural test. Live and player-visible; no money moved and **nothing was stored, so nothing was backfilled** - the server's answer was right every time it was asked, and the defect is that it was asked once. **Two surfaces were deliberately still stale**: the play screen's standings sidebar, and the dashboard contest cards, where `ContestsSidebar` polled challenges every ten seconds and read competitions from static props. **The cards were closed the next day** (`13` **s5.1b**, 11 Sep 2026) - so that half is correct as history and stale as a present fact, and **say which** - by a narrow endpoint rather than a page re-read, because `getComprehensiveDashboardData` cannot be polled; the play screen's sidebar is still one-shot and the negative guard above still forbids the obvious fix for it. Never verified by eye |
| **"The game says ten minutes and stops me at five"** | **R66, closed 10 Sep 2026** (`21` **s4.1p**), and the report was exactly right. `stateFor` published `endsAt = gameplayEndsAt(round)` - the title's own length from `startedAt` - while `playability` refuses once `expiresAt` has passed, and the platform sets that expiry to `Math.min(now + maxDurationSeconds, playWindowEnd)`. So a player starting a ten-minute sprint with five minutes of contest left watched a clock counting from **10:00** and was stopped with **5:00** on it. **There were TWO wrong numbers, read separately**, and a summary will merge them: `endsAt` drove the ticking clock, while the sentence before Start is composed by `introCopy` from `durationSeconds`, which was the *configured* length - so the player was told ten minutes in words as well as shown them. **The third finding is the transferable one: `hardDeadline` already mins `[expiresAt, gameplay, ceiling]`, is documented as the hard ceiling section 6 requires, and was CALLED BY NOTHING** - the seventh declared-written-dead symbol after `requiresSyncPlay`, `isPaused`, `lastSuccessfulRoundAt`, `family`, `playModeOverride` and the SEO field; **an exported, correct, uncalled helper is evidence that its caller computed the answer itself**, and in all seven cases the thing beside it was wrong. The fix is a **call, not a calculation**, so `playableSeconds` defers to it and a fourth constraint added later reaches the clock, the promise and the gate together. **The platform was already telling the truth**, which makes this a contradiction rather than an omission - `RoundPreflight` discloses `shortenedMs` under `until_window_closes`, so the pre-flight screen and the game showed different numbers for one round. **Not cosmetic, and R48 is why**: once a partial run counts, the clock is the only input to how a player *paces* a paid attempt. **The obvious over-correction is worse than the defect and is probed for twice** - "show the contest's remaining time" fixes the report and tells a player with an hour left that a two-minute sprint runs for an hour; the rule is the **tighter** of the two. Three properties read as fussiness and are not: **anchored on `startedAt`, never `now`**, or the promise shrinks while the player watches; **floored, never rounded**, since an overstatement is a promise the server breaks; and **a promised zero is honoured, not treated as absent**, because a falsy check reinstates the full-length promise at the one moment it is most wrong. **`durationSeconds` is now sent for both titles** - a fixed-set title quotes no time, which is exactly why nobody noticed the contest cuts a Perfect round short too, so the shortening gets its own sentence there. **10 tests, 10 probes red on exactly the named test.** Live and player-visible; scores and payouts were correct throughout, so **nothing was backfilled**. Never verified by eye. **AMENDED 11 Sep 2026: THE FIX IS CORRECT IN CODE AND WAS STILL NOT RUNNING.** The owner reported the identical symptom the next day - a contest with 6m left, the game counting from 10m - and the code was verified correct: the defect is that **R66 changed TypeScript, `games-service/dist` is not tracked in git, and the server was still running its own last build.** Proven rather than assumed - `git show d63c29fd^:games-service/src/rounds/play.ts` returns `const endsAt = gameplayEndsAt(round);`, which ignores the window entirely, which is exactly the report. **`git pull` alone does nothing for this service**, and that is the general rule: **since `21` s4.1i a change under `public/play` needs only pull + `pm2 restart chartvolt-games`, but a `.ts` change needs `npm run build` between them** - and a document repeating s4.1m's "needs no rebuild" as though it covered a source change is describing R52 reproduced. No cache purge is needed: R55's fingerprinted path segments closed that half. **This is the fifth time a games-service fix has appeared to fail because it was not deployed**, so treat "the owner reports the same symptom again" as a deploy question first and a code question second |
| **"When i choose 60 the game didn't finish"** | **R69, closed 12 Sep 2026** (`21` **s4.1t**), and the owner put it first in a message that also rejected the leaderboard. **One line in the game service capped every round at another title's maximum**: `hardDeadline` resolved its ceiling from `PERFECT.maxDurationSeconds` - 600 seconds - whatever title the round belonged to. **Correct by accident** until `12` s2.9 raised Sprint's maximum to an hour on 8 September, at which point every Sprint round was capped at ten minutes however long the operator configured it. **The structural half is why a cap became a HANG rather than a stop, and it is the transferable finding:** `hardDeadline` decides the countdown the player watches, while `playability` - which decides when the round actually ends - **weighed the deadlines itself and never consulted it**. The two agreed for as long as their lists agreed and stopped agreeing the moment the ceiling did anything, so the clock reached 0:00, the client asked the server (rightly, rather than deciding for itself), the server answered *still playable*, and the board sat at zero for the remaining fifty minutes. **A clock that reaches zero and ends nothing is worse than a clock that is wrong**, because there is no state to report and nothing to log. There is now one deadline: `playability` asks `hardDeadline` **whether** and `expiresAt` only **which** wording - a sprint timer running out is `completed`, an unfinished board caught by the contest window is `expired`. Third finding: `findFinishedClocks`' guard against an over-long config was **two constants compared for truthiness** and could not fire, so it now filters on `hardDeadline` too, making the sweeper the **third of three readers - display, gate, sweeper - to go through one function**; a deadline three pieces of code derive separately is three chances for the player's clock and the server's answer to differ, and only one of them ever tells anybody. **An unknown title applies no ceiling** rather than borrowing another one. **LIVE and player-visible**, scores correct throughout, **nothing backfilled**. **The 409 in the pasted log is NOT diagnosed** - most plausibly a platform round already terminal with no stored score, which gate 8 refuses and the route maps to 409; **the R44 cut-off is ruled out** because `unresolved -> expired` is legal - and it could not be diagnosed at all because `attemptDelivery` printed the status and **threw the platform's explanation away**, which is `classify, never merely count` applied to the counter and not to the string the counter replaced. **305 tests (was 299), 14 probes red. Deploy needs `npm run build`** between the pull and the restart, because `dist` is untracked - **R66 was reported twice for exactly this.** |
| **"When i choose custom no box comes"** | **R70, closed 12 Sep 2026** (`12` **s2.13**). **Two correct rules made the option unreachable.** `DurationControl` **derived** custom mode from the stored value matching no preset, and picking Custom deliberately changed no value so that an operator opening it to look did not silently edit the contest. Alone, each is right. Together: the default is ten minutes, ten minutes is a preset, so the click changed nothing, the derived mode stayed false, the select snapped back and no box appeared - **an operator could only use Custom if they had somehow already used it.** Keeping the value untouched was never the problem; **deriving the MODE from it was**, so one piece of state fixes it and both original guarantees survive. It is the shape this programme keeps finding - a control that renders correctly, reports nothing and does nothing - after a provider with no adapter, a `rankingMethod` a provider game ignores, `isPaused` (R41) and the green badge over a refused create (R47). **Fixed with it, and found by reading the docblock that DEFENDS the design:** `resolveExpiry`'s generosity was **borrowed from the gap between the catalogue ceiling and the configured length, and that gap closes** - pick the longest round a title allows and the expiry lands one round after the round was **created** while the game's clock runs one round from **Start**, so every full-length round was cut off by the frame's load time and filed `expired`. Not a wrong payment (a partial run counts, R48), but `lastSuccessfulRoundAt` never refreshes and every full-length round lands in the expiry bucket on the screen deciding whether a title keeps running; `ROUND_EXPIRY_HEADROOM_SECONDS` now states the slack instead of inferring it. **Ten probes across two platform harnesses had been reporting nothing and two guarded these very rules** - nine moved anchors plus two stale **claims**, which the anchor failure hid - and re-aiming them exposed two real test weaknesses, both fixed by strengthening the test: **a slice whose end marker has moved returns -1, so `slice(0, -1)` hands back almost the whole file and every assertion is trivially true** (assert both ends exist), and **a per-branch claim asserted file-wide**, since `contest-preflight.ts` states the same fact as a refusal and as a warning (count the occurrences). **LIVE and operator-visible, nothing to backfill. Never verified by eye.** |
| **"The timer must be the same graphics as trading's"** | **Done 10 Sep 2026** (`13` **s4.1j**), and it is the fourth and last item of that day's owner report. **No defect was found and there is no risk number**, which is worth saying because R65, R66 and R67 all came from the same message: the game lobby's clocks were correct, they were merely small - three lines of `InlineCountdown` text where the trading lobby has a panel of four large cells. So **nothing was backfilled, no money was ever involved, and this is presentation work.** The panel was **extracted** into `components/competitions/CountdownPanel.tsx` rather than copied, because a second copy of a panel that detailed drifts on the first edit and the drift is invisible until somebody holds the two screens side by side. **The APPEARANCE is shared and the CLOCK deliberately is not, which is the whole reason this is two components rather than one reused twice**: trading reads the browser's clock as it always has, while the game lobby's new `ContestCountdown` reads the **server's**, because every rule about when a contest opens, when an attempt may start and when entry shuts is enforced on our servers against our time - so a clock working from a visitor's own computer is wrong in both of the directions that cost something, a button offered when the window has shut or withheld while it is genuinely open, and **neither records an error because neither is one.** `CountdownPanel` is handed a number of milliseconds and knows nothing about clocks at all; a later "consistency" pass unifying them is a behaviour change to a screen nobody was asked to touch, so **there is a probe in each direction** because both read as tidying up. **The second reason not to reuse trading's component is a latent trap worth knowing about:** `LiveCountdown` renders nothing at all for a `type`/`status` pair it does not recognise - including `status: "completed"`, which its own props admit and nothing handles - so a mismatch produces an empty space and no error. **Verified latent rather than assumed**: both existing callers always pass matched pairs, so it is **pinned by a test and deliberately left alone**, since changing it in the same edit as the move would destroy the only evidence nothing moved. **Three things drift easily.** **The negative assertion is the load-bearing half** - "does the screen import the shared panel" is trivially satisfied by one that imports it *and* hand-rolls its own cells beside it, so six of the panel's literals are required to appear in `CountdownPanel.tsx` and in **no consumer**; picking them needed a grep first, because `grid-cols-4` alone appears in the lobby's own hero and `uppercase tracking-wider` appears in about forty files, so a guard built on either would have failed on **correct** code. **An unparseable target must refuse, never clamp** - `splitDuration` treats anything non-finite or negative as zero and **zero is the FINISHED state**, so without the `Number.isNaN` guard one bad stored date renders "Competition has ended!" over a running contest; the same reasoning keeps the target prop wide rather than a narrowed ISO string, since `toISOString()` **throws** and converting at the call site would take the whole page down instead of one panel. And **`RoundPreflight`'s four time readouts stay inline text on purpose** - they are clocks embedded in sentences explaining a *gate*, not the contest, and trading has no large-panel equivalent for them either; likewise the hero tile stays small because trading's hero tile is small, so the test pinning **exactly three** `InlineCountdown` instances is still green and unmodified: this slice **added** a clock rather than replacing one. **18 tests, 16 probes red on exactly one failure**, full suite 2091 passing, typecheck error **lists** identical before and after, `check:mirrors` correctly silent since nothing here exists in `apps/admin`. **Never verified by eye** - both lobbies are behind sign-in |
| **"The rules are not shown anywhere"** | **Fixed 11 Sep 2026** (`13` **s4.1k**), and the cause is one step behind the report. **R63 had made `rulesSummary` and `howToPlay` storable the day before** - they had been demanded of providers, validated on arrival and discarded by a parse bug - and nobody then checked whether anything *read* them. Nothing did: **`getGamePresentation`, the one reader every player-facing game screen goes through, did not name either field in its projection**, so a paying player saw the pot, the entry fee, the clock, the prize split and the standings and **was never told what a winning score was.** On a lower-is-better title that is not a gap in the copy - it is the difference between playing the game and playing it backwards. **Live and player-visible, nothing stored, nothing backfilled.** One panel, `components/games/GameRulesPanel.tsx`, rendered on the lobby and in the arena. **Six facts drift easily.** **This is a different defect from R63 and a document merging them is wrong in both directions** - R63 fixed the sync and made the fields editable, this made them visible, so say which half. **The NEGATIVE assertion is the load-bearing guard**: importing the panel is trivially satisfied by a screen that then hand-rolls its own rules block beside it, which is how two screens describe one game's scoring differently, so the two block headings must appear in the panel and in **no** consumer. **That negative had to be narrowed for the `13` s4.1g reason** - the first version forbade "is scored" anywhere in a consumer and **failed on correct code**, because the lobby's `UNRESOLVED_POLICY_COPY.score_zero` legitimately says "it is scored zero and the competition still settles on time"; a guard that fires on correct code is the kind the first person it inconveniences deletes. **Only the scoring rule gets the emphasis and the asymmetry is deliberate** - a player can discover the controls by trying them and cannot discover the *direction* by any amount of playing - so the amber treatment is asserted to appear **exactly once** and **before** "How to play". **An absent field renders nothing, not a heading over an empty box**, and that is the common case rather than an edge one: every title synced before R63 carries neither value, so **until task 17's catalogue re-sync runs this panel is invisible on every contest** - a stored `""` is normalised to absent and the panel trims before deciding. And **the lobby's own `ProviderGame.findOne` was collapsed onto the shared reader as part of the deliverable**: with its own projection and its own hand-written `.lean<{...}>` generic, adding a field to `GamePresentation` shows the rules on the play screen and silently not on the lobby, which is **task 20.1's defect exactly** - two hand-written projections of one document mean a new field arrives `undefined` at whichever caller nobody remembered, with nothing failing. Two pre-existing tests pinning the old projection were **flipped, not deleted**. **11 new tests, 13 probes red on exactly one failure.** Two things are **recorded rather than built**: `ArenaContestPanel`'s "Round time" tile still states the title's nominal length, because `RoundPreflight` already discloses the shortened figure on the server clock and the game's own countdown is authoritative, so a third server-rendered statement of one clock would be stale the instant it was drawn; and the footer strip deliberately stays a help link, which is a different thing from the rules of one game. **Never verified by eye**, and here for a second reason - there is nothing to render until the catalogue is re-synced. **AMENDED TWICE ON 11 SEP 2026 and two facts here are correct as history only, so say which**: the arena's copy of this panel became the compact `strip` and **no longer carries the scoring rule at all** (`13` s4.1u, owner's instruction), and the lobby's copy **no longer names the game in its heading** (`13` **s4.1v**) - both screens read `How it works`, because `gameName` is operator free text of unbounded length and the live title carries its tagline inside it. The rule itself is unchanged and is still on the lobby in full |
| **"Show what each player solved"** | **Built 11 Sep 2026** (`13` **s4.1n**), after the owner rejected the previous day's arena redesign outright - *"you didn't do anything I ask... this is unacceptable"* - and **the rejection was right**: that slice made three structural corrections, left the screen looking like the application's neutral shell beside a bright mock, and answered a question nobody had asked (where the repository boundary runs) instead of the two that were. **THE FINDING IS THAT A PLAYER'S PROGRESS WAS ALREADY ARRIVING AND NO BOARD READ IT.** Every game reports a `scoreBreakdown` beside its score - `{ boardsCompleted, boardsAttempted, fastestBoardMs, ... }` for this catalogue - and `game_round` has stored it since X3; it was read by exactly two screens, the player's own results page and the admin per-user performance tab, and **never by the contest board, which is where every player looks.** So a summary describing this as a provider change, a spec change or a version bump is wrong on all three: it is a read that was never written, and it closes the previous entry's claim that the RECENT PLAYERS feed had no data source - **that sentence was wrong and is corrected in place rather than tidied away.** **No risk number and nothing was backfilled**: nothing was computed wrongly, a figure that existed was not shown. `lib/services/games/contest-activity.service.ts` is the read, `lib/utils/round-activity.ts` turns a row into words, and both boards plus a new `ArenaActivityFeed` consume them. **Four facts drift easily.** **THE PLATFORM MUST NOT CHOOSE AMONG A GAME'S METRICS** - `01` s3.2 makes the breakdown display-only, free-form and **ordered by the provider**, so the entries are handed over in the game's own order and filtered only by whether a screen can render the value; a table saying *for a puzzle show `boardsCompleted`* is a `switch` on game code wearing a different hat, and it makes the no-developer-needed claim false for the next title while every existing test still passes, so a structural test forbids any metric name, game code or provider key in all four files and a probe reorders a breakdown. **Which rounds carry a number is decided once, in the service**, because `roundContributesScore` is the ingestion path's rule: a **`voided`** round stores `rawScore: 0` **deliberately**, so passing it through puts a number beside a cancelled round and, on a lower-is-better title, **the best one on the board**, while an **`unresolved`** round is `unresolvedRoundPolicy`'s question and answering it here answers it twice - guarded **negatively on all three consumers**, which is the load-bearing half, since a service that computes it correctly is trivially satisfied by a component that then decides for itself. **`expired` wording describes the clock, never the player**: `createRound` clamps `expiresAt` to `playWindowEnd`, so it is the **ordinary** ending for anybody still playing at the whistle and since R48 those runs count, which makes "gave up" a sentence blaming a player for attending. And **an absent activity map renders no second line at all**, which is not the same as rendering "not played" - a caller that has not read the activity is saying *I do not know*, and answering *nobody has played* on its behalf is false for every row. **Two deviations are recorded rather than absorbed**: the feed is in the **sidebar** rather than the reference's three-panel bottom band, for the reason the previous slice learned the hard way (a layout cannot see that its child rendered nothing, and the band's other two members return `null` until the catalogue re-sync); and **the score in the feed is printed plain with no `+` sign**, because the mock's `+240 ⚡` is right for an upward game and exactly backwards for a time trial, where the direction is resolved once server-side. **28 tests, 13 probes red on exactly 1 failure**, two of which had to be re-aimed first - **`DID NOT APPLY` means the target moved, never that the run was quiet.** **The artwork the owner explicitly asked to be recreated is NOT built**, nor is the games-service half, and Hint, Undo, Moves/Best Time/Combo, the LEVEL bar and the GLOBAL/FRIENDS/COUNTRY tabs still have no data source on either side. **Never verified by eye** |
| **"It is not showing live the boards the user finished"** | **Built 11 Sep 2026**, in two halves the same day (`13` **s4.1o** then **s4.1p**) - and a document calling it half built is correct as history and stale as a present fact, so **say which**. **The second half is the protocol change s4.1o filed as its own piece of work:** there was **no mid-round reporting anywhere on either side of the seam**, so `POST /api/games/providers/{key}/progress` now writes `game_round.scoreBreakdown` and a new `progressAt`, our games-service fires one after each solved board, and the requirements document went **1.4 -> 1.5**. **The whole design is one sentence - it must not become a second scoring door**: a separate route rather than a `final: false` flag (a flag is how the second door opens), an explicit two-path `$set` rather than a spread (**a spread is how the next field arrives, and the field after that is `rawScore`**), `status` read and never written, and a sanitiser keeping primitives only and 24 at most, because an unbounded breakdown written once per solved board is **a store that fills up by succeeding**. **The read side needed almost nothing** - `getContestActivity` already passed a live round's breakdown through, so the board and the feed show progress with **no component change**; the one edit is `activityAt`, where `progressAt` sits above `startedAt` or every live player's feed entry stays frozen at the moment they pressed Play. **The game's half is the opposite of `deliver.ts` deliberately** - no retry, no queue, a 3s timeout and `void` rather than `await`, because a lost report costs one stale line for seconds while **retrying is strictly worse than not**, a queue of stale reports behind a finished round being a board that goes backwards, and the `void` sits between a player solving a board and being handed the next one in a round they **paid** for. **Refusals are not logged**, this being the highest-rate provider route by a wide margin. **Nothing backfilled** - it is a capability that did not exist. **Two deploys, not one**, TypeScript having changed on both sides. Below, correct as history: **Half built 11 Sep 2026** (`13` **s4.1o**), the second of the three things the owner named when they rejected the arena a second time - and **the half that is built and the half that is not are worth stating separately, because a summary will merge them.** **Nothing was computed wrongly and there is no risk number**: the board and the per-player activity line were both correct and both were **server props rendered once**, so a round that landed while the player sat at the game appeared only after a reload - **nothing backfilled, no money involved.** **THE OBVIOUS FIX IS FORBIDDEN HERE AND THAT IS THE WHOLE DESIGN CONSTRAINT**: `LiveContestRefresher` is what every other contest surface uses and a test bans it on this page, because the arena hosts a live round in an iframe and a `router.refresh()` timer underneath an attempt somebody has **paid** for can disturb it - so the rail polls `GET /api/competitions/[id]/standings` and swaps only its own two panels. **THE SAFETY PROPERTY IS A PROPERTY OF WHERE THE STATE LIVES, not of what it contains**: `ArenaLiveProvider` takes the rest of the arena as `children`, and a `children` element handed down from a server component is the **same object** on every re-render, so React reconciles it by identity and never descends into it - which means making `ProviderRoundHost` a *consumer* instead of a child turns a board refresh into a reloaded game, so that is asserted rather than commented and a probe injects exactly that mutation. **One producer, because agreement beats liveness** - the `dashboard-live` rule in a new place: the page and the route both call `getArenaStandings` and **neither reads the board or the activity itself**, since any field where the poll and the first render differ reads to a player as *the value having changed*, and two compositions is precisely how they come to differ. **An endpoint is a second reader only if it composes an answer of its own**, which is why `LiveContestRefresher`'s header claim that "there is no player-facing JSON API that returns a competition's ranking" is now false and **corrected in place rather than tidied into the present tense** - its conclusion still holds, its reason is narrower. Three smaller rules each have an inverse that reads as correct: **liveness comes from the stored status, never a clock in the browser**, because a contest whose end time has passed is still `active` until a cron finalizes it and a client deciding for itself freezes the board exactly while the last rounds are scored; **a bad response leaves the last good board on screen**, because an empty rail here says *nobody has played*, a false statement about a contest in progress rather than a missing one; and **the count pill became a consumer**, `standingsCount` going from a number to a `ReactNode`, since a count rendered once disagrees with the list beneath it the moment somebody joins. **7 tests, 15 probes red on exactly 1 failure**, three of which had to be re-aimed - two because the mutation injected an *import* where the assertion bans a **call** (the fourth cause of a green probe, a mutation with no observable) and one because the harness was **not parameterised on the suite**, so `-t` matched nothing and vitest reported a passing run over zero tests. **WHAT IT DOES NOT FIX IS THE OWNER'S THIRD POINT**: there is **no mid-round reporting anywhere on either side of the provider seam** - a score exists only after `finishRound`, the frame's `postMessage` carries none by construction, and the games-service's own per-board record never leaves that database - so a player still at the board reads "Playing now" however many boards they have solved, and closing that needs a signed progress callback writing only `game_round.scoreBreakdown`, which is a protocol change and its own piece of work. **Never verified by eye** |
| **"The round header, the board, Hint/Undo/Clear and SUBMIT SOLUTION... do as well"** | **Built 11 Sep 2026** (`21` **s4.1q**), and it is the owner's explicit authorisation of the half `13` s4.1m had recorded as belonging to **games-service** - a separate repository sharing no code with this one by design (`npm run check:isolation`). **NOTHING WAS BROKEN and there is no risk number**: the surface was readable and correct after s4.1f and s4.1m, it simply was not the reference, so a document describing a fixed calculation, a wrong figure or a defect of any kind is describing s4.1p rather than this - **nothing was backfilled** and no money figure exists near this code. What shipped is a round header of three boxed cells, a vertical `Undo / Clear` rail, a coverage meter with stat tiles beside the board, and one wide `Submit solution`. **The two rails beside the board were moved later the same day** (`21` **s4.1r**) because a board in that frame is **width-bound** and they were taking a third of the width - so a document describing the arena as rail, board and rail is correct as history and stale as a present fact, and **say which**. **THE TWO REFUSALS ARE THE LOAD-BEARING PART and a summary will drop them.** The reference puts a running **SCORE** in the header: `PlayState` carries no score, no rank and no prize, deliberately, and `resultCopy` is pinned by a test that hands it all three and asserts none reaches the screen - so a score cell could be filled only by **computing one here**, a second scoring authority and the single thing the provider seam exists to prevent, or by **sending one down**, which is a protocol change and the owner's decision rather than a styling one; a probe injects the figure somebody would actually write, a plausible `solved * 250` derived from real facts that nothing authoritative agreed to. And the reference's **HINT with a count of three is forbidden outright** by the platform's fairness rule - it tells a player something about the solution they had not worked out, which improves a score in a paid contest, and a consumable count is exactly the marketplace mechanic that rule names - so a probe adds `hintState` beside `undoState` and the test goes red. **`LEVEL` and `Combo` have no source either**, and the honest reading of the LEVEL bar is the board's own **coverage**, which is a real fact the client holds. **Undo IS built and the distinction is the argument**: it is **strictly weaker than the Clear button that has always been here**, the same position being reachable by touching that pair's terminal and drawing it again, so it changes no rule and reveals nothing. Three findings from it. **The order is the order pairs were DRAWN, never the order they were joined** - a player who abandons a route half way has still drawn it, so keying on "joined" removes something from several moves ago, and the test asserts **which** pair survives on a hand-written grid, because counting alone passes either way and a generated puzzle cannot answer it. **A guard that read as careful was UNREACHABLE and it made two real resets unprobeable** - the first `lastDrawn` walked backwards past entries whose paths had gone, nothing can empty one path without removing its entry, and with the walk in place **removing `drawOrder = []` from `clear` changed no observable at all**: two guards each covering for the other, R42's shape, **deleted rather than probed**. And **`undo()` must RETURN false rather than merely doing nothing**, because `app.js` plays the clearing sound on a true. **Two layout decisions would have been wrong the obvious way**: `chromeHeightOf` **measures the arena and subtracts the board**, so the rails count as chrome when they stack and cost nothing when they sit beside it, with no media query restated in JavaScript; and the phone rearrangement uses **explicit `grid-row`s in a max-width query, never `order`**, because grid auto-placement follows *order-modified* document order - the mistake already made once on the platform's own arena layout. **15 new tests (285 in the service, from 270), 48 probes red on exactly the named test**, four green first time on two known causes. **Deferred and not to be summarised as done**: Best Time as a record across rounds, Combo, the LEVEL number and the GLOBAL/FRIENDS/COUNTRY tabs have no data source on either side. **Deploy: `public/play` needs only a pull and a `pm2 restart`** - the served set is read from the directory at boot and the assets carry a fingerprint - but **`npm run build` is still required in the same movement**, because R66 and the progress callback are TypeScript and have never been built on the server. **Never verified by eye** |
| **"The names are cut of and we dont have the avantars of the player"** | **Fixed 11 Sep 2026** (`13` **s4.1s**), and it is **two defects wearing one screenshot** - merging them is the mistake a summary invites. **THE AVATARS WERE BUILT, WIRED, TESTED AND PROBED THE DAY BEFORE AND COULD NEVER HAVE APPEARED** (**R68**): `getUsersByIds` filtered on the `id` **field** alone while Better Auth's MongoDB adapter keeps an account's identity in `_id`, so the query matched nothing, the map came back empty, every row correctly had no picture and `NeonAvatar` correctly drew initials - **no error, no log line, every layer reporting success**. **The evidence was ten lines above it**: `getUserById` has tried `id`, then `_id` as an ObjectId, then `_id` as a string since it was written, because one was not enough; the batch version had one of the three, and the platform's global leaderboard shows faces only because it goes through `getAllUsers()`, which applies **no id filter at all**. **Live and player-visible on both game boards, nothing stored, nothing backfilled**, and **no other screen was affected** - the lookup has exactly one caller, added the day before, so this arrived with the feature that first used it rather than being a long-standing hole. **The guard has to be behavioural and that is the transferable part**: `{ id: { $in: ids } }` reads perfectly and no structural assertion could call it wrong, so the test seeds three real documents - `id` field, ObjectId `_id`, string `_id` - and asserts all three are found **under the id the caller passed in**; the map is keyed under both ids, because a document fetched through `_id` whose `id` says something else would otherwise be found and then **lost on the way out**, the same silent empty answer one step later. A fix handling only `_id` is the same defect facing the other way, so that case is asserted too and a probe removes each half separately. **THE NAMES WERE CUT OFF BY THE ROW'S OWN FURNITURE, NOT BY THE RAIL** - which had already been widened 280 → 300 → 360 by counting characters, twice. On the name line beside the name sat **a second gold crown**, while `NeonRankBadge style="plates"` was already drawing one two columns to the left, and a **`= #1` tie chip** repeating the rank that plate states. Both facts are **still on the row** - the crown in the plate, the tie beside the score it is a statement about, as a bare `=` with the long form on a `title` - which is the test for removing furniture rather than shrinking it. Beside that the two numeric columns were cut to the width of a five-figure score and a `12:34` clock, the plate column to the plate's own size and the gaps from 8px to 6px: about **40px handed back** to the one column whose content has no upper bound. **The tie marker is counted and its position asserted**, because a second copy left on the name line satisfies any positional check aimed at the first - the shape that has defeated four structural tests here - and the numeric widths are counted because the template is written twice, once for the heading and once for the rows, so a widening applied to one misaligns every row while reading correctly in a diff. **One pre-existing assertion had to be re-pointed and the claim is unchanged**: it sliced backwards from the score to the nearest `<span`, which was the score's own until the tie marker moved into that cell and became the nearest one, so it began failing on a **correct** file - **a slice taken backwards finds whatever construct happens to be closest**, the third time that has cost a false result here, and anchoring on the token cannot drift. **2242 tests green, typecheck at the 194 baseline exactly, lint clean, 25/25 avatar-and-leaderboard probes red on exactly one failure.** **Never verified by eye** - the arena is behind sign-in, and the avatar half cannot be seen at all until this runs against a real database |
| **"Boards done needs to replace solved... rearange bottom info to be alling"** | **Built 11 Sep 2026** (`21` **s4.1s**), the games-service half of the same message as the row below. **NOTHING WAS COMPUTED WRONGLY and there is no risk number** - the owner had spotted a **duplicate**, not a wrong number: s4.1q's header cell said `SOLVED` and s4.1r's figures strip said `BOARDS DONE`, two correct statements of one count in two wordings. **Which copy survives is the whole question, and the wording is the evidence**: `BOARDS DONE` names the unit the score is made of, so the header keeps the cell and takes the clearer label while the strip loses the tile - **the opposite of the tidier-looking change**, which deletes the header cell and thereby moves the figure a player watches out of the row they watch it in. **"Align" was TWO faults and the second is the one a diff hides**: the boxes were already the same size, and what differed was where their captions sat - the coverage meter centred its caption while every tile beside it started at the left, because `.screen` centres text and only the tiles said otherwise, and the meter put label and figure at opposite ends of one line where a tile stacks them, so one figure in a row of four sat at a different height. A document describing this as a sizing fix is describing the half that was already right. **A fixed share is only ever right for one tile count**, so the strip is two flex children - the meter and a grid of equal tile columns - with `app.js` publishing the count as `--tiles` and the meter claiming one part of `1 + n`; `playStatTiles` omits a tile rather than rendering an empty value, so the count genuinely varies and a hard-coded three-column rule loses this silently. **A guard naming only the surviving label is satisfied by a build that still has both**, so the probes restore the header's old label and the strip's old tile separately. **6 new tests (299 in the service, from 297). Verified by eye** through `tools/smoke-play.ts`. **Deploy: `public/play` needs only a pull and a `pm2 restart`** - though `npm run build` is still owed for R66 and the progress callback |
| **"They dont align, one bigger than the other... have images next to letters icons"** | **Built 11 Sep 2026** (`13` **s4.1t**), the owner's third look at the arena's bottom band, plus *"when i have mouse over the buttons need to have the hand"*. **NOTHING WAS COMPUTED WRONGLY AND THERE IS NO RISK NUMBER** - a short panel, an absent picture and an arrow cursor all render perfectly and report success - so every guard here is structural and **nothing was backfilled**. **THE WRAPPERS WERE NEVER THE PROBLEM, which is why the obvious fix is a no-op that reviews as correct**: the band's three slots are flex items and a flex item stretches by default, so they were already level while the owner was looking at panels that were not - each **panel** sizes to its own text and leaves the rest of its stretched wrapper empty. `[&>*]:h-full` reaches through; `h-full` on the wrapper itself changes nothing, and a guard asserting the band "mentions `h-full`" is green against it, so the probe restores it verbatim. It belongs on the wrapper rather than in the panels because **two of them are also rendered in the lobby sidebar**, where a forced full height would stretch one card down the whole column - and `empty:hidden` and `flex-wrap` are asserted beside it, because s4.1r's revert is still the governing fact. **THE PICTURES ARE OURS, NOT THE PROVIDER'S, and that decides everything else about them**: `howToPlayImageUrl` and `highlightsImageUrl` on `provider_game`, uploaded through the existing artwork route and edited in the content dialog under their own heading, in **no sync allow-list at all** - not even `firstSyncOnlyFields` - so no provider is asked for them, `ChartVolt-Game-API-Requirements.html` is unchanged and **there is no version to bump**. **The fallback is the point of the component rather than a courtesy**: no title carries either image, so the natural implementation - render the `src`, render nothing otherwise - is green against any test that supplies a URL and ships a band illustrated on nothing and broken-looking on everything; `NeonIllustration` draws a two-ring emblem instead, so an unset value is **a deliberate look and an upload replaces it rather than filling a hole**, the same arrangement as `bannerUrl` falling through to `banners.ts`. **The glyph is chosen by the calling panel, never by the title** - the rules panel draws the rules glyph whatever game it is describing - and the emblem **carries no caption**, asserted as "no text-bearing element and no default `alt`" rather than as a list of banned nouns, because a noun list here would have to permit `classes.tile` and `aspect-square` and a guard forced to make that exception will eventually make the wrong one; note this component is in `components/neon/`, which the arena's own agnostic guard does not read. **A condition that would have looked careful is a defect**: the arena passes `layout="column"`, so gating the picture on `"wide"` reviews as considered, passes any test that only checks it exists in the file, and leaves it off the one screen it was asked for. **The hand cursor is Tailwind v4's preflight, not a per-component omission** - every button in both apps lost it on the upgrade and `globals.css` had been putting it back one component class at a time, which is why it kept coming back - so it is now on both `button.tsx` copies and on `components/neon/Buttons.tsx`'s `BASE`, with `disabled:pointer-events-none` and `cursor-not-allowed` asserted alongside, because a hand over a disabled control promises a click that does nothing. **13 tests, 15 probes red on exactly one failure**, full suite 2255 green, typecheck at the 194 and 223 baselines exactly, lint clean, `check:mirrors` OK. Three probes came back green first time **on three different causes** - a mutation with no observable (the caption probe replaced a comment), an import is not a use (`indexOf("NeonIllustration")` found the import on line two, so the layout gate's slice examined the file header and the exact injected defect passed), and a weak test (the projection guard counted occurrences, and deleting the field from the `.select(...)` left three other mentions standing). **Never verified by eye**, and for a second reason on two of the three panels: they render nothing at all until the catalogue is re-synced. **AMENDED later the same day by `13` s4.1u** - the owner rejected the result as ~600px tall against a reference measuring 986 x 103 - so two facts here are correct as history and stale as present facts, and **say which**: `[&>*]:h-full` now sits beside a fixed `sm:h-24` on the row rather than alone, and **both pictures are fixed small squares beside the copy** rather than flexible blocks under it. The `layout === "wide"` warning is unchanged and is now one of three layouts |
| **"STOP. The current implementation is structurally wrong"** | **Built 11 Sep 2026** (`13` **s4.1u**), the owner's **fourth** rejection of this band, and the count is part of the finding. **THE MEASUREMENT IS THE SPECIFICATION**: the reference is **986 x 103 pixels** and *"your current implementation stretches the section to approximately 600px+ high. THIS IS WRONG."* **NOTHING WAS COMPUTED WRONGLY AND THERE IS NO RISK NUMBER** - a 600px band renders perfectly and reports success, the only witness is a screenshot, which is why it took three earlier passes before a guard existed - so every assertion is structural and **nothing was backfilled**. **ONE NUMBER DOES NOT ENFORCE A HEIGHT, AND THAT IS THE TRANSFERABLE PART**: `sm:h-[104px]` is the height, the spellings the owner named (`min-height`, vertical `flex: 1`, `height: 100%`, `grid-auto-rows: 1fr`) are asserted **absent** because any one of them re-inflates the row while the fixed height stays in the file reading as though it still governs, and **each card caps its own items** - three steps, four tips, three feed rows - because `overflow-hidden` alone is a card silently dropping the operator's fourth tip with nothing on screen to say so. **104 IS ARRIVED AT, NOT CHOSEN, and the first attempt at 96 was wrong in a way nothing would have reported** - `NeonIllustration` takes its height from its **width** through an aspect ratio, so the owner's 65-75px picture needs 66 of a body that 96 left at 49, and it would have been cropped by the `overflow-hidden` backstop on the two acceptance points that ask whether the pictures are there; caught by doing the arithmetic for the self-check rather than by any test, which is why a card here writes **a width and never a height**. The counts are not arbitrary either: a 104px card less its 30px `dense` heading is 74px of body, which is three 24px rows - and **both halves of a row height are asserted or neither is**, the feed's 20px avatar plus `py-0.5` being 24 while one step up to `py-1` pushes the third row out of sight with `FEED_LIMIT` still reading 3 two lines away. They are the reference's own counts because that is what the owner measured. **A BAN ON A SUBSTRING FIRES ON CORRECT CODE** - `not.toMatch(/h-full/)` failed on the required `[&>*]:h-full` the first time the suite ran, so the rule is a **count** of occurrences, the s4.1g lesson again. **THE GRID IS A RECORDED DEVIATION FROM THE OWNER'S OWN CSS**: he supplied `grid-template-columns: 1.15fr 1.15fr 1fr` and the build uses flex bases dividing the row identically at 34/34/32, because s4.1r's trap still governs - a hidden grid item leaves its track behind and puts one panel adrift beside an empty third of the page, which is the common case. **TWO RECORDED DECISIONS WERE REVERSED AND BOTH COST SOMETHING.** `What to expect` became `Game tips` on his explicit override, and the original argument survives the rename - the seeded highlights still read as marketing, so the card promises tips and shows selling points until somebody rewrites them. And the scoring rule left the arena, which `13` **s4.1k** put there on the owner's own earlier instruction: it is **not deleted**, the lobby carries it in full and every route into the arena passes through the lobby, but **a player halfway through a contest cannot re-read it without leaving the board** - stated rather than smoothed over. **THE REFERENCE'S WORDS ARE NOT IN THE CODEBASE**: the steps and tips he listed verbatim are the operator's `howToPlay` lines and `highlights` titles read from the catalogue, the arena's agnostic guard forbids a game-shaped sentence in a quoted string, and a probe hard-codes his own steps to prove it. **TWO OF THE TEN ACCEPTANCE POINTS ARE ANSWERED BY DATA, NOT CODE, AND THE HINTS ARE WHERE THAT GETS SAID** - the two uploads are in swapped slots in the live catalogue, and the strip numbers each **line** of `howToPlay`, so his three instructions appear the moment an operator writes three lines while today's one-paragraph copy correctly draws one; the field accepts both and saves both, so the alternative to saying it beside the field is a screen that looks wrong for a reason that is nowhere on the screen. `ARENA_STEP_LIMIT` and `ARENA_HIGHLIGHT_LIMIT` are those numbers, and a test reads the literals out of the admin and player files and asserts they agree, because `apps/admin` cannot import from `components/games/` and a number that drifts makes the hint a lie. **A dead prop was deleted rather than left** - `ArenaHighlights`'s `layout?: "strip"`, narrowed to one value and read by nothing, the declared-written-dead shape on the `shouldBlockEntry` precedent. **29 tests, 32 probes red on exactly one failure**, full suite 2283 green, typecheck at the 194 and 223 baselines exactly, lint clean, `check:mirrors` 80 agree / 0 drifted. Two probing lessons: one came back **GREEN on the fourth known cause**, since `flex-col` puts the picture under the lines while leaving document order untouched, so the flex **direction** is now asserted too; and six reported `PROBE BROKEN - no test matched`, because vitest's `-t` is a **regular expression** and five test names carried apostrophes - a fragment matching nothing produces a passing run over zero tests, which reads exactly like a guard that does not work, so the names were made regex-safe rather than the probes escaped. **Never verified by eye** |
| **"Change also this How Circuit Sprint: Fast and Fun Spatial Puzzles is scored"** | **Built 11 Sep 2026** (`13` **s4.1v**), and it exists because of a sentence in the report of the row above: told that the scoring rule was *"not deleted - the lobby carries it in full"*, the owner went and looked at the lobby. **IT IS THE SAME TWO FAULTS ONE PAGE ALONG, and that is the transferable part** - a heading running the width of the page, and the operator's small graphic drawn beneath the text at the cell's full width, so a panel of two short paragraphs was a screen tall with a hero image in it. Both had just been corrected on the band, and both were **left untouched in that commit on purpose**, because the instruction said *"do not modify the rest of the Circuit Sprint page"*: **a fault corrected on one screen is worth looking for on every other screen rendering the same component**, since the reason it was scoped out is a reason about the request rather than about the code. **NOTHING WAS COMPUTED WRONGLY and there is no risk number** - a page-wide heading and an oversized picture both render perfectly - and **nothing was backfilled**. **THE HEADING FAULT IS NOT THE LENGTH OF ONE NAME**: `How ${presentation.gameName} is scored` reads correctly against a short name and produced `How Circuit Sprint: Fast and Fun Spatial Puzzles is scored` against the live one, because `gameName` is **operator-editable free text of unbounded length** and this title carries its tagline inside it - so **interpolating a field like that into a sentence is the fault**, any title with a subtitle reproduces it, and shortening this one name would have fixed nothing. Both screens now read `How it works`, which is also one fewer thing that can differ between them, and the name survives on the picture's `alt` where a length is harmless; the two assertions pinning the template were **flipped to forbid it rather than deleted**, because the sentence *"the lobby has the room and is where a player is deciding whether to pay"* was believed for a day and is the reason nobody looked. **A WIDTH AND A POSITION ARE TWO DEFECTS AND ONE FIX IS NOT THE OTHER** - `NeonIllustration` takes its height from its width, so uncapped in a 300px cell a 4/3 graphic is 225px tall, and `w-[132px]` is a width and never a height (s4.1u's rule); but a capped picture still **below** three lines of text adds its own height, so the body became **one row** rather than a two-column grid, which was the other half of the emptiness the owner drew - two columns make the panel as tall as its tallest cell, so the short one was a column of nothing. `md:grid-cols-2` is asserted **absent** as well as the row being present, because both can sit in one file at once and an inner wrapper left behind restores the taller shape while reading as harmless, and the position guard asserts the **flex direction** rather than document order, a `flex-col` being the fourth known cause of a green probe and having already cost one in the band's suite. **A THIRD DEAD LAYOUT DELETED** - `layout` offered `column`, which was the default and which no caller ever passed, so it was a stacked layout nobody could see; deleted and the prop made **required**, because a default is how a third unreachable branch arrives without a caller, the second instance in two days after `ArenaHighlights`. The picture also moved to `contain`, for the band's reason: these two uploads are graphics rather than photographs, so a crop takes the corners off a badge, while `cover` stays the kit's default because the slots that came first are a logo and a hero banner. **3 tests, 4 probes red on exactly one failure**, full suite 2287 green, typecheck at the 194 and 223 baselines exactly, lint clean. The harness gained a **per-probe `Suite`**: these guards live in `game-rules-panel.test.ts` rather than the band's own suite, and run against the default one a probe reports "no test matched", which reads like a broken harness rather than a moved target. **Never verified by eye** - the lobby is behind sign-in |
| **"The current banner is far too tall"** | **Built 11 Sep 2026** (`13` **s4.1w**), the owner's hero reference beside the built one, and it is the **second rebuild of this component in a day with the same fault both times** - the header kept growing until it was the largest thing on a page whose entire purpose is the board below it. **NOTHING WAS COMPUTED WRONGLY and there is no risk number**: a 400px banner renders perfectly, throws nothing and logs nothing, so the only witness is a screenshot and **nothing was backfilled**. **A MINIMUM HEIGHT IS A FLOOR CONTENT IS FREE TO EXCEED, and that is the whole mechanism** - `min-h-[220px]` and then let the content decide, after which every addition was individually reasonable (a badge, a heading wrapping to two lines because the catalogue's name carries its own subtitle, a three-line description, the contest's name, three bordered cards) and the sum was the screenshot, **with no rule broken anywhere**; it is the bottom band's fault one element up, which is why the height is now **fixed at 118px** and `min-h-`, `h-auto` and the old `p-5 sm:p-7` are asserted **absent** - a floor beside a ceiling is the ceiling losing while still reading as though it governs. **`sm:` only**, because a phone has no room for a logo, five lines and four features on one row and a fixed 118 there would crop the heading instead. **THE ARTWORK DECISION IS ARITHMETIC, NOT TASTE**: the banners are 1280x720 and `object-cover` across a 1350x118 panel shows a **16%-tall horizontal slice**, so the trophy the owner asked to keep would have been a band of glare - and under `cover` a box wider than 1.78x its height never crops horizontally at all, so **no value of anything makes a full-bleed 16:9 background work in a 118px strip**. Two passes instead: a dim full-width copy for continuity, then the picture at 300px tall inside a **330px window anchored right**, cropping it to the half carrying the trophy and the pot. **Their order is load-bearing and asserted by position** - absolutely positioned siblings with no z-index paint in document order, so written the other way the wash covers the piece and the trophy disappears behind a 40%-opacity copy of itself, which looks like a dim banner rather than like two layers in the wrong order. **ONE OF THE FOUR REQUESTED LABELS WAS REFUSED and it must not be quietly added**: `BIG REWARDS` is a promise this banner cannot check - what a contest pays depends on its prize pool, the hero receives no prize figure, and a free contest would carry the phrase too - so the trophy slot holds `Skill based`, the platform's own guarantee, and a probe puts the reference's wording back. The other three are **derived rather than written**: `Fast rounds` from `maxDurationSeconds`, which is **a ceiling and not a length**, so at or under five minutes it claims shortness without a figure, above it says "up to", and undeclared says **nothing**; `Real players` prefers the contest's own `2-100 players`, a figure being something a promise cannot be; `Global leaderboard` is true of every contest and survives the agnostic guard because `\bboard\b` does not match inside "leaderboard". **Four is not padded to** - a title declaring nothing renders three. **The features are icon and label with no box**, the previous bordered cards being what made three small facts read as a second section, and the 8px labels are **a recorded deviation** from the kit's 11px `NEON_LABEL`, which wraps "Global leaderboard" to three lines in a 68px column and takes the banner with it. **Every line of copy is clamped and the clamps are COUNTED**, four of the five being operator fields with no length limit, where one wrap pushes the line below it into `overflow-hidden`. **The two-line title comes from splitting the name at its own colon and nothing else** - the s4.1v fault one field along - so any title punctuated that way splits, any other is one heading, and nothing is dropped either way. **The contest's name left the banner deliberately**, having repeated the `Back to {name}` link directly above it, with the guard running in **both** directions. **15 tests, 18 probes red on exactly one failure**, full suite 2302 green, typecheck at the 194 baseline exactly, lint clean, `check:mirrors` OK. Two probes were green first time and **both were the test's fault rather than the claim's**: `overflow-hidden` also sits on the artwork's window, so a banner-wide match stayed green while the panel itself let the copy spill - the fifth time one identifier has defeated a structural test here - and a slice anchored on the bare token `NEON_PANEL_LIT` opened at the **import** two hundred lines early while still satisfying its own length assertion. **Never verified by eye**. **AMENDED LATER THE SAME DAY BY `13` s4.1x and four of these facts are correct as history only, so say which**: the height is **150, not 118**, the labels **10px, not 8** with 24px glyphs, the description **two lines, not one**, and the logo's track **168px, not 132**; the mechanism is unchanged and is the part that must not drift, and the refusal of "Big rewards" now stands as a refusal to **derive** one rather than as an absolute, an operator being able to write those words themselves - see the row below |
| **"The icons and info needs to be bigger... must be able to change them from the games edit content in admin"** | **Built 11 Sep 2026** (`13` **s4.1x**), the owner's reply to the 118px banner with the four features circled. **FIVE REQUESTS AND ONLY THE LAST IS STRUCTURAL** - bigger icons, a bigger logo, the description actually showing, permission to grow the banner, and the strip becoming operator-editable - so a document describing this as a sizing pass is describing four fifths of it. **NOTHING WAS COMPUTED WRONGLY and there is no risk number**: small labels and a clipped description render perfectly, and **nothing was backfilled**. **RAISING A HEIGHT IS THE EXACT THING s4.1w WAS BUILT TO PREVENT, so 150 is ARRIVED AT rather than chosen** - the badge, a 25px heading, the subtitle, the tagline and two clamped description lines is about 112px plus 16 of padding, with the logo at 120 in the same box - and **`min-h-`, `h-auto`, `p-5` and `p-7` stay asserted absent**, because the fault both rebuilds shared was never the number but a floor content could exceed sitting in the file reading as though it decided; **a bigger banner is a different measurement, a banner that can grow is the defect back.** **THE DESCRIPTION WAS NOT SMALL, IT WAS CUT OFF** - `line-clamp-1` clipped the live copy at *"...precision are key. Connect"*, which reads as a rendering fault rather than a summary - so it is two lines, **three is still forbidden**, and the test was **renamed rather than rewritten** because the claim is unchanged. **"The game logo bigger" IS TWO NUMBERS and asserting either alone passes a broken screen** - a bigger box in a 132px column is a bigger box that is clipped, a wider track under the old box is a small logo in a wide gap - so the test reads both out of the file and compares them, and the **monogram** is checked too, being the fallback for a title with no uploaded artwork, which is every title today. **THE STRIP IS NOW CATALOGUE CONTENT AND AN UNSET FIELD MEANS DERIVED, NEVER "SHOW NONE"**: nothing carries authored features, so the opposite reading would have stripped four facts off every hero the day this shipped with nothing failing and nothing logged - the `resolveAllowedGameTypes` reading and the **opposite** of `entryBlockThreshold`'s, the question being which way the failure falls - and it comes free, since `game-content.service.ts` already `$unset`s an empty array so clearing the list restores them. **One authored row replaces all four**, because a merge leaves an operator unable to tell which lines are theirs or to remove ours, and the dialog says so beside the field. **`resolveHeroFeatures` is the only place that decision is taken**, a second copy being the "one rule, two copies" shape with two answers on the screen a player reads immediately before paying. **The vocabulary is mirrored, model-free and NOT an enum** - model-free by requirement (**R58**: a `"use client"` file naming a driver-reaching module cannot build, which no typecheck, test or dev server can see), and `icon` is a `String` because a missing enum value rejects the **whole write**, so an unforeseen slug would cost the entire catalogue row on a scheduled sync; an unrecognised one **keeps its row and loses its picture**, the label being the operator's statement. `check:mirrors` compares models, so the guarantee is a byte-for-byte test. **The validator REFUSES an unknown icon where the genre field normalises one**, which looks inconsistent on the same screen and is the right answer to a different question: a genre arrives as free text from provider syncs and pre-vocabulary titles so refusing it would block an unrelated tagline edit, and **nothing but this dialog has ever written an icon slug**. **The assistant is barred with a reason recorded**, each slot being a **fact position** the platform otherwise fills from the round ceiling, the declared family and the contest's player range - a model writes "3 minute rounds" on a ten-minute ceiling, right about the genre and wrong about the number. **"BIG REWARDS" IS NOW AVAILABLE AND s4.1w's REFUSAL STILL STANDS**: the platform will not **derive** a rewards claim, the hero receiving no prize figure and a free contest carrying the phrase too, but an operator typing it has a name and a timestamp against it - **the refusal was never about the words** - and the guard on the derived set is unchanged. **30 tests, 30 probes red on exactly one failure**, full suite **2317** green, typecheck at the 194 and 223 baselines exactly, lint clean, `check:mirrors` 80 agree / 0 drifted. Two new probes are worth their shape: one asserts the **two**-line clamp is present rather than only forbidding three, since a guard banning three lines is equally happy with the one line the owner rejected; and one **exhausts the vocabulary** against the component's glyph map, since a spot check passes while the slug somebody adds next renders nothing. **Never verified by eye**, and for a second reason - there is nothing authored to render until an operator writes a strip |
| **"STOP. The leaderboard implementation is structurally wrong"** | **Built 12 Sep 2026** (`13` **s4.1y**), the owner's **fifth** rejection in this area, against a ten-row target and the two-row panel beside it. **MOST OF THE TARGET WAS ALREADY BUILT AND SAYING SO IS WHAT MAKES THE REST CREDIBLE** - the Trophy heading, a players count, a scope strip, the four columns, 28px avatars, a gold leader row with a crown and the full-width CTA were all there, so a document describing this as building a leaderboard from nothing is describing a fifth of it; four things were genuinely wrong, and the height is the one that mattered. **NOTHING WAS COMPUTED WRONGLY and there is no risk number** - a short panel renders perfectly, throws nothing and logs nothing - so **nothing was backfilled**. **THE HEIGHT WAS A SPLIT-OWNERSHIP FAULT, NOT AN ARITHMETIC ONE**: `GameArenaLayout` composed the chrome while a separate consumer supplied the rows, so **nothing owned the panel's height** - a heading at its text height, a rows box capped at `max-h-[460px]` and a footer at its content height, inside a grid cell as tall as the game board - and the arena backdrop showing through underneath **is** the owner's *"oversized empty dark area"*, which was **the layout's doing rather than the board's**, so a document describing it as the board being too tall has it backwards. One component owning the whole rail is what makes the fix expressible. **`h-full` ON A GRID ITEM'S WRAPPER IS A NO-OP THAT REVIEWS AS CORRECT** - a grid item already stretches to its row, so the wrapper was the right height all along, which is exactly why every reading of the arithmetic came back clean, and only **`[&>*]:h-full`** reaches the panel inside it; the same mistake was made once already on the bottom band (s4.1t), so carry the rule and not the two instances. Scoped **`xl:`**, because below that the rail is full width beneath the board and a forced height stretches a short list down an empty column. **`min-h-[400px]` IS ROOM, NEVER A CAP** - how many players a contest has is not ours to decide, so what is guaranteed is space for about ten rows and the panel grows past it to fill the row; a `max-h` reads as a sensible precaution and **is** the defect - and **exactly one child may grow**, because two `flex-1` children divide the slack and the footer drifts up by whatever the list does not need, which is the same empty area wearing a different cause. **NOT ONE OF THE FOUR REMOVALS LOST A FACT, and they are one `variant` prop rather than three booleans**: the activity lines moved to a PLAYERS roster tab and remain in the recent-players feed, the `YOU` word became a tint on the name - **the one marker that survives on the leader's gold row**, where a row background cannot - and two rows sharing a rank plate replaces the `= #1` pill; each removal is only worth making as part of the same decision, since each one is what buys a compact row. **TWO OF THE THREE FILTER TABS CANNOT BE ANSWERED AND ARE DRAWN ANYWAY** - the claim that there is no friends model was **false** (`Friendship.getUserFriends` lives in `database/models/messaging/friend.model.ts`); the rail does not call it, and the owner asked for both FRIENDS wiring and COUNTRY later rather than now - so `GLOBAL` is lit and the other two are `aria-disabled` labels with the reason in a title attribute, **rendered as spans so there is no control that appears to work**; that **reverses a guard recorded in s4.1q** asserting the rail draws no dead scope tabs, and the test was **flipped rather than deleted** so the original objection stays visible as the reason for *how* they are drawn - second time the owner has overridden a recorded guard here, so a document citing the old rule as current is stale. **THE POLLING CONTRACT IS UNTOUCHED and that is the constraint that shaped the rebuild** - `LiveContestRefresher` still banned on this page by a test, the round host still a **child** of the live provider rather than a consumer of it, `getArenaStandings` still the single producer for both the first render and the poll - and the new panel is a second **consumer** of the same context which **fetches nothing of its own**, asserted, with the consumer count now summed across two files rather than one. **THREE STRUCTURAL GUARDS HAD BEGUN FIRING ON CORRECT CODE and had to be sliced**: a file-wide wrapper count could not tell the band's three selectors from the rail's, a `flex-1` count caught a legitimate one on a roster row, and a whole-file ban on `Standings` was satisfied by an **import path** - **a guard that fires on correct code is the one the next reader deletes**, so slice to the construct and count the construct. Fourth instance of an older rule beside it: `describeRoundActivity`'s first occurrence in a file is its **import**, so a position assertion measured from `indexOf` is trivially true - use `lastIndexOf`. **24 tests across two suites, 19 probes red on exactly one failure**, full suite **109 files / 2331** green, typecheck at the 194 baseline exactly with nothing new in a touched file and nothing disappearing, lint clean, `check:mirrors` OK. **Two probes were green first time and both were the fourth cause** - a mutation with no observable: one moved the footer button "inside" the scroller while still leaving a `<NeonButton` after the scroller's opening tag, which every check was satisfied by, so the test now requires the scroller to have **closed** first. **Nothing here is mirrored. Never verified by eye** - the play screen is behind sign-in |
| **R50 on challenges - a seat is not a score** | **Closed 12 Sep 2026**, the field half only. `ChallengeParticipant.score` is optional with no default in both copies; the accept route's builder writes no `score` key; the pinning test was flipped, not deleted. **Latent twice over:** neither `challenge-finalize.actions.ts` copy reads the field, and nothing writes a real score yet. **The read path is still not built**, so a document implying a challenge now ranks on score is wrong. No challenge migration: every existing seat is trading. Capital fields stay unconditionally required until a provider challenge is actually seated. Friends-model claim corrected in the same pass - `Friendship` exists under `database/models/messaging/`. **AMENDED 13 Sep 2026: the read path IS built now** - `challenge-settlement.service.ts` ranks on `score` and gate 11b writes it - and the capital fields are written **explicitly per game** by `buildChallengeParticipantSeat`, so both of the last two sentences are correct as history and stale as a present fact; **say which**. What survives unchanged is the reason the field carries no default |
| **A provider game can be challenged, end to end** | **Built 13 Sep 2026.** Pick trading or a game in `ChallengeCreateDialog`, resolve the title through `challengeable-titles.service.ts`, pre-flight it in `format: "challenge"`, derive the play window from **one** producer at acceptance (`challenge-window.ts`, mirrored), seat both players, host a real round at `/challenges/[id]/play`, carry the score through **gate 11b of the single ingestion door**, and settle on `challenge-settlement.service.ts` (mirrored) - **the shared settlement stages, not the old inline copy**, which is the part that matters, since the challenge payout had been an entirely separate implementation in both apps. 17 settlement tests against a real database, 17 probes. **One probe was green and the CLAIM was wrong, not the test:** liquidation is game-agnostic on purpose and `isTrading` scopes only the trade floor - which had no trading-side test until the probe went looking. **Deferred:** the opponent picker, open challenges, willingness and matchmaking (X10 / X11.5) - **the opponent picker landed 14 Sep 2026** (`13` s4.1aa), so that first item is correct as history and stale as a present fact; **say which**. A `scheduled` title stays unchallengeable by the 8 Sep decision. **Never played by a person** - no provider challenge has run outside the harness |
| **"The user enter 10 min and the game didn allow him... is too late"** | **Three owner reports on the challenge screens, closed 13 Sep 2026** (`13` **s4.1z**), and **only the middle one was a defect - R73**. A ten-minute Circuit Sprint challenge refused a round **for its entire life**, with nine minutes on its own clock, because `roundStartPolicy: "reserve_full_round"` reserves the **catalogue ceiling** (`12` s2.9) and that ceiling is an hour. The rule is right for a competition, which has an operator who chooses it, a schema default behind them and a pre-flight refusing a too-short contest; **a challenge has none of the three.** A challenge now never reserves - `CHALLENGE_ROUND_START_POLICY`, imported by all three writers rather than written out - so a late presser gets a **shortened** round, disclosed. **Live and player-visible, nothing to backfill:** the round was refused before it was created, so no attempt was consumed and no money moved. The other two: the game chooser is a **scrollable list** that says *why* a title cannot be challenged rather than greying it out, and the dialog now renders **the title's own `configSchema`** with the admin wizard's `defaultConfigValues` - no game is named anywhere in it, asserted. The play screen moved onto the **same `GameArenaLayout`** as the competition arena (`competitionId` became `backHref`), with a separate `ChallengeStandingsPanel` because a challenge has **two players and no rank until settlement**. **Not built:** per-title challenge defaults, so the settings are choosable but not pre-chosen — **that last clause is history from later the same day**, see the row below, so **say which** |
| **"We need to be able to specify the default settings for challenges for each specific game"** | **Built 13 Sep 2026** (`12` **s4.2d**, the authoritative account; `22` s5 and `13` s4.1z amended). The other half of the instruction that produced R73: a player creating a 1v1 is **not** an operator drafting a contest — no view of the catalogue, no reason to hold an opinion about a board size, no way to tell a sensible answer from a bad one — so a fourth per-title control now chooses the join rule, the challenge length and the game's own settings, and the player's form opens pre-filled. **NOTHING WAS BROKEN and there is no risk number**: the settings were already choosable (`13` s4.1z), and **nothing was backfilled** — a title that has chosen nothing behaves exactly as it did. **Nothing here is a second source of truth:** the provider's `configSchema` still decides which settings exist and what they accept, and this only picks among the answers it already permits, validated against it twice — so a title with a board size, a difficulty and a lives count needs no code, asserted. **The write is STRICT and the read is LENIENT, and that asymmetry is the design:** an operator is sitting in front of the form, so an out-of-range length is refused with the range named, while a player is not — a stored setting the schema has since **stopped** accepting is dropped back to its declared default and an out-of-range length is **clamped**, because refusing on the read path takes the challenge dialog down for a title that is otherwise perfectly playable, for a reason no player can act on. **The strict half refuses a reservation the title cannot honour, and that is what stops the new control recreating R73** — no declared round length means nothing to reserve, and a round as long as the whole challenge means every round refused from the first second, which is the owner's report exactly. **Only the keys the operator SENT are stored:** `validateConfigValues` fills a missing field from its default, which is right for reading and wrong for writing — storing what it filled in would freeze the provider's own defaults onto the title and the frozen copy would win for ever, even after a later sync changed them. **The field is ours:** in **no** sync allow-list at all, not even `firstSyncOnlyFields`, on `NEVER_EDITABLE_CONTENT_FIELDS` beside `playModeOverride`, cleared with `$unset` and never `{}`. **The bounds are fetched, never retyped** — they live on the Challenge settings screen, so a copy in the dialog offers a length the save then refuses, naming a range the form itself allowed; a test asserts `1440` appears nowhere in it. **One reading of the join rule**, `resolveChallengeStartPolicy`, because there were three the moment this shipped and the drift is the quiet kind — a challenge created under one rule and played under the other, where the refusal names the clock rather than the setting behind it — with **absent meaning permissive**, the opposite of `contest-config.ts`'s reading of the same field name. **The create path stores what the pre-flight was RUN against**, returned rather than re-imported, or a title whose operator reinstated the reservation is approved permissively and stored strictly. **54 tests, 23 probes red on exactly the expected test.** **Not built:** no bulk edit, and no per-provider default — a board size is a property of a game, not of the company supplying it. **Never verified by eye** — the admin control is behind an admin sign-in and the player's form behind a player one |
| **Can a player choose who they are challenging?** | **YES, from 14 Sep 2026** (`13` **s4.1aa**) - the opponent half of X10, and the first challenge that can be started from a screen which is not already about one person. `ChallengeCreateDialog`'s `challengedUser` is now **optional**; when it is absent the dialog opens on an **opponent picker** - friends first, from `Friendship.getUserFriends` through the new `GET /api/challenges/opponents`, then a search box on the **existing** `GET /api/messaging/search/users`. **NOTHING WAS BROKEN and there is no risk number**: every existing caller passes an opponent and behaves exactly as it did, so a document describing a fixed calculation or a payout defect is describing something nobody found, and **nothing was backfilled**. **The load-bearing decision is precedence**: `opponent` is `challengedUser ?? pickedOpponent`, so a prop-supplied opponent always wins and the picker is withheld entirely - written the other way, which is the natural spelling if you think of the picker as the newer source, a player opening the dialog from somebody's profile could have the picker's state shadow the prop and **send the challenge to a different person**, debiting two wallets with nothing thrown and nothing logged; both directions are asserted, because the positive test alone is green on the reversal. **`null` and absent are deliberately the same fact**, inverting this codebase's usual rule that a stored value and an absent one differ, because every live caller passes `null` while its own user record loads and that is genuinely indistinguishable from not knowing. **The search is REUSED rather than reimplemented** - the messaging endpoint already applies both directions of the block list and the friend-request privacy setting - so a second search written for this screen would be a second answer to "may I see this person", and the copy that forgets shows somebody who has blocked you; one consequence is inherited rather than chosen, that endpoint matching on email, unchanged from the friend search every signed-in player already reaches. **Friends, NOT matchmaking**: `matchmaking.service.ts` ranks opponents by **trading** skill, so for a provider game it returns a happy well-ordered list sorted by something with no bearing on the game - risk X13, and it belongs with X11.5. The route returns the **other** party of each friendship, pinned by a test, since `userDetails` holds both and taking the first entry is a way to challenge yourself. Willingness is **not** filtered here, deliberately - a list that quietly omits people is indistinguishable from the person not existing, so the create route refuses with a reason instead. **Deferred and not to be summarised as done: open challenges** (nobody named - conditional opponent fields in both model copies, an atomic claim on accept, and the sites that assume two named players) **and per-game willingness**. **Never verified by eye**. **AMENDED later the same day: open challenges are BUILT**, so that half of the deferral is correct as history and stale as a present fact - **say which** - while **per-game willingness is still genuinely outstanding**; see the row below |
| **Can a challenge be left open to anyone?** | **YES, from 14 Sep 2026** (`03` **s2.4a**, `04` s2.1) - the other half of X10's opponent work. A player may open a seat rather than naming somebody, and the first player to take it becomes the opponent. **NOTHING WAS BROKEN and there is no risk number**: no challenge could be open before, so a document describing a fixed calculation or a payout defect is describing something nobody found, and **nothing was backfilled**. **It REVERSES `03` s2.4 point 1, which recommended keeping `challengedId` required and materialising a `Challenge` from a separate `OpenChallenge` collection on acceptance** - a document proposing that collection is describing the plan, not the code, and the reason is that a second collection makes the materialising step a **second writer of a money document**, the shape this programme has spent weeks removing; the three opponent fields are **conditionally** required instead, on a predicate `check:mirrors` cannot see, so a byte-comparison test pins the two copies. **Openness is an explicit `openToAnyone` flag, never inferred from an absent opponent**, and the direction of failure is the argument: inferred, a bug dropping `challengedId` from a **directed** challenge hands a stranger a seat offered to a named friend and debits a real entry fee, where the flag turns the same bug into a challenge nobody can accept - visible, refundable and complained about. **The flag stays true after the seat is claimed**, so it answers "how was this created" and not "is this joinable"; `isUnclaimedOpenChallenge` is the one answer to the second. **The claim is ATOMIC and that is the money-critical part** - one `findOneAndUpdate` filtered on `pending`, `openToAnyone` and an empty seat, then **continuing from the returned document**, because continuing from the pre-claim copy is the natural spelling and debits both of two simultaneous accepters; "empty" is matched in **all three** shapes, absent, `null` and `""`, since only the first is what the create path writes and the other two look correct in a document dump. **The create route REFUSES a request carrying both facts** rather than letting one win, a precedence rule stated in the browser and again on the server being one rule in two copies with the untested one in front of the player. **No notification and no decline** - there is nobody to notify, and the decline route already requires the caller to be the named opponent, so refusal is by construction rather than by a new check. **Open question 15 is answered for this slice only:** any signed-in player except the creator, through the same standing, balance and fraud gates. **Six screens assumed two named players** - the card, the list (a new **Open** tab), the detail page, the provider lobby, the entry actions, and the public landing feed, which invented a "Player 2" for a seat nobody had taken - and the shared answer is `lib/utils/open-challenge.ts`, **model-free by requirement** because its callers are `"use client"` (**R58**). **47 tests, 21 probes red on exactly the expected test**, one of which was **green first time and the TEST was at fault**: a fixed-width slice after `const handleSubmit` reached into `proceedAfterTerms`, whose identical guard satisfied the match - the sixth instance of one identifier defeating a structural test, and the fix is to bound a slice by the construct that follows it, never by a character count. **Never verified by eye**. **AMENDED later on 14 Sep 2026 - the "no notification" and "inherits `acceptDeadline`" clauses are BOTH closed**, so they are correct as history and stale as present facts; **say which**. See the two rows below |
| **Does anything tell you a challenge changed?** | **Yes, from 14 Sep 2026** - in-app, by email where a template says so, instantly on the bell, and as a **pop-up with a click target** for every challenge event (`13` **s11.1a**). The owner's report was that somebody taking your open seat gives you "no email, no in-app alert, no badge" and you "find out by opening the page", which matters more than it sounds because the clock starts on acceptance. **MOST OF THE PIECES EXISTED AND NONE WERE CONNECTED, and that is why the report read as three absences**: the in-app row was always stored, `NotificationTemplate.channels` was declared on every template and **read by nothing** (the seventh declared-written-dead field, and the first on a control an operator can see and toggle), `emailNotificationBridge` had **zero production callers** checked with `rg`, and the socket server had no notification case at all. So the deliverable is **one seam, not four features**: `notificationService.send()` hands every notification it stores to `deliverNotification`, which pushes it and emails it when the template says to - a document describing per-event wiring is describing the version where the next template is silently silent. **The socket server stays generic and a test forbids an event name in it**, because it deploys as its own process and a `case` per template means redeploying it to add a notification. **The push is fire-and-forget**, sitting inside the accept route that debits two wallets. **Email is one generic `notification_alert` template**, never one per event, for the same reason an aggregate must not enumerate game types. **`shouldSendEmail` was reading a singular category against plural preferences**, so the user opt-out was being **bypassed rather than honoured** - a real defect found only by giving the bridge its first caller. **`send()` now seeds on demand**, having previously returned silently when a template was missing, which is a notification nobody gets on every database older than the template while the call site reports success. **`notification-push.ts` is deliberately dependency-free and mirrored** so the admin app can push without the user lookup or the email bridge, held byte-identical by a test since `check:mirrors` compares models. **Two templates lacked an `actionUrl`** and seeding is `$setOnInsert`, so it cannot repair a row that already exists - hence a per-category fallback in the popup card. **The popup is a category filter, not a challenge feature**: `NotificationPopupCard` knows nothing about challenges, `challenge_received` is the one suppression because it has its own accept/decline card, and a continuous trading event must never reach it. **The bell updates without a second websocket connection** - `useWebSocket` opens one per call - so `ChallengePopup` broadcasts a browser event, and it broadcasts **before** the popup filter, or a notification correctly not worth a popup silently stops counting. **Never verified by eye** |
| **How long does an open seat stay up?** | **Its own setting from 14 Sep 2026** (`03` **s2.4b**) - `ChallengeSettings.openChallengeExpiryMinutes`, resolved by the mirrored `lib/services/challenges/accept-deadline.ts`. Until then it reused `acceptDeadlineMinutes`, **a number nobody chose with a public notice board in mind**: it answers how long to hold a seat for one specific person who has been told, which is not the question "how long do I leave a notice up for any passer-by", and the two were forced to be equal. **NOTHING WAS COMPUTED WRONGLY** and **nothing was backfilled**. **The open fallback is its own default of 24 hours, never the directed one** - no deployment holds a value for a field that did not exist yesterday, so falling back to `acceptDeadlineMinutes` makes the whole change invisible everywhere with every structural test still green, the branch being present and merely answering the same thing. **A non-positive or non-finite stored value reads as unset**, these arriving from `parseFloat` on an admin form where a `0` expires an open challenge on creation (R31's rule). **It is resolved once, at every writer**, or a challenge is created under one rule and expired under the other with a probe aimed at either copy staying green. **The admin control sends `null` when cleared**, because `JSON.stringify` drops `undefined` and an omitted field cannot unset a stored one - without it an operator can raise the number and never lower it. **The player route exposes the resolved figure**, not the stored one. Two templates exist because of this: **`challenge_seat_taken`**, because `challenge_accepted` says the opponent "accepted your challenge" and implies an invitation, and **`challenge_open_expired`**, because `challenge_expired` says a named opponent "did not respond in time" - a false statement about a challenge nobody was invited to, and unfixable by rewording afterwards since seeding never touches an existing row. **Expiry is written in three places** - server action, worker, admin - so the notification is a shared helper; **the worker is the one that runs in production**, which is why leaving it out is the version where nothing fires and every test about the other two passes |
...
| **Why the admin panel was down** | **R58, closed 9 Sep 2026, and it is the first defect here whose harm is that the application did not exist.** `GameScoringDialog.tsx` is `"use client"` and imported **one number** from `game-scoring-rules.service.ts`, whose first import is `@/database/mongoose`, whose first import is `MongoClient` from `mongodb`. Turbopack traced the whole driver into the browser bundle, where `fs`, `net`, `tls`, `dns` and `child_process` do not exist, so `next build` failed with 17 errors, `apps/admin` had no `.next` directory, and PM2 restarted it **283 times**. No money moved, no document is wrong, **nothing to backfill** - a build either emits a bundle or it does not, and the main app stayed up. **The typecheck is structurally blind to this whole class**: the import is valid TypeScript for a value of type `number`, and nothing about a module's fitness for a *browser bundle* is expressible in the type system - both apps' `tsc --noEmit` were clean, every test passed, and the dev server never had reason to build the dashboard's client bundle, so **it is only ever caught by a full production build, which is to say by deploying.** Fixed by moving the constant into `apps/admin/lib/admin/score-eligibility-copy.ts`, which reaches no driver, with the service importing it back so there is still one definition. **Extending the new guard to the main app is what turned a fixed bug into a finding**: `MarketStatusBanner.tsx` was one keystroke from the same outage, building only because both its bindings are interfaces used in type positions, so **the safety was the bundler's unused-import elision and not a decision anyone had made** - promote either to a class and the *player* app fails identically with no warning from any tool. The guard forbids `mongodb` and **deliberately not `mongoose`**, which has a browser build: the first draft banned reaching a model, flagged five files that build perfectly well, and would have been deleted by the first person it inconvenienced. **Two things about the report point away from the cause and cost time** - the deploy script printed the build failure and then restarted the processes anyway, so the readable tail was a boot loop with the 17 errors scrolled off, and every error names a file inside `node_modules`, which reads as a dependency problem; **the one useful line is the bottom of each import trace.** 7 tests, 2 probes. **Both apps now emit a `.next/BUILD_ID`, verified locally - the owner still has to redeploy** |
| **Why the genre list looked empty** | **R60, closed 9 Sep 2026.** The Genre drop-down opened as a tall panel with one word in it, and **nothing was missing** - all fifteen options were there and all fifteen were white on white. **A browser paints a native `<select>`'s list itself**, taking the background from the element's own `background-color` while the options inherit `color`; this surface is themed `bg-white/5 text-white`, and a *translucent* white composites over the browser's light list surface to effectively white. The one legible row was the highlighted one, against the operating system's selection band. **A fault in which nothing fails reads as a data problem**, which is why it was reported as items not showing. **Two conditions are needed**: a native `<select>` AND a translucent background. The surface has **eleven** pickers on the shared `Select`, which draws its own list in a portal; this was the twelfth and the only native one there. The **33 remaining native selects in admin and 11 in the player app sit on an opaque `bg-gray-*` or set none**, and mostly set no colour, so they are correct - **which is why the guard bans the COMBINATION and not the element**, an outright ban firing on forty-odd working files being the kind the first person it inconveniences deletes (`13` s4.1g). **THERE WERE TWO INSTANCES**: `MessagingSection.tsx:2110`'s "Transfer to" picker has the same pair, so **every employee name in it is white on white too**. It is **deliberately unfixed for a mechanical reason** - the change is one token, but that file carries ~30 pre-existing lint warnings and the pre-commit hook lints staged files at zero, so touching it demands an unrelated cleanup of a 2,180-line component - and it is a **named exception with a test asserting the exception is still an offender**. **The guard's own near-miss is the transferable part:** the scan was first `<select\b[^>]*>`, which stops at the `>` inside `onChange={(e) => ...}`, so it never reached `className`, matched nothing, and **was green over an unexamined tree** - caught only by the exception test reporting the known offender as clean. **Radix throws on a `SelectItem value=""`**, so "no genre" travels as a sentinel mapped back at the boundary, and its spelling is load-bearing: `normaliseCategorySlug` strips every non-alphanumeric run, so `__no_genre__` is unreachable from the slug namespace while `none` or `custom` is a genre somebody can type and would then find clearing the field. **Low and live, nothing backfilled** - but it made the genre unsettable in practice, so `category` is the field most likely to be stale on any title touched before today, and it is the grouping key tasks 21-24 join on. **4 tests, 6 probes; never verified by eye** |
| **Why every dialog was too narrow** | **R59, closed for the games surface 9 Sep 2026.** The owner reported the Games list clipped at its fourth column. The dialog asks for `max-w-4xl` and **renders at 32rem**, because `DialogContent`'s own class list ends with `sm:max-w-lg`: `twMerge` keys a conflict on the utility group **and the modifier**, so a bare width does not displace a `sm:`-prefixed one and **both reach the DOM**, after which Tailwind emits the `sm:` rule *later* at equal specificity and the cap wins. **Measured, not argued** - compiling both candidates with this app's Tailwind puts them at bytes 4541 and 4749. **The "appears to work and does nothing" shape again**, after a provider with no adapter, a `rankingMethod` a provider game ignores and `isPaused` on a provider contest. **Platform-wide: 55 inert width classes, of which 31 are clipping content, 6 too roomy, and 16 accidentally correct because `lg` is what the cap already says** - and those 16 are why it survived, the pattern working on a third of its uses. **Two of the 31 are in the PLAYER app.** Fixed for all five game dialogs through one model-free token module, with the catalogue's table given `min-w-[68rem]` inside an `overflow-x-auto` wrapper, because seven columns of controls fit no laptop at any dialog width. **The main app already had the answer** - its own copy of the primitive grew a `size` prop whose variants are all `sm:`-prefixed - so the right sweep for the remaining 29 is probably **to port that prop**, not to spread a token across 24 files. **The other 29 admin dialogs and 2 player dialogs are an owner decision, recorded rather than swept:** each becomes visibly wider, none can be checked by eye from here, and bundling them destroys the only evidence the reported fix is safe. **7 tests, 7 probes; never verified by eye**, the screen being behind an admin sign-in |
...
5. Then the admin programme, per the owner's admin-first sequencing. **Most of X6 is now
   ...
   **game contest wizard was rebuilt on trading's chrome with a game-aware assistant**
   ...
   closed `12` s9's group criterion at the same time. On 9 September a **Play style control**
   was added to the Games list and the wizard's game picker (`12` s4.2c, `22` s9), so an
   operator can now say whether a title is played at one appointed moment or joined whenever
   - previously only the provider's catalogue could answer that, which is useless for the
   game company we own. **What is left of X6 is the Game Master
...
| **9 Sep 2026** | **Image optimisation happens ON UPLOAD ONLY. Existing files are left alone** - no retro-scan of game artwork, no rewriting of stored filenames | Owner decision, and it closes task doc 15/16 as **scoped down rather than done**. The optimiser converts in place, renames to `.webp` and **deletes the source**, while a filename is stored in `provider_game.thumbnailUrl` / `bannerUrl` **and duplicated into `branding_asset`** - so pointing it at existing artwork leaves the row and the Mongo copy both naming a file that no longer exists, and a filesystem scan **cannot see** a logo that only ever reached one server's disk, which is the entire reason the Mongo copy exists (R56). **Optimising on upload is safe for the opposite reason, and it is the reason rather than a coincidence: the filename is settled before any row is written**, so there is nothing to keep in step. **Do not "finish the job" later by adding a retro-scan** - the missing piece is not the scan, it is a writer that updates both stores as it renames, and until that exists a scan is a data-loss tool with a progress bar |
| **9 Sep 2026** | **100 credits = EUR 1 is authoritative. `AppSettings.credits.valueInEUR` must DERIVE from `CreditConversionSettings.eurToCreditsRate` rather than be a second stored number** | Owner decision, closing the disagreement recorded at the foot of `format-volts.ts`. **It is not a tie between two defaults - one side is live and wrong by 100x.** `WalletBalanceDisplay.tsx:32` renders `balance * valueInEUR` at 1 credit = EUR 1, while `app/api/wallet/withdraw/route.ts:115` pays out on `eurToCreditsRate` at 100 credits = EUR 1 - so a player holding 1,000 credits is shown **EUR 1,000.00** and can withdraw **EUR 10**. The `/help` page renders **both** numbers, from the two different models, on one page. **This is its own slice with its own tests and it is NOT part of the Volts work**, which deliberately removed the fiat equivalent from contest surfaces rather than correcting it. Two things to get right when it is built: `valueInEUR` is an **operator-editable field** on `CurrencySettingsSection.tsx`, so deriving it removes a control somebody may already have set - read what is stored before overwriting it - and the change **moves the player's wallet line by a factor of a hundred**, which needs saying out loud rather than shipping as a tidy-up. **BUILT 14 Sep 2026 as R74**, so this row is a decision fulfilled rather than an outstanding one - and both cautions held: the stored field is **left in place, not migrated**, because it may carry a deliberate value, and the admin control is **withheld read-only rather than deleted**, because an operator needs telling where the number went |
| **10 Sep 2026** | **The countdown graphics come BEFORE the dashboard contest cards** | Owner sequencing decision, taken after R67 shipped and the cards turned out not to be fixable the same way. Both came from one message, so the order is worth recording rather than inferring. The graphics are **self-contained and visible** - the game screens' clocks are lines of text where the trading page has a large four-cell countdown - while the cards need an **extraction first**: `router.refresh()` cannot be used on the dashboard, because `getComprehensiveDashboardData` runs unbounded `TradeHistory` and `WalletTransaction` reads, a 10,000-row participant fetch, two aggregations and, on a cold cache, a global leaderboard rebuild its own comment puts at ~7 seconds. **And a light endpoint cannot simply be written**, because `comprehensive-dashboard.actions.ts` already computes contest ranks with **its own sort** (`:616-657`) rather than through `getCompetitionLeaderboard` - so a new reader would be a **third** answer and the card would visibly flip on its first poll. The work is therefore to lift that computation out and have the page and the poll share it, so the live answer is the same answer **by construction**. Note the comment beside that sort claims it is *"same as competition leaderboard"* - an **unverified aside**, and the eighth instance of the class. **Both halves have now shipped** - the graphics the same day (`13` s4.1j), the cards on 11 Sep 2026 (`13` **s5.1b**) - so a document saying the extraction is outstanding is stale. Two corrections to this row's own reasoning, recorded rather than tidied away: the extraction found **four** copies of the comparator and not one, two of them in the public `/api/dashboard/competitions` arena route, which is **recorded as a named exception with a canary test** rather than fixed; and the *"same as competition leaderboard"* aside is still unverified and is still wrong in the direction that matters, since the action reads a stored `pnl` where the leaderboard does not - which is why the new endpoint deliberately reports the stored value too, so the card cannot jump on its first refresh |
...
| Challenge opponent | ~~**Always a direct invitation** to a required `challengedId`. No open challenge exists, and the help page says so~~ **Stale from 14 Sep 2026, correct as history - say which.** A challenge is a direct invitation **or** an open seat: `openToAnyone` is an explicit stored flag, the three opponent fields are conditionally required, and the first claimant wins through an atomic `findOneAndUpdate`. ~~**The help page still says otherwise and has not been reworded**~~ **- reworded the same day (`13` s9.1a), so that clause too is correct as history and stale as a present fact; say which.** An open seat also has **its own expiry** rather than the directed `acceptDeadline`, and its claim **notifies the creator** (`03` s2.4b) | `POST /api/challenges`, `POST /api/challenges/[id]/accept` |
...
| `22-contest-shape-and-synchronisation.md` | **New 8 Sep 2026.** Answers open question 18 - how a contest differs when every player must start together, as a race does, rather than turning up whenever they like. **Sections 1-6 were the proposal; the competition half was BUILT the same day and `section 8` is the authoritative account of what exists. `Section 9` adds the operator's own answer** - a Play style control per title, 9 Sep 2026 - **and changes no rule.** Challenges (s5) are not built |
...
| **11** - Game-level supported modes | **Done** (11.1). A title may declare it supports **both** shapes, with the operator picking one **per contest** - which **reversed** `22` s8.3's "never from caller input". This row said "Not started" until 10 Sep 2026 and was **stale**, not a description of the code |
| **12** - Required timing / runtime settings | **Done** (12.1). The wizard already adapted; the **editor** had never heard of the play shape (**R61**). Four of the fields the task lists are **derived on purpose** and must not be added back; the check-in / lobby / countdown trio is **blocked**, like 10.2 |
| **13** - Data-driven game configuration | **Done** (13.1). **The schema it sketches already existed; its closing SENTENCE is what shipped.** The assumption left was in ingestion, where it decided money (**R62**). `capabilities.attempts` was considered and **deliberately not added** - it would be read by nothing. A second cluster is **recorded, not fixed**: the global leaderboard, matchmaking, badges and the journey all rank on `TradeHistory` alone, which is `05` / X7 / X11, not this task |
...
- **100 credits = EUR 1 is authoritative**, and `AppSettings.credits.valueInEUR` derives from
  ...
  **moves the wallet's euro line by a factor of a hundred**, so it is a money-visible change
  and not a tidy-up. Recorded at the foot of `lib/utils/format-volts.ts`. **BUILT 14 Sep 2026
  as R74** - one mirrored resolver, `valueInEUR` derived by both `/api/settings` routes, the
  admin PUT refusing the path and the control withheld read-only. The stored field is kept and
  **not migrated**, so a document describing a data migration here is describing something
  nobody ran.
...
`challengedId`)~~ **both built 14 Sep 2026, so that clause is correct as history and stale as
a present fact - say which**; still outstanding are
per-game willingness (`UserPresence.acceptingChallenges` is still one
...
**The three sentences this section used to carry are all history now, and each was true on a
different day, so say which.** The **data model has been ready since X1**: `Challenge` carries
`gameType` and an immutable `gameKey` with indexes on both, and `ChallengeParticipant` carries
`score` and `gameKey` with a game-agnostic ranking index. **`challenge-finalize.actions.ts`
holding its own inline copy of the settlement stages was true through 11 Sep 2026**: both apps
now call the shared `payContestPrizes()` and `settleFeesAndGameMasters()` through
`challenge-settlement.service.ts`, exactly as a competition does - only the third stage,
`completeContest()`, stays unreused, because a challenge has no matching lifecycle step. And
**"`ChallengeCreateDialog.tsx` mentions no game field of any kind, so a challenge can only be a
trading one" was true through 12 Sep 2026 and is FALSE from 13 Sep**: the dialog carries the
game picker, the title's own `configSchema` and, since later that day, the per-title defaults it
opens pre-filled with. A document repeating it is repeating a claim the file disproves. What
remains outstanding is the **opponent** half listed above, not the game half.
...
instruction. **(1)** The whole challenge lifecycle now reports itself - stored in-app,
pushed to the bell instantly, emailed where the template says so, and shown as a
**pop-up with a click target** (`13` s11.1a). **(2)** An open seat has an expiry of its
own rather than inheriting the directed `acceptDeadline`, with `challenge_seat_taken` and
`challenge_open_expired` templates written because neither existing one could be reused
(`03` s2.4b). **(3)** A game challenge's two clocks - how long the challenge runs and how
long the game runs - are **stated as clocks rather than offered as controls**, locked to
the title's defaults (`13` s4.1ab). 89 tests, 36 probes, all red on exactly the expected
test. **Nothing was computed wrongly anywhere in this slice and nothing was backfilled.**

**The finding worth carrying:** most of the notification machinery already existed and
none of it was connected, which is why one absence was reported as three.
`NotificationTemplate.channels` was declared on every template and **read by nothing** -
the seventh declared-written-dead field after `requiresSyncPlay`, `isPaused`,
`lastSuccessfulRoundAt`, `family`, `playModeOverride` and the SEO field, and the first one
that is **a control an operator can see and toggle**. `emailNotificationBridge` had zero
production callers, checked with `rg`. The socket server had no notification case at all.
So the deliverable is **one seam, not four features**, and giving the bridge its first
caller is what exposed a real defect underneath: `shouldSendEmail` compared a singular
template category against plural user preferences, so the opt-out was being **bypassed
rather than honoured**.

**Four rules from it.** A guard that must survive a redeployment cannot live in a process
that redeploys separately - the socket server stays generic and a test forbids an event
name in it. A notification path inside a money route must be **fire-and-forget**, or a
dead socket server is latency on two wallet debits. **Seeding is `$setOnInsert` and
therefore cannot repair a row that already exists**, which is why two templates missing
an `actionUrl` also needed a per-category fallback in the popup card, and why a badly
worded existing template must be replaced rather than reworded. And **an open fallback
must be its own default**: falling back to `acceptDeadlineMinutes` would make the entire
expiry change invisible on every deployment, with every structural test still green,
because the branch is present and merely answers the same thing.

**Files touched:** `lib/services/notification.service.ts`,
`lib/services/notifications/delivery.ts`, `notifications/notification-push.ts` (new,
mirrored), `lib/services/email-notification-bridge.ts`,
`lib/services/challenges/accept-deadline.ts` (new, mirrored),
`challenges/expiry-notifications.ts` (new), `lib/utils/notification-events.ts` (new),
`components/notifications/NotificationPopupCard.tsx` (new),
`components/challenges/ChallengePopup.tsx`, `NotificationDropdown.tsx`,
`components/challenges/create/ChallengeDurationClock.tsx` (new),
`ChallengeBattleSettings.tsx`, `ChallengeSettingsFields.tsx`,
`components/competitions/CountdownPanel.tsx`, both `challenge-settings.model.ts` copies,
both `notification-template.model.ts` copies, both `email-template.model.ts` copies, the
three expiry writers, `websocket-server/index.ts`, the challenge create / accept /
settings routes, the admin challenge-settings route and
`apps/admin/components/admin/ChallengeSettingsSection.tsx`, plus two new test files, two
new probe scripts, `03` (s2.4b), `13` (s11.1a, s4.1ab) and this file.

**Next chat should:** ask the owner about X6 - and in code, **per-game willingness** is
what remains of X10's opponent work.

---

### 14 Sep 2026 - THE HELP PAGE DESCRIBED A FEATURE WE NO LONGER HAVE

**Shipped:** the `1v1 Challenges` section of the help page, rewritten to match what a
challenge actually is. **No code behaviour changed, no route changed, nothing was
backfilled** - this is documentation, recorded because a help page is the one screen whose
readers cannot tell it has gone stale. Three claims were false. It said *"there's no public
lobby - every challenge is an invitation from one player to another"*, which stopped being
true when open challenges shipped **earlier the same day**; it framed every challenge as a
**trading** contest with virtual capital, six ranking methods and liquidation rules, all of
which are false for a game challenge; and it used **`duel`**, a banned noun, nine times. 14
tests, 11 probes, all red on exactly one failure.

**Files touched:** `app/(root)/help/page-content.tsx`,
`__tests__/challenges/help-page-challenges.test.ts` (new),
`tools/probe-help-page-challenges.ps1` (new), `External game plans/13-user-ui-and-routes.md`
(new s9.1a).

**Deviated from plan:** the task on the list was item 3, *letting a player choose trading or a
game when creating a challenge*. **It was already built** - the picker, the adaptive settings
form and the scoped market gate all shipped on 13 September - so the work became verifying
that and then fixing what the verification turned up. The general form is worth keeping:
**checking whether a planned item is already done is not a formality, and here it is what
found the defect.** Three things were checked against the code before a word was changed -
`gameNeedsMarketHours(gameLabel.gameType)` really does scope the market gate,
`getProviderRankingValue` really does ignore `rankingMethod`, and the decline route really
does admit only the named `challengedId` - so every sentence the page now carries is a claim
somebody verified rather than inferred.

**Owner tested:** not yet. **Never verified by eye** - the page was not opened in a browser.

**Deferred:** **nine `duel` uses remain outside this file** and are listed in `13` s9.1a with
their counts - landing-page templates (5), the admin landing builder's defaults (5), hero
settings defaults (2), two landing sections (2 each), the arena H2H scene and the theme data
(1 each). Three of those are **seeded defaults**, so editing the constant does not change rows
already written; that sweep is a migration question rather than a wording one, which is why
the guard is scoped to the help page. **Do not let a summary imply the vocabulary is clean
platform-wide.**

**Next chat should:** either sweep the nine remaining `duel` sites (deciding first what to do
about the three seeded-defaults files, since those need a migration or an explicit "new rows
only"), or move to the next unbuilt challenge item - **per-game willingness** and **a
notification when somebody takes your open seat**, both named as not built in `13` s4.1aa.

---

### 14 Sep 2026 - ONE CREDIT IS WORTH ONE NUMBER (R74)

**Shipped:** the last live money-adjacent defect on the decided-but-not-built list. What a
credit is worth was stored **twice** - `AppSettings.credits.valueInEUR` defaulting to 1 credit
= EUR 1, `CreditConversionSettings.eurToCreditsRate` defaulting to 100 credits = EUR 1 - and the
first drove the wallet's "approximately" line while the second is what withdrawals actually pay
on. A player holding 1,000 credits read **EUR 1,000.00** above a button that would give them
**EUR 10**. `valueInEUR` is now **derived**, in one mirrored resolver, served by both
`/api/settings` routes and `/api/help-settings`, refused by the admin PUT, and shown read-only
on the currency screen with a pointer to Credit Conversion. 26 tests, 16 probes.

**Files touched:** `lib/utils/credit-value.ts` (new, mirrored into `apps/admin/lib/utils/`),
both `app-settings.model.ts` copies (comment only), both `/api/settings` routes,
`app/api/help-settings/route.ts`, both `AppSettingsContext.tsx` copies,
`components/trading/WalletBalanceDisplay.tsx`, `app/(root)/help/page-content.tsx`,
`apps/admin/components/admin/CurrencySettingsSection.tsx`, both `format-volts.ts` copies
(comment only), `__tests__/services/credit-value.test.ts` (new),
`tools/probe-credit-value.ps1` (new).

**Deviated from plan:** the 9 Sep decision row warned that `valueInEUR` is an operator-editable
field and deriving it removes a control somebody may already have set. Both cautions were
honoured rather than argued away, and the second one inverts a precedent worth naming. The
stored field is **kept and left unmigrated** - it may carry a deliberate value, and dropping it
is a mirrored migration for a cosmetic gain - so both model comments are corrected **in place**
and say the field is inert. And the admin control is **withheld read-only rather than deleted**,
which is the opposite of the `shouldBlockEntry` precedent: a dead helper invites the defect back,
but a number that simply vanishes from a screen sends an operator hunting for it in the
database, so the row stays and names where the decision now lives.

**Owner tested:** not yet. The wallet line is behind sign-in, so **never verified by eye** - and
this one moves a figure a player reads by a factor of a hundred, so it is worth looking at.

**Deferred:** nothing from this slice. Two things are recorded rather than closed. `format-volts.ts`
still converts nothing and that refusal **no longer depends on this defect** - a competition is
denominated in credits and has no business quoting a second unit - so the assertion stays and its
reasoning was rewritten rather than deleted. And the fix is not retroactive in one narrow sense:
any operator who typed a value into `valueInEUR` will find it ignored, which is the intent.

**Next chat should:** take the challenges block in the owner's stated order - **R50's read half
is done**, so the next item is item 3, letting a player choose trading or a game when creating a
challenge.

---

### 14 Sep 2026 - A CHALLENGE NOBODY IS NAMED ON

**Shipped:** the other half of X10's opponent work, built directly on the picker earlier the
same day. A player may open a seat to anybody instead of naming an opponent; the first player
to take it becomes the opponent. `openToAnyone` on both `Challenge` copies, the three opponent
fields made conditionally required, an **atomic claim** on accept, `GET /api/challenges?type=open`
and an **Open** tab, and a pass over the six screens that assumed two named players. 47 tests,
21 probes red on exactly the expected test.

**Files touched:** `lib/utils/open-challenge.ts` (new), both `challenge.model.ts` copies,
`app/api/challenges/route.ts`, `app/api/challenges/[id]/accept/route.ts`,
`app/api/landing/challenges/route.ts`, `app/(root)/challenges/[id]/page.tsx`,
`app/(root)/challenges/page-content.tsx`, `components/challenges/ChallengeCreateDialog.tsx`,
`components/challenges/create/OpponentPicker.tsx`, `components/trading/ChallengeCard.tsx`,
`components/trading/ChallengeEntryActions.tsx`, `components/games/ProviderChallengeLobby.tsx`,
`__tests__/challenges/open-challenges.test.ts` (new), `tools/probe-open-challenges.ps1` (new).
Only the model is mirrored, so `check:mirrors` says nothing about the rest.

**This reverses a recommendation in `03` s2.4, and the reversal is recorded rather than
quietly written over.** That chapter says `challengedId` should stay required and an open
challenge should live in a separate `OpenChallenge` collection that materialises a `Challenge`
on acceptance. The caution behind it is right - this model sits on the money path and is
mirrored - but the remedy costs more than it saves: the materialising step is **a second
writer of a money document**, which is the shape this programme has spent weeks removing
(four competition entry paths, ten finalize sites, two settlement copies), and until the
moment somebody claims it an open challenge would be a different kind of object that every
list, notification, cancellation and fee path has to learn about. Conditional requirement
keeps one collection, one writer and one lifecycle for one predicate. It is deliberately not
a drop to `required: false`: **a directed challenge with no opponent is still a bug**, and the
schema is the only thing that catches it. Both copies carry the same predicate body, pinned by
a byte comparison - `check:mirrors` compares field paths and enum values and **not predicate
bodies**, so a conditional requirement that differed between the apps would be a validation
rule whose outcome depends on which process saved the document, with the guard staying green.

**Openness is an explicit flag, never inferred from an absent opponent, and the direction of
failure is the whole argument.** Inferred, a bug that drops `challengedId` from a **directed**
challenge silently converts it into one anybody may claim - a stranger takes a seat offered to
a named friend and a real entry fee is debited, with nothing thrown and nothing logged. With
the flag, the same bug produces a challenge nobody can accept: visible, refundable and
complained about. A capability gate fails closed. The flag **stays true after the seat is
claimed**, because it states how the challenge was created rather than what state it is in,
which is what lets a screen explain why a stranger is in it - so it is not the "is this
joinable" question, and `isUnclaimedOpenChallenge` is the one answer to that.

**The atomic claim is the money-critical part.** Accept does a single `findOneAndUpdate`
filtered on `status: "pending"`, `openToAnyone: true` and an empty seat, and then **continues
from the returned document**. Continuing from the pre-claim copy is the natural spelling and
is exactly how two simultaneous accepters both get debited. "Empty" is matched in **all three**
of its shapes - absent, `null` and `""` - because only the first is what the create path
writes, and the other two are what a bad edit or a half-run migration leaves behind while
looking perfectly correct in a document dump.

**Three refusals are by construction rather than by a new check**, which is worth saying
because each reads like an omission. The create route **refuses a request carrying both
`openToAnyone` and `challengedId`** rather than letting one win, since a precedence rule
stated in the browser and again on the server is one rule in two copies with the untested one
in front of the player. **No notification is sent**, there being nobody to notify. And
**declining is impossible**, the decline route already requiring the caller to be the named
opponent.

**Open question 15 is answered for this slice only:** an open challenge may be claimed by any
signed-in player except its creator, through the same standing, balance and fraud gates a
directed accept already passes. Per-game willingness is untouched.

**Deviated from plan:** the `OpenChallenge` collection, above. Nothing else.

**Owner tested:** no. **Never verified by eye** - every screen is behind sign-in and the
automated browser has no session.

**Deferred:** **per-game willingness** - the opt-out storage the owner chose on 13 Sep 2026,
the create and accept gates that read it, and the toggle the help page already promises. Also
not built: an expiry rule specific to open challenges (they inherit the ordinary
`acceptDeadline`), and any notification when somebody takes your open seat. **And the help
page is now wrong** - `app/(root)/help/page-content.tsx` still says a challenge is always a
"Direct invite" and that "the opponent has to be chosen before you can create the challenge"
(lines ~1865 and ~1886). It is player-facing documentation of a rule that no longer holds, so
it is a defect rather than a tidy-up, and it belongs with X8's player wording pass only if
nobody gets to it first.

**Next chat should:** build per-game willingness, or move to the next item on the owner's
order - the four-part clock on the game countdowns.

**One probing finding worth carrying.** A probe restoring `!opponent` in `handleSubmit` came
back **green**, and the test was at fault rather than the claim: the assertion sliced a fixed
200 characters from `const handleSubmit`, which reaches into `proceedAfterTerms`, whose
identical `if (!hasRecipient)` satisfied the match. Sixth instance of one identifier defeating
a structural test, after `!expectedOrigin`, the fixed-character Edit guard,
`canTransitionRound`, `MIN_REASON_LENGTH` and the Image Optimizer's refusal count - and the
general fix is the same each time: **bound a slice by the construct that follows it, never by
a character count**, and assert each gate separately when there are two.

---

### 14 Sep 2026 - A PLAYER CAN CHOOSE WHO THEY ARE CHALLENGING

**Shipped:** the opponent half of X10's first slice. `ChallengeCreateDialog` no longer needs a
pre-selected opponent: `challengedUser` is optional, and when it is absent the dialog opens with
an **opponent picker** - the player's friends first, then a search box for anybody else. A
**New Challenge** button on `/challenges` is the entry point, which is what makes "challenge
somebody" reachable from a screen that is not already about one person. 15 tests, 11 probes red
on exactly the expected test.

**Files touched:** `components/challenges/create/OpponentPicker.tsx` (new),
`app/api/challenges/opponents/route.ts` (new), `components/challenges/ChallengeCreateDialog.tsx`,
`app/(root)/challenges/page-content.tsx`, `__tests__/challenges/opponent-picker.test.ts` (new),
`tools/probe-opponent-picker.ps1` (new). **Nothing here is mirrored** - `apps/admin` has no
challenge creation screen - so `check:mirrors` says nothing about any of it.

**The load-bearing decision is precedence, and it is the one that costs money.** `opponent` is
`challengedUser ?? pickedOpponent`, so a prop-supplied opponent always wins and the picker is
withheld entirely when the caller named somebody. Written the other way round - which is the
natural spelling if you think of the picker as "the newer source" - a player opening the dialog
from somebody's profile could have the picker's state shadow the prop and **send the challenge
to a different person**, debiting two wallets with nothing thrown and nothing logged. Both
directions are asserted, because the positive test alone is green on the reversal.

**`null` and absent are deliberately the same fact here**, which inverts this codebase's usual
rule that a stored value and an absent one are different. Every existing caller passes `null`
while its own user record is still loading, and that is genuinely indistinguishable from not
knowing who the opponent is - both mean *ask*. The opposite reading would make three live
screens open a dialog that renders nothing.

**The search is the messaging one and is reused rather than reimplemented.**
`GET /api/messaging/search/users` already applies both directions of the block list and the
friend-request privacy setting, and returns only id, name and avatar. A second search written
for this screen would be a second answer to "may I see this person", and the copy that forgets
shows somebody who has blocked you. One consequence is inherited rather than chosen: that
endpoint matches on email and falls back to the email when a player has no name, so a searcher
can confirm an address is registered - unchanged from the friend search every signed-in player
already reaches, and narrowing it here would change a screen nobody asked to touch.

**Friends come from `Friendship.getUserFriends` and NOT from matchmaking.**
`lib/services/matchmaking.service.ts` exists and ranks opponents by **trading** skill, so for a
provider game it would return a happy, well-ordered list of people sorted by something with no
bearing on the game being played - risk X13, and it belongs with X11.5 rather than with a
picker. The route returns the **other** party of each friendship, pinned by a test: `userDetails`
holds both, so taking the first entry puts the player in their own opponent list, which reads as
a rendering oddity and is a way to challenge yourself.

**Willingness is not filtered here, deliberately.** Whether a player accepts challenges at all is
enforced by the create route against their stored presence, so a refusal names the reason - a
list that quietly omits people is indistinguishable, from the searcher's seat, from the person
not existing.

**Deviated from plan:** none of substance. The refactor that made room for this - splitting the
728-line dialog into `components/challenges/create/` - was a **separate, behaviour-neutral
commit** taken first, with a shared `readChallengeCreateScreen()` helper so the existing
structural suites keep reading the whole logical component rather than passing vacuously against
a file the code has left.

**Owner tested:** no. **Never verified by eye** - the dialog is behind sign-in and the automated
browser has no session.

**Deferred:** ~~**open challenges** (nobody named)~~ **- built the next day, 14 Sep 2026, so that
clause is correct as history and stale as a present fact; say which** - and **per-game
willingness**, which is still
outstanding and must not be summarised as done. Open challenges needed `challengedId`,
`challengedName` and `challengedEmail` to become conditional in both model copies, an atomic
claim on accept so a simultaneous second accepter is refused rather than both being debited, and
a pass over the sites that assume two named players; all three are done. Per-game willingness needs the opt-out
storage the owner chose on 13 Sep 2026, the create and accept gates that read it, and the toggle
the help page already promises. Also deferred: the friends list shows the **snapshot** name
stored on the friendship, so a renamed player reads stale - accepted, because the create route
resolves the current name from the user record, so what is stored on the challenge is always
right whatever the list said.

**Next chat should:** build open challenges, starting by counting the writers and readers that
assume a named opponent rather than trusting the figure in any plan. **Done 14 Sep 2026 - see
that day's entry.**

---

### 13 Sep 2026 - THE CHALLENGE DEFAULTS AN OPERATOR CHOOSES PER TITLE

**The instruction, from the same message that produced R73:** *"We need to be able to specify the
default settings for challenges for each specific game from the game providers are. For example
when users can join, like any time or specific, the size of the board etc so its easier for the
user to create challenges."*

**Nothing was broken, and there is no risk number.** The settings were already choosable - the
dialog has rendered the title's own `configSchema` since `13` s4.1z earlier the same day - and
**nothing was backfilled**, because a title whose operator has chosen nothing behaves exactly as
it did. What was missing is that **a player creating a 1v1 is not an operator drafting a
contest**: they cannot see the catalogue, have no reason to hold an opinion about a board size,
and cannot tell a sensible answer from one that makes the game unplayable. The fourth per-title
control chooses the join rule, the challenge length and the game's own settings, and the player's
form opens pre-filled with them.

**It is not a second source of truth, and that is the first thing a summary will get wrong.** The
provider's `configSchema` still decides which settings exist and what values they accept; this
picks among answers that schema already permits, validated against it on the way in and again on
the way out. So a title with a board size, a difficulty and a lives count needs no code, and a
test forbids a game code, a provider key or a metric name anywhere in the dialog.

**The write is strict and the read is lenient, deliberately.** An operator is sitting in front of
the form, so an out-of-range length is **refused with the range named**. A player is not, so a
stored setting the schema has since stopped accepting is **dropped back to its declared default**
and an out-of-range length is **clamped** - refusing on the read path takes the challenge dialog
down for a title that is otherwise perfectly playable, for a reason no player can act on. The
refusals also **accumulate rather than stopping at the first**, on the contest pre-flight's
precedent: an operator fixing one per submission gives up.

**The strict half is what stops the new control recreating the defect it shipped beside.** A
`reserve_full_round` default is refused when the title declares no round length (nothing to
reserve, so the switch would do nothing) and when a round is as long as the whole challenge (every
round refused from the first second, which is the owner's report verbatim). Left unguarded, the
control would be a way to reintroduce R73 through a form labelled "make it easier for players".

**Only the keys the operator actually SENT are stored.** `validateConfigValues` fills a missing
field from its declared default, which is right for reading and wrong for writing: storing what it
filled in would freeze the provider's own defaults onto the title, and the frozen copy would then
win for ever, even after a later sync changed them. An empty `settings` object therefore stores
nothing - the same reading `resolveAllowedGameTypes` gives an empty array, since nothing offers
"answer none of the questions", so nothing means it.

**The field is ours, not the provider's**: in **no** sync allow-list at all, not even
`firstSyncOnlyFields`, on `NEVER_EDITABLE_CONTENT_FIELDS` beside `playModeOverride`, with **no
schema default** - absent means the platform's own answers apply, which is a different stored fact
from an operator having chosen them - and cleared with `$unset`, never `{}`.

**The bounds are fetched, never retyped.** The minimum, maximum and fallback length are
administered on the Challenge settings screen, so a number in the dialog's `min`/`max` is a second
copy of a rule the server enforces, and the two drift in the quiet direction: the form offers a
length the save then refuses, naming a range the form itself allowed. A test asserts `1440`
appears nowhere in the component.

**One reading of the join rule.** There were three writers of that ternary the moment this
shipped - the round config, the resolver and the create-time pre-flight - so it became
`resolveChallengeStartPolicy`, imported by all three, with the negative half of the guard as the
load-bearing one: importing it is trivially satisfied by a file that imports it and spells the
comparison out again five lines later. The drift is the quiet kind, a challenge created under one
reading and played under the other, where the refusal names the clock rather than the setting
behind it. **Absent means permissive**, which is the opposite of `contest-config.ts`'s reading of
the same field name, and the model comments on both copies say so.

**The create path stores what the pre-flight was RUN against**, returned by the resolver rather
than re-imported at the call site: a title whose operator has reinstated the reservation would
otherwise be approved permissively and stored strictly.

**Tests:** 54 in `__tests__/admin/game-challenge-defaults.test.ts`, behavioural against a real
database for the write path and the sync, structural for the two components. **23 probes red on
exactly the expected test**, `tools/probe-challenge-defaults.ps1`.

**One probe came back green and the test was at fault, not the claim.** "The guard runs before the
body is read" was measured **file-wide**, so `code.search` found the `GET`'s guard - which sits
above the `PATCH` entirely - and compared it against the `PATCH`'s body read. A `PATCH` that read
its body first still compared a guard at line 45 against a body at line 73 and looked correct.
This is the count-per-handler rule one step along, so `handlerSlices` joined
`__tests__/helpers/route-guard-audit.ts` rather than being written locally, and the assertion now
runs per handler that reads a body. **A position comparison over a whole file cannot see an
ordering defect in the second handler.**

**Files touched:** `lib/services/games/challenge-defaults.ts` (mirrored into
`apps/admin/lib/services/games/`, byte-identical test), `challengeDefaults` on both
`provider-game.model.ts` copies, `apps/admin/lib/services/game-providers/challenge-defaults.service.ts`,
`apps/admin/app/api/games/providers/[providerKey]/games/challenge-defaults/route.ts`,
`apps/admin/components/admin/games/GameChallengeDefaultsDialog.tsx` wired into
`ProviderCatalogueDialog.tsx`, `challengeable-titles.service.ts`,
`challenge-provider-resolution.ts`, `challenge-round-config.ts`, `ChallengeCreateDialog.tsx`,
and `defaultConfigValues` moved into `lib/services/games/config-schema.ts` so the admin wizard and
the player's form seed from one definition.

**Deviated from plan:** nothing was planned - `22` s5 recorded per-title challenge defaults as not
built and this closes it.

**Deferred:** no bulk edit, and **no per-provider default** - a board size is a property of a game,
not of the company supplying it, so a provider-level control would be a value nothing could
sensibly hold.

**Owner tested:** no. **Never verified by eye** - the admin control is behind an admin sign-in and
the player's form behind a player one.

**Next chat should:** ask the owner to open the Games catalogue, set a challenge default on
`circuit-sprint`, and create a 1v1 on it - the pre-filled form is the only part of this that
cannot be checked from here.

---

### 13 Sep 2026 - THREE OWNER REPORTS ON THE CHALLENGE SCREENS (R73)

**The reports, in the owner's order:** the game picker must be a list because the games will be
many, and the dialog must adapt to any game's settings; a player who set a ten-minute challenge
was told **"TOO LATE TO START A ROUND"** and could not play at all; and the challenge play
screen is missing everything the competition arena has.

**The middle one is R73 and it is the only defect here** - the other two are screens that were
never built for more than one game. It was **live and player-visible on every provider
challenge**, it is the report a real player made, and **nothing was backfilled**, because no
money moved and nothing was stored wrongly: the round was refused before it was created, so the
attempt was never consumed either. Every figure on the screen was correct.

**The cause is a rule that was right for a competition and wrong for a challenge, and both
halves matter.** `POST /api/challenges` stored `roundStartPolicy: "reserve_full_round"`, and
that policy reserves the **catalogue ceiling** rather than the configured length (`12` s2.9), so
a challenge whose whole window is shorter than the title's maximum round refuses **every**
attempt for its entire life - not merely near the end. Circuit Sprint's ceiling is an hour, so a
ten-minute challenge could never start a round at any moment of its existence. The competition
side has an operator who chooses, a schema default behind them, and a pre-flight that refuses a
contest shorter than the ceiling; **a challenge has none of those**, so the same value produced
a screen with one button on it saying no.

**Owner decision, and it is a decision rather than a repair: a challenge never reserves.**
`CHALLENGE_ROUND_START_POLICY = "until_window_closes"` in `challenge-round-config.ts` is the one
definition, imported by the three writers that must agree - the config resolver, the pre-flight
in `challenge-provider-resolution.ts`, and the create route that stores the field. Written out
in each, a challenge could be created under one rule and played under another, and **the
disagreement reads as a clock problem rather than a settings one**. A late presser now gets a
round shortened by `resolveExpiry`'s existing clamp to `playWindowEnd`, which `RoundPreflight`
already discloses; since R48 a partial run counts, so a shortened round is a worse score rather
than a wasted fee.

**Absent means PERMISSIVE here, the opposite of `contest-config.ts`'s reading of the same field
name**, and that inversion is deliberate: a competition's unset policy is an operator's choice
with a schema default behind it, while a challenge has no operator, so an unset value is a
challenge created *before there was a rule* rather than one whose two players chose the strict
one. Only an explicit `reserve_full_round` reserves. **It stays a stored field rather than
becoming a hard-coded read**, so a per-title challenge default can narrow it later without every
existing challenge silently changing rule underneath its players.

**The fallback in `challenge-round-status.service.ts` was the sibling nobody would have found.**
It read `?? "reserve_full_round"` - unreachable, because the resolver always resolves the field,
but naming a value the resolver had stopped producing. Left alone it is a refusal reachable from
no configuration, sitting in the branch that claims to be defaulting *from* that configuration.
It names the constant now.

**The arena.** `/challenges/[id]/play` rendered a bare frame in a plain page; it now renders the
same `GameArenaLayout` the competition arena does, with the hero, the rules panel, the bottom
band and a standings rail. Two things about it are load-bearing. `GameArenaLayout` took a
**`competitionId` and built `/competitions/${id}` itself**; it now takes a `backHref`, because
building the path here means the layout knowing which of two contest kinds it is holding, which
is one branch away from the game-specific knowledge that folder is deliberately kept free of -
**the caller already knows, so the caller says.** And the rail is a **new
`ChallengeStandingsPanel`, not `ArenaLeaderboardPanel` reused**: both existing boards are built
around a **rank**, with a rank plate, a crown and tie markers, and a challenge has no rank until
it settles, so a rail that ordered the pair itself would be a second place the score direction is
decided - **the exact shape of R37**, where the board and the payout each worked it out
separately and disagreed. It reports and never orders: viewer first, opponent second, no
position, and the footer says why. An absent score is a **dash, never a nought** (R50's read
side).

**The picker and the settings form.** `ChallengeGamePicker` was a horizontal strip of buttons -
fine for two entries and unusable for a catalogue - and is now a **scrollable vertical list with
a capped height**, each row carrying the title's own facts and, when it cannot be challenged, the
reason. **Trading stays first**, because it is the entry most players want and a fetched list
must not push it below the fold. Beside it, `ChallengeSettingsFields` renders **the title's own
`configSchema`**, which is what "adapts to any game settings" has to mean: the field list now
travels to the client on `ChallengeableTitle.settingsFields`, the form branches on the declared
**type and format** and never on a field or game name, and a test forbids a game code, a
provider key or a field name anywhere in it. `defaultConfigValues` moved into the shared
`config-schema.ts` so the admin wizard and the player dialog seed a form from **one** definition;
a probe restoring the admin form's own copy turns red.

**The clock is minutes in the control and seconds in the payload**, resolved by
`format: "duration-seconds"` and never by a field called `durationSeconds` - the same rule as
`12` s2.9, and probed in both directions.

**The dialog's two states move TOGETHER**, and the guard for it had to be rewritten. The first
version compared two counts (`setGameSettings` occurrences at least `setSelection` occurrences)
and a probe deleting one seed came back **GREEN**, because a third `setGameSettings` exists
legitimately - the per-field `onChange` that merges one value - so `2 >= 2` held with the defect
in place. **A tally cannot tell a paired writer from an unrelated one**; each `setSelection` is
now required to carry a seed within a couple of hundred characters of it. Sixth "probe stayed
green" instance, answer: **weak test**.

**A settings change replaces the previous game's values WHOLE, never merging them** - keys from
the old schema are meaningless against the new one, and the create route refuses them while
naming a field the player never saw.

**Files touched:** `lib/services/games/challenge-round-config.ts`,
`challenge-round-status.service.ts`, `challenge-provider-resolution.ts`,
`challengeable-titles.service.ts`, `challenge-game-copy.ts`, `config-schema.ts` (mirrored),
`app/api/challenges/route.ts`, `app/(root)/challenges/[id]/play/page.tsx`,
`components/games/arena/GameArenaLayout.tsx`, the new
`components/games/arena/ChallengeStandingsPanel.tsx`,
`components/challenges/ChallengeGamePicker.tsx`, `ChallengeCreateDialog.tsx` and the new
`ChallengeSettingsFields.tsx`, plus `apps/admin/components/admin/games/ConfigSchemaFields.tsx`
onto the shared defaults.

**Tests:** `__tests__/games/challenge-arena.test.ts` and
`__tests__/games/challenge-game-picker.test.ts`, with `challengeable-titles` and
`challenge-provider-resolution` extended - **41 cases across the two picker suites, all green**.
`tools/probe-challenge-game-picker.ps1` is **21 probes, all red on exactly the expected test**
after the pairing assertion was fixed, plus `tools/probe-challenge-arena.ps1`.

**Also recorded: one probe in `tools/probe-challenge-settlement.ps1` reported STILL GREEN and
the guard is fine.** Reproducing the mutation by hand turns the expected test red. **A single
probe result is not evidence on its own** - re-run it before believing it, the same way a
`DID NOT APPLY` means the target moved rather than that the run was quiet.

**Not built, and it is the owner's own second sentence:** **per-title challenge defaults in
admin** - a join policy, a round length and a board size an operator sets per catalogue title,
which the create dialog would then pre-fill. The stored `roundStartPolicy` field is deliberately
left in place *for* it. Until it exists, a player choosing a game gets the schema's declared
defaults, which is the same set an untouched form submits.

**Owner tested:** not yet. **Never verified by eye** - every screen here is behind sign-in.

**Next chat should:** build the per-title challenge defaults, then have the owner play one
provider challenge end to end in a browser with two sessions.
...
**Deferred:** ~~the opponent picker, open challenges~~ (**both built 14 Sep 2026** - correct as
history, stale as a present fact, so say which), per-game willingness and matchmaking (X10
...
### 12 Sep 2026 - R50'S CHALLENGE HALF - A SEAT IS NOT A SCORE

**Shipped:** `ChallengeParticipant.score` is optional with no default in both apps, the accept
route seats both players through one named builder that writes no `score` key, and the test
that pinned the old default was flipped rather than deleted. The field half of R50 is closed
on challenges **before** the flow that would have fallen into it. **Latent twice over and
nothing backfilled:** nothing writes a real challenge score, nothing reads the field, every
existing seat is a trading seat, and trading's `hasResult` is unconditionally true.

**Files touched:** both `challenge-participant.model.ts` copies,
`lib/services/challenges/challenge-participant-seat.ts` (new),
`app/api/challenges/[id]/accept/route.ts`,
`__tests__/services/game-label-and-score.test.ts` (flipped),
`__tests__/services/challenge-participant-seat.test.ts` (new),
`tools/probe-challenge-phantom-score.ps1` (new). Docs: `17` R50, `11` seam 2, `05` s9.2,
`22` s8's challenges bullet, this file, the paired-document row.

**Two things were checked rather than assumed, and both change the shape.** Neither copy of
`challenge-finalize.actions.ts` mentions `score` - the winner comes from a fifth private copy
of the ranking comparator, trading metrics only. And `IChallengeParticipant` is imported
nowhere, so the typecheck cannot protect this change; the guard is a test. There is **one**
create writer, not two: the accept route inserts both seats in one call.

**No challenge migration, deliberately.** That is the opposite of the competition half. A
stored zero on an existing trading row is inert. The optional report-only count of challenge
zeros was proposed and not taken.

**Deviated from plan:** the three capital fields stay unconditionally required. Making them
conditional is a change to trading's contract and belongs with the step that actually seats a
provider challenge - the same order as `score` in X1 and capital in X5.

**Also corrected, because a claim that is false is a defect:** there *is* a friends model,
in `database/models/messaging/friend.model.ts`. Four documents had said otherwise after a
search that stopped at the top of `database/models`. FRIENDS wiring and COUNTRY on the board
are later work; the owner asked for both.

**Gates:** targeted suites 32/32; full platform **110 files / 2339** (was 109 / 2331 — the new
seat file is the tenth file and the eight new tests); one lock-timeout flake in
`competition-cancel-refund` on the busy run, green alone; `tsc --noEmit` 194 main / 223 admin,
no new error in a touched file and none disappearing; eslint `--max-warnings=0` clean;
`check:mirrors` 80 agree / 0 drifted; **9/9 probes red on exactly one named test**.

**Owner tested:** nothing. This is a schema and a writer. No player-visible screen changed.

**Deferred, and not to be summarised as done:**

- Challenge settlement still holds its own copy of all three stages in both apps. Item 2.
- The create dialog still offers trading only. Items 3-5.
- `getRankingValue`'s `join_time` tiebreaker reads `enteredAt`, a field this model does not
  declare - it declares `joinedAt` - so that branch silently falls back to `Date.now()`.
  Logged, not fixed.
- The three capital fields, as above.

**Next chat should:** put challenge settlement on the shared payout code (item 2), then the
trading-or-game picker. Do not imply a player can create a game challenge yet.

---

### 12 Sep 2026 - THE STANDINGS RAIL AS A COMPETITIVE LEADERBOARD

**Owner rejection**, the third item of the same message and the **fifth** rejection in this area:
*"STOP. The leaderboard implementation is structurally wrong. The target is not a small player
status card. It is a full competitive leaderboard sidebar with tabs, filters, many rows, and a
bottom CTA. Also make the whole sidebar extend to the same bottom edge as the gameplay board."*
Closing: *"Fix only the leaderboard panel. Do not redesign the rest of the page."* Two images, a
ten-row target and the current two-row panel, plus a ten-point acceptance checklist.

**Shipped:** the rail is now one component owning its own chrome, with two heading tabs, three
filter tabs, a four-column table, room for about ten compact rows, a bottom CTA pinned to the
bottom edge, and a height that stretches to the board beside it. `13` **s4.1y**. **No risk number
and nothing backfilled** - a short panel renders perfectly and reports success.

**Files touched:** `components/games/arena/ArenaLeaderboardPanel.tsx` (new),
`components/games/arena/GameArenaLayout.tsx`, `components/games/arena/ArenaLiveStandings.tsx`,
`components/games/ProviderLeaderboard.tsx`, `components/neon/{tokens,Cards,LeaderboardRow}.tsx`,
`app/(root)/competitions/[id]/play/page.tsx`, plus one new suite, four re-pointed suites and two
probe harnesses.

**Most of the target was already built, and saying so is what makes the rest credible.** The rail
already had the Trophy heading, a players count, a scope strip, the four columns, avatars, a gold
leader row with a crown and the full-width CTA. Four things were genuinely wrong: the height, two
heading tabs rather than a title and a pill, three filter tabs rather than one, and room for ten
rows.

**The height was a split-ownership fault, not an arithmetic one.** `GameArenaLayout` composed the
chrome while a separate consumer supplied the rows, so **nothing owned the panel's height** - a
heading at its text height, a rows box capped at 460px and a footer at its content height, inside a
grid cell as tall as the board. The arena backdrop showing underneath is the owner's *"oversized
empty dark area"*, and it was the **layout's** doing rather than the board's. Two rules came out of
the fix: **`h-full` on a grid item's wrapper is a no-op that reviews as correct** - a grid item
already stretches, so only `[&>*]:h-full` reaches the panel, which is the same mistake s4.1t made
once already - and **exactly one child may grow**, because two `flex-1` children divide the slack
and the footer drifts up by whatever the list does not need.

**`min-h` is room, never a cap.** How many players a contest has is not ours to decide, so what is
guaranteed is space for ten rows; a `max-h` reads as a sensible precaution and is the defect.

**Deviated from plan:** two of the owner's filter tabs cannot be answered *yet*. The claim
that there is no friends model was **false** - `Friendship` and `FriendRequest` live in
`database/models/messaging/friend.model.ts` with `getUserFriends` and `areFriends`. The
tabs were greyed because the rail does not call them, not because the data does not exist.
The owner then asked for both FRIENDS wiring and COUNTRY on the board, scheduled as later
work rather than this slice. All three scopes are drawn, `GLOBAL` lit and the other two as
disabled labels with the reason in a title attribute - **rendered as spans, so there is no
control that appears to work.** That **reverses a guard recorded in s4.1q** asserting the
rail draws no dead scope tabs; the test was **flipped rather than deleted**. Second time
the owner has overridden a recorded guard here.

**Nothing on the removal list lost a fact.** The activity lines moved to a PLAYERS roster tab and
remain in the recent-players feed; the `YOU` word became a tint on the name, which is the one
marker that survives on the leader's gold row; and two rows sharing a rank plate replaces the
`= #1` pill. They are one `variant` prop rather than three booleans, because each removal is only
worth making as part of the same decision.

**The polling contract is untouched** - `LiveContestRefresher` still banned on this page, the round
host still a **child** of the live provider, `getArenaStandings` still the single producer - and
the new panel fetches nothing of its own, asserted.

**Owner tested:** no. **Never verified by eye**: the play screen is behind sign-in and the
automated browser has no session.

**Found on the way:** **three structural guards had begun firing on correct code** and had to be
sliced - a file-wide wrapper count could not tell the band's selectors from the rail's, a
`flex-1` count caught a legitimate one on a roster row, and a whole-file ban on `Standings` was
satisfied by an import path. **A guard that fires on correct code is the one the next reader
deletes.** And a fourth instance of an older rule: `describeRoundActivity`'s first occurrence in a
file is its **import**, so a position assertion measured from `indexOf` is trivially true.

**Gates:** platform **109 files / 2331 tests**; `tsc --noEmit` **194** main, matching baseline
exactly with nothing new in any touched file and nothing disappearing; eslint clean on every
changed tree; `check:mirrors` OK. Probes: **19/19 red** on
`probe-arena-leaderboard-panel.ps1` and **15/15** on `probe-arena-live-standings.ps1` after
re-aiming its one stale probe. Two of the new probes came back green first time and both were the
**fourth cause** - a mutation with no observable: one left a `<NeonButton` after the scroller's
opening tag while moving it inside, and the test was tightened to require the scroller to have
**closed** first.

**Deferred:** the actual cause of the pasted HTTP 409, unchanged from the entry below.

**Next chat should:** deploy both halves, then move to the challenges side - the data model has
been ready since X1, the flow does not exist, and it inherits **R50**, because
`ChallengeParticipant.score` still defaults to `0`.

> **Done, 12 September 2026, the field half only.** `ChallengeParticipant.score` no longer
> defaults. The flow (picker, adaptive setup, play, settlement unification) is the rest of
> X10 and is still unbuilt. A document repeating the sentence above as current is stale;
> one implying a player can now create a game challenge is equally wrong.

---

### 12 Sep 2026 - AN HOUR-LONG ROUND WAS TOLD IT HAD TEN MINUTES (R69, R70)

**Owner report**, first of three items in one message and marked *"first"*: *"fix problem with more
than 10 min game - when i choose 60 round the game didn't finish, check all times to work correctly.
Also when i choose custom in wizard no box comes to add custom round time. Also make sure that when
admin specify time the game respect that and finish - check all."* Pasted with it, two `pm2 logs`
lines showing a round's result refused with **HTTP 409** and retried.

**Shipped:** six fixes across both repositories - the game service's ceiling, its playability gate,
its sweeper query, its delivery log, the wizard's Custom control and the platform's expiry headroom.
**R69** and **R70**.

**Files touched:** `games-service/src/rounds/lifecycle.ts` (the ceiling, the gate, the sweeper),
`games-service/src/callback/deliver.ts`, `apps/admin/components/admin/games/ConfigSchemaFields.tsx`,
`lib/services/games/round.service.ts`, plus `games-service/tools/test-play.ts` (+6),
`__tests__/services/round-lifecycle.test.ts` (+1), `__tests__/admin/contest-round-clock.test.ts`
(two pre-existing weaknesses strengthened) and three probe harnesses.

**The primary fault was one line.** `hardDeadline` read `PERFECT.maxDurationSeconds` - 600 seconds -
for every round, whatever title it belonged to. Correct by accident when it was written, because
Perfect's maximum was then the largest in the catalogue; a **ten-minute cap on every Sprint round**
from the moment `12` s2.9 raised Sprint's maximum to an hour on 8 September.

**Why it hung instead of stopping, which is the finding to carry.** `hardDeadline` decides the
countdown; `playability`, which decides when a round ends, weighed the deadlines itself and never
called it. The clock reached zero, the client asked the server rather than deciding for itself, the
server said *still playable*, and the board sat at 0:00 for fifty minutes. There is now one
deadline - `playability` asks `hardDeadline` **whether**, and `expiresAt` only chooses **which**
terminal state.

**Deviated from plan:** nothing. Every change is inside code these chapters already own.

**Owner tested:** not yet, and it needs a deploy of both halves before it can be. `games-service`
is TypeScript with an untracked `dist`, so `git pull` alone changes nothing - **R66 was reported
twice for exactly that reason.**

**Deferred:** the actual cause of the pasted 409, which needs the two round documents on the
production database. Most plausibly a round already terminal with no stored score, which gate 8
refuses and the route maps to 409; the R44 settlement cut-off is ruled out, because
`unresolved -> expired` is legal. From this deploy the reason travels in the log line.

**Found on the way, and not planned:** **ten probes across two platform harnesses had been reporting
nothing** - nine anchors moved by s2.8, s2.9, s2.10 or R61, and two with stale *claims* as well,
which the anchor failure hid. Two of them guarded these very clock rules. Re-aiming them exposed two
genuine test weaknesses, both fixed by strengthening the test rather than loosening the probe: a
slice whose end marker had moved (`indexOf` returns -1, `slice(0, -1)` hands back almost the whole
file, every assertion trivially true - assert both ends exist), and a per-branch claim asserted
file-wide (`contest-preflight.ts` states the same fact as a refusal *and* as a warning - count the
occurrences).

**Gates:** platform 108 files / **2320 tests**; `tsc --noEmit` **194** main and **223** admin,
matching baseline exactly; eslint clean on every changed file; `check:mirrors` OK;
`games-service` **305 tests** with `check:isolation` passing. Probes: 23/23 red on
`probe-contest-round-clock.ps1`, 11/11 on `probe-round-start-policy.ps1`, 14/14 on
`probe-round-clock.ps1`.

**Next chat should:** rebuild the arena leaderboard sidebar to the owner's reference - it is the
third item of the same message and a **fifth** rejection in that area. Three things on his removal
list exist for recorded reasons and must be reported rather than silently faked: `"Playing now"` is
the honest state for a live round, an absent score renders `-` and never `0` (R50), and the `YOU`
marker identifies the viewer. `FRIENDS`, `COUNTRY` and a player count have **no data source**, and
"around 10 rows" is a property of the contest rather than of the panel.

> **Done the same day, in the entry above** (`13` **s4.1y**). All three recorded reasons survived
> rather than being faked: the activity lines moved to a roster tab, the `YOU` word became a tint,
> and the two unanswerable scopes are drawn as disabled labels with the reason on them. A document
> citing this as the outstanding next step is stale.

---

### 11 Sep 2026 - THE HERO AS ONE THIN BANNER

**Owner instruction:** the built hero beside a reference for it - *"the current banner is far too
tall and has unnecessary content/cards underneath... TARGET: a single compact horizontal banner,
approximately 110-125px high on desktop... the banner must stay visually SHORT"* - with a target
structure of `| logo | title and text | 4 small features | trophy art |`.

**Shipped.** `13` s4.1w. The arena's hero is a fixed **118px** single row: a 132px logo, the
catalogue name split at its own colon into a title and subtitle, a one-line tagline, a one-line
description, four unboxed icon-and-label features, and the game's artwork bleeding to the right
edge. 15 tests, 18 probes red on exactly one failure.

**Files touched:** `components/games/arena/ArenaIdentity.tsx` (rewritten),
`components/games/arena/GameArenaLayout.tsx` (the hero block only),
`components/games/arena/arena-facts.ts` (`splitGameTitle`, `roundLengthLabel`, `heroFeatures`);
`__tests__/games/arena-hero.test.ts` and `tools/probe-arena-hero.ps1` new.

**The mechanism, because it is the second rebuild of this component in a day.** The previous
version set `min-h-[220px]` and let its content decide. **A minimum height is a floor content is
free to exceed**, so everything added afterwards was individually reasonable and the sum was the
~400px header in the screenshot, with no rule broken anywhere - which is why it needed a
photograph to find. The height is now fixed, and `min-h-`, `h-auto` and the old `p-5 sm:p-7` are
asserted **absent**: a floor beside a ceiling is the ceiling losing while still reading as though
it governs. Same fault as the bottom band, one element up.

**Deviated from plan:** three things, each recorded rather than absorbed.

  1. **`BIG REWARDS` was refused.** It is one of the four labels the instruction names, and it is
     a promise this banner cannot check - what a contest pays depends on its prize pool, the hero
     receives no prize figure, and a **free** contest would carry the phrase too. The trophy slot
     holds `Skill based`, the platform's own guarantee, which is the strongest claim that is
     always true. A probe puts the reference's wording back.
  2. **The 8px feature labels are not the kit's.** `NEON_LABEL` is 11px with wide tracking, which
     wraps "Global leaderboard" to three lines in a 68px column and takes the banner with it.
  3. **The artwork is not a full-bleed background, and that is arithmetic rather than taste.** The
     banners are 1280x720; `object-cover` across a 1350x118 panel shows a 16%-tall horizontal
     slice, and under `cover` a box wider than 1.78x its height never crops horizontally at all -
     so no value of anything makes a 16:9 picture fill a 118px strip. It is a dim full-width copy
     for continuity plus the picture at 300px tall in a 330px window anchored right. Their order
     is asserted by **position**: written the other way the wash covers the piece and the trophy
     disappears behind a 40%-opacity copy of itself.

**Owner tested:** not yet. **Never verified by eye** - the arena is behind sign-in and the
automated browser has no session.

**Deferred:** nothing new. The two operator actions carried from s4.1u still stand - the panel
illustrations are in swapped upload slots, and three numbered steps appear once `howToPlay` is
written as three separate lines.

**Next chat should:** deploy, then take the owner's next screenshot.

---

### 11 Sep 2026 - THE SAME TWO FAULTS ON THE LOBBY'S PANEL

**Owner instruction:** a screenshot of the **lobby**, with *"see image change also this How Circuit
Sprint: Fast and Fun Spatial Puzzles is scored and do it like the other you fixed"*. It exists
because of a sentence in the entry below: told that the scoring rule was *"not deleted - the lobby
carries it in full"*, he went and looked at the lobby.

**Shipped.** `13` s4.1v. Both screens read `How it works`; the lobby's body is one row rather than
a two-column grid; the diagram is `w-[132px]` and `contain`, beside the instructions rather than
under them; the unused `column` layout is deleted and `layout` is required. 3 tests, 4 probes red
on exactly one failure.

**Files touched:** `components/games/GameRulesPanel.tsx`;
`__tests__/games/game-rules-panel.test.ts`, `__tests__/games/arena-band.test.ts`;
`tools/probe-arena-band.ps1`.

**Nothing was computed wrongly and there is no risk number.** A page-wide heading and an oversized
picture both render perfectly; nothing was backfilled.

**The finding, and it is about scope rather than about CSS.** These are the **same two faults** the
band was rejected for, one page along, and they were **left untouched in that commit on purpose** -
the instruction said *"do not modify the rest of the Circuit Sprint page"*. So: **a fault corrected
on one screen is worth looking for on every other screen that renders the same component**, because
the reason it was scoped out is a reason about the request, not about the code.

**The heading fault is not the length of one name.** `How ${presentation.gameName} is scored` reads
correctly against a short name and produced `How Circuit Sprint: Fast and Fun Spatial Puzzles is
scored` against the live one, because `gameName` is operator-editable free text of unbounded length
and this title carries its tagline inside it. Any title with a subtitle reproduces it, so shortening
this one name would have fixed nothing. The two assertions pinning the template were **flipped to
forbid it rather than deleted**, because *"the lobby has the room and is where a player is deciding
whether to pay"* was believed for a day and is the reason nobody looked.

**A width and a position are two defects, and one fix is not the other.** `NeonIllustration` takes
its height from its width, so uncapped in a 300px cell a 4/3 graphic is 225px tall - but a capped
picture still *below* the text adds its own height to the panel. Hence one row, with `md:grid-cols-2`
asserted **absent** as well as the row being present, and the position guard asserting the flex
**direction** rather than document order: a `flex-col` is the fourth known cause of a green probe and
has already cost one in the band's suite.

**Deferred:** nothing new. The two illustrations are still in swapped upload slots in the live
catalogue and `howToPlay` is still one paragraph - both operator actions, both said in the admin
field hints.

**Next chat should:** pick up the 35-task document at tasks 15-16, or the challenges work, whichever
the owner names.

---

### 11 Sep 2026 - THE BAND AS A THIN INFORMATION STRIP

**Owner instruction:** the band beside the reference a fourth time, opening *"STOP. The current
...
600px+ high. THIS IS WRONG."* He named the spellings that produce it (`min-height: 400px`,
vertical `flex: 1`, `height: 100%`, `grid-auto-rows: 1fr`), gave the three panels' contents line
by line, and closed with a ten-point acceptance test and *"Do not modify the rest of the Circuit
Sprint page."*

**Shipped.** `13` s4.1u. A fixed `sm:h-[104px]` row of three cards at 34/34/32, each with a
compact heading, a capped list of one-line items and a small picture beside the text rather than
under it. 29 tests, 32 probes red on exactly one failure.
...
before a guard existed - and **nothing was backfilled**.

**Six things from it that generalise.**

- **A fixed height and an aspect ratio are two numbers that must be solved together, and the
  first attempt did not.** The band was built at 96px, which after a 30px heading and padding
  left 49px of body - and the owner had also specified the pictures at 65-75px. `NeonIllustration`
  derives its height from its **width** through an aspect ratio, so a 66px-wide square is 66px
  tall and would have been cropped by the `overflow-hidden` backstop: **silently, on the two
  acceptance points that ask whether the pictures are there and small.** Caught by doing the
  arithmetic for the acceptance self-check rather than by any test. 104px was arrived at
  backwards from the picture, and is inside the owner's own 95-115. Two rules follow: **a card
  writes a width and never a height** when its content is aspect-ratio sized, or the two
  disagree the moment either moves; and **both halves of a row height are asserted or neither
  is** - the feed's 20px avatar plus `py-0.5` is 24px, and one step up to `py-1` pushes the third
  row under `overflow-hidden` while `FEED_LIMIT` still reads 3 two lines away.
- **A measurement is a specification, and one number does not enforce it.** `sm:h-[104px]` is the
  height; the banned spellings are asserted **absent**, because any one of them re-inflates the
  row while the fixed height stays in the file reading as though it still governs; and each card
  **caps its own items**, because `overflow-hidden` alone is a card silently dropping the
  operator's fourth tip with nothing on screen to say so.
- **A ban on a substring fires on correct code.** `not.toMatch(/h-full/)` failed on the required
  `[&>*]:h-full` the first time the suite ran, so the rule became a **count** - every occurrence
  must be the child-selector form. Same lesson as s4.1g's narrowed `GameIcon` ban, and a guard
  that fails on a correct file is the kind the first person it inconveniences deletes.
- **Deviating from an owner's own CSS needs the reason recorded, not the diff hidden.** He
  supplied `grid-template-columns: 1.15fr 1.15fr 1fr`; the build uses flex bases dividing the
  row identically, because s4.1r's trap still governs - a hidden **grid** item leaves its track
  behind and puts one panel adrift beside an empty third of the page, which is the common case.
- **Reversing a recorded decision means naming what it cost.** Two here. `What to expect` became
  `Game tips` on the owner's explicit override, and the original argument survives the rename -
  the seeded highlights still read as marketing, so the card promises tips and shows selling
  points until somebody rewrites them. And the scoring rule left the arena, which `13` s4.1k put
  there on the owner's own earlier instruction: it is **not deleted**, the lobby carries it in
  full and every route into the arena passes through the lobby, but **a player halfway through a
  contest cannot re-read it without leaving the board.**
- **Two of the ten acceptance points are answered by data, not code, and the hints are where
  that gets said.** The two uploads are in swapped slots in the live catalogue, and the strip
  numbers each **line** of `howToPlay` - so the reference's three instructions appear the moment
  an operator writes three lines, and today's one-paragraph copy correctly draws one. The field
  accepts both and saves both, so the alternative to saying it beside the field is a screen that
  looks wrong for a reason that is nowhere on the screen.

**And two probing lessons.** One probe came back **GREEN on the fourth known cause**: adding
`flex-col` puts the picture *under* the lines while leaving document order untouched, so the
positional assertion had no observable for it and the flex **direction** is now asserted too.
And six reported `PROBE BROKEN - no test matched`, because vitest's `-t` is a **regular
expression** and five test names carried apostrophes - a fragment matching nothing produces a
passing run over zero tests, which reads exactly like a guard that does not work. The names were
made regex-safe rather than the probes escaped.

**Never verified by eye** - the play screen is behind sign-in.

---

### 11 Sep 2026 - THE BAND'S PICTURES, AND THE PANELS FINALLY LEVEL

**Owner instruction:** the arena's bottom band again, all three panels circled - *"still dont like
these, first they dont align, one bigger than the other, they must be the same"* - beside the
reference showing a small graphic against each block of copy: *"have images next to letters icons,
recreate the images and icons, or better create a place in the games add content in admin and when
i put images there will show them"*. And, last, *"when i have mouse over the buttons need to have
the hand"*.

The same message also carried the games-service half, shipped as `21` s4.1s and recorded in its
own status row: *"boards done needs to replace solved and solved deled, no need both, and rearange
bottom info to be alling"*.

**Shipped.** `13` s4.1t. Three panels the same height, a picture beside two of them with an emblem
drawn where there is none, both pictures uploadable per title in the game content dialog, and a
hand cursor over every control in both apps. Plus `21` s4.1s in the other repository: one figure
in one place, and the strip beneath the board made even.

**Nothing was computed wrongly and there is no risk number.** A short panel, an absent picture and
an arrow cursor all render perfectly and report success, so every guard here is structural and
**nothing was backfilled**.

**Four things from it that generalise.**

- **A flex item already stretches, so the wrapper was never what was short.** The band's three
  slots were the same height while the owner was looking at panels that were not: each panel sizes
  to its own text and leaves the rest of its stretched wrapper empty. `h-full` on the wrapper is
  therefore **a no-op that reviews as correct**, and a guard asserting the band "mentions
  `h-full`" is green against it. `[&>*]:h-full` reaches through to the thing that was actually
  short. It goes on the wrapper rather than in the three panels because **two of them are also
  rendered in the lobby sidebar**, where a forced full height would stretch one card down the
  length of the whole column.
- **When the absent case is the NORMAL case, the fallback is the component.** No title in the
  catalogue carries either new image, so "render the `src`, render nothing otherwise" is green
  against every test that supplies a URL and ships a band illustrated on nothing and
  broken-looking on everything. `NeonIllustration` draws a two-ring emblem instead, so an unset
  value is a deliberate look and an upload **replaces** it rather than filling a hole - the same
  arrangement as `bannerUrl` falling through to `banners.ts`.
- **A condition can look careful and deliver nothing.** The rules panel takes a `layout` prop and
  the arena passes `column`, not `wide` - so `layout === "wide"` around the illustration reviews
  as considered, passes any test that only checks the picture exists in the file, and leaves it
  off the one screen it was asked for.
- **The hand cursor was a framework default, not a per-component omission.** Tailwind v4's
  preflight no longer sets `cursor: pointer` on a `<button>`, so every button in both apps lost it
  on the upgrade and `app/globals.css` had been putting it back one component class at a time -
  which is exactly why it kept reappearing. Fixed on both `button.tsx` copies and on the kit's
  `BASE`, with `disabled:pointer-events-none` and `cursor-not-allowed` asserted beside it, because
  a hand over a disabled control promises a click that does nothing.

**The two fields are OURS, and that is the distinction that decides where they live.** A logo and
a hero banner identify a provider's title; `howToPlayImageUrl` and `highlightsImageUrl` illustrate
**our** panels at our size. So they are in **no sync allow-list at all**, not even
`firstSyncOnlyFields`, no provider is asked for them, and
`ChartVolt-Game-API-Requirements.html` is unchanged with **no version to bump**.

**Three probes came back green first time, on three different causes**, which is worth keeping
together: a **mutation with no observable** (the caption probe replaced a comment), **an import is
not a use** (`indexOf("NeonIllustration")` found the import on line two, so the layout gate's
slice examined the file header and the exact injected defect passed - the `canTransitionRound`
trap again), and **a weak test** (the projection guard counted occurrences of the field name, and
deleting it from the `.select(...)` left the interface, the lean generic and the return still
naming it).

**Gates:** 13 new tests, 15 probes red on exactly one failure, full suite **2255** green,
typecheck at the **194** and **223** baselines exactly, lint clean, `check:mirrors` OK.

**Never verified by eye** - the arena is behind sign-in, and for a second reason on two of the
three panels: they render nothing until the catalogue is re-synced.

---

### 11 Sep 2026 - THE LARGER THE BOARD, THE SMALLER IT WAS DRAWN

**Owner instruction:** *"this is the large board, when we choose medium and large the system makes
the board smaller. This is not what I want, I want the board to be big, fill like I show in red
arrows"*, with four red arrows across the empty space around the board and two green circles round
the `UNDO / CLEAR` rail and the stat column - *"you need to rearrange the info and buttons in green
circles"*.

**He is right, and the direction of the fault is what matters: a bigger grid really did get a
smaller board.**

**Shipped.** `21` s4.1r. The arena is one column - board, then figures - the actions moved into the
footer beside Submit, `desiredFrameHeight` derives its cell from the width the frame actually has,
and `fitBoard` and the height request share one measurement. 6 new tests (297 in the service, from
291), 13 probes red on exactly the named test.

**The cause.** The board was **width-bound**. The frame is 450-650 pixels; a cell is square, so
`boardCellPx` takes whichever axis answers smaller, and s4.1q had put a rail on each side of the
board inside the same grid - about **220 pixels of a 500-pixel frame**. An 8x8 divided what was
left into cells at their 34-pixel floor; a 4x4, dividing the same remainder four ways, looked
healthy. **`boardCellPx` was correct throughout and was being handed a third of the room.**
Separately `desiredFrameHeight` asked for room for a 72-pixel cell the width would not permit, and
that surplus is the band the vertical arrows point at. **Two faults, one report.**

**Findings.**

- **Third distinct cause of a small board on this screen, and all three presented the same way.**
  s4.1f's frame-height feedback loop, s4.1m's `align-items: center`, now the width. Every one was
  a layout fault that read as an arithmetic one, and every review of `boardCellPx`, `fitBoard` and
  `desiredFrameHeight` came back clean each time.
- **The safe axis and the unsafe axis.** Reading the **width** is safe because nothing we report
  changes it - it is the host's column. Reading the **height** is the original defect, because the
  height *is* what we report, so a height derived from it is a fixed point at the host's opening
  320 pixels. The two reads look identical in a diff, so the distinction is in the function's own
  comment.
- **`chromeHeightOf` needed no change, by construction.** It sums the bars then adds
  `arena - board`, so a strip inside the arena is counted by the subtraction and buttons in the
  footer by the sum. What it could never count was a rail beside the board as a *column*: no
  taller than the board, so worth nothing in height while costing a third of the width.
- **A recorded hazard was honoured rather than discarded.** s4.1q put Clear in the rail because
  "a destructive control beside the confirming one, **both the same width**, is how a player wipes
  a finished grid instead of sending it". The clause that carries that argument is the **width** -
  Clear is a 62-pixel ghost icon, Submit takes the leftover and lights cyan - so the comment was
  rewritten in place to say that equalising them reinstates the hazard, because a reader seeing
  them share a row will otherwise assume the rule was abandoned.
- **One measure, two callers.** `fitBoard` and the height request each read the wrap's box; they
  now both call `boardSpace()`. Two measurements of one box drift on the first margin, and the
  symptom of that drift is a board sized for one width inside a frame sized for another - **this
  defect arriving by a different road.**
- **One probe came back green on two guards covering each other**, and it is **recorded as
  unprobed with the reason** rather than shipped: the width half of `widthBoundCellPx`'s first
  guard changes no answer, because `spaceForGrid` reports 0 for a non-positive width and the line
  below returns the target anyway. The `cols` half genuinely decides something - `grid / 0` clamps
  to `MAX_CELL_PX` - so it carries the probe instead. A green probe would have taught the next
...
**Deferred.** Best Time across rounds, Combo, the LEVEL number and the GLOBAL/FRIENDS/COUNTRY tabs
still have no data source on either side of the seam, and a Hint is forbidden outright.

**Next chat should:** take the owner's next report. His sentence trailed off mid-word ("so
rearrange a"), so if the arrangement is not what he meant, the pieces now move independently.

**Deploy:** `cd /var/www/chartvolt/games-service && git pull && npm run build && pm2 restart
chartvolt-games` - the build is for R66 and the progress callback, not for this change.

---

### 11 Sep 2026 - THE LEADERBOARD ROW, AND THE LOOKUP THAT FOUND NOBODY

**Owner instruction:** *"our leaderboard first doesnt fit the wording, the names are cut of and we
dont have the avantars of the player fix it fast"*, with a crop of the arena's leaderboard.

**Two defects wearing one screenshot**, and they are not in the same layer.

**Shipped.** `lib/utils/user-lookup.ts`'s batch lookup now finds a player three ways, exactly as
the single lookup beside it always has; and the leaderboard row's name line carries the name and
the "you" marker and nothing else. 5 new tests in `__tests__/games/user-lookup-batch.test.ts`
against a real database, 2 new structural tests, 6 new probes - 25/25 red on exactly one failure.
2242 tests green, typecheck at the 194 baseline exactly, lint clean.

**Findings.**

- **The avatars were built, wired, tested and probed yesterday and could never have appeared**
  (**R68**). `getUsersByIds` filtered on the `id` **field** alone; Better Auth's MongoDB adapter
  keeps the identity in `_id`, so the query matched nothing, the map came back empty, every row
  correctly had no picture and `NeonAvatar` correctly drew initials. **No error, no log line,
  every layer reporting success.**
- **The evidence was ten lines above it.** `getUserById` tries `id`, then `_id` as an ObjectId,
  then `_id` as a string - three fallbacks, written because one was not enough. The global
  leaderboard shows faces only because it goes through `getAllUsers()`, which applies **no id
  filter at all**.
- **No other screen was affected, and say that precisely**: the lookup has exactly one caller,
  added the day before. This arrived with the feature that first used the function.
- **The guard has to be behavioural.** `{ id: { $in: ids } }` reads perfectly, so no structural
  assertion could call it wrong. Three real documents, three id shapes, all found **under the id
  the caller passed in** - and the map keyed under both, because a document fetched through `_id`
  whose `id` says something else is otherwise found and then lost on the way out.
- **The names were cut off by the row's own furniture, not by the rail**, which had already been
  widened twice by counting characters. A second crown while the rank plate drew one, and a
  `= #1` chip repeating the rank that plate states. Both facts are still on the row - which is
  the test for removing furniture rather than shrinking it.
- **A pre-existing assertion had to be re-pointed and its claim is unchanged.** It sliced
  backwards from the score to the nearest `<span`; the tie marker became the nearest one, so it
  failed on a **correct** file. A slice taken backwards finds whatever is closest - third time
  that has cost a false result here.

**Deviated.** Nothing.

**Owner tested.** Not yet - the arena is behind sign-in, and the avatar half cannot be seen at all
until this runs against a real database.

**Next chat.** Deploy is platform-only. The catalogue re-sync (task 17) is still outstanding, and
without it the band's first two panels render nothing.

---

### 11 Sep 2026 - THE ARENA'S FOOT, ON THE REFERENCE AFTER ALL

**Owner instruction:** *"image 1 must look like image 2 recreate the graphics to look like that
all the info there must be the same do this fast"* - the arena's bottom band as built, beside the
reference's three panels.

**Shipped.** The band is the reference's: **How the game is scored** (`GameRulesPanel`, content
unchanged, now in the kit's headed shell), **What to expect** (`ArenaHighlights` in a new `list`
layout of ticked lines), and **Recent players** (`ArenaActivityFeed`, moved out of the sidebar,
with the reference's `Live activity` label). 195 tests green in the touched suites, 2235 in the
platform suite, typecheck at the 194 baseline exactly, lint clean, 28/28 theme probes and 17/17
live-standings probes red on exactly the named test.

**Findings.**

- **The band had been tried and reverted that morning, and the diagnosis is what made it
  buildable.** All three slots render nothing when their content is absent, and absent is the
  common case; **a layout cannot see that its child returned `null`**, so a grid column holding
  one is still a column. **`:empty` is how CSS sees what React cannot** - `empty:hidden` on each
  wrapper, and `flex-wrap` rather than `grid-cols-3`, because a hidden *grid* item leaves its
  track behind while a hidden flex item leaves the line.
- **Nothing was broken.** Every fact on the old band was correct and is still there. There is no
  risk number and nothing was backfilled.
- **The mock's own words are game-specific, and the arena may not hold them.** A test bans
  game-shaped nouns in quoted strings in that folder, so the numbered steps and the ticked lines
  arrive as catalogue data or not at all - the list appears the moment an operator writes line
  breaks. The feed's score still carries **no `+` sign**, for s4.1n's reason.
- **`GAME TIPS` became `What to expect`,** because these are the operator's feature cards and not
  advice. A caption is a claim.

**Deviated from plan:** nothing new. It reverses s4.1m's revert and s4.1n's sidebar placement,
both amended in place with the history kept rather than rewritten.

**Owner tested:** no. **Never verified by eye** - the arena is behind sign-in.

**Next chat should:** carry on with the 35-task document. Deploying this needs the platform build
only; nothing in `games-service` changed.
...
three boxed cells, a vertical `Undo / Clear` rail, a coverage meter and stat tiles beside the
board, and one wide `Submit solution`. `21` **s4.1q**.

**Why it is this repository's other half.** `13` s4.1m answered the platform's share of the
owner's reference and recorded that the middle of that picture is drawn by **games-service** - a
separate repository that shares no code with this one by design. The owner then authorised that
half in as many words ("do as well"), so this closes the reference rather than explaining it
again.

**Nothing was broken and there is no risk number.** The surface was readable and correct after
s4.1f and s4.1m; it simply was not the reference. **Nothing was backfilled**, no money figure
exists anywhere near this code, and the one behaviour addition is Undo.

**THE TWO REFUSALS ARE THE LOAD-BEARING PART, and a summary will drop them.** The reference puts a
running **SCORE** in the round header and a **HINT** button with a count of three beside the
board. Neither is built, and neither is an effort decision.

- **`PlayState` carries no score, no rank and no prize**, deliberately, and `resultCopy` is pinned
  by a test that hands it all three and asserts none reaches the screen. So a score cell could be
  filled only by **computing one here** - a second scoring authority, which is the single thing
  the provider seam exists to prevent - or by **sending one down**, which is a protocol change and
  the owner's decision rather than a styling one. A probe injects the figure somebody would
  actually write, a plausible `solved * 250` derived from real facts that nothing authoritative
  ever agreed to.
- **A hint is forbidden outright by the platform's fairness rule.** It tells a player something
  about the solution they had not worked out, which improves a score in a paid contest, and a
  consumable count is exactly the marketplace mechanic that rule names - *"extra time, retries,
  hints, skips and easier content are not"*. A probe adds `hintState` beside `undoState`.
- **`LEVEL` and `Combo` have no source either.** The honest reading of the reference's LEVEL bar
  is the board's own **coverage**, which is a real fact the client holds and one a player under a
  clock acts on, so that is what the meter shows. Combo is not built.

**Undo IS built, and the distinction from a hint is the argument.** It removes the path drawn
last, and it is **strictly weaker than the Clear button that has always been here** - the same
position is already reachable by touching that pair's terminal and drawing it again - so it
changes no rule of the puzzle, reveals nothing and cannot improve a score. It is an input
...
- **The order is the order pairs were DRAWN, never the order they were joined.** A player who
  ...
  silently skips their most recent work and removes something from several moves ago - and a
  control that undoes the wrong thing is worse than none, because the player then repairs it under
  a clock. The test asserts **which** pair survives, on a hand-written grid: counting alone passes
  either way, which is how its first version let the defect through, and a generated puzzle cannot
  answer it because `generated.solution` has no fixed relation to `puzzle.pairs`.
- **A guard that read as careful was UNREACHABLE, and it made two real resets unprobeable.** The
  ...
  was empty and the walk skipped the lot. Two guards each silently covering for the other, which
  is R42's shape. **Deleted rather than probed**, and the two resets are the real mechanism.
- **`undo()` must RETURN false rather than merely doing nothing**, because `app.js` plays the
  clearing sound on a true - so a false success is the game making the noise of an action it did
  not take, and a disabled button is still keyboard-reachable in some browsers.

**Two layout decisions that would have been wrong the obvious way.** `chromeHeightOf` **measures
the arena and subtracts the board**, so the rails count towards the frame height when they stack
on a phone and cost nothing on a desktop, without the function knowing which arrangement is in
force and without a media query restated in JavaScript; a fixed allowance would shrink every
desktop board. And the phone rearrangement uses **explicit `grid-row`s inside a max-width query,
never `order`** - grid auto-placement follows *order-modified* document order, so an `order` rule
firing only on a wide screen leaves a phone arranged by DOM order, which is the mistake already
made once on the platform's own arena layout (`13` s1.1h).
...
**Files touched:** `games-service/public/play/presentation.js` (six pure functions -
`boardProgress`, `formatBoardTime`, `bestBoardTime`, `roundHeaderCells`, `playStatTiles`,
`undoState`), `board.js` (`drawOrder`, `noteDrawn`, `lastDrawn`, `undo()`, `canUndo()`, and
`{ settled: true }` on pointerup so a completed drag can be counted), `index.html`, `app.css`,
`app.js`, `tools/test-board.ts`, `tools/test-presentation.ts`, `tools/probe-presentation.ps1`.
**Nothing in the platform repository changed at all**, so `check:mirrors` and every platform test
say nothing about any of it.

**Gates:** 15 new tests (**285 in the service**, from 270), **48 probes red on exactly the named
test**. Four came back green on the first run, both causes already known - one weak test (counting
instead of identifying) and one unreachable guard covering for two real ones. `check:isolation`
green, main typecheck at the 194 baseline, eslint clean.

**Deferred, and it must not be summarised as done:** Moves counts completed drags and is real, but
the reference's **Best Time** as a personal record across rounds, **Combo**, the **LEVEL** number
and the **GLOBAL / FRIENDS / COUNTRY** tabs all have no data source on either side of the seam.

**Owner action:** `public/play` needs only a pull and `pm2 restart chartvolt-games` - the served
set is read from the directory at boot (s4.1i) and the assets carry a content fingerprint
(s4.1o), so no build and no cache purge. **`npm run build` is still required in the same
movement**, because R66's clock fix and the progress callback are TypeScript and have never been
built on the server.
...
### 11 Sep 2026 - A ROUND IN FLIGHT NOW SAYS WHAT IT HAS DONE (protocol 1.4 -> 1.5)

**Shipped:** mid-round progress reporting, end to end. A signed
`POST /api/games/providers/{key}/progress` writes `game_round.scoreBreakdown` and a new
`progressAt`, and our own games-service fires one after each solved board - so a contest board
says what a player has solved **while they are still solving it**, instead of "Playing now".

**Files touched:** new `lib/services/games/round-progress.service.ts`,
`app/api/games/providers/[providerKey]/progress/route.ts`,
`__tests__/games/round-progress.test.ts`, `tools/probe-round-progress.ps1`,
`games-service/src/callback/progress.ts`, `games-service/tools/test-progress.ts`; modified both
`game-round.model.ts` copies (`progressAt`), both `round-types.ts` and `contract.ts` and
`chartvolt-games.adapter.ts` copies (`progressCallbackUrl`), `round.service.ts` (**two** call
sites, not one), `round-launch.service.ts`, `contest-activity.service.ts`,
`games-service/src/{store/round.model,rounds/create,rounds/play}.ts`,
`games-service/package.json`; docs `01` s4/s5.5/s7/s11, `04`, `06` s2.2, `13` s4.1p,
`ChartVolt-Game-API-Requirements.html` **1.5**.

This is the **third** of the three things the owner named on rejecting the arena a second time,
and the one s4.1o filed as a protocol change: *"and have also not showing live the boards the
user finished"*.

**Nothing was computed wrongly and there is no risk number.** There was no mid-round reporting
**anywhere on either side of the seam** - a score exists only after `finishRound`, the frame's
`postMessage` types carry no score field by construction, and the games-service's per-board
record never left that database. Nothing to backfill; this is a capability that did not exist.

**The whole design is one sentence: it must not become a second scoring door.** Four decisions
carry it, and each one's obvious alternative is worse.

- **A separate route, not a `final: false` flag on `/events`.** A flag is how the second door
  opens - a body claiming it is not final reaching the scoring path is one plausible-looking
  branch away.
- **An explicit two-path `$set`, never a spread** of anything the provider sent. A spread is how
  the next field arrives, and the field after that is `rawScore`. A test asserts there is exactly
  one `$set`, that it does not begin with a spread, and that the file never contains `rawScore`,
  `CompetitionParticipant`, `syncParticipantScore`, `applyResult` or `calculateRankings`.
- **`status` is read and never written**, so the route can neither finish a round nor revive a
  finished one, and it refuses anything not `pending` or `launched`.
- **The sanitiser keeps primitives only, 24 at most**, drops `__proto__` / `constructor` /
  `prototype` and writes through `Object.defineProperty`. A breakdown is unbounded provider data
  written once per solved board, so this is **a store that fills up by succeeding** - the
  `brandingFiles` failure in a new place.

**The read side needed almost nothing, which is why this was affordable.**
`getContestActivity` already passed a live round's breakdown through, so the board and the feed
render progress with **no component change at all**. The single edit is `activityAt`, where
`progressAt` sits below `completedAt` and **above** `startedAt` - without it every live player's
entry is frozen at the moment they pressed Play, so a contest in which four people have each
just solved a board orders them by who started first and never moves again.

**The game's half is the opposite of `deliver.ts` in every way, deliberately.** No retry, no
queue, no delivery record, a 3s timeout, and `void` rather than `await`. A lost result is a
contest nobody can settle; a lost progress report is one stale line for a few seconds, and
**retrying would be strictly worse than not** - a queue of stale reports arriving out of order
behind a finished round is a board that goes backwards. The `void` is the load-bearing half: it
sits between a player solving a board and being handed the next one, in a round they **paid**
for. It is sent on the **continuing branch only**, pinned with `lastIndexOf` because the same
finishing call appears earlier in `resumeRound` and measured from the first one the positional
assertion is trivially true.

**Refusals on this route are deliberately not logged.** It is the highest-rate provider route by
a wide margin - once per solved board, per player, per contest - so a line per refusal turns one
misconfigured game into a flood that buries the warnings that matter. The status code is the
whole diagnosis and `01` s5.5 tells providers what each one means.

**Deviated from plan:** nothing planned this; it came out of s4.1o's own "what this does not
fix". Two things are recorded rather than absorbed. `progressAt` is **stored** rather than
derived from `updatedAt`, which moves for any write at all and so cannot answer whether a
player's game has gone quiet. And the callback uses the **same credentials** as the result
callback - a second credential for a lower-value endpoint is a second thing to rotate and the
first one somebody leaves behind.

**Gates:** main typecheck **194** (baseline, unchanged), eslint clean, **102 files / 2202
tests**, `check:mirrors` 80 mirrored / **0 drifted**, games-service **270 tests / 0 failed**,
`check:isolation` green. 22 new platform tests, 11 new games-service tests, **20 probes red on
exactly 1 failure each**.

Two probing findings worth carrying. One probe came back **green** because `-t` is a **regular
expression** and the test was named `both apps' contracts agree` - the apostrophe matched
nothing and vitest reported a pass over zero tests; fixed by **renaming the test** to something
regex-safe, not by escaping the probe. Another reported **DID NOT APPLY**, which is always the
target having moved - and chasing it found the test itself was weak, because `indexOf` matched
an identical call earlier in the file.

**Deferred:** the provider spec is issued at 1.5 but **no third-party provider exists**, so only
our own games-service implements it.

**Next chat should:** the owner has authorised the games-service half of the arena reference -
the boxed round-header cells, the Hint/Undo/Clear rail, the LEVEL/Moves/Best Time/Combo column
and the wide SUBMIT SOLUTION. Note `PlayState` and `resultCopy` are test-pinned to carry **no
score**, several of those figures have no data source, and a **paid** hint is forbidden outright.

---

### 11 Sep 2026 - THE ARENA'S BOARD IS LIVE WITHOUT THE PAGE BEING

**Shipped:** the arena's standings, its per-player activity lines, the players count and the
Recent players feed all refresh on a 15-second poll while the contest is running. The page - and
in particular the iframe holding a paid round - is never re-rendered.

**Files touched:** new `lib/services/games/arena-standings.service.ts`,
`app/api/competitions/[id]/standings/route.ts`,
`components/games/arena/ArenaLiveStandings.tsx`,
`tools/probe-arena-live-standings.ps1`; modified
`app/(root)/competitions/[id]/play/page.tsx`,
`components/games/arena/GameArenaLayout.tsx`,
`components/competitions/LiveContestRefresher.tsx`,
`__tests__/games/{live-contest-refresh,provider-play-ui}.test.ts`, `13` s4.1o.

This is the **second** of the three things the owner named on rejecting the arena a second time:
*"and have also not showing live the boards the user finished"*.

**Nothing was computed wrongly and there is no risk number.** The board was right, the activity
line under each name was right, and both were server props rendered once. Nothing to backfill.

**THE OBVIOUS FIX IS FORBIDDEN HERE, AND THAT IS THE WHOLE DESIGN CONSTRAINT.**
`LiveContestRefresher` is what every other contest surface uses, and a test bans it on this page
- deliberately, since `13` s1.1j - because the arena hosts a live round in an iframe and a
`router.refresh()` timer underneath an attempt somebody has **paid** for can disturb it,
intermittently and unreproducibly. So the rail polls a narrow endpoint and swaps only its own
panels.

**THE SAFETY PROPERTY IS A PROPERTY OF WHERE THE STATE LIVES, not of what it contains.**
`ArenaLiveProvider` takes the rest of the arena as `children`, and a `children` element handed
down from a server component is the **same object** on every re-render, so React reconciles it by
identity and never descends into it: the frame cannot remount. That is destroyed the moment
`ProviderRoundHost` reads the context instead of being a child of it - the natural next step for
somebody adding a feature - so it is asserted rather than commented, and a probe injects exactly
that mutation.

**One producer, because agreement beats liveness.** The `dashboard-live` rule in a new place: the
page and the route both call `getArenaStandings`, and **neither reads the board or the activity
itself**. Any field where the poll and the first render differ reads to a player as the value
having changed, and two compositions is precisely how they come to differ.

**An endpoint is a second reader only if it composes an answer of its own.**
`LiveContestRefresher`'s header opened its reasoning with "there is no player-facing JSON API
that returns a competition's ranking", which is now false. Its conclusion still holds and its
reason is narrower, so the sentence is **corrected in place** rather than tidied into the present
tense.

Three smaller rules, each with an inverse that reads as correct: **liveness comes from the stored
status, never a clock in the browser**, since a contest whose end time has passed is still
`active` until a cron finalizes it and a client deciding for itself freezes the board exactly
while the last rounds are scored; **a bad response leaves the last good board on screen**, since
an empty rail here says *nobody has played*, a false statement about a contest in progress rather
than a missing one; and **the count pill became a consumer**, `standingsCount` going from a number
to a `ReactNode`, since a count rendered once disagrees with the list beneath it the moment
somebody joins.

**7 tests, 15 probes red on exactly 1 failure.** Three had to be re-aimed and each failed for a
different one of the known reasons. Two injected an *import* where the assertion bans a **call**,
which is the fourth cause of a green probe - a mutation with no observable - and the assertion was
kept rather than loosened, because importing a reader is harmless and composing a second answer
with it is the defect. The third reported green because **the harness was not parameterised on the
suite**: that guard lives in `provider-play-ui.test.ts`, `-t` matched nothing in the default
suite, and vitest reports a **passing** run over zero tests. `Invoke-Probe` now takes a `-Suite`.

**WHAT THIS DOES NOT FIX IS THE OWNER'S THIRD POINT.** There is **no mid-round reporting anywhere
on either side of the provider seam**: a score exists only after `finishRound`, the frame's
`postMessage` carries none by construction, and the games-service's own per-board record never
leaves that database. A player still at the board reads "Playing now" however many boards they
have solved. Closing that needs a signed progress callback writing only
`game_round.scoreBreakdown` - a protocol change, and its own piece of work.

**Never verified by eye.**

---

### 11 Sep 2026 - WHAT EACH PLAYER SOLVED

**Shipped:** the contest board now shows what every player has been doing, not just their score,
on **both** the lobby and the arena; a **Recent players** feed on the arena; contest facts as
icon tiles with a real state pill; ordinal place labels on the prize table; a taller hero with
the artwork visible; and numbered *How to play* steps.

**Files touched:** new `lib/services/games/contest-activity.service.ts`,
`lib/utils/round-activity.ts`, `components/games/arena/ArenaActivityFeed.tsx`,
`__tests__/games/contest-activity.test.ts`, `tools/probe-contest-activity.ps1`; modified
`components/games/{ProviderLeaderboard,ProviderContestLobby,GameRulesPanel}.tsx`,
`components/games/arena/{ArenaContestPanel,ArenaIdentity,GameArenaLayout}.tsx`,
`components/competitions/PrizeTable.tsx`, `components/neon/Cards.tsx`,
`app/(root)/competitions/[id]/play/page.tsx`, `13` s4.1n.

**THE OWNER REJECTED THE PREVIOUS ENTRY OUTRIGHT** - *"you didn't do anything I ask... this is
unacceptable"* - and the rejection was right. The arena redesign made three structural
corrections and left the screen looking like the application's neutral shell beside a bright
mock, and it answered a question nobody had asked (where the repository boundary runs) instead of
the two things asked for, **both of which are on our side of it.**

**THE FINDING: a player's progress was already arriving and no board read it.** Every game
reports a `scoreBreakdown` beside its score and `game_round` has stored it since X3. It was read
by exactly two screens - the player's own results page and the admin performance tab - and never
by the contest board, which is where every player looks. So this is a read that was never
written, not a provider change: **no spec change, no version bump.**

**No risk number and nothing was backfilled.** Nothing was computed wrongly; a figure that
existed was not shown.

**Three rules worth carrying:**

1. **The platform must not choose among a game's metrics.** `01` s3.2 makes the breakdown
   display-only, free-form and **ordered by the provider**, so the entries are handed over in
   the game's own order and filtered only by renderability. A name table is a `switch` on game
   code wearing a different hat - it makes the no-developer-needed claim false for the next
   title while every existing test still passes.
2. **Which rounds carry a number is decided once, in the service.** A `voided` round stores
   `rawScore: 0` **deliberately**, so passing it through puts a number beside a cancelled round
   and, on a lower-is-better title, the best one on the board. Guarded **negatively on all
   three consumers**, which is the load-bearing half.
3. **`expired` wording describes the clock, never the player.** `createRound` clamps `expiresAt`
   to `playWindowEnd`, so it is the ordinary ending for anybody still playing at the whistle,
   and since R48 those runs count. "Gave up" blames a player for attending.

**Deviated from plan:** the reference puts **RECENT PLAYERS** in a three-panel bottom band and
it is in the sidebar, for the reason the previous entry learned - a layout cannot see that its
child rendered nothing, and the band's other two members return `null` until the catalogue
re-sync. The feed's score is printed **plain, with no `+` sign**: the mock's `+240` is right for
an upward game and exactly backwards for a time trial.
...
> **The RECENT PLAYERS half of that sentence was WRONG and was corrected the same day** (`13`
> s4.1n): `game_round.scoreBreakdown` has carried every player's progress since X3 and nothing
> on a board read it, so the feed had a source all along and is now built. The other four
> genuinely have none. **Correct as history, stale as a present fact** - and the owner rejected
> this entry's screen partly *because* of that misreading, so do not restore the claim.
...
data" instruction, and the item their own 10 September sequencing decision put after the
countdown graphics.

**No risk number, and the reason matters.** Nothing was computed wrongly and no money was
involved: the cards were made game-aware on 6 September, and what they lacked was any reason to
change after they were drawn. Competitions came from props frozen at page load and the countdown
was computed once, at render, from a clock nothing re-read. **Nothing was backfilled.**

**Two commits, deliberately.** The first moved the rank calculation and changed no behaviour;
the second added the polling. **The extraction's only guarantee is that nothing moved, and any
behaviour change in the same diff destroys it** - the same reasoning that preserved a known
one-character defect verbatim while the settlement stages were extracted. The comparator was
moved character for character, the four pre-existing ranking guards were re-pointed at its new
home with a note that the claims are unchanged and only the location moved, and the typecheck sat
at the 194-error baseline on both sides.

**The finding is the counting rule, again, and this time on a public screen.** The plan for this
slice said "the dashboard sorts its own ranks", which reads as one copy. There are **four**: the
dashboard action, and **two more in `app/api/dashboard/competitions/route.ts`**. That route is
...
field where the two disagree *looks like the value changed*. So the endpoint reports the
**stored** `pnl` exactly as the action reads it and deliberately does **not** recompute
unrealized profit from open positions and live prices - the challenge endpoint next door does,
and is right to, but here it would make the number jump on first refresh for every trading
contest, **a defect dressed as an improvement**. The strongest guard is a text comparison of the
two select strings, token for token.

**Files touched:**

- `lib/services/games/dashboard-contest-rank.service.ts` - **new**, the one definition. Not
  mirrored.
- `lib/actions/comprehensive-dashboard.actions.ts` - 43 lines of inline sorting replaced by one
  call; still imports `getDashboardRankingValue` for **challenge** `isLeading`.
- `app/api/competitions/dashboard-live/route.ts` - **new**, session-guarded, same selects and
  same caps as the action.
- `lib/utils/performance.ts` - `COMPETITION_LIVE_DATA: 15000`.
- `components/dashboard/ContestsSidebar.tsx` - the poll, and `TimeLeft` now ticks.
- `__tests__/games/provider-dashboard-cards.test.ts` - 18 tests to **21**, four re-pointed.
- `__tests__/games/dashboard-live-refresh.test.ts` - **new**, 15 tests.
- `tools/probe-dashboard-rank.ps1` and `tools/probe-dashboard-live.ps1` - **new**, 7 and 14
  probes, every one red on exactly one failure.

**Deviated from plan:** the cards could not be fixed the way the lobbies were.
`LiveContestRefresher` re-reads the page because a lobby page is cheap;
`getComprehensiveDashboardData` cannot be polled at all - twenty-plus round trips, unbounded
`TradeHistory` and `WalletTransaction` reads, a ten-thousand-row participant fetch and a
`getGlobalLeaderboard(999999)` that takes seconds cold. A narrow endpoint is exactly the second
reader the lobbies avoided, **which is why the rank came out first.** Also: the countdown ticks
on the **browser** clock, not the server's, because s4.1j settled that trading surfaces read the
browser and the dashboard is one - adding a tick is additive, changing the clock is a behaviour
change to a screen nobody asked to touch.

**Owner tested:** nothing. The dashboard is behind sign-in and the automated browser has no
session.
...
- **Typing the `.lean()` rows** in the new service and route. Deliberately not done in this
  commit: a hand-written lean generic makes the compiler check an invented interface instead of
  the schema, which is where the missing `participant.score` read hid for a day (R32/R33).
- **Task 17's catalogue re-sync** and **the `games-service` rebuild and restart** are both still
  operator actions, and both still gate things this repository cannot fix.

**Next chat should:** the challenges side - the data model has been ready since X1 and the flow
does not exist, and it inherits **R50**, because `ChallengeParticipant.score` still defaults to
`0`.

---

### 11 Sep 2026 - THE RULES AN OPERATOR WROTE WERE SHOWN TO NOBODY, AND R66 WAS NEVER DEPLOYED

**Shipped.** `13` s4.1k. One owner message, two items, and **only one of them needed code.**

**Item 1 - the timer. NO CODE CHANGE. R66's fix is correct and was not running.**

The owner reported the identical symptom to R66 the day after it was closed: a contest with about
six minutes left, the game counting down from nine, having started at ten. Before writing
anything, the code was checked and is correct - `stateFor` publishes `hardDeadline(round)`, which
mins `[expiresAt, gameplay, ceiling]`.

What was wrong is the deployment, and it was **proven rather than inferred**:

- `git ls-files games-service/dist` returns **nothing**. The build output is not tracked, so the
  server runs whatever it last built for itself.
- R66 changed **TypeScript only** (`src/rounds/play.ts`, `src/rounds/lifecycle.ts`).
- Decisively, `git show d63c29fd^:games-service/src/rounds/play.ts` returns
  `const endsAt = gameplayEndsAt(round);` - `startedAt` plus the configured length, with **no
  reference to the contest window at all.** That is exactly the reported symptom, so the running
  build is identifiably the pre-R66 one.

The fix is three commands the owner must run: `git pull`, then `npm run build` **inside
`games-service`**, then `pm2 restart chartvolt-games`. **No cache purge** - R55's fingerprinted
path segments closed that half.

**The general rule, because `21` s4.1m's note is easy to over-read:** since s4.1i the served
asset set is derived from the directory at boot, so a change under `public/play` needs only pull
+ restart. **A `.ts` change needs a build between them.** A document repeating "needs no
rebuild" as though it covered a source change is describing R52 reproduced in full.

**Item 2 - the rules surface. This is the code.**

`getGamePresentation` - the one reader every player-facing game screen goes through - did not
select `rulesSummary` or `howToPlay`. R63 had made both storable and editable the previous day
and **nobody checked whether anything read them**, so a paying player saw the pot, the entry fee,
the clock, the prize split and the standings and was never told what a winning score was. On a
lower-is-better title that is the difference between playing the game and playing it backwards.

**Files touched:**

- `lib/services/games/game-presentation.service.ts` - both fields added in **three** places
  (projection, lean generic, return), normalising a stored `""` to absent. `UNKNOWN_GAME_NAME`
  exported so the lobby can tell the placeholder apart from a real name.
- `components/games/GameRulesPanel.tsx` - **new**, the one definition. A server component, not
  mirrored.
- `components/games/ProviderContestLobby.tsx` - the panel mounted above the leaderboard; its own
  `ProviderGame.findOne` **collapsed onto the shared reader**; two stale comments corrected in
  place with the old wording quoted.
- `components/games/arena/GameArenaLayout.tsx` - a **required** `rules: ReactNode` slot.
- `app/(root)/competitions/[id]/play/page.tsx` - fills it.
- `__tests__/games/game-rules-panel.test.ts` - **new**, 11 tests.
- `__tests__/games/provider-play-ui.test.ts` - two tests **flipped, not deleted**.
- `tools/probe-game-rules-panel.ps1` - **new**, 13 probes.

**Deviated from plan:** `13` s4.1d listed "a rules surface for a provider title" as not built
*because the catalogue stores no rules text*. That reason was already false when it was written -
R63 proved the fields had been demanded, validated and discarded. Both notes are **amended with
the old wording kept**, since the stated reason is why nobody looked.

**Owner tested:** nothing. Both screens are behind sign-in, and there is a second reason here -
**until task 17's catalogue re-sync runs there is nothing for the panel to render.**

**Deferred, and not to be summarised as done:**

- **`ArenaContestPanel`'s "Round time" tile** still states the title's nominal length. Recorded
  rather than built: `RoundPreflight` already discloses the shortened figure on the server clock
  and the game's own countdown is authoritative, so a third server-rendered statement of one
  clock would be stale the instant it was drawn.
- **Task 17's catalogue re-sync** is an operator action and it gates everything above being
  visible.
- **The `games-service` rebuild and restart** is an operator action and the timer stays wrong
  until it happens, whatever this repository says.

**Next chat should:** the dashboard contest cards, starting by lifting
`comprehensive-dashboard.actions.ts`'s inline rank computation out and sharing it - polling
anything before that gives the card a **third** answer to "what rank am I", and it would visibly
flip on the first refresh. **Done 11 Sep 2026** (`13` **s5.1b**), and the extraction turned up
**four** copies of the comparator rather than the one this paragraph assumed.

---

### 10 Sep 2026 - THE GAME LOBBY GOT THE TRADING LOBBY'S CLOCK

**Shipped.** `13` s4.1j. The owner's fourth and last item from the 10 September report: **"the
timer must be the same graphics as the counter in trading competition countdown"**.

**No defect, and no risk number.** That is worth stating first, because the other three items
from the same message are R65, R66 and R67. The game lobby's clocks were **correct** - they were
small. Three lines of `InlineCountdown` text where the trading lobby has a panel of four large
cells, days / hours / minutes / seconds. So nothing was ever wrong, **nothing was backfilled**,
and no money was involved at any point.

**What was built.** `components/competitions/CountdownPanel.tsx` is now the one definition of the
panel; `components/trading/LiveCountdown.tsx` renders it, keeping its own clock and its own
status gate; `components/games/ContestCountdown.tsx` renders it on the server's clock; and
`ProviderContestLobby.tsx` mounts that under the entry control, the same position the trading
sidebar uses. Nothing is mirrored - `apps/admin` has no player lobby - so `check:mirrors` is
correctly silent.

**The appearance was extracted and the clock deliberately was not.** This is the whole reason
the result is two components rather than one reused twice, and a later reader will be tempted to
"finish the job". The two callers disagree about what time it is, on purpose. Trading reads the
browser's clock, as it always has. The game screens read the **server's**, because every rule
about when a contest opens, when an attempt may start and when entry shuts is enforced on our
servers against our time - and a clock working from a visitor's own computer is wrong in both of
the directions that cost something: a button offered when the window has shut, or withheld while
it is genuinely open. **Neither records an error, because neither is one.** `CountdownPanel` is
therefore handed a number of milliseconds and knows nothing about clocks at all. There is a probe
in **each** direction - trading moved onto the server's clock, and the game moved onto the
browser's - because both read as tidying up.

**The second reason not to reuse trading's component is a latent trap.** `LiveCountdown` renders
nothing at all for a `type`/`status` pair it does not recognise, including `status: "completed"`,
which its own props admit and nothing handles. A mismatch produces an empty space and no error,
which is how a countdown goes missing without anybody being told. **Verified latent rather than
assumed**: both callers - the trading sidebar and the challenge page - always pass matched pairs.
So it is **pinned by a test and left alone**, because changing it in the same edit as the move
would destroy the only evidence that nothing moved. `ContestCountdown` has no such gate; the
caller decides whether a clock belongs on the screen, in the open.
...
- **The negative assertion is the load-bearing half.** "Does the screen import the shared panel"
  ...
  So six of the panel's own literals must appear in `CountdownPanel.tsx` and in **no consumer** -
  and picking those six needed a grep first, because `grid-cols-4` alone appears in the lobby's
  own hero and `uppercase tracking-wider` appears in about forty files. A guard built on either
  would have failed on **correct** code, which is the kind the first person it inconveniences
  deletes.
- **An unparseable target refuses rather than clamps.** `splitDuration` treats anything
  non-finite or negative as zero, and **zero is the FINISHED state** - so without the
  `Number.isNaN` guard, one bad stored date renders "Competition has ended!" over a contest that
  is running. A missing clock is recoverable by reading the schedule panel; a confident wrong one
  is not. The same reasoning keeps the target prop as `string | number | Date` rather than a
  narrowed ISO string: `new Date(x).toISOString()` **throws**, so converting at the call site
  would put that throw inside the lobby's render and take the whole page down instead of one
  panel.
- **It is mounted once and reuses `countdownTarget`.** The trading sidebar writes two blocks, one
  per state; here a single block switches on `isActive`, because `countdownTarget` is already the
  answer to "which clock matters now" and is what the hero tile and the play-window row count
  down to. A second block, or a separately resolved date, is a third chance for one screen to
  contradict itself. Position is asserted rather than presence, so the order stays entry control,
  clock, details - the same as trading's.

**Deliberately unchanged.** `RoundPreflight`'s four time readouts stay inline text: they are
clocks embedded in sentences that explain a *gate*, not the contest, and four-cell grids would
destroy the sentences - trading has no large-panel equivalent for them either. The hero tile's
small countdown stays small because trading's hero tile is small too, which is why the existing
test pinning **exactly three** `InlineCountdown` instances is still green and unmodified: this
slice **added** a clock rather than replacing one.

**Verification.** 18 tests in `__tests__/games/contest-countdown.test.ts`, 16 probes in
`tools/probe-contest-countdown.ps1`, all red on exactly one failure. Full suite **2091 passing**;
the single failure was a MongoDB `IX lock` contention artifact in
`competition-cancel-refund.test.ts`, a file untouched here, which passes 11/11 alone. Typecheck
error **lists** identical before and after - 194 both ways, none added and none disappeared.
Lint clean at zero warnings.

**Deferred.** The dashboard contest cards, which is the owner's own sequencing decision of
10 September and the next item.

**Not verified by eye.** Both lobbies are behind sign-in and the automated browser has no
session, so the owner's review is the first time this is seen.

---

### 10 Sep 2026 - R67 - THE LOBBY WAS A PHOTOGRAPH

**Shipped.** `13` s1.1j, risk **R67**. A game lobby now notices that its contest has started, and
both lobbies re-read their standings while one is running. 13 tests, 12 probes red on exactly the
named test, typecheck error lists identical before and after, `__tests__/games` **200/0**.

**Files touched.** `components/competitions/LiveContestRefresher.tsx` (new),
`app/(root)/competitions/[id]/page.tsx` (both branches),
`__tests__/games/live-contest-refresh.test.ts`, `tools/probe-live-contest-refresh.ps1`.
**Nothing is mirrored** - `apps/admin` has no contest lobby.

**Two owner reports, one cause.** The game branch of the lobby page returns the whole screen
before the trading markup, which is right - and means anything mounted above that markup is not
mounted for a game. `CompetitionStatusMonitor` was such a thing, and it is entirely game-agnostic,
so the game lobby went on deciding from a `status` prop frozen at render and a paying player had
to **reload to see Play**. Mounting it does not fix the second report, because it only fires on a
status *change*: a running contest sits at `active` throughout, so **both** lobbies showed the
standings that existed when the page loaded.

**Why a refresh and not an endpoint.** There is no player-facing JSON API that returns a
competition's ranking - `getCompetitionLeaderboard` is a **server action** and holds the whole
rule, so an endpoint would be a second reader that can drift from it. `router.refresh()` re-runs
the page that already calls it.

**Deferred, and deliberately:** the play screen's own standings sidebar and the dashboard contest
cards. The play screen is the interesting one - it hosts a live round in an iframe, so a timer
calling `router.refresh()` there could disturb an attempt somebody paid for. A test forbids
mounting the refresher on it, because that is the obvious wrong fix.

**One probe came back green and the test was at fault**, not the claim: a bare `visibilitychange`
match is satisfied by the *teardown* line, so the listener could be left unattached with every
assertion passing. Sixth instance of one identifier defeating a structural test.

**Next chat should:** give the game countdowns the four-cell trading clock (the owner's remaining
report from the same message), noting `LiveCountdown` computes from `new Date()` while the game
screens deliberately run on `useServerClock` - so the **appearance** must be extracted rather than
the component reused.

---

### 10 Sep 2026 - R66 - THE GAME'S CLOCK COUNTED DOWN TO THE WRONG MOMENT

**Shipped.** `21` s4.1p, risk **R66**. A round the contest will cut short now counts down to the
contest, and says so in words before the player presses Start. 10 tests, 10 probes red on exactly
the named test, `games-service` typecheck clean, `test:play` **78/0** and `test:presentation`
**40/0**.

**Files touched.** `games-service/src/rounds/lifecycle.ts` (`playableSeconds`),
`src/rounds/play.ts`, `public/play/presentation.js`, `tools/test-play.ts`,
`tools/test-presentation.ts`, new `tools/probe-round-clock.ps1`, `package.json`. **Nothing here
is in the platform repository** - `check:mirrors`, `check:isolation` and every platform test say
nothing about any of it.

**The finding, beneath the report.** The owner reported the countdown; there were **two** wrong
numbers, read separately. `endsAt` drove the ticking clock and was `gameplayEndsAt(round)` - the
title's length from `startedAt`, with no reference to the contest. `durationSeconds` drove
`introCopy`'s "You have ten minutes" and was the configured length. So the player was told ten
minutes, shown ten minutes, and given five.

**The third finding is the transferable one.** `hardDeadline` already mins
`[expiresAt, gameplay, ceiling]`, is documented as the hard ceiling section 6 requires, and was
**called by nothing** - the seventh declared-written-dead symbol in this programme after
`requiresSyncPlay`, `isPaused`, `lastSuccessfulRoundAt`, `family`, `playModeOverride` and the SEO
field. **An exported, correct, uncalled helper is evidence that its caller computed the answer
itself**, and in all seven cases the thing beside it was wrong. The fix is a call, not a
calculation.

**The platform was already right**, which is what makes this a contradiction: `RoundPreflight`
discloses `shortenedMs` under `until_window_closes`, so the pre-flight screen and the game showed
different numbers for one round.

**Not cosmetic, because of R48.** Once a partial run counts, the clock is the sole input to how a
player paces a paid attempt.

**The over-correction is worse and is probed for.** "Show the contest's remaining time" fixes the
report and tells a player with an hour left that a two-minute sprint runs for an hour. The rule
is the tighter of the two.

**Deferred:** nothing from this slice. **Live and player-visible; scores and payouts were correct
throughout, so nothing was backfilled.** Never verified by eye - the surface needs a signed launch
token and a contest whose window is closing.

**Next chat should:** the three remaining owner reports from the same message, in order - live
standings during a contest, the Play button appearing without a reload, and the game countdowns
adopting the trading page's four-cell clock.
...
actually played, beside their trading figures rather than instead of them. One row per game,
measured on what that game reported. Task doc **21.1**, risk **R64**. **33 tests, 22 probes,
each red on exactly one failure.**

**Files touched:** `lib/services/games/round-types.ts` and its admin mirror
(`SCORE_PRODUCING_ROUND_STATUSES`), `lib/services/games/participant-score.service.ts` (built
from it), `apps/admin/lib/utils/humanize-metric.ts` (mirrored),
`apps/admin/lib/services/games/player-game-performance.service.ts` (new),
`apps/admin/components/admin/games/PlayerGamePerformance.tsx` (new),
`apps/admin/app/api/users/[userId]/performance/route.ts`,
`apps/admin/components/admin/UserFullDetailPanel.tsx`,
`__tests__/admin/player-game-performance.test.ts`,
`tools/probe-player-game-performance.ps1`.

**TASK 21's PREMISE IS FALSE WHERE IT POINTS, and establishing that came before writing
anything.** Three independent checks: the only screen called Game Performance
(`GamePerformanceSection.tsx`) measures the round lifecycle and contains no `gameKey ===`, no
switch on a game and no metric named after one; `rg` for laps, best time, lines and combo
across both apps' components returns nothing; and the player's own result surfaces have
rendered the reported breakdown generically through `humanizeMetric` since 7 September, with
three distinct empty states - so **tasks 22, 23 and 24 were already satisfied there.** A fix
aimed at the task's sentence would have changed the one thing that was already right. Third
instance after **R7** and **R31**, so carry the class rather than the cases.

**What was genuinely broken is one screen along, and it is worse than a wrong label.** The
per-user route computed eleven figures, every one from `TradeHistory` and `startingCapital`,
and the panel gated **the whole tab** on `!perfStats || perfStats.totalTrades === 0` - one
card reading *"This client has no closed trades yet."* So an operator opening a games-only
player got a true sentence presented as the answer to *how is this player doing*, with every
ranked round, score and prize invisible. No error, no empty column, nothing in a log. `05`
**s10** in its plainest form. **Live, but a reporting defect and never a payment one; nothing
stored, so nothing to backfill.**

**Five findings:**

- **A single "no data" flag covering two questions IS the defect.** The trading block can
  legitimately be empty for a games player and the games list can be empty for a pure trader,
  and one flag for both is how the second case ate the first. The state is two values now,
  and the tab renders the games block on **both** sides of the trades gate - which is also
  why the guard counts the occurrences and asserts their **position** against the gate: a test
  merely checking the panel renders the component is **green** on the original bug, because the
  component would be there, in the branch nobody reaches.
- **A second defect was hiding inside the first one.** `netRoi` is prizes against entry fees -
  real wallet money, entirely game-agnostic - and it was gated on a *trade* count too. Worth
  stating because it is the general shape: a gate that is wrong about its subject is usually
  also wrong about something adjacent that happens to sit behind it.
- **Task 22's declared metric schema is a deliberate deviation, and implementing it literally
  would be worse in both directions.** A per-category table of metric names is the single
  failure mode the platform is built to avoid - anything that enumerates games means the next
  game silently fails to appear - and it is *also* a second source that can disagree with what
  the provider sends, so a metric declared and never sent is a permanent blank row while one
  sent and never declared is real data hidden. The reported `scoreBreakdown` is the only
  source, which satisfies the task's actual requirement (*only show values actually supported
  by the game*) **by construction rather than by configuration**.
- **The coupling was deleted rather than detected.** The admin app has to answer *which rounds
  produced a score* and cannot import `participant-score.service.ts`, which is unmirrored so
  there is exactly one ingestion door. A third copy plus a test noticing the drift was the
  obvious move; the list moved into the mirrored, model-free `round-types.ts` instead -
  `21` s4.1i's rule. The type is spelled out as a literal union rather than importing
  `RoundStatus`, because that module is reached from a client bundle and must stay model-free
  (**R58**).
- **A `voided` round stores `rawScore: 0` deliberately, which makes "has a number" the wrong
  test twice over.** It counts a support action as a result the player earned, and on a
  lower-is-better title that zero sorts **first** and would be nominated as their best round -
  showing an operator a breakdown for a round the leaderboard ignored. The shared status list
  is the test.

**Two probing lessons, both recurrences.** A probe collapsing the empty-breakdown check came
back green on the **fourth cause** - the mutation changed no observable, because the fixture had
`{}` on the losing round. Fixing that exposed something better: **the fixture could not express
the case at all**, since Mongoose's `minimize` default deletes an empty object before saving, so
`scoreBreakdown: {}` cannot be stored through the model and a fixture going through it silently
tests the *absent* case instead. Seeded with the raw driver, which is the one situation where
that is the honest choice, and the check is recorded in the test as a **tripwire** rather than
claimed as a live guard.

**Verified:** 2045 tests pass, `check:mirrors` green (80 agree, 0 drifted), main typecheck at
194 and admin at 223 - **both baselines exactly, with the lists diffed rather than the counts**,
which mattered: a first measurement read 493 because `Select-String` piped to a file wraps at
the console width and split long error lines.

**Deferred:** the **player's own dashboard has the same gap** and is the natural next slice -
which is why the service is deliberately **not mirrored**, a file mirrored before anything
imports it being **R42** exactly. No cross-game total and no ranking, which is chapter `05` and
risk **X13**. **Never verified by eye**, the screen being behind an admin sign-in.

**Next chat should:** take the five live-data defects the owner reported on 10 September - the
entry-closes wording, the in-game clock that shows the game's own length to a late joiner, the
standings that need a page refresh, the Play button that needs one too, and the countdown
graphics - both of the first two being confirmed against the code already.
...
### 10 Sep 2026 - TASK 19 (ASSISTANT HALF) - MARKETING COPY, AND NOTHING THE PROVIDER OWNS

**Shipped:** an operator opening a title's content dialog can ask for copy, steer it in a
sentence, and take the suggestions field by field or all at once. **It proposes into the
form and never saves** - the operator still presses Save, and every value goes through the
same `validateGameContent` their own typing does. Task doc **19.2**. Task 19 is now done.

**Built:** `apps/admin/lib/admin/ai-game-content-vocabulary.ts` (the prompt),
`ai-game-content-suggestion.ts` (the parser), `POST /api/ai/generate-game-content` guarded
by `game-providers`, `GameContentAiPanel.tsx`, and `AI_WRITABLE_CONTENT_FIELDS` /
`AI_NEVER_WRITABLE_CONTENT_FIELDS` in `game-content-fields.ts` beside the operator's own
two lists. `TRADING_WORDS` widened from contest-local to exported, one word of diff, so
both assistants share one banned-word list. **21 tests, 22 probes red on exactly one
failure**, 2,000 tests green, typechecks at the 194 / 223 baselines with none in a touched
file, admin `next build` clean, eslint clean. **Nothing here is mirrored.**
...
- **The bar is enforced by the SHAPE, not by remembering.** `GameContentSuggestion` has no
  ...
  field rather than spreading the model's reply - so a reply carrying them yields an object
  with four keys and the extras are gone before any caller sees them. Same reasoning as the
  play screen's frame message having no score field (`13` s1.1a). **Probe 1 is the one that
  matters**: it makes the parser spread the payload, which is what somebody does in good
  faith while adding a fifth permitted field, and the test catches it only because it
  asserts the **whole** object against a reply that does carry both.
- **"Do not state", never "do not invent".** A model told not to invent rules will state the
  ones it has assumed and consider itself obedient, so the rule forbids the *acts* - saying
  how the game is played, what the controls are, how points are earned, how long a round
  lasts, how ties break, or describing a specific level, board or opponent. The type stops
  the text being **stored**; the prompt stops it appearing inside a description that is.
- **A separate vocabulary module from the contest one, sharing the FACTS rather than the
  prompt.** A game's page outlives every contest run on it, so one prompt with a mode flag
  would drift into a `switch`. What is shared is `describeSubject`, `describeWinningRule`,
  `TRADING_WORDS` and `NO_FIAT_RULE`, so a title's facts cannot be described one way on one
  screen and another way on the next.
- **Guarded by the section that owns the calling screen** (R51), not by a general "AI" grant
  and not by folding it into `generate-competition`, which is granted `competitions` -
  folding would have locked out a catalogue operator with no contest grant, or widened
  `competitions` to cover the catalogue. Pinned by **counting exported handlers against
  guard calls**, comments stripped first, because these files name `guardSection` in prose.
- **A latent bug of my own, from the storage half, found by the tests.** `onSaved` handed
  the parent row seven of the nine fields, having not been updated when 19.1 added two.
  Nothing failed - the save succeeded and the server had the new text - but the parent kept
  the **old** value, so reopening showed the text the operator had just replaced and saving
  again wrote it back over the server's copy. **A silent revert of an edit that reported
  success**, this codebase's recurring shape. Adding a field to a form is not finished when
  the form saves it.

**Two smaller things.** `AI_WRITABLE_CONTENT_FIELDS` is `as const` rather than an explicit
tuple type specifically so a probe adding a field stays valid TypeScript - a probe that
cannot compile is indistinguishable from a guard that works. And the new module imports its
siblings **relatively**, not through `@/`: vitest aliases `@` to the repository root, so
`@/lib/admin/...` inside `apps/admin` resolves under `next build` and fails in the suite
(**R58**). The neighbours get away with `@/lib/services/...` only because those files are
mirrored and a root copy happens to exist.

**Not built: no SEO or meta description.** A provider title has no public page with a
`<head>` of its own yet, so the field would be written by the assistant and read by nothing
- the sixth declared-written-dead field after `requiresSyncPlay`, `isPaused`,
`lastSuccessfulRoundAt`, `family` and `playModeOverride` on a head-to-head title. **Never
verified by eye**: the screen is behind an admin sign-in the automated browser has no
session for.

---

### 10 Sep 2026 - TASK 19 (STORAGE HALF) / R63 - THE SYNC DISCARDED CONTENT THE CONTRACT DEMANDS

**Shipped:** every content field `01` s3.1 requires of a provider is now parsed, stored and
editable. **The AI panel task 19 asks for is NOT built** - this is the prerequisite it
exposed, and a summary saying task 19 is done is wrong.

Setting out to add AI generation for the content this task lists turned up the reason most
of it could not be generated anywhere: there was nowhere to put it, **and the provider was
already sending it.**

| Field | Provider sends | Stored before | Now |
|---|---|---|---|
| `displayName`, `description`, `thumbnailUrl`, `category` | yes | yes | unchanged |
| `tagline`, `bannerUrl` | yes | **no** - field existed, in neither allow-list | seeded on first sync |
| `rulesSummary`, `howToPlay` | yes | **no field, and not on `ProviderCatalogueGame`** | new fields, seeded on first sync |

**Built:** four fields added to `ProviderCatalogueGame` in `contract.ts` and assigned by
both adapters (mirrored); `firstSyncOnlyFields` widened from four to eight and spread into
`ProviderGame.create()`, so an absent value is left **unset** rather than stored as `""`;
`rulesSummary` and `howToPlay` declared on both `provider-game.model.ts` copies; both added
to `EDITABLE_CONTENT_FIELDS` with a 2,000-character limit and to `GameContentDialog`;
`mock-trivia` supplies all four and `mock-sprint` deliberately supplies none, so the pair
proves a supplied value travels end to end **and** an absent one is left alone.

**Ten tests** in `__tests__/services/provider-content-fields.test.ts`, **12 probes** in
`tools/probe-provider-content-fields.ps1`, all red on exactly one failure. 1,976 tests green
(1,966 before, so exactly the ten new), `check:mirrors` OK, typechecks at the 194 / 223
baselines with none in a touched file, admin `next build` clean, eslint clean.

**Five findings, and the first is the one that generalises furthest.**

- **The drift ran DOCUMENT to CODE, and no guard looks in that direction.** `01` and
  `ChartVolt-Game-API-Requirements.html` were correct and agreed with each other, so the
  paired-document rule was satisfied and reported nothing; every test touching the sync
  asserted the fields it already knew about. So the guard **parses section 3.1's own
  table** and requires each `Yes` row to be stored by a real sync or listed in `NOT_STORED`
  with a reason. **Probe 12 is the point of the whole suite**: it adds a seventh required
  field to the issued specification and **changes no code at all**. Green there would mean
  the tripwire is decoration and the next forgotten field ships in silence.
- **The comment said the omission was deliberate, which is why nobody re-read it.** It
  described these as fields "no provider ever supplies" and "in no contract at all", both
  false for two of the three. **Seventh instance** of an aside being a claim rather than a
  fact, and the first where the wrong sentence *actively deterred* the reading that would
  have found the defect. Corrected **in place**, with the old wording quoted, rather than
  adjusted into the present tense.
- **Prove storage behaviourally, never by matching an allow-list.** A field named in
  `firstSyncOnlyFields` that the schema then discards satisfies any structural check
  perfectly - which is exactly the shape of every strict-mode defect in this programme. The
  suite runs a real sync against a real database and reads the row back.
- **first-sync-only versus provider-owned is a decision about who wins an argument.** These
  are sentences a player reads, so an operator must be able to fix grammar, tone or language
  and have the edit survive the next scheduled sync. `scoreDirection` and `playMode` beside
  them are the provider's statements about how their own game works and must not be
  overridable. The suite pins **both** directions - the operator's edit surviving, and the
  provider still winning where the field is theirs - and the second half is not decoration:
  a "sync never overwrites anything" bug passes the first assertion alone.
- **Both green probes were the same weak test, and the cause was the same one that has bitten
  four times before**: the identifier appears twice in the file, so a backwards search finds
  the surviving copy. Confirmed by **counting** rather than assumed, then fixed by slicing
  the construct with a length assertion and by asserting an occurrence count.

**Harm, in both directions.** **Live, and occurring on every sync**, which is rarer here than
latent - but **nothing was destroyed and nothing was backfilled**, because the values live on
the provider and a re-sync populates them. The cost was a game page with no rules text, which
is why `13` s4.1h's **View Rules** button points at `/help/competitions`. That button can now
do what it says, which is the natural next slice and is player-facing.

**No provider-spec change and no version bump** - `01` and the requirements HTML were right
throughout, verified with one search before assuming a restatement needed changing.

**Owner decision, taken during the work:** the AI **never** writes `rulesSummary` or
`howToPlay`. `01` s3.1 calls the rules summary the first text support quotes back when a
player disputes a prize, so invented wording there is invented wording in a money argument,
and the operator can already edit the provider's text by hand on the same screen. The AI
half is scoped to marketing copy - tagline, description, highlights.

---

### 10 Sep 2026 - TASK 13 / R62 - THE ADAPTER DECIDED HOW A GAME RANKS

**Shipped:** how a title ranks is read from the catalogue everywhere, including the one path
that aggregates a player's attempts. `scoreDirection` no longer exists on a round result.

**Task 13's schema already existed; its closing SENTENCE is what shipped.** Almost every field
in its example has been on `provider_game` since X2, and three of its blocks were completed by
tasks 9, 11 and 14 over the two preceding days - so building the sketch would have meant
renaming working fields. The deliverable was the objective line, *remove game-specific
assumptions from UI and business logic*, and the one that mattered decided money.

`chartvolt-games/normalise.ts` held `TITLE_DIRECTIONS`: a map of game code to score direction
with **two entries**, `circuit-sprint` upward and `circuit-perfect` downward, defaulting
anything else upward with a `console.warn`. Gate 11b of `result-ingestion.service.ts` passed
its answer to `syncParticipantScore`, which uses the direction to pick the best of several
attempts. **A third title shipped by our own games service, with nobody editing that platform
file, would have had every player on a `best_of_n` contest scored on their worst attempt.**

**Say the harm precisely, because it reads worse than it was and also better.** It is **not** a
reversed leaderboard: settlement and both boards have always resolved the direction from the
catalogue, so the contest would have paid the correct order **of the wrong runs**. It is not
cosmetic either - on a lower-is-better title, the gap between a player's best and worst attempt
is the entire reason for offering several. **Latent, nothing backfilled**, and precisely: the
per-round scores are on `game_round`, so a wrongly-aggregated contest *could* have been
recomputed, and there is not one.

**Why nobody had found it.** The failure is uniform across every player of the affected title,
so no board looks impossible, no total fails to add up and nothing throws. The whole signal was
a warning on a server nobody watches. And the old suite asserted that the guess for an unknown
title **matched settlement's default exactly** - an assertion correct about the code, which was
pinning the very property that made the defect invisible.
...
contract field it filled was required. Both true. The mistake was in the **consumer**:
`result-ingestion.service.ts` is async, holds `round.gameKey`, and gate 10 had already read the
very row that declares the direction, forty-five lines earlier, for the range check - then
discarded it. **A correct explanation of why one file cannot do something is not an
explanation of why the thing is not done.**

**The field left the contract rather than being read twice.** Two rules meet there: a supplier's
opinion is an input and never a decision, so even a provider volunteering a direction on a
result body must not settle it; and R32/R33 established that a duplicated ranking input lets two
rows in **one** leaderboard disagree, which is incoherent rather than merely wrong. Ingestion
resolves through `resolveScoreDirection`, the definition settlement and both boards already
share, so **adding a title is now a catalogue sync.** Resolved there rather than by widening
gate 10's read, deliberately - `scoreWithinRange` answering a second unrelated question would
make its name a lie.

`ProviderCatalogueGame.scoreDirection` is **kept, and a test pins it.** A provider declaring how
their own game ranks is exactly right; sweeping it away for consistency leaves no source at all.

**The provider spec needed no change and checking cost one search.** `01` s3 and
`ChartVolt-Game-API-Requirements.html` ask for `scoreDirection` on the **catalogue** and have
never asked for it on a result body. The issued contract was right; the per-round copy was ours.
**No version bump.**

**Guard:** `__tests__/services/game-agnostic-result-ingestion.test.ts`, 9 tests, and
`tools/probe-game-agnostic-ingestion.ps1`, 8 probes, every one red on exactly one failure. Four
things generalise:
...
- **The eighth probe is behavioural and the other seven could not be.** Structural tests stay
  ...
  hard-codes the upward default - exactly what the deleted map returned for an unknown title -
  and the ranking test turns red.
- **`toHaveProperty` names a field as a STRING, so the compiler cannot see it.** Removing the
  field turned six type-position uses red and left that one assertion green until the suite ran.
  **A structural rename is only as complete as the assertions that are typed.**
- **Two sources must disagree before a test can prove which one was read.** The behavioural test
  used to seed the title as `lower_is_better` *and* override the direction on both payloads, so
  it could not say which was consulted - and it was the payload. Removing the override is what
  strengthened it: a fallback to the default now scores 140 rather than 92.
- **A length assertion caught a slice that had found nothing.** `indexOf("buildResult(")`
  matched a **call site**, so the slice ran to the enclosing method's brace - 24 characters
  containing none of what the test claimed to examine. Same class as every other
  scan-towards-it failure here; anchor on the declaration and assert the slice is non-trivial.

**Recorded and NOT fixed, because it is a different phase rather than a smaller job:** the
global leaderboard, matchmaking, badge evaluation and the journey map all rank on `TradeHistory`
alone, so a player who wins provider contests and never trades ranks last on the platform's own
leaderboard, is matched as a beginner and unlocks no badges. That is `05` cross-game scoring,
risk **X13**, phases X7 and X11. **Do not let a summary imply task 13 closed it.**

**Verified:** 1,966 main-app tests across 91 files, `check:mirrors` clean, both typechecks at
their exact baselines (main 194, admin 223) with none in a touched file and **none
disappearing**, admin `next build` clean, lint clean. Nothing is player- or operator-visible, so
there was nothing to verify by eye.

**Next chat should:** task 19 (AI content generation on the game content screen), or the
challenges flow, which is the largest remaining gap - the data model has been ready since X1
and `ChallengeCreateDialog.tsx` still has no game field at all.

---

### 10 Sep 2026 - TASK 12 / R61 - THE WIZARD KNEW THE CONTEST'S SHAPE AND THE EDITOR DID NOT

**Shipped:** both contest setup screens now adapt to the contest's play shape, from one rule.

**Most of task 12 was already built and one screen had never heard of it.** The wizard has
adapted since `22` s8 - labels, hints, the round-start control and the attempts control all
come from `playShapeRules`. `ProviderContestEditor.tsx` did not import `play-shape.ts` at all.

That is **R61**, and it is not a labelling fix. `applyEdit` forces `attemptsPolicy` and
`roundStartPolicy` from the shape, unconditionally, and reads `forcedAttemptsPolicy` **before**
it looks at what the operator sent. So an operator could open a race, choose "Best of several",
save, be told the edit succeeded, and have `single` stored - no error, no log line, and the
screen still showing their choice until they reloaded. **A control that appears to work and
does nothing**, in the place this codebase keeps finding it.

The dates were the quieter half: "Contest starts" and "Contest ends" with no hint, on a shape
where the start is **also the moment entry closes**. That is the one fact an operator needs
before touching it, because moving the start moves the entry deadline with it.

**The class, which is the part worth carrying.** Third instance of the sibling-screen shape:
the competitions list learned Edit-routes-by-game in `12` s2.2 and the contest view page was
missed until s2.4; the wizard learned the play shape and the editor was missed until now.
**A rule enforced by one screen is not a rule the product enforces.** So the guard is
deliberately not "the editor hides two controls" - it is the invariant **a control is withheld
exactly when the shape forces its value**, which is a property of `play-shape.ts` rather than
of either screen. Four of the eleven probes therefore mutate that module rather than a
component, and a third mode that forces something while offering its control turns them red on
the day it is added.

**Four of the fields task 12 asks for are derived on purpose, and adding them back would undo
`12` s2.3.** Registration Closes and the entry deadline come from `resolveContestEntryDeadline`;
Play Window Opens and Closes come from `deriveWindow`. There is one contest clock, and four
date fields are how you build a contest nobody can win fairly. Recorded as a table in 12.1 so
the next reader does not "finish" the task by reintroducing the defect.

**Blocked rather than deferred:** Player Check-In Window, Lobby Opens and Countdown. They are a
pre-match lobby - a player-facing screen, a check-in state on the participant, a no-show rule -
and **no title declares `scheduled`**, so it would be a lobby for a shape nothing uses. Same
reasoning as turn-based and heat-based in 10.2.

**Files touched:** `lib/services/games/play-shape.ts` (mirrored, byte-identical) gains
`copy.attemptsWithheld`; `apps/admin/app/api/games/contests/[competitionId]/route.ts` resolves
the shape and sends `playMode`; `ProviderContestEditor.tsx` and `wizard/StepPrizes.tsx`.

**Deviated from plan:** the withheld sentence **moved** out of `StepPrizes.tsx` rather than
being copied into the editor. A literal in each screen is the "one rule, two copies" shape
behind `referenceId`, `failedReason`, `challengeId` and the Game Master `||`, none of which
`check:mirrors` can see. The move turned a pre-existing test red - it asserted the sentence
appeared as a literal in the component - and that test was **flipped, not deleted**: the claim
it makes is unchanged, so it now asserts the shared field rather than the words.

Also: both attempts controls were put under **one** guard in a fragment. A grid lays out a
fragment's children as its own, so nothing moves, and it removes the shape a structural test
cannot hold - with the condition written twice, deleting the second leaves the first satisfying
any check that looks backwards from the attempts count for a guard.

**Owner tested:** nothing. **Not verified by eye** - the screen is behind an admin sign-in, and
**no contest can be put into the shape this fixes until a title declares `scheduled`.**
...
**Deferred:** the lobby trio above. Nothing else from task 12.

**Next chat should:** resume **task 13** of the 35-task document - game configuration must be
data-driven. Note that the two rows above it in the pasted status tables were stale: tasks 9,
11 and 14 are **done**, and the `PROGRESS.md` table row for 11 said "Not started" until today.

---

### 9 Sep 2026 - R60 - THE GENRE LIST WAS DRAWN BY THE BROWSER, IN WHITE, ON WHITE

**Shipped:** the Genre picker on the game content dialog is on the same primitive as its eleven
siblings.

The owner reported it opening as a tall, almost empty panel with one word in it. **Nothing was
missing.** All fifteen options were there, correctly generated from `GAME_CATEGORIES`, and all
fifteen were white on white.

**A browser paints a native `<select>`'s list itself.** It takes the list background from the
element's own `background-color` and lets the options inherit `color`. Every field on this
surface is themed `bg-white/5 text-white` - and `bg-white/5` is `rgba(255,255,255,0.05)`, a
*translucent* white, which composites over the browser's light list surface to effectively
white. The one legible row was the highlighted one, readable only against the operating
system's own selection band. **A fault in which nothing failed reads as a data problem, which
is why the report was "the list is not shown the items".**

**Two conditions are needed: a native `<select>` AND a translucent background.** This surface
has **eleven** pickers on the shared `Select`, which draws its own list in a portal and never
asks the browser for one; this was the twelfth and the only native one left there. The **33
remaining native selects in admin and 11 in the player app** sit on an opaque `bg-gray-*` or set
no background at all, and mostly set no `color` either, so they carry two independent
protections and are correct.

**That is why the guard bans the combination and not the element.** Banning `<select>` outright
fires on forty-odd working files and gets deleted by the first person it inconveniences - the
reasoning that narrowed the `GameIcon` ban in `13` s4.1g.

**THERE WERE TWO INSTANCES, and counting them is what corrected this entry.** It was first
written claiming the genre picker was the only one. It was not: the **"Transfer to" employee
...
the *condition* after the fix rather than before - the seventh instance of the counting rule.
**It is deliberately not fixed for a mechanical reason, not a priority one:** the change is one
token, but that file carries ~30 pre-existing lint warnings and the pre-commit hook lints staged
files at `--max-warnings=0`, so touching it demands an unrelated cleanup of a 2,180-line
component in the same commit. It is a **named exception inside the guard, with a test asserting
the exception is still an offender**, so the entry must be deleted for the suite to stay green
once it is fixed.

**The guard's own near-miss is the most transferable thing here.** The platform-wide scan was
first written as `<select\b[^>]*>`, which stops at the `>` inside `onChange={(e) => ...}` - which
every real call site has - so the captured tag ended before `className`, no file could ever
match, and **every assertion was green while the tree was unexamined**. What caught it was the
exception-still-offends test reporting the *known* offender as clean. Two rules: **a scan for a
combination needs a fixture proving it reaches the second half**, and **a deliberately-listed
exception doubles as a canary for the scan itself.**

**The one non-obvious thing the fix had to solve.** Radix reserves `""` for "nothing is
selected" and **throws** on a `SelectItem` carrying it, so `<option value="">No genre</option>`
could not be ported across as it stood. "No genre" travels as a sentinel mapped back to `""` at
the boundary, so stored state is unchanged - absent is still absent. **The sentinel's spelling
is load-bearing:** `normaliseCategorySlug` strips every non-alphanumeric run, so `__no_genre__`
is unreachable from the slug namespace, whereas `none` or `custom` is a genre an operator can
legitimately type and would then find clearing the field. Same shape as
`GamePlayStyleControl`'s `__follow_provider__`.

**Files touched:** `apps/admin/components/admin/games/GameContentDialog.tsx`;
`__tests__/admin/game-categories.test.ts` (+4 tests, 34 total);
`__tests__/admin/native-select-legibility.test.ts` (new, 8 tests, the platform-wide half);
`tools/probe-genre-picker.ps1` (new, 9 probes); `17` R60; this file; the `.mdc` rule.
**`MessagingSection.tsx` is NOT touched** - see the exception above.

**Severity, in both directions.** **Low and live, on two controls.** No money moved, nothing is
stored wrongly, **nothing backfilled** because there is nothing to backfill. But it made the
genre unsettable in practice - an operator could only land on a value by arrowing blindly
through an invisible list - so `category` is the field most likely to be stale on any title
touched before today, and that is the grouping key tasks 21-24 will join on.

**Owner tested:** no. **Never verified by eye** - the screen is behind an admin sign-in the
automated browser has no session for, so the mechanism is proven and the pixels are not. Admin
`next build` clean, `tsc --noEmit` at the **223** baseline exactly with none in the touched
files and none disappearing, **691 admin tests** green plus the 8 new ones, 9 probes red on
exactly the expected test with a blast radius of one.

**Next chat should:** resume **task 12** of the 35-task document.

---

### 9 Sep 2026 - R59 - EVERY DIALOG THAT ASKED TO BE WIDE WAS RENDERING AT 32rem

**Shipped:** the game catalogue dialog is as wide as the screen allows, and the mechanism that
was silently discarding its width request is now a shared module with a behavioural guard.

The owner reported the Games list as too narrow, with the Prize eligibility heading clipped to
"PR" and the play-style controls crushed. The dialog asked for room the obvious way, and had
since it was written: `<DialogContent className="max-h-[85vh] max-w-4xl ...">`. **It was
rendering at 32rem, and so was every other admin dialog that asked the same way.**

**The cause needs both halves to be stated or it sounds like a style preference.**
`DialogContent`'s own class list ends with `sm:max-w-lg`. `cn()` is `twMerge`, which keys a
conflict on the utility group **and the modifier together** - so a bare `max-w-4xl` does not
displace a `sm:`-prefixed cap and **both reach the DOM**. Tailwind then emits `.sm\:max-w-lg`
*after* `.max-w-4xl` at equal specificity, so the cap wins from 640px up. Measured rather than
argued: compiling both candidates with this app's own Tailwind 4.1.18 puts them at bytes 4541
and 4749, and `twMerge` on the real base string returns `w-full sm:max-w-lg max-w-4xl`. **Ten
lines of Node settles it, and the intuition - the later class in `className` wins - is wrong
twice over.**

**This is the "appears to work and does nothing" shape in a new place**, after a provider with
no adapter, a `rankingMethod` a provider game ignores, `isPaused` on a provider contest and
`playModeOverride` written to a provider-owned field. The class is present, the diff reads
correctly, review passes, and the author's intent is discarded in silence.

**The scope is platform-wide and the arithmetic is the part to carry.** 55 `DialogContent`s
across both apps carry an inert unprefixed `max-w-*`: **31 asked for more than 32rem and are
clipping content**, 6 asked for less, and **16 asked for `lg`, which is what the cap already
says, so they are accidentally correct.** Those 16 are why it survived - the pattern
demonstrably works on a third of its uses, so the first person to meet a clipped dialog
suspects their own value rather than the mechanism. **Two of the 31 are in the PLAYER app**
(`components/trading/IndicatorSelector.tsx`, `components/profile/TradingArsenalSection.tsx`).

**And the main app already had the answer, which reframes the sweep.** Its copy of
`components/ui/dialog.tsx` grew a `size` prop - `sm | default | lg | xl | full` - whose
variants are **all `sm:`-prefixed**, which is exactly the mechanism that works. The admin copy
never got it. So the two clipped player dialogs are two call sites passing a className instead
of `size="xl"`, and the right sweep for the remaining 29 admin dialogs is probably **to port
that prop** rather than to spread a token across 24 files. Same rule as `12` s1.1: **before
building a mechanism a screen needs, check whether the sibling app already has one.**

**Files touched:** `apps/admin/lib/admin/dialog-widths.ts` (new, three tokens, model-free);
`ProviderCatalogueDialog.tsx`, `GameContentDialog.tsx`, `GameScoringDialog.tsx`,
`ProviderCredentialsDialog.tsx`, `ProviderRegisterDialog.tsx`;
`__tests__/admin/dialog-widths.test.ts` (new, 7 tests); `tools/probe-dialog-widths.ps1` (new,
7 probes); `17` R59; this file; the `.mdc` rule.

**Deviated from plan:** nothing planned this; it is an owner report. Two decisions are
deliberate. The catalogue's table gets `min-w-[68rem]` inside an `overflow-x-auto` wrapper
rather than being allowed to compress - seven columns of controls do not fit a small laptop at
any dialog width, and a select squeezed to 90px is worse than a scrollbar. And the primitive
was **not** touched, so the blast radius of this commit is five files.

**The guard is behavioural, because the string is not the property.** It runs the real
`twMerge` against the base class list **read out of `dialog.tsx`**, and the **control test** -
asserting that an unprefixed `max-w-4xl` *still* leaves the cap in place - is what gives the
others meaning: without it, a future `twMerge` that stopped conflicting at all would leave
every assertion green while nothing was being measured. The token is asserted **inside the
`DialogContent` opening**, not anywhere in the file, because a file-wide match is satisfied by
the import line alone. Probe 6 mutates the **primitive**, which is what proves the control
reads `dialog.tsx` rather than carrying its own copy of the cap.

**Owner tested:** no. **Never verified by eye** - the screen is behind an admin sign-in the
automated browser has no session for, so the width is proven by the merge and not by a
screenshot. Admin `next build` clean, `tsc --noEmit` at the **223** baseline exactly with none
in the touched files and none disappearing, 687 admin tests green, 7 probes red on exactly the
expected test with a blast radius of one.

**Deferred:** the other **29 admin dialogs**, and the **2 player dialogs**. Recorded rather
than swept: each is a screen becoming visibly wider, none can be checked by eye from here, and
doing it in the same commit as the reported fix destroys the only evidence the reported fix is
safe - the reasoning that kept the Game Master `||` verbatim while settlement was extracted.
**It is an owner decision, and the honest framing is that 29 admin dialogs are showing less
than their authors wrote.**

**Next chat should:** resume **task 12** of the 35-task document (required timing and runtime
settings per mode), unless the owner asks for the dialog sweep first.

---

### 9 Sep 2026 - R58 - A CLIENT COMPONENT IMPORTED ONE NUMBER AND TOOK THE ADMIN PANEL DOWN

**Shipped:** `SCORE_UNIT_MAX_LENGTH` moved out of `game-scoring-rules.service.ts` and into
`apps/admin/lib/admin/score-eligibility-copy.ts`, which reaches no MongoDB driver, with the
service importing it back from there so there is still exactly one definition.
`components/trading/MarketStatusBanner.tsx` changed to `import type`. Guarded by
`__tests__/admin/client-bundle-model-imports.test.ts` (7 tests) and probed by
`tools/probe-client-bundle-guard.ps1` (2 probes, both red on the expected test). **Both apps
now produce a `.next/BUILD_ID`** - admin `rQxHvj-c9thlJviVmGqzA`, main `TpEw69COxpH5Jff2Uwe38`.

**What broke, and it is the first entry here whose harm is that the application did not
exist.** `GameScoringDialog.tsx` is `"use client"` and imported one constant from a service
whose first import is `@/database/mongoose`, whose first import is `MongoClient` from
`mongodb`. Turbopack traced the whole driver into the browser bundle, `next build` failed on 17
unresolvable Node builtins, `apps/admin` had no `.next` directory, and PM2 restarted it **283
times**. No money moved, no document is wrong, **nothing to backfill** - a build either emits a
bundle or it does not. The main app stayed up.

**Files touched:** `apps/admin/components/admin/games/GameScoringDialog.tsx`,
`apps/admin/lib/admin/score-eligibility-copy.ts`,
`apps/admin/lib/services/game-providers/game-scoring-rules.service.ts`,
`components/trading/MarketStatusBanner.tsx`, plus the new test and probe.

**Deviated from plan:** nothing was planned - this was a production break reported by the
owner. Two deliberate choices are worth recording. The guard covers **both** apps rather than
the one that broke, and the service imports the constant by a **relative** path rather than
`@/`, because vitest aliases `@` to the repository root and not the admin root, so the aliased
form resolves under `next build` and fails in the suite.

**Four things that generalise:**

- **The typecheck is structurally blind to this whole class.** The import is valid TypeScript
  for a value whose type is `number`, and nothing about a module's fitness for a *browser
  bundle* is expressible in the type system. Both apps' `tsc --noEmit` were clean, every test
  passed, and the dev server never built the dashboard's client bundle. **It is only ever
  caught by a full production build, which is to say by deploying.**
- **Extending the guard to the main app is what turned a fixed bug into a finding.**
  `MarketStatusBanner.tsx` was one keystroke from the same outage: it built only because both
  its bindings are interfaces used in type positions, so the bundler dropped the import. **The
  safety was the bundler's unused-import elision, not a decision anyone had made.** Promote
  either interface to a class, or import one more binding, and the *player* app fails
  identically with no warning from any tool.
- **The guard forbids `mongodb` and deliberately not `mongoose`.** The first draft banned
  reaching a model, flagged five files that build perfectly well, and would have been deleted
  by the first person it inconvenienced - `mongoose` has a browser build, the driver under it
  does not. Same reasoning that narrowed the `GameIcon` ban in `13` s4.1g: **forbidding what
  actually fails is what keeps a guard believed.**
- **The deploy output pointed away from the cause twice.** The script reported the build
  failure and then restarted the processes anyway, so the last hundred lines were a boot loop
  with the 17 errors scrolled off; and every error names a file inside `node_modules`, which
  reads as a dependency problem. **The one useful line is the bottom of each import trace.**

**Owner tested:** not yet. The deploy is what found it, so a redeploy is the test.

**Deferred:** two pre-existing warnings are untouched and neither caused this - the NFT-list
warning on `apps/admin/next.config.ts`, and the multiple-lockfile workspace-root warning, which
wants `outputFileTracingRoot` set.

**Two incidental changes the pre-commit lint gate forced, recorded so neither reads as part of
the fix.** The guard's import matcher was first written with an optional `(\s+type)?` group,
which nests a quantifier inside a quantifier and is refused by `security/detect-unsafe-regex`;
it now captures the whole clause between the keyword and `from` and inspects it as a string,
and the directive check strips comments and compares strings rather than matching a pattern
that has to skip a 70-line block comment. **The probes were re-run afterwards** rather than
trusted, since the mechanism they aim at had changed - both still red on exactly one failure.
And `MarketStatusBanner.tsx`'s empty `catch (error)` became a bare `catch`, because the file
was already being touched and `--max-warnings=0` blocks a commit on the unused binding.

**Next chat should:** have the owner redeploy, then resume task 12.

---

### 9 Sep 2026 - X6 / `22` s10 - ONE TITLE, BOTH SHAPES (TASK 11), AND A DECISION REVERSED

**Shipped:** `provider_game.supportedPlayModes` on both model copies, `Competition.playMode` on
both, and three new functions on `lib/services/games/play-shape.ts` (mirrored, byte-identical
test): `resolveSupportedPlayModes`, `isPlayModeSupported` and `resolveContestPlayMode`. The
create service refuses a pick outside the supported set and **names what is supported**; the
wizard offers `ContestPlayModeField` when - and only when - a title supports more than one
shape; `GamePlayStyleControl` gained a supported-set control beside the Play style one. **49
tests, 37 probes all red on exactly one failure.**

**THIS REVERSES A RECORDED DECISION, which is the fact worth carrying.** `22` s8.3 and
`play-shape.ts`'s own header said the shape is a property of the **title** and **never** comes
from caller input. The create service now takes it from its caller. The rule was not wrong when
it was written - it was correct for a world where a title had exactly one shape - and the
owner's example is what ended that world: **a multiplayer racing game with a synchronised race
*and* an asynchronous time trial.** One title, two shapes, and no way to express it. Recorded in
`22` s10 rather than by editing s8, on the standing rule that a reversed direction is exactly
what a new chat needs to see.

**What survives is the safety, and this is the distinction rather than the two sentences.** The
fear behind "never from caller input" was an operator declaring a race `anytime` so entry stays
open after the gun. `resolveSupportedPlayModes` is precisely what prevents that: the pick is
validated against a **stored** set, so a race can be run staggered only if somebody has declared
that this race has a legitimate time-trial form. All three properties that made the old rule safe
are intact - validated against stored data rather than input, consequences stamped on at **write**
time so not one runtime gate reads a mode, and **no player-facing path supplies a shape at all**,
which was always the absolute half. **The general form: a rule that blocks a legitimate
requirement is usually several rules wearing one sentence - find which of its jobs is the safety
one before either keeping it or deleting it.**

**The defect this closed, and why the model change was unavoidable.** The edit service resolved
the shape from the **title**. Correct with one shape per title; a live defect the moment a title
has two, because the title's answer is then its *default* rather than what a given contest was
created as. An ordinary edit to a name would have re-forced a staggered contest to one attempt,
`until_window_closes` and entry closing at the gun - **under people who had already paid to
enter**, with no error and nothing in a log. So the contest stores its own `playMode` and the
edit path reads that. **The general form: adding a second possible value to a field is never
purely additive - grep every read that reaches it through its parent.** The flipped structural
test now asserts `resolvePlayShape` is **absent** from both services, where it used to require it.

**Frozen in three places, because one is a suggestion.** `Competition.playMode` is absent from
`EditProviderContestInput`, absent from `toEditRequestBody` and named in `NEVER_EDITABLE_FIELDS`.
It decides **when entry closes** and **how many attempts a paying entrant gets**, so a contest
that should be the other shape is a new contest - the same answer as refusing a game-type change
on a zero-participant draft (`12` s2.2), where the permission buys nothing and costs a guarantee.

**Two questions, two functions, and merging them is the tidy-looking mistake.**
`isPlayModeSupported` runs **before anything is written**, so a refusal leaves no draft behind;
`resolveContestPlayMode` reads what a contest already is and deliberately does **not** re-check
it. Folded together, every read of an existing contest would re-litigate a decision already
taken - so narrowing a title's supported set would retroactively change the shape of contests
already running under it.

**Three smaller rules from the same build.** The supported set **unions the title's own resolved
style in**, because that is what the Play style control says the game *is* and what every contest
already created on it was created as - a set excluding it makes a declared style unselectable;
same reasoning as `resolveAllowedGameTypes` treating `[]` as the default. **`head_to_head` beats
the operator here too**, which makes the picker's options a **server** answer: a component
deriving its own would offer a staggered chess match the create service is about to refuse, and a
test asserts the two helpers **disagree** on that case. And `supportedPlayModes` has **no schema
default**, on the `playModeOverride` precedent - a schema default *is* a stored value, and one
here would opt the whole catalogue into a control nobody asked for.

**Probing lesson, and it cost two real guards.** Two pre-existing probes reported `PROBE DID NOT
APPLY`, having matched `resolvePlayMode`'s original one-line body since the operator override
(`22` s9) replaced it with the shared `storedMode` helper. That reads like a broken harness
rather than a moved target, so the guards behind them had been sitting unprobed. Both re-aimed
and both now red. **When a probe reports DID NOT APPLY, the target moved - it is not noise.**

**Files touched:** `lib/services/games/play-shape.ts` (+ admin mirror),
`database/models/games/provider-game.model.ts` (+ admin), `database/models/trading/competition.model.ts`
(+ admin), `apps/admin/lib/services/game-providers/{provider-contest,provider-contest-edit,game-play-style}.service.ts`,
`apps/admin/app/api/games/providers/[providerKey]/games/play-style/route.ts`,
`apps/admin/lib/admin/{game-content-fields,competition-update-fields}.ts`,
`apps/admin/components/admin/games/{ContestPlayModeField,GamePlayStyleControl,ProviderContestWizard,contest-draft,contest-types,provider-types}.*`,
`apps/admin/components/admin/games/wizard/{StepSchedule,StepPrizes}.tsx`,
`__tests__/services/play-shape.test.ts`, `tools/probe-play-shape.ps1`.

**Deviated from plan:** the task document asks only that unsupported modes be unselectable. The
build also **stores the choice on the contest**, which the task does not mention and which is the
larger half - without it the feature is a picker whose answer the first edit throws away.

**Owner tested:** no. Both screens are behind an admin sign-in the automated browser has no
session for. Full suite 1,919 green, `check:mirrors` clean, typechecks at baseline (main **194**,
admin **223**) with nothing in a touched file and nothing disappearing.

**Deferred:** nothing from task 11. **Latent, nothing backfilled** - no title declares a second
shape, so `resolveSupportedPlayModes` returns one entry for the whole catalogue, the picker is
withheld everywhere and every contest resolves exactly as it did on 8 Sep.

**Next chat should:** task **12** (required timing / runtime settings), then **13**
(data-driven game configuration). **The challenges side is still gated behind those two** - it
needs a shape-aware setup page, and the shape settings it would adapt to are what 12 and 13
define.
...
### 9 Sep 2026 - X6 - WHICH SCORES ARE WORTH A PRIZE IS NOW A PER-TITLE SETTING (TASK 14)

**Shipped:** three operator-owned fields on both `provider_game` copies -
`zeroIsValidResult`, `minimumEligibleScore`, `scoreUnit` - read once per contest by
`resolveScoringRules` (mirrored), threaded onto every participant row by
`provider-settlement.service.ts` (mirrored) and consumed by `providerHasResult` (mirrored).
Admin side: `game-scoring-rules.service.ts`, a `PATCH` route guarded on `game-providers` with
its own audit line, `score-eligibility-copy.ts`, and a **Prize eligibility** control in the
per-title row of `ProviderCatalogueDialog`. 55 tests in
`__tests__/admin/game-scoring-rules.test.ts`, **28 probes red on exactly the expected test**
in `tools/probe-game-scoring-rules.ps1`. Full account in the task document at **14.1**.

**THE RULE DID NOT CHANGE - IT BECAME A DEFAULT.** This morning's task 2 decision shipped as
a hard-coded `> 0` inside `providerHasResult`. Every one of the three fields absent produces
the identical answer, so **nothing settles differently** until an operator turns it on for a
title, and a row synced before the fields existed is unaffected. A summary claiming eligibility
was loosened is wrong in both directions.

**Three of the six things task 14 lists were already there and were not rebuilt.**
`scoreDirection`, `scoreType` and `scoreRange` are **provider-owned** - in
`providerOwnedFields`, so a control writing to them saves, toasts and is reverted by the next
sync with no error and nothing in a log. That split is the design: *how* a game scores is the
provider's fact, *whether a score is worth paying* is ours.

**Decided:**

- **`minimumEligibleScore` is directional**, and "minimum" is the trap - `>=` upward, `<=`
  downward. The both-directions-upward spelling refuses every finisher of a race under the
  bar, so **the better a player did the more certainly they are excluded.** Wrong winner paid,
  not a wrong screen.
- **A stored `0` and an absent value are opposites here** - on a higher-is-better title a bar
  of zero *admits* a zero score - so the collapse is probed at four separate layers, because
  `|| null`, `?? ""` and a truthiness test each reintroduce it somewhere different.
- **`zeroIsValidResult` carries no schema default.** `default: false` matches today's
  behaviour exactly and writes a real `false` onto every row the sync creates, making an
  explicit "no" indistinguishable from "nobody has said" - so a future reversal of the
  platform rule could not tell them apart. Found by a test asserting the sync **invents**
  nothing, which went red on the default alone.
- **All three are on `NEVER_EDITABLE_CONTENT_FIELDS`**, following `playModeOverride`: two of
  them decide who is paid, so the content door would let a prize rule change as a side effect
  of fixing a typo with the audit trail recording a content edit. `scoreUnit` is harmless
  there and is barred anyway, so one screen's three fields cannot have two audit lines.
- **The preview is pinned behaviourally against the gate.** A copy test passes for ever while
  the wording describes the opposite of what settlement does, and the operator sets a money
  rule from that sentence.

**Deferred:** placement scoring and win/loss records, two of the five models the task lists,
need nothing here - a provider reporting a placing reports a number and the platform ranks
numbers. A title needing the platform to understand a ladder is a scoring *model*, which is
task 13.

**Four probing lessons, every one a recurrence.** Removing a field from the never-editable
list stayed **green**, because it is absent from the editable allow-list too and still fell
through to the unknown-field refusal whose message also names the field - identical to
`competition-update-fields.ts` and `gameKey`, and the test now pins *which* refusal fired.
Deleting the whole `newValue` block stayed **green** against `toMatch(/zeroIsValidResult/)`,
because the human-readable description one line above names both fields: **fifth instance of
one identifier defeating a structural test.** `-t '$unset'` matched **nothing**, because
vitest treats the filter as a regex and `$` is an anchor, so the probe reported a guard absent
while the named test never ran. And an **em-dash in a pattern cannot match the file** - this
script is UTF-8 with no BOM and PowerShell 5.1 decodes it with the system codepage - so keep
probe anchors ASCII and anchor on the condition rather than the prose when the prose is what
is being tested.

**Next chat should:** the challenges side - picking trading-or-game, then a title, then
adapting the setup page to that title's shape. Note it is **gated**, and two things about it
are already decided: a simultaneous-start title is **not challengeable at all** (`22` s6
answer C), and provider challenges inherit **R50** unfixed because
`ChallengeParticipant.score` still defaults to `0`.
...
advertised to signed-out visitors read `+$30.00`. **70 tests, 27 probes**, every one red on
exactly the expected test with a blast radius of one, plus one control probe green on purpose.

**Corrected within the day, and read this before the paragraphs below: it is the credit SYMBOL,
not the credit name.** The first cut rendered `AppSettings.credits.name`, the word `Volts`; the
owner asked for `credits.symbol` from Settings → Currency, defaulting to `⚡`, so an entry fee
reads **`50 ⚡`**. One field along on the right object - it typechecks, it renders, and it is
only wrong to somebody who opens the currency screen. Task doc **1.2** is the authoritative
account; every sentence here naming `credits.name` is correct as history and stale as a present
fact. **Singularisation went with it and its two tests were inverted rather than deleted**: a
word has a plural, a glyph does not, and `replace(/s$/, "")` on a configured symbol strips its
last character. **Both of those probes came back green, on the fourth cause - the mutation
changed no observable**, since re-adding the singulariser to `⚡` yields `⚡`; the tests only
asserted the default, and the fix was a configured symbol ending in `s`, the field being free
text. The symbol is written **after** the number, matching the wallet, pinned by a test because
the fiat convention is what a later edit corrects it to. A **codemod overreached into four
files** where `creditName` was prose - a knowledge-base article, a CSV header - and was
reverted, the tell being duplicate object keys: **a pattern-driven rename cannot tell a
formatter argument from a sentence.** And one **measurement error**, kept because it looked like
a live defect: a whole-file regex for the symbol default returned a euro sign, which reads as
the formatter and the schema having drifted - but `currency:` precedes `credits:` in
`app-settings.model.ts`, so that was the **fiat** field, where a euro sign is right. The test
slices from `credits:` and was correct throughout. **Scope a schema probe to the block you
mean, and check a finding against the assertion that already passes before believing it.**
...
pick the wrong rate. **(Closed on its own as R74, 14 Sep 2026. The out-of-scope call above is
correct as history; the disagreement is not still live.)**
...
### 9 Sep 2026 - X6 - AN OPERATOR CAN NOW SET A GAME'S PLAY STYLE (TASK 10, FIRST STEP)

**Shipped:** `provider_game.playModeOverride`, a **Play style** control on every row of the
Games list, and the resolved style badged on the contest wizard's game picker. Yesterday's
`22` s8 made the shape real and enforced; it left the declaration entirely with the provider,
which is right for a third party and **wrong for the one provider we run ourselves** - for
ChartVolt Games the value is a TypeScript literal in `games-service/src/games/titles.ts`, so
making a title a race meant an edit, a build and a redeploy of a separate service. There was
no screen anywhere that could say it. **37 tests, 19 probes**, every one red on exactly the
expected test.

**Files touched:** `lib/services/games/play-shape.ts` and its admin mirror (the override on
`PlayShapeInput`, the `storedMode` normaliser, the new precedence, `canOverridePlayMode`,
`PLAY_MODE_COPY`); both `provider-game.model.ts` copies;
`apps/admin/lib/services/game-providers/game-play-style.service.ts` (new);
`apps/admin/app/api/games/providers/[providerKey]/games/play-style/route.ts` (new);
`apps/admin/lib/admin/game-content-fields.ts`;
`apps/admin/components/admin/games/GamePlayStyleControl.tsx` (new),
`ProviderCatalogueDialog.tsx`, `provider-types.ts`,
`wizard/StepChooseGame.tsx`; `__tests__/admin/game-play-style.test.ts` (new);
`tools/probe-game-play-style.ps1` (new).

**Deviated from plan:** nothing in `22` s4 asked for an operator control - the chapter assumed
the provider always knows. Recorded as `22` s9 and `12` s4.2c rather than by rewriting s8, and
inserted into the owner's task document as **10.1**, with **10.2** putting `TURN_BASED` and
`ROUND_BASED` on the blocked list beside the per-round provider cost. That is the owner's
instruction and the reasoning is worth keeping: **a bracket needs opponents.** Nothing in the
catalogue takes turns or runs heats, no provider is signed, and `01`'s issued contract carries
one round producing one score - so designing either now means inventing a protocol, a schedule
and a UI against no counterparty, then finding out on the first real integration which half
was wrong, while two extra values ride through every gate, wizard step and migration
meanwhile. It costs nothing to wait: `supportsOneVsOne` already refuses a title that cannot be
challenged, and `playMode` covers the two shapes we can actually run.

**Owner tested:** not yet. Verified by the 37 tests, the 19 probes, `check:mirrors` green, and
both typechecks at their baselines (main **198**, admin **223**) with none in the changed
files and **none disappearing**.

**Deferred:** no bulk edit and no per-contest override - the shape is a property of the title,
resolved from the stored catalogue row and never from caller input, and a per-contest control
is exactly the way to switch off whichever half of the rule is inconvenient. Challenges are
still untouched (`22` s5, X10 / E8), and they still inherit **R50**.

**Three findings worth carrying.**

**A control writing to a field the catalogue sync owns is a control that appears to work.**
`playMode` is in `providerOwnedFields`, so a control editing it would save, toast, render
correctly and be reverted by the next sync with no error and nothing in a log - the shape
already on record for a provider enabled with no adapter, a `rankingMethod` a provider game
ignores and `isPaused` on a provider contest. Hence a second field. **The part that makes it a
rule rather than a note is that the safety lives in a different file:** `playModeOverride` is
safe because of an allow-list in `catalogue.service.ts`, not because of anything visible where
the field is declared or written. So the test runs a **real sync** and asserts the provider's
own `playMode` **was** rewritten in the same pass - an assertion that only checks the override
survived is equally green against a sync that did nothing. And the mirror matters
behaviourally here: the sync test exercises the main app's copy while `apps/admin` has its
own, which is the one that runs when an operator presses Sync, so a probe adding the field to
the admin allow-list alone leaves every behavioural test green. Only the byte-for-byte
comparison catches it - `check:mirrors` compares models.

**`head_to_head` beats the operator, and getting the precedence the natural way round would
have created the fifth dead field of this programme.** An override cannot make two people play
each other at different times, so a value stored against such a title is read by nothing -
after `requiresSyncPlay`, `isPaused`, `lastSuccessfulRoundAt` and `family`. The service
refuses it and the control withholds itself with the reason, both from `canOverridePlayMode`,
because a form offering a choice the server rejects with a 400 reads to an operator like a
permissions problem. Both call the one helper; a probe giving the component its own
`family ===` test is red.

**The empty string inverts this rule rather than merely being untidy.** "Missing" has three
shapes and `""` is the one a half-run migration or a blank form leaves behind - and read
literally it would mask a provider's `scheduled` declaration behind an override nobody chose,
turning a race into a staggered contest. `storedMode` admits only the two real values, and
clearing uses **`$unset`** so the document afterwards is indistinguishable from one that never
had an override.
