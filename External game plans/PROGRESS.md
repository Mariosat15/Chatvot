...
5. Then the admin programme, per the owner's admin-first sequencing. **Most of X6 is now
   ...
   screen** (`12` s5.1a). **What is left of X6 is the Game Master contest creation API**,
   plus three items inside `12` s5: `AdminOverviewDashboard.tsx`, the
...
**Deferred:** the trading form is **not** on the shared shell yet - deliberately a separate
commit so it can be reverted without losing the game wizard's look - and the client-side market
block on trading creation is still there, which is the 4 September decision that was never
applied to the screen.

**Next chat should:** switch the trading form onto the shared shell, then delete its market
block.
