...
| **Why the genre list looked empty** | **R60, closed 9 Sep 2026.** The Genre drop-down opened as a tall panel with one word in it, and **nothing was missing** - all fifteen options were there and all fifteen were white on white. **A browser paints a native `<select>`'s list itself**, taking the background from the element's own `background-color` while the options inherit `color`; this surface is themed `bg-white/5 text-white`, and a *translucent* white composites over the browser's light list surface to effectively white. The one legible row was the highlighted one, against the operating system's selection band. **A fault in which nothing fails reads as a data problem**, which is why it was reported as items not showing. **Two conditions are needed and this was the only place in either app with both**: a native `<select>` AND a translucent background. The surface has **eleven** pickers on the shared `Select`, which draws its own list in a portal; this was the twelfth and the only native one. The **~24 native selects elsewhere in admin sit on an opaque `bg-gray-*`** and mostly set no colour, so they are correct - **which is why the guard is scoped to this folder**, a platform-wide ban firing on two dozen working files being the kind the first person it inconveniences deletes (`13` s4.1g). **Radix throws on a `SelectItem value=""`**, so "no genre" travels as a sentinel mapped back at the boundary, and its spelling is load-bearing: `normaliseCategorySlug` strips every non-alphanumeric run, so `__no_genre__` is unreachable from the slug namespace while `none` or `custom` is a genre somebody can type and would then find clearing the field. **Low and live, nothing backfilled** - but it made the genre unsettable in practice, so `category` is the field most likely to be stale on any title touched before today, and it is the grouping key tasks 21-24 join on. **4 tests, 6 probes; never verified by eye** |
...
**Two conditions are needed, and this was the only place in either app with both.** A native
`<select>` *and* a translucent background. This surface has **eleven** pickers on the shared
`Select`, which draws its own list in a portal and never asks the browser for one; this was the
twelfth and the only native one left. The **~24 native selects elsewhere in the admin app sit
on an opaque `bg-gray-*`** and mostly set no `color` at all, so they have two independent
protections and are correct.

**That is why the guard is scoped to this folder and does not ban the element platform-wide.** A
platform-wide ban fires on two dozen working files and gets deleted by the first person it
inconveniences - the reasoning that narrowed the `GameIcon` ban in `13` s4.1g.
...
`tools/probe-genre-picker.ps1` (new, 6 probes); `17` R60; this file; the `.mdc` rule.

**Severity, in both directions.** **Low and live.** No money moved, nothing is stored wrongly,
**nothing backfilled** because there is nothing to backfill. But it made the genre unsettable
in practice - an operator could only land on a value by arrowing blindly through an invisible
list - so `category` is the field most likely to be stale on any title touched before today,
and that is the grouping key tasks 21-24 will join on.
...
files and none disappearing, **691 admin tests** green, 6 probes red on exactly the expected
test with a blast radius of one.
