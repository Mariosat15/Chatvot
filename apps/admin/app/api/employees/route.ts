  ...
  try {
    ...
    return NextResponse.json({
      ...
      employees: employees.map((emp) => {
        const isSuper =
          emp.email.toLowerCase() === defaultAdminEmail ||
        ...
        const recentlyActive =
          emp.lastLogin && new Date(emp.lastLogin) > thirtyMinutesAgo;
  ...
  try {
    ...
    if (action === "create") {
      ...
      if (roleTemplateId) {
        ...
        allowedSections = customSections.filter((s: string) =>
      ...
      console.log(
        `📝 Password type: ${isManualPassword ? "manual" : "auto-generated"}`,
      );
      ...
      console.log(
        `✅ Employee created: ${newEmployee.email}, role: ${roleName}, sections: ${allowedSections.length}`,
      );
      console.log(
        `✅ Password hash stored (first 20 chars): ${newEmployee.password.substring(0, 20)}...`,
      );

      // Verify password was hashed correctly by testing it
      const passwordVerify = await newEmployee.comparePassword(password);
      console.log(
        `✅ Password verification test: ${passwordVerify ? "PASSED" : "FAILED"}`,
      );
      if (!passwordVerify) {
        console.error(
          "❌ WARNING: Password verification failed immediately after save!",
        );
      }
      ...
      adminEventsService.employeeCreated(newEmployee._id.toString(), {
        id: auth.adminId!,
        email: auth.email!,
      });
    ...
    if (action === "send_credentials") {
      ...
      if (!emailSent) {
        ...
        if (!smtpHost) {
          return NextResponse.json({
            ...
            emailSent: false,
            message:
              "Password updated but email could not be sent. SMTP is not configured.",
      ...
      return NextResponse.json({
        ...
        message: emailSent
          ...
          : "Password updated but email failed to send. Check server logs.",
    ...
    return NextResponse.json(
      { error: "Failed to manage employees" },
      { status: 500 },
    );
  }
}
...
async function sendEmployeeCredentialsEmail(
  ...
  try {
    console.log(
      `📧 Attempting to send credentials email to ${employee.email}...`,
    );

    // Get email template
    let template = await EmployeeEmailTemplate.findOne({
    ...
    if (!template) {
      ...
      template = await EmployeeEmailTemplate.findOne({
        templateId: "employee_welcome",
        isActive: true,
      });
    }
    ...
    if (!template) {
      console.error(
        "❌ Employee welcome email template not found even after initialization",
      );
      return false;
    }
    ...
    if (!fromEmail) {
      console.error(
        "❌ Email not configured - check nodemailerEmail in WhiteLabel settings or NODEMAILER_EMAIL env",
      );
      return false;
    }
