 ...
 */
async function redirectActiveConversations(
  employeeId: string,
  ...
  if (!availableEmployee) {
    console.log(
      `⚠️ [Availability] No available employees to redirect conversations from ${employeeEmail}`,
    );
    return;
  }
      ...
      { _id: conv._id },
      {
        $set: {
          ...
          assignedEmployeeName:
            availableEmployee.name || availableEmployee.email,
  ...
  console.log(
    `📋 [Availability] Redirected ${activeConversations.length} conversations from ${employeeEmail} to ${availableEmployee.email}`,
  );
}
