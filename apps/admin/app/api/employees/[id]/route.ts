...
export async function PUT(
  ...
  try {
    ...
    if (
      isEmployeeOriginal &&
      employee._id.toString() !== currentAdmin._id.toString()
    ) {
      return NextResponse.json(
    ...
    if (previousRole !== employee.role) {
      await auditLogService.logEmployeeRoleChanged(
        adminInfo,
        id,
        employee.name,
        previousRole || "None",
    ...
    adminEventsService.employeeUpdated(id, {
      id: auth.adminId!,
      email: auth.email!,
    });
...
export async function DELETE(
  ...
  try {
    ...
    const reassignResult =
      await customerAssignmentService.reassignEmployeeCustomers(
        id,
        deletedName,
        deletedEmail,
        performedBy,
    ...
    console.log(
      `🔄 Customer reassignment result for ${deletedEmail}: ${reassignResult.reassigned} reassigned, ${reassignResult.failed} failed/unassigned`,
    );
    ...
    await auditLogService.logEmployeeDeleted(
      adminInfo,
      id,
      deletedName,
      deletedEmail,
    ...
    // Broadcast real-time event
    adminEventsService.employeeDeleted(id, {
      id: auth.adminId!,
      email: auth.email!,
    });

    console.log(
      `✅ Employee deleted: ${deletedEmail} (was force logged out first)`,
    );

    // Include reassignment info in response
    const message =
      reassignResult.reassigned > 0
...
export async function PATCH(
  ...
  try {
    ...
    if (action === "suspend") {
      ...
      await auditLogService.logEmployeeSuspended(
        adminInfo,
        id,
        employee.name,
        reason,
      );
      adminEventsService.employeeStatusChanged(id, "disabled", {
        id: auth.adminId!,
        email: auth.email!,
      });
    ...
    if (action === "unsuspend") {
      ...
      await auditLogService.logEmployeeUnsuspended(
        adminInfo,
        id,
        employee.name,
      );
      adminEventsService.employeeStatusChanged(id, "active", {
        id: auth.adminId!,
        email: auth.email!,
      });
    ...
    if (action === "toggle_status") {
      ...
      adminEventsService.employeeStatusChanged(id, newStatus, {
        id: auth.adminId!,
        email: auth.email!,
      });
    ...
    if (action === "reset_password") {
      ...
      await auditLogService.logEmployeePasswordReset(
        adminInfo,
        id,
        employee.name,
      );
      ...
      console.log(
        `✅ Password reset for ${employee.email}, forceLogoutAt set to invalidate old sessions`,
      );

      return NextResponse.json({
        success: true,
        message:
          "Password reset successfully. Employee will need to log in again.",
      });
    }
    ...
    if (action === "force_logout" || action === "toggle_lockout") {
      ...
      if (newLockedOutState) {
        ...
        adminEventsService.employeeStatusChanged(id, "locked_out", {
          id: auth.adminId!,
          email: auth.email!,
        });

        console.log(
          `🔒 Employee ${employee.email} locked out by ${auth.email}`,
        );
        ...
        adminEventsService.employeeStatusChanged(id, "unlocked", {
          id: auth.adminId!,
          email: auth.email!,
        });
