...
export async function GET(request: NextRequest) {
  try {
    ...
    const UserLevel = (await import("@/database/models/user-level.model"))
      .default;
    const UserBadge = (await import("@/database/models/user-badge.model"))
      .default;

    // Get all related data
    const [wallet, transactions, userLevel, badges, notifications] =
      await Promise.all([
        CreditWallet.findOne({ userId: resolvedUserId }).lean(),
        WalletTransaction.find({ userId: resolvedUserId })
          .sort({ createdAt: -1 })
          .lean(),
    ...
    if (completedDeposits.length > 0 && !userLevel) {
      issues.push(
        "❌ Has completed deposits but no UserLevel - badge evaluation never ran",
      );
    }
    ...
    if (pendingDeposits.length > 0) {
      issues.push(
        `⚠️ Has ${pendingDeposits.length} pending deposits - may be waiting for webhook`,
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to diagnose user",
      },
      { status: 500 },
    );
  }
