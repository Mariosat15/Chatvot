...
const IMAGE_DIRECTORIES = {
  production: [
    { path: '/var/www/chartvolt/public/uploads/marketplace', label: 'Marketplace Uploads' },
    { path: '/var/www/chartvolt/public/assets/avatars', label: 'Default Avatars' },
    { path: '/var/www/chartvolt/public/uploads/cosmetics', label: 'Cosmetics' },
    { path: '/var/www/chartvolt/public/uploads/indicators', label: 'Indicators' },
    { path: '/var/www/chartvolt/public/uploads/strategies', label: 'Strategies' },
    { path: '/var/www/chartvolt/public/uploads/gamemaster', label: 'Game Master' },
    { path: '/var/www/chartvolt/public/uploads', label: 'General Uploads' },
  ],
  development: [
    { path: path.join(process.cwd(), '..', '..', 'public', 'uploads', 'marketplace'), label: 'Marketplace Uploads' },
    { path: path.join(process.cwd(), '..', '..', 'public', 'assets', 'avatars'), label: 'Default Avatars' },
    { path: path.join(process.cwd(), '..', '..', 'public', 'uploads', 'cosmetics'), label: 'Cosmetics' },
    { path: path.join(process.cwd(), '..', '..', 'public', 'uploads', 'indicators'), label: 'Indicators' },
    { path: path.join(process.cwd(), '..', '..', 'public', 'uploads', 'strategies'), label: 'Strategies' },
    { path: path.join(process.cwd(), '..', '..', 'public', 'uploads', 'gamemaster'), label: 'Game Master' },
    { path: path.join(process.cwd(), '..', '..', 'public', 'uploads'), label: 'General Uploads' },
    { path: path.join(process.cwd(), 'public', 'uploads', 'marketplace'), label: 'Admin Marketplace' },
    { path: path.join(process.cwd(), 'public', 'uploads'), label: 'Admin Uploads' },
  ],
};
...
async function findAllImageDirectories(): Promise<DirectoryInfo[]> {
  const allDirs = [...IMAGE_DIRECTORIES.production, ...IMAGE_DIRECTORIES.development];
...
function getImageType(filename: string, dirLabel: string): string {
  ...
  if (dirLower.includes('gamemaster') || dirLower.includes('game master')) return 'gamemaster';
  ...
  if (lower.includes('gamemaster') || lower.includes('gm-')) return 'gamemaster';
  ...
  try {
    ...
    if (directories.length === 0) {
      return NextResponse.json({
        ...
        message: 'No directories containing images were found. Make sure uploads exist.',
      }, { status: 404 });
    }
  ...
  try {
    ...
    for (const img of imagesToProcess) {
      try {
        ...
        const settings = IMAGE_SETTINGS[img.imageType] || IMAGE_SETTINGS.default;
        ...
        console.log(`✅ Optimized ${img.filename}: ${formatBytes(fileStats.size)} → ${formatBytes(optimizedBuffer.length)} (${savedPercent.toFixed(1)}% saved)`);
      } catch (error) {
        results.push({
          filename: img.filename,
          fullPath: img.fullPath,
