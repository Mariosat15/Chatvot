...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Confirmation required. Send { confirm: "DELETE_ALL_SIMULATION_DATA" }',
        },
        { status: 400 },
      );
    }
    ...
    console.log(
      `Found ${simCompetitionIds.length} simulator competitions to clean up`,
    );

    // 3. Delete competition participants first (before competitions)
    const participantDeleteResult = await db
      .collection("competitionparticipants")
      .deleteMany({
        $or: [
          {
            competitionId: {
              $in: simCompetitionIds.map(
    ...
    const chalPartDeleteResult = await db
    ...
    const posDeleteResult = await db.collection("tradingpositions").deleteMany({
      $or: [
        ...
        {
          competitionId: {
            $in: simCompetitionIds.map((id) => new mongoose.Types.ObjectId(id)),
          },
        },
        { competitionId: { $in: simCompetitionIds } },
      ],
    ...
    const historyDeleteResult = await db
...
export async function GET() {
  try {
    ...
    counts.competitionParticipants = await db
      .collection("competitionparticipants")
      .countDocuments({
        $or: [
          {
    ...
    counts.challengeParticipants = await db
      .collection("challengeparticipants")
      .countDocuments({
        $or: [
          { userId: { $in: simUserIds } },
    ...
    counts.deviceFingerprints = await db
      .collection("devicefingerprints")
      .countDocuments({
        $or: [
          { browser: "SimBrowser" },
