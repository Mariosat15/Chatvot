...
export async function GET(
  ...
  try {
    ...
    try {
      const challengeParticipations = await db
      ...
      const challengeIds = challengeParticipations
        .map((p) => p.challengeId)
        .filter(Boolean);
      const challenges =
    ...
    try {
      ...
      if (gmSubscription) {
        ...
        // Renewal history
        if (
          gmSubscription.renewalHistory &&
          Array.isArray(gmSubscription.renewalHistory)
        ) {
          for (const renewal of gmSubscription.renewalHistory) {
            history.push({
              ...
              type: "gamemaster",
              category:
                renewal.status === "success" ? "renewal" : "renewal_failed",
    ...
    try {
      ...
      for (const earning of gmEarnings) {
        ...
        const isChallenge =
          earning.transactionType === "gamemaster_challenge_referral" ||
