...
export async function GET(
  ...
  try {
    ...
    const history = await AccountLockout.find({
      $or: [{ userId }, { email: lockout?.email }].filter(Boolean),
    })
      .sort({ createdAt: -1 })
      .limit(10)
      .lean();
...
export async function DELETE(
  ...
  try {
    ...
    const result = await AccountLockout.updateMany(query, {
      $set: {
        isActive: false,
        unlockedAt: new Date(),
    ...
    if (userEmail) {
      try {
        ...
        console.log(
          `📡 [Admin] Calling main app to clear in-memory lockouts...`,
        );
        console.log(`📡 [Admin] Main app URL: ${mainAppUrl}`);

        const response = await fetch(
          `${mainAppUrl}/api/admin/lockouts/unlock`,
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          console.log(
            `✅ [Admin] In-memory lockouts cleared for user: ${userId}`,
            responseData,
          );
          inMemoryCleared = true;
        } else {
          console.error(
            `❌ [Admin] Main app returned error:`,
            response.status,
            responseData,
          );
        }
        ...
        console.error(
          "❌ [Admin] Could not call main app to clear in-memory lockouts:",
          memoryError,
        );
    ...
    console.log(
      `🔓 [Admin] Account unlocked for user ${userId} by ${session.email}`,
    );
