  ...
  try {
    ...
    if (userIdFilter) {
      ...
      if (users.length === 0) {
        try {
          ...
          if (ObjectId.isValid(userIdFilter)) {
            users = await db
              .collection("user")
              .find({ _id: new ObjectId(userIdFilter) })
              .toArray();
          }
        } catch {
      ...
      if (users.length === 0) {
        users = await db
          .collection("user")
          .find({ _id: userIdFilter as any })
          .toArray();
      }

      totalCount = users.length;
      console.log(
        `📊 Fetching user with ID: ${userIdFilter} - Found ${users.length} user(s)`,
      );
    } else {
      ...
      console.log(
        `📊 Fetching users - Page ${page}, Limit ${limit}, Found ${users.length}/${totalCount} user(s)`,
      );
    }
    ...
    const gmSubscriptions = await db
      .collection("gamemastersubscriptions")
      .find({
        userId: { $in: userIds },
      })
      .toArray();
    const gmSubscriptionMap = new Map(
      gmSubscriptions.map((gm: any) => [gm.userId, gm]),
    );
    ...
    const usersWithData = users.map((user: any) => {
      ...
      const activeCompetitions = userComps.filter(
        (p: any) => p.status === "active",
      ).length;
      const completedCompetitions = userComps.filter(
      ...
      const activeChallenges = userChalls.filter(
      ...
      const marketplaceSpent = userPurchs.reduce(
        (sum: number, p: any) => sum + (p.pricePaid || 0),
        0,
      );
      const marketplaceItems = userPurchs
      ...
      const storedRole =
        user.role ||
        (user.email?.toLowerCase() === adminEmail ? "admin" : "trader");
        ...
        gameMaster: gmSubscription
          ? {
              ...
              scheduledForDeletion:
                gmSubscription.scheduledForDeletion || false,
            }
          : {
              isGameMaster: false,
            },
