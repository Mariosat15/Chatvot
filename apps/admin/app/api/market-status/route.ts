...
async function getUpcomingHolidays(): Promise<MarketHoliday[]> {
  ...
  try {
    ...
    const holidays = (data?.response || []).filter(
      (h: MarketHoliday) =>
  ...
  for (const holiday of holidays) {
    ...
    if (holidayDate >= startDate && holidayDate <= endDate) {
      if (holiday.status === "closed") {
        warnings.push(
          `⚠️ Market closed on ${holiday.date} for ${holiday.name}`,
        );
      } else if (holiday.status === "early-close" || holiday.close) {
        warnings.push(
          `⚠️ Early market close on ${holiday.date} for ${holiday.name}`,
        );
      }
    }
  }
...
export async function GET(request: Request) {
  try {
    ...
    if (startDateStr && endDateStr) {
      ...
      const holidayWarnings = checkHolidayOverlap(
        startDate,
        endDate,
        upcomingHolidays,
      );
    ...
    if (!currentStatus.isOpen) {
      ...
      warnings.push(
        "❌ Cannot create instant challenges while market is closed",
      );
    }
