...
const Candle1m =
  mongoose.models.Candle1m || mongoose.model("Candle1m", Candle1mSchema);
...
const TIMEFRAME_CONFIG: Record<
  string,
  { collection: string; minutes: number }
> = {
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `🔍 [Gap Detection] Checking ${symbols.length} symbols × ${timeframesToCheck.length} timeframes`,
    );
    ...
    for (const sym of symbols) {
      ...
      if (liveCandles.length > 0) {
        console.log(
          `📊 [Gap Detection] ${sym} candles_1m: ${liveCandles.length} candles`,
        );
        ...
        if (gapMinutes > 60) {
          ...
          if (!isWeekend) {
            console.log(
              `🚨 [Gap Found] ${sym} candles_1m START OF DAY: ${startOfDay.toISOString()} → ${oldestLiveDate.toISOString()} (${gapMinutes} min)`,
            );
            allGaps.push({
              symbol: sym,
              timeframe: "1m (live)",
        ...
        for (let i = 1; i < liveCandles.length; i++) {
          ...
          if (timeDiff > 120) {
            ...
            if (!isWeekendGap && missingMinutes > 10) {
              console.log(
                `🚨 [Gap Found] ${sym} candles_1m: ${prevDate.toISOString()} → ${nextDate.toISOString()} (${missingMinutes} min)`,
              );
              allGaps.push({
                symbol: sym,
                timeframe: "1m (live)",
      ...
      for (const tf of timeframesToCheck) {
        ...
        if (candles.length > 0) {
          ...
          console.log(
            `📊 [Gap Detection] ${sym} ${tf}: ${candles.length} candles (${oldestDate.toISOString()} → ${newestDate.toISOString()})`,
          );
          ...
          if (trailingGapSeconds > expectedGapSeconds * 2) {
            ...
            if (config.minutes <= 240) {
              // 4h or less
              isWeekendTrailing =
                (newestDay === 5 || newestDay === 6) &&
                trailingGapMinutes >= 2880 &&
                trailingGapMinutes <= 4500;
            } else if (config.minutes === 1440) {
              // 1d
              isWeekendTrailing =
                newestDay === 5 &&
                trailingGapMinutes >= 2880 &&
                trailingGapMinutes <= 4320;
            }

            if (!isWeekendTrailing && trailingGapMinutes > config.minutes * 2) {
              const missingCandles = Math.floor(
                trailingGapMinutes / config.minutes,
              );
              console.log(
                `🚨 [Gap Found] ${sym} ${tf} TRAILING: ${newestDate.toISOString()} → ${now.toISOString()} (${trailingGapMinutes} min, ~${missingCandles} missing candles)`,
              );
              allGaps.push({
                symbol: sym,
                timeframe: tf,
          ...
          for (let i = 1; i < candles.length; i++) {
            const prevTime =
              new Date(candles[i - 1].timestamp).getTime() / 1000;
            ...
            if (timeDiff > expectedGapSeconds * 2) {
              ...
              if (!isWeekendGap && missingMinutes > config.minutes * 2) {
                console.log(
                  `🚨 [Gap Found] ${sym} ${tf}: ${prevDate.toISOString()} → ${nextDate.toISOString()} (${missingMinutes} min, ${Math.round(missingMinutes / 1440)} days)`,
                );
                allGaps.push({
                  symbol: sym,
                  timeframe: tf,
    ...
    console.log(
      `✅ [Gap Detection] Complete: ${allGaps.length} gaps found across ${symbols.length} symbols and ${timeframesToCheck.length} timeframes`,
    );
...
const TIMEFRAME_TO_MASSIVE: Record<
  string,
    ...
    for (const sym of symbols) {
      for (const tf of timeframesToFill) {
        ...
        if (candles.length < 50) {
          ...
          console.log(
            `📥 [Gap Fill] ${sym} ${tf}: API returned ${apiCandles.length} candles`,
          );
          ...
          for (let i = 1; i < candles.length; i++) {
            ...
            if (gap > expectedGapMs * 2) {
              ...
              console.log(
                `🔧 [Gap Fill] ${sym} ${tf}: Filling ${gapMinutes} minute gap`,
              );
        ...
        if (tfCandlesFilled > 0) {
          ...
          console.log(
            `✅ [Gap Fill] ${sym} ${tf}: Filled ${tfCandlesFilled} candles`,
          );
        }
      }
    }
