...
> {
  ...
  console.log(
    `🌐 [Massive API] Requesting: ${ticker} from ${new Date(fromMs).toISOString()} to ${new Date(toMs).toISOString()}`,
  );
  ...
  try {
    ...
    return NextResponse.json({
      ...
      symbolStats: symbolStats.map((s) => ({
        ...
        daysOfData:
          s.oldest && s.newest ? Math.round((s.newest - s.oldest) / 86400) : 0,
      })),
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `📥 [Seed History] Starting: ${symbols.length} symbols, ${days} days`,
    );
    console.log(
      `📥 [Seed History] From: ${fromDate} (${fromMs}ms = ${new Date(fromMs).toISOString()})`,
    );
    console.log(
      `📥 [Seed History] To: ${toDate} (${toMs}ms = ${new Date(toMs).toISOString()})`,
    );
    ...
    (async () => {
      ...
      for (const symbol of symbols) {
        try {
          ...
          while (chunkStart < toMs) {
            ...
            try {
              const candles = await fetchCandlesFromMassive(
                symbol,
                chunkStart,
                chunkEnd,
              );
              ...
              if (candles.length > 0) {
                ...
                const result = await Candle1m.bulkWrite(bulkOps, {
                  ordered: false,
                });
      ...
      console.log(
        `🎉 [Seed History] Background job ${jobId} completed in ${duration}s - Total: ${totalFetched} fetched, ${totalInserted} inserted`,
      );
    })();
