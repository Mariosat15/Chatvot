...
> = {
  "1m": {
    minutes: 1,
    collectionName: "candles_historical_1m",
    apiMultiplier: 1,
    apiTimespan: "minute",
  },
  "5m": {
    minutes: 5,
    collectionName: "candles_historical_5m",
    apiMultiplier: 5,
    apiTimespan: "minute",
  },
  "15m": {
    minutes: 15,
    collectionName: "candles_historical_15m",
    apiMultiplier: 15,
    apiTimespan: "minute",
  },
  "30m": {
    minutes: 30,
    collectionName: "candles_historical_30m",
    apiMultiplier: 30,
    apiTimespan: "minute",
  },
  "1h": {
    minutes: 60,
    collectionName: "candles_historical_1h",
    apiMultiplier: 1,
    apiTimespan: "hour",
  },
  "4h": {
    minutes: 240,
    collectionName: "candles_historical_4h",
    apiMultiplier: 4,
    apiTimespan: "hour",
  },
  "1d": {
    minutes: 1440,
    collectionName: "candles_historical_1d",
    apiMultiplier: 1,
    apiTimespan: "day",
  },
  "1w": {
    minutes: 10080,
    collectionName: "candles_historical_1w",
    apiMultiplier: 1,
    apiTimespan: "week",
  },
  "1M": {
...
const FetchStatus =
  mongoose.models.historical_fetch_status ||
...
> {
  ...
  console.log(
    `🌐 [Download History] Fetching ${ticker} ${timeframe}: ${new Date(fromMs).toISOString()} to ${new Date(toMs).toISOString()}`,
  );
  ...
  if (!response.ok) {
    ...
    console.error(
      `❌ [Download History] API Error ${response.status}: ${errorText}`,
    );
...
export async function GET() {
  try {
    ...
    const statuses = await FetchStatus.find()
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (useDateRange) {
      console.log(
        `📥 [Download History] DATE RANGE MODE: ${symbols.length} symbols, timeframes: ${validTimeframes.join(", ")}`,
      );
      console.log(
        `📥 [Download History] From: ${dateRangeFrom?.toISOString()} To: ${dateRangeTo?.toISOString()}`,
      );
    } else {
      console.log(
        `📥 [Download History] YEARS BACK MODE: ${symbols.length} symbols, timeframes: ${validTimeframes.join(", ")}, ${yearsBack} years`,
      );
    }
    ...
    (async () => {
      ...
      for (const symbol of symbols) {
        for (const timeframe of validTimeframes) {
          ...
          if (useDateRange && dateRangeFrom && dateRangeTo) {
            ...
            console.log(
              `📊 [Download History] ${symbol} ${timeframe}: DATE RANGE ${dateRangeFrom.toISOString()} → ${dateRangeTo.toISOString()}`,
            );
          }
          // YEARS BACK MODE: Calculate from last candle backwards
            ...
            if (oldestHistorical?.timestamp) {
              ...
              console.log(
                `📊 [Download History] ${symbol} ${timeframe}: Existing data, oldest at ${endDate.toISOString()}`,
              );
          ...
          try {
            ...
            console.log(
              `📊 [Download History] ${symbol} ${timeframe}: ${new Date(startMs).toISOString()} to ${new Date(endMs).toISOString()}`,
            );
            ...
            while (chunkStart < endMs) {
              ...
              try {
                const candles = await fetchFromMassive(
                  symbol,
                  timeframe,
                  chunkStart,
                  chunkEnd,
                ...
                if (candles.length > 0) {
                  // ALWAYS align timestamps before storing to ensure consistency:
                  ...
                  const documents = candles.map((c) => ({
                    symbol,
                    timestamp: new Date(
                      alignTimestamp(c.t, tfConfig.minutes, timeframe),
                    ),
                    open: c.o,
                    high: c.h,
                    low: c.l,
                  ...
                  // Track date range for logging (use aligned timestamps)
                  const firstCandleDate = documents[0].timestamp;
                ...
                console.error(
                  `⚠️ [Download History] Chunk error ${symbol} ${timeframe}: ${chunkError}`,
                );
              }
            ...
            console.log(
              `✅ [Download History] ${symbol} ${timeframe}: fetched ${totalFetched}, saved ${totalSaved}`,
            );
      ...
      console.log(
        `🎉 [Download History] Background job ${jobId} completed in ${duration}s - Total: ${grandTotalFetched} fetched, ${grandTotalSaved} saved`,
      );
    })();
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (symbol && timeframe) {
      ...
      return NextResponse.json({
        success: true,
        message: `Reset ${symbol} ${timeframe}`,
      });
    }
