...
export async function POST(
  ...
  try {
    ...
    const {
      ...
      reason,
      transferType = "chat",
      transferAllConversations = false,
    } = body;
    ...
    if (transferAllConversations && customerId) {
      ...
      console.log(
        `📋 [Transfer] Found ${conversationsToTransfer.length} conversations to transfer for customer ${customerName}`,
      );
    }
    ...
    try {
      ...
      for (const convId of conversationsToTransfer) {
        const conv = convId.equals(convObjectId)
          ? conversation
          : await db
              .collection("conversations")
              .findOne({ _id: convId }, { session });
        if (!conv) continue;
      ...
      const systemMessage =
        systemMessages.find((m) => m.conversationId.equals(convObjectId)) ||
        systemMessages[0];
      ...
      if (customerId) {
        await db.collection("customer_audit_trails").insertOne(
          {
            customerId,
            action: transferAllConversations
              ? "all_conversations_transferred"
              : "chat_transferred",
            category: "messaging",
            performedBy: {
              id: decoded.adminId,
      ...
      await db.collection("employee_notifications").insertOne(
        {
          ...
          type: transferAllConversations
            ? "bulk_chat_transfer_received"
            : "chat_transfer_received",
          title: transferAllConversations
      ...
      console.error(
        "❌ [Transfer] Transaction error, rolling back:",
        transactionError,
      );
