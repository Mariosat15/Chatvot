...
const MessagingSettingsSchema = new mongoose.Schema(
  {
    ...
    outsideHoursMessage: {
      type: String,
      default: "We're currently outside working hours.",
    },
    ...
    if (newAIEnabled !== undefined && newAIEnabled !== previousAIEnabled) {
      ...
      if (db) {
        if (newAIEnabled) {
          ...
          console.log(
            `🤖 [AI Enabled] Updated ${result.modifiedCount} conversations to AI handling`,
          );
        } else {
          ...
          console.log(
            `👤 [AI Disabled] Updated ${result.modifiedCount} conversations to employee handling`,
          );
        }
      }
    }
