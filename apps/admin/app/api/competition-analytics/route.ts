...
export async function GET(request: NextRequest) {
  try {
    ...
    const competitionIdStrings = competitionIds.map((id) =>
      (id as any).toString(),
    );
    ...
    const competitionAnalytics = await Promise.all(
      competitions.map(async (comp) => {
        ...
        const platformFeeFromTransactions =
          (compPlatformFee as any)?.amount || 0;
        ...
        const disqualifiedParticipants = compParticipants.filter(
          (p: any) =>
            ...
            (comp.finalLeaderboard &&
              comp.finalLeaderboard.find(
                (l: any) =>
                  l.userId?.toString() === p.userId?.toString() &&
    ...
    const challengeAnalytics = challenges.map((challenge) => {
      ...
      const platformFee = challengePlatformFees.find(
        (t: any) => t.sourceId === chalId,
      );
      const unclaimedPool = challengeUnclaimedPools.find(
        (t: any) => t.sourceId === chalId,
      );
      ...
      return {
        ...
        platformFeeAmount:
          challenge.platformFeeAmount || (platformFee as any)?.amount || 0,
        ...
        unclaimedPool: bothDisqualified
          ? (unclaimedPool as any)?.amount || challenge.winnerPrize || 0
          : 0,
