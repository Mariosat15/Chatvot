...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!subscription) {
      return NextResponse.json(
        { error: 'No active Game Master subscription found. Please purchase and activate a Game Master package from the marketplace.' },
        { status: 403 }
      );
    }
    ...
    if (new Date(subscription.endDate) < new Date()) {
      return NextResponse.json(
        { error: 'Your Game Master subscription has expired. Please renew to continue.' },
        { status: 403 }
      );
    }
