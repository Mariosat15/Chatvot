...
export async function POST(
  ...
  try {
    ...
    for (const comp of compensations) {
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Each compensation must have userId, positive amount, and reason",
          },
          { status: 400 },
        );
      }
    ...
    console.log(
      `💰 [Compensation] Processing ${compensations.length} compensations for incident ${incidentId}`,
    );
    ...
    for (const result of results) {
      if (result.success) {
        incident.resolution.compensations.push({
          ...
          reason:
            compensations.find((c) => c.userId === result.userId)?.reason || "",
          status: "paid",
          paidAt: new Date(),
        });
      }
    ...
    if (totalCompensated > 0) {
        ...
        [
          {
            ...
            compensationDetails: {
              ...
              compensationPerUser:
                successCount > 0 ? totalCompensated / successCount : 0,
      ...
      console.log(
        `   📊 [PlatformTransaction] Recorded expense: -€${totalCompensated.toFixed(2)}`,
      );
    }
    ...
    console.log(
      `💰 [Compensation] Complete: ${successCount}/${compensations.length} successful, €${totalCompensated.toFixed(2)} total`,
    );
