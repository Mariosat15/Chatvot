...
import {
  generateCustomerKnowledgeBase,
  getSettingsSummary,
} from "@/lib/services/dynamic-knowledge-generator";
  ...
  try {
    ...
    if (type === "admin" || type === "all") {
      ...
      if (existingAdminSource && !force) {
        ...
        if (existingAdminSource && force) {
          await aiKnowledgeService.deleteSource(
            existingAdminSource._id.toString(),
          );
        }
    ...
    if (type === "customer" || type === "all") {
      ...
      if (existingCustomerSource && !force) {
        results.customer = {
          alreadyIndexed: true,
          source: existingCustomerSource,
        };
      } else {
        if (existingCustomerSource && force) {
          await aiKnowledgeService.deleteSource(
            existingCustomerSource._id.toString(),
          );
        }
        ...
        console.log(
          `[AI Knowledge] Generating customer FAQ with settings: ${settingsSummary}`,
        );
        ...
        const customerSource = await aiKnowledgeService.createSource({
          ...
          metadata: {
            ...
            description:
              "Public-facing FAQ for customer support AI - generated from platform settings",
