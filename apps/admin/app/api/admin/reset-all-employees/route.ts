  ...
  try {
    ...
    if (confirmation !== "DELETE_ALL_EMPLOYEES") {
      return NextResponse.json(
        {
          error:
            'Invalid confirmation. You must type "DELETE_ALL_EMPLOYEES" exactly.',
        },
        { status: 400 },
      );
    }
    ...
    const defaultEmailAdmin = await Admin.findOne({
      email: defaultAdminEmail,
    }).select("_id");
    if (
      defaultEmailAdmin &&
      !superAdminIds.includes(defaultEmailAdmin._id.toString())
    ) {
      superAdminIds.push(defaultEmailAdmin._id.toString());
    }

    console.log(
      `   🛡️ Protecting super admin(s): ${superAdminIds.length} account(s)`,
    );
    ...
    console.log(
      `   ✅ Deleted ${deleteEmployeesResult.deletedCount} employee accounts`,
    );
    ...
    console.log(
      `   ✅ Deleted ${deleteCustomTemplatesResult.deletedCount} custom role templates`,
    );
    ...
    for (const template of DEFAULT_ROLE_TEMPLATES) {
      try {
        ...
        console.log(
          `   ⚠️ Could not reset template ${template.name}:`,
          templateError,
        );
      }
    }
    console.log(
      `   ✅ Reset ${DEFAULT_ROLE_TEMPLATES.length} default role templates`,
    );

    // 5. Delete all customer assignments (collection name: customer_assignments)
    try {
      const customerAssignmentsCollection = db.collection(
    ...
    try {
      const customerAuditTrailCollection = db.collection(
    ...
    console.log(
      `🚨 [ADMIN] EMPLOYEE DATA RESET COMPLETE - ${totalDeleted} total items affected`,
    );
