...
export async function POST(request: NextRequest) {
  try {
    ...
    const providerDoc = await PaymentProvider.findOne({
      slug: provider,
      isActive: true,
    });
...
async function autoConfigureStripe(
  ...
  const webhookSecretCred = providerDoc.credentials.find(
    (c: any) => c.key === "webhook_secret",
  );
...
async function autoConfigurePaddle(
  ...
  if (!listResponse.ok) {
    ...
    throw new Error(
      `Paddle API error: ${error.error?.message || "Failed to list destinations"}`,
    );
  }

  const { data: destinations } = await listResponse.json();
  const existing = destinations?.find(
    (d: any) => d.endpoint_url === webhookUrl,
  );
  ...
  if (existing) {
    ...
    console.log(
      `📝 Updating existing Paddle notification destination: ${existing.id}`,
    );

    const updateResponse = await fetch(
      `${baseUrl}/notification-destinations/${existing.id}`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${apiKey}`,
    ...
    if (!updateResponse.ok) {
      ...
      throw new Error(
        `Failed to update Paddle destination: ${error.error?.message}`,
      );
    }
    ...
    console.log(
      `✨ Creating new Paddle notification destination: ${webhookUrl}`,
    );
    ...
    if (!createResponse.ok) {
      ...
      throw new Error(
        `Failed to create Paddle destination: ${error.error?.message}`,
      );
    }
