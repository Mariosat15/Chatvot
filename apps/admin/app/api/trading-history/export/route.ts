...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all trades
    const trades = await TradeHistory.find(dateFilter)
      .sort({ closedAt: -1 })
      .lean();
    ...
    const competitionIds = [
      ...new Set(
        trades.filter((t) => t.competitionId).map((t) => t.competitionId),
      ),
    ];
    ...
    const competitionMap = new Map(
      competitions.map((c) => [c._id.toString(), c.name]),
    );

    const challengeIds = [
      ...new Set(trades.filter((t) => t.challengeId).map((t) => t.challengeId)),
    ];
    const challenges = (await Challenge.find({ _id: { $in: challengeIds } })
      .select("_id name")
      .lean()) as unknown as Array<{ _id: any; name: string }>;
    const challengeMap = new Map(
      challenges.map((c) => [c._id.toString(), c.name]),
    );

    // Filter trades
    let filteredTrades = trades.filter((t) =>
      allowedUserIds.has(t.userId.toString()),
    );

    if (contestType === "competition") {
      filteredTrades = filteredTrades.filter(
        (t) => t.competitionId && !t.challengeId,
      );
