...
export async function GET(
  ...
  try {
    ...
    const challenges = await Challenge.find({
      ...
      .select("_id slug status challengerName challengedName entryFee duration")
      .lean();
    ...
    const calculateSummary = (contestTrades: any[]) => {
      ...
      const totalPnl = contestTrades.reduce(
        (sum, t) => sum + (t.realizedPnl || 0),
        0,
      );
      const wins = contestTrades
        .filter((t) => t.realizedPnl > 0)
        .map((t) => t.realizedPnl);
      const losses = contestTrades
        .filter((t) => t.realizedPnl < 0)
        .map((t) => t.realizedPnl);
      const totalHoldingTime = contestTrades.reduce(
      ...
      return {
        ...
        losingTrades,
        winRate:
          contestTrades.length > 0
            ? (winningTrades / contestTrades.length) * 100
            : 0,
        totalPnl,
        ...
        averageHoldingTime:
          contestTrades.length > 0
            ? totalHoldingTime / contestTrades.length
            : 0,
      };
    };
    ...
    return NextResponse.json({
      user: {
        ...
        winRate:
          allTrades.length > 0 ? (totalWins / allTrades.length) * 100 : 0,
        totalPnl: allTrades.reduce((sum, t) => sum + (t.realizedPnl || 0), 0),
        competitions: competitions.length,
        challenges: challenges.length,
      },
      contests: contests.sort(
        (a, b) => b.summary.totalTrades - a.summary.totalTrades,
      ),
    });
  } catch (error) {
