...
export async function PUT(request: Request) {
  try {
    ...
    if (body.assetSchedules !== undefined) {
      for (const [asset, schedule] of Object.entries(body.assetSchedules)) {
        if (schedule && typeof schedule === "object") {
          (settings.assetSchedules as Record<string, unknown>)[asset] =
            schedule;
        }
      }
    ...
    if (body.holidays !== undefined) {
      ...
      settings.holidays = body.holidays.map(
        (h: {
          _id?: string;
          name: string;
          date: string;
    ...
    if (body.blockTradingOnHolidays !== undefined) {
      if (body.blockTradingOnHolidays !== settings.blockTradingOnHolidays) {
        changes.push(
          `blockTrading: ${settings.blockTradingOnHolidays} → ${body.blockTradingOnHolidays}`,
        );
      }
    ...
    if (body.blockCompetitionsOnHolidays !== undefined) {
      if (
        body.blockCompetitionsOnHolidays !==
        settings.blockCompetitionsOnHolidays
      ) {
        changes.push(
          `blockCompetitions: ${settings.blockCompetitionsOnHolidays} → ${body.blockCompetitionsOnHolidays}`,
        );
      }
    ...
    if (body.blockChallengesOnHolidays !== undefined) {
      if (
        body.blockChallengesOnHolidays !== settings.blockChallengesOnHolidays
      ) {
        changes.push(
          `blockChallenges: ${settings.blockChallengesOnHolidays} → ${body.blockChallengesOnHolidays}`,
        );
      }
