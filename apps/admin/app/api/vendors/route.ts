...
export async function GET(request: NextRequest) {
  try {
    ...
    if (upcoming) {
      ...
      vendors = await VendorSubscription.find(query).sort({ nextPaymentDate: 1 });
    } else {
      vendors = await VendorSubscription.find(query).sort({ nextPaymentDate: 1 });
    }
    ...
    const byServiceType: Record<string, { count: number; monthlyTotal: number }> = {};
    for (const vendor of activeVendors) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.name || !body.amount || !body.nextPaymentDate) {
      return NextResponse.json(
        { success: false, error: 'Name, amount, and next payment date are required' },
        { status: 400 }
      );
    }
