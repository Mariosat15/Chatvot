...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, amount, source, reference, notes, category, vendor, invoiceNumber, paymentMethod, description } = body;
    ...
    if (action === 'add_balance') {
      ...
      console.log(`💰 Admin Balance Added: €${amount} from ${source} by ${admin.email}`);
      ...
      const transaction = await PlatformTransaction.create({
        ...
        sourceType: 'manual',
        description: description || `Custom expense: ${category}${vendor ? ` - ${vendor}` : ''}`,
        notes,
        expenseDetails: {
          category,
          vendor,
      ...
      console.log(`💸 Custom Expense Recorded: €${amount} for ${category} by ${admin.email}`);
      ...
      return NextResponse.json(
        { success: false, error: 'Invalid action. Use "add_balance" or "add_expense"' },
        { status: 400 }
      );
    }
  } catch (error) {
