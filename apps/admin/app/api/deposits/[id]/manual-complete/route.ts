...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const failedDeposit =
      await WalletTransaction.findById(transactionId).session(session);
    ...
    const eurAmount =
      failedDeposit.metadata?.eurAmount ||
    ...
    await WalletTransaction.findByIdAndUpdate(
      transactionId,
      {
        $set: {
          ...
          "metadata.manualResolutionId":
            manualCreditTransaction[0]._id.toString(),
