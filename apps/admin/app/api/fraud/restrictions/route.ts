...
export async function GET(_request: NextRequest) {
  try {
    ...
    console.log(
      "   Active:",
      restrictions.filter((r: any) => r.isActive).length,
    );
    console.log(
      "   Banned:",
      restrictions.filter(
        (r: any) => r.restrictionType === "banned" && r.isActive,
      ).length,
    );
    console.log(
      "   Suspended:",
      restrictions.filter(
        (r: any) => r.restrictionType === "suspended" && r.isActive,
      ).length,
    );

    // Get unique user IDs
    const userIds = [
      ...new Set(restrictions.map((r: any) => r.userId.toString())),
    ];
