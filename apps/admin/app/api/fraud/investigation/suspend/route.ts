  ...
  try {
    ...
    if (!alertId || !userIds || !Array.isArray(userIds) || !suspendUntil) {
      return NextResponse.json(
        {
          success: false,
          error: "Alert ID, user IDs, and suspension duration required",
        },
        { status: 400 },
      );
    }
    ...
    // user-facing /account/review page, so we clamp + trim even though the
    // client already does the same. Mirrors the per-user restriction API.
    const parsedEta =
    ...
    const restrictionPromises = userIds.map((userId) =>
      UserRestriction.create({
        ...
        customReason:
          customReason ||
          `Account suspended until ${new Date(suspendUntil).toLocaleString()}`,
        canTrade:
        ...
        // defaulted it to "allowed", so a suspended account could still accept
        // paid 1v1 challenges.
        canEnterChallenges:
      ...
      await FraudHistoryService.logSuspension(
        ...
        `Account suspended until ${new Date(suspendUntil).toLocaleString()}. Reason: ${customReason || reason}`,
        {
          ...
          durationDays: Math.ceil(
            (new Date(suspendUntil).getTime() - Date.now()) /
              (1000 * 60 * 60 * 24),
          ),
        },
        adminInfo,
    ...
    console.log(
      `✅ Suspended ${userIds.length} accounts until ${suspendUntil} (Alert: ${alertId})`,
    );
    ...
    try {
      ...
      console.log(
        `🔔 Sent suspension notifications to ${userIds.length} users`,
      );
    } catch (notifError) {
