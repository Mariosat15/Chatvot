...
export async function POST(request: NextRequest) {
  try {
    ...
    let restrictions;
    if (restrictionId) {
      restrictions = await UserRestriction.find({
        _id: restrictionId,
        isActive: true,
      });
    } else {
    ...
    if (relatedAlertIds.size > 0) {
      ...
      console.log(
        `📝 Marked ${updateResult.modifiedCount} fraud alert(s) as cleared`,
      );
      console.log(`   Alert IDs: ${Array.from(relatedAlertIds).join(", ")}`);
      console.log(`   Cleared at: ${clearanceTimestamp.toISOString()}`);
      console.log(
        `   → Future fraud activity by these users will generate NEW alerts`,
      );
    }
    ...
    try {
      ...
      for (const lifted of liftedRestrictions) {
        await notificationService.notifyAccountRestored(
          lifted.userId.toString(),
        );
      }
      console.log(
        `🔔 Sent account restored notifications to ${liftedRestrictions.length} users`,
      );
    } catch (notifError) {
      console.error(
        "Error sending account restored notifications:",
        notifError,
      );
    }
