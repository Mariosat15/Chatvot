...
export async function POST(request: Request) {
  try {
    ...
    if (rawUserId) {
      user = await db.collection('user').findOne({ id: rawUserId }, { projection: USER_PROJECTION });
      ...
      if (!user) {
        user = await db.collection('user').findOne(
          { _id: rawUserId } as Record<string, unknown>,
          { projection: USER_PROJECTION }
        );
      }
    }
    ...
    const [
      ...
      FraudAlert.find({
        $or: [
          ...
          ...(resolvedEmail ? [{ 'evidence.data.email': resolvedEmail }, { 'evidence.data.userEmail': resolvedEmail }] : [])
        ]
      }).sort({ detectedAt: -1 }).limit(MAX_ITEMS).lean(),
      DeviceFingerprint.find({ userId: resolvedUserId }).sort({ lastSeen: -1 }).limit(MAX_ITEMS).lean(),
      UserRestriction.find({ userId: resolvedUserId }).sort({ createdAt: -1 }).limit(MAX_ITEMS).lean(),
      AccountLockout.find({
        $or: [
          { userId: resolvedUserId },
          ...(resolvedEmail ? [{ email: resolvedEmail }] : [])
        ]
      }).sort({ lockedAt: -1 }).limit(MAX_ITEMS).lean(),
      userObjectId
        ? PaymentFingerprint.find({ userId: userObjectId }).sort({ lastUsed: -1 }).limit(MAX_ITEMS).lean()
        : Promise.resolve([]),
      FraudHistory.find({
        $or: [
          { userId: resolvedUserId },
      ...
      }).sort({ createdAt: -1 }).limit(MAX_ITEMS).lean()
    ]);

    const summary = {
      alertsTotal: alerts.length,
      alertsPending: alerts.filter((alert) => alert.status === 'pending').length,
      alertsInvestigating: alerts.filter((alert) => alert.status === 'investigating').length,
      devicesTotal: devices.length,
      devicesHighRisk: devices.filter((device) => device.riskScore >= 70).length,
      restrictionsActive: restrictions.filter((restriction) => restriction.isActive).length,
