...
export async function POST(request: NextRequest) {
  try {
    ...
    const adminAuth = { isAuthenticated: true as const, adminId: guard.admin.id, email: guard.admin.email };
    ...
    const paymentFingerprintCountBefore =
      await PaymentFingerprint.countDocuments();
    const behavioralSimilarityCountBefore =
      await BehavioralSimilarity.countDocuments();
    const tradingProfileCountBefore =
      await TradingBehaviorProfile.countDocuments();
    ...
    console.log(
      `   - Behavioral Similarities: ${behavioralSimilarityCountBefore}`,
    );
    ...
    console.log(
      `✅ Device fingerprints deleted: ${deviceDeleteResult.deletedCount}`,
    );

    // DELETE all user restrictions (unban/unsuspend all users)
    const restrictionDeleteResult = await UserRestriction.deleteMany({});
    console.log(
      `✅ User restrictions deleted: ${restrictionDeleteResult.deletedCount}`,
    );

    // DELETE all suspicion scores (Device Match, Mirror Trading, Payment, etc.)
    const suspicionScoreDeleteResult = await SuspicionScore.deleteMany({});
    console.log(
      `✅ Suspicion scores deleted: ${suspicionScoreDeleteResult.deletedCount}`,
    );
    ...
    console.log(
      `✅ Payment fingerprints deleted: ${paymentFingerprintDeleteResult.deletedCount}`,
    );

    // DELETE all behavioral similarity data
    const behavioralSimilarityDeleteResult =
      await BehavioralSimilarity.deleteMany({});
    console.log(
      `✅ Behavioral similarities deleted: ${behavioralSimilarityDeleteResult.deletedCount}`,
    );
    ...
    console.log(
      `✅ Trading profiles deleted: ${tradingProfileDeleteResult.deletedCount}`,
    );
    ...
    const allCleared =
      alertCountAfter === 0 &&
      deviceCountAfter === 0 &&
      restrictionCountAfter === 0 &&
      suspicionScoreCountAfter === 0;
    ...
    if (!allCleared) {
      ...
      console.log(
        `   All users are now unrestricted and can access the platform`,
      );
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        behavioralSimilaritiesDeleted:
          behavioralSimilarityDeleteResult.deletedCount,
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          error instanceof Error ? error.message : "Failed to reset alerts",
      },
      { status: 500 },
    );
  }
