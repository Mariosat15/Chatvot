    ...
    const stats = await FraudHistory.aggregate([
      { $match: { $or: [{ userId: userObjectId }, { userId: userId }] } },
      {
        $group: {
          ...
          suspensionLiftCount: {
            $sum: {
              $cond: [{ $eq: ["$actionType", "suspension_lifted"] }, 1, 0],
            },
          },
          banCount: {
          ...
          investigationCount: {
            $sum: {
              $cond: [{ $eq: ["$actionType", "investigation_started"] }, 1, 0],
            },
          },
          criticalIncidents: {
...
function determineCurrentStatus(stats: Record<string, number>): string {
  ...
  const netSuspensions =
    (stats.suspensionCount || 0) - (stats.suspensionLiftCount || 0);
