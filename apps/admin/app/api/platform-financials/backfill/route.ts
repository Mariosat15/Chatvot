  ...
  try {
    ...
    for (const deposit of completedDeposits) {
      ...
      const platformFeeAmount =
        (eurAmount * platformDepositFeePercentage) / 100;
    ...
    for (const withdrawal of completedWithdrawals) {
      ...
      const eurAmount =
        withdrawal.metadata?.eurAmount || Math.abs(withdrawal.amount);

      const platformWithdrawalFeePercentage =
        feeSettings.platformWithdrawalFeePercentage || 0;
      const platformFeeAmount =
        (eurAmount * platformWithdrawalFeePercentage) / 100;

      const bankWithdrawalFeePercentage =
        feeSettings.bankWithdrawalFeePercentage || 0.25;
      const bankWithdrawalFeeFixed = feeSettings.bankWithdrawalFeeFixed || 0.25;
      const bankFeeTotal =
        (eurAmount * bankWithdrawalFeePercentage) / 100 +
      ...
      if (platformFeeAmount > 0) {
        try {
          ...
          errors.push(
            `Failed to backfill withdrawal ${withdrawalId}: ${error}`,
          );
        }
      }
    }
    ...
    console.log(
      `💰 Backfill complete: ${backfilledCount} deposits, ${withdrawalBackfilledCount} withdrawals`,
    );
