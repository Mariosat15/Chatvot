...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (isNaN(settings.platformFeePercentage))
    ...
    if (isNaN(settings.defaultStartingCapital))
      settings.defaultStartingCapital = 10000;
    if (isNaN(settings.minStartingCapital)) settings.minStartingCapital = 1000;
    if (isNaN(settings.maxStartingCapital))
      settings.maxStartingCapital = 100000;
    ...
    if (isNaN(settings.defaultDurationMinutes))
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to update challenge settings",
      },
      { status: 500 },
    );
  }
