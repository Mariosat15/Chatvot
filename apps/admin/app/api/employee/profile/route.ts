...
export async function GET(request: NextRequest) {
  try {
    ...
    const employee = await Admin.findById(auth.adminId)
      .select("-password -forceLogoutAt -tempPasswordExpiresAt")
      .lean();
  ...
  try {
    ...
    if (name !== undefined || avatar !== undefined) {
      try {
        ...
        console.error(
          "Error syncing employee profile to messaging:",
          syncError,
        );
    ...
    console.log(
      `✅ Employee profile updated: ${auth.email} - Fields: ${changedFields.join(", ")}`,
    );
