...
export interface IFraudSettings extends Document {
  ...
  // a real player out with no admin lever; see the main app's
  // `entry-fraud-gate.service.ts` section 4. Only `autoSuspendThreshold`, with
  // `autoSuspendEnabled` on, causes an automatic block, and that one creates a
  // liftable restriction.
...
const FraudSettingsSchema = new Schema<IFraudSettings>(
  {
    ...
    deviceFingerprintBlockThreshold: {
      type: Number,
      default: 70,
      min: 0,
      max: 100,
    ...
    duplicateKYCSuspendMessage: {
      type: String,
      default:
        "Your account has been suspended due to a security concern with your identity verification. Please contact support for assistance.",
    },
    ...
    registrationChallengeProvider: {
      type: String,
      default: "none",
      enum: ["none", "recaptcha", "turnstile", "hcaptcha"],
    },
...
const FraudSettings =
  models.FraudSettings ||
  model<IFraudSettings>("FraudSettings", FraudSettingsSchema);
