...
// Methods
PaymentFingerprintSchema.methods.addLinkedUser = function (
  linkedUserId: mongoose.Types.ObjectId,
): void {
  // Check if already linked
  const exists = this.linkedUserIds.some(
    (id: mongoose.Types.ObjectId) => id.toString() === linkedUserId.toString(),
  );
...
PaymentFingerprintSchema.statics.findByFingerprint = function (
  fingerprint: string,
  provider: string,
) {
  return this.find({
...
const PaymentFingerprint =
  models.PaymentFingerprint ||
  model<IPaymentFingerprint>("PaymentFingerprint", PaymentFingerprintSchema);
