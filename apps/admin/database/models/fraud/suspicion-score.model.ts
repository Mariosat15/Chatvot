...
SuspicionScoreSchema.methods.addPoints =
  SuspicionScoreSchema.methods.addPercentage;
...
SuspicionScoreSchema.methods.addLinkedAccount = function (
  ...
  const exists = this.linkedAccounts.some(
    (acc: ILinkedAccount) => acc.userId.toString() === linkedUserId.toString(),
  );
...
SuspicionScoreSchema.methods.resetScore = function (): void {
  ...
  Object.keys(this.scoreBreakdown).forEach((key) => {
    const breakdown =
      this.scoreBreakdown[key as keyof ISuspicionScore["scoreBreakdown"]];
    if (breakdown) {
...
const SuspicionScore =
  models.SuspicionScore ||
  model<ISuspicionScore>("SuspicionScore", SuspicionScoreSchema);
