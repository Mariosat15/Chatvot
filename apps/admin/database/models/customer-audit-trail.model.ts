...
CustomerAuditTrailSchema.index({
  customerId: 1,
  actionCategory: 1,
  timestamp: -1,
});
...
export function getActionDescription(
  ...
  const descriptions: Record<AuditActionType, string | ((m: any) => string)> = {
    ...
    customer_transferred: (m) =>
      `Customer transferred from ${m?.fromEmployee?.name} to ${m?.toEmployee?.name}`,
    ...
    email_changed: (m) =>
      `Email changed from ${m?.previousValue} to ${m?.newValue}`,

    // Financial
    deposit_processed: (m) =>
      `Deposit of ${m?.currency || "€"}${m?.amount?.toFixed(2) || "0.00"} processed`,
    withdrawal_requested: (m) =>
      `Withdrawal of ${m?.currency || "€"}${m?.amount?.toFixed(2) || "0.00"} requested`,
    withdrawal_approved: (m) =>
      `Withdrawal of ${m?.currency || "€"}${m?.amount?.toFixed(2) || "0.00"} approved`,
    withdrawal_rejected: (m) =>
      `Withdrawal rejected: ${m?.reason || "No reason provided"}`,
    withdrawal_completed: (m) =>
      `Withdrawal of ${m?.currency || "€"}${m?.amount?.toFixed(2) || "0.00"} completed`,
    withdrawal_failed: (m) =>
      `Withdrawal failed: ${m?.reason || "Unknown error"}`,
    refund_issued: (m) =>
      `Refund of ${m?.currency || "€"}${m?.amount?.toFixed(2) || "0.00"} issued`,
    credits_adjusted: (m) =>
      `Credits adjusted by ${m?.amount > 0 ? "+" : ""}${m?.amount}`,
    ...
    kyc_rejected: (m) =>
      `KYC verification rejected: ${m?.reason || "No reason provided"}`,
    kyc_reset: "KYC status reset",
    kyc_documents_uploaded: "KYC documents uploaded",
    kyc_documents_reviewed: "KYC documents reviewed",

    // Fraud
    fraud_alert_created: (m) =>
      `Fraud alert created: ${m?.alertType || "Unknown type"}`,
    fraud_alert_investigated: "Fraud alert under investigation",
    fraud_alert_dismissed: (m) =>
      `Fraud alert dismissed: ${m?.reason || "No reason provided"}`,
    fraud_alert_escalated: "Fraud alert escalated",

    // Trading
    position_closed_by_admin: (m) =>
      `Position ${m?.positionId} closed by admin`,
    ...
    user_suspended: (m) =>
      `User suspended: ${m?.reason || "No reason provided"}`,
    user_unsuspended: "User unsuspended",
    user_restricted: (m) =>
      `User restricted: ${m?.reason || "No reason provided"}`,
    ...
    xp_adjusted: (m) =>
      `XP adjusted by ${m?.amount > 0 ? "+" : ""}${m?.amount}`,
    level_changed: (m) =>
      `Level changed from ${m?.previousValue} to ${m?.newValue}`,
...
export const CustomerAuditTrail =
  mongoose.models.CustomerAuditTrail ||
  mongoose.model<ICustomerAuditTrail>(
    "CustomerAuditTrail",
    CustomerAuditTrailSchema,
  );
