...
const UserRestrictionSchema = new Schema<IUserRestriction>(
  {
    ...
    // Reason: default true so pre-existing restrictions never accidentally
    // block challenges. Only an explicit `false` blocks.
    // Reason: default flipped true → false on 2 Sep 2026, mirroring the main app.
    // It was the only one of these five flags defaulting to "allowed", and 10 of
    // the 11 writers that create a restriction never set it - so every suspended
    // and banned account could still accept paid 1v1 challenges. A challenge is
    // the easier shape to abuse, being exactly two players with the pot returning
    // to the pair minus the fee, so this was the wrong flag to fail open.
