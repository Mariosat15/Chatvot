 ...
 */
export function calculateCompetitionDifficulty(
  ...
  const hours =
    params.durationHours || (params.duration ? params.duration / 60 : 24);
  if (hours <= 1) {
    score += 10;
  ...
  if (params.rules) {
    if (params.rules.minimumTrades && params.rules.minimumTrades >= 10) {
      score += 8;
      factors.push({
        factor: `${params.rules.minimumTrades}+ trades required`,
        impact: "high",
        score: 8,
      });
      ...
      score += 2;
      factors.push({
        factor: `${params.rules.minimumTrades} trade required`,
        impact: "low",
        score: 2,
      });
    }
    ...
    if (params.rules.minimumWinRate) {
      score += 6;
      factors.push({
        factor: `${params.rules.minimumWinRate}% win rate`,
        impact: "medium",
        score: 6,
      });
    }
  ...
  if (params.riskLimits?.enabled) {
    if (
      params.riskLimits.maxDrawdownPercent &&
      params.riskLimits.maxDrawdownPercent <= 30
    ) {
      score += 5;
    ...
    if (
      params.riskLimits.dailyLossLimitPercent &&
      params.riskLimits.dailyLossLimitPercent <= 10
    ) {
      score += 3;
  ...
  if (params.levelRequirement?.enabled && params.levelRequirement.minLevel) {
    if (params.levelRequirement.minLevel >= 8) {
      score += 10;
      factors.push({
        factor: `Level ${params.levelRequirement.minLevel}+ only`,
        impact: "high",
        score: 10,
      });
      ...
      score += 6;
      factors.push({
        factor: `Level ${params.levelRequirement.minLevel}+ required`,
        impact: "medium",
        score: 6,
      });
