...
// Participant stats interface - includes all possible ranking metrics
export interface ParticipantStats {
  oderId?: string;
  pnl: number; // Realized P&L (closed trades only)
  pnlPercentage: number; // ROI based on realized P&L
  ...
  startingCapital?: number; // For calculating true ROI
  // Add more stats as needed for future ranking methods
  [key: string]: any; // Allow custom stats for extensibility
}
 ...
 */
export function getTotalPnL(stats: ParticipantStats): number {
...
// Configuration for each ranking method
export interface RankingConfig {
  getValue: (stats: ParticipantStats) => number;
  label: string;
...
const RANKING_REGISTRY: Record<string, RankingConfig> = {
  pnl: {
 ...
 */
export function getRankingLabel(method: RankingMethod): string {
  const config = getRankingConfig(method);
  return config.label;
}
 ...
 */
export function getMyStatLabel(method: RankingMethod): string {
  const config = getRankingConfig(method);
 ...
 */
export function getAvailableRankingMethods(): string[] {
