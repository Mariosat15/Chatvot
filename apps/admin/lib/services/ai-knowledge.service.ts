 ...
 */
export function splitTextByHeadings(text: string, maxTokens: number = 500): TextChunk[] {
  ...
  for (const line of lines) {
    ...
    if (headingMatch) {
      ...
      if (potentialTokens > maxTokens && currentChunk.trim()) {
        ...
        const contextPrefix = currentHeadingPath.length > 0 
...
export class AIKnowledgeService {
  ...
  async getSettings() {
    ...
    if (!settings) {
      settings = await AIKnowledgeSettings.create({
        ...
        categories: ['General', 'Trading', 'Competitions', 'Challenges', 'Wallet', 'Technical'],
      });
    }
   ...
   */
  async getSources(filters?: { type?: SourceType; status?: string; isActive?: boolean }) {
    await connectToDatabase();
    const query: any = {};
  ...
  async processSource(sourceId: string, content: string) {
    ...
    try {
      ...
      console.log(`[AI Knowledge] Processing source ${sourceId}: ${chunks.length} chunks`);
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const newChunk = await AIKnowledgeChunk.create({
          ...
          metadata: {
            sectionTitle: chunk.headingPath[chunk.headingPath.length - 1] || source.name,
          },
          isActive: true,
        });
      ...
      console.log(`[AI Knowledge] Source ${sourceId} processed: ${createdChunks.length} chunks, ${totalTokens} tokens`);
      
      return { chunks: createdChunks.length, tokens: totalTokens };
      
    } catch (error) {
      console.error(`[AI Knowledge] Error processing source ${sourceId}:`, error);
  ...
  async search(query: string, options?: {
    ...
    if (options?.category) {
      ...
      if (matchQuery.sourceId) {
        matchQuery.sourceId = { $in: sources.map(s => s._id).filter(id => 
