...
import { publishProviderContest } from "./provider-contest-publish.service";
...
export interface CreateProviderContestInput {
  ...
  createdBy: string;
  /**
   * When a Game Master creates the contest (23 Sep 2026). Optional so the admin wizard
   * stays unchanged - absent means an operator-created contest, same as before.
   */
  gameMasterId?: string;
...
export async function createProviderContest(
  ...
  try {
    const competition = await Competition.create({
      ...
      // Reason: only set when a Game Master creates the contest. Leaving them absent on the
      // admin path keeps every existing operator-created document indistinguishable from
      // before this field was threaded through.
      ...(input.gameMasterId
        ? {
...
/**
 * Game Master path: create a draft then publish it in one shot so the contest lands
 * `upcoming` like a trading GM contest (23 Sep 2026).
 *
 * Admin operators keep the draft → explicit Publish control. A Game Master has no admin
 * publish UI, and creating a permanently invisible draft would look like a successful
 * create that never appears on the platform.
 *
 * If publish fails after create, the draft is left in place and the error is returned -
 * better a recoverable draft than deleting a money-adjacent document from a route.
 */
export async function createAndPublishProviderContest(
  ...
  if (!published.success) {
    return {
      ...
      warnings: [
        ...(created.warnings ?? []),
        "The contest was saved as a draft but could not be published.",
      ],
