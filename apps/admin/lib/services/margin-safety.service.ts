...
export function calculateMaxSafeTradeAmount(
  ...
  const projectedMarginLevel =
    projectedUsedMargin > 0 ? (equity / projectedUsedMargin) * 100 : Infinity;

  const reason =
    marginLimitedAmount < availableCapital ? "margin_limit" : "capital_limit";
...
export function getMarginSafetyMessage(
  ...
  return {
    status: "safe",
    message: mode === "game" ? "✅ Safe to trade!" : "✅ Margin level healthy",
    canTrade: true,
  };
}
