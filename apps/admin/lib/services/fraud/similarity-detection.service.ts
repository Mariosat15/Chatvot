import TradingBehaviorProfile, { ITradingBehaviorProfile } from '@/database/models/fraud/trading-behavior-profile.model';
import BehavioralSimilarity, { IBehavioralSimilarity, ISimilarityBreakdown } from '@/database/models/fraud/behavioral-similarity.model';
...
export class SimilarityDetectionService {
   ...
   */
  private static calculateOverallScore(breakdown: ISimilarityBreakdown): number {
    // Weighted average
    const weights = {
      pairSimilarity: 0.15,
   ...
   */
  static async detectHighSimilarity(threshold = 0.7): Promise<IBehavioralSimilarity[]> {
    await connectToDatabase();
