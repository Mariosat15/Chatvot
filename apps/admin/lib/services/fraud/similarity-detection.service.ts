...
import BehavioralSimilarity, {
  IBehavioralSimilarity,
  ISimilarityBreakdown,
} from "@/database/models/fraud/behavioral-similarity.model";
...
export class SimilarityDetectionService {
  ...
  private static readonly HIGH_SIMILARITY_THRESHOLD = 0.7;
  ...
  static async calculateSimilarity(
    ...
    console.log(
      `🔍 Calculating similarity between ${userId1.substring(0, 8)}... and ${userId2.substring(0, 8)}...`,
    );
    ...
    if (
      overallScore >= this.HIGH_SIMILARITY_THRESHOLD &&
      !similarity.flaggedForReview
    ) {
      similarity.flaggedForReview = true;
      console.log(
        `🚨 HIGH SIMILARITY DETECTED: ${overallScore.toFixed(2)} between ${userId1} and ${userId2}`,
      );
        ...
        evidence: [
          {
            ...
            data: {
              ...
              timingSimilarity:
                (breakdown.timingSimilarity * 100).toFixed(1) + "%",
    ...
    console.log(
      `✅ Similarity calculated: ${(overallScore * 100).toFixed(1)}%`,
    );
   ...
   */
  static compareTradingPatterns(
    profile1: ITradingBehaviorProfile,
    profile2: ITradingBehaviorProfile,
  ): ISimilarityBreakdown {
    const p1 = profile1.patterns;
    const p2 = profile2.patterns;

    // Pair similarity (Jaccard)
    const pairSimilarity = this.jaccardSimilarity(
      p1.preferredPairs || [],
      p2.preferredPairs || [],
    );

    // Timing similarity (cosine of hour distribution)
    const timingSimilarity = this.cosineSimilarity(
      p1.tradingHoursDistribution || Array(24).fill(0),
      p2.tradingHoursDistribution || Array(24).fill(0),
    );

    // Size similarity (normalized difference)
    const sizeSimilarity = this.normalizedDifference(
      p1.avgTradeSize || 0,
      p2.avgTradeSize || 0,
      10, // max difference of 10 lots
    );

    // Duration similarity
    const durationSimilarity = this.normalizedDifference(
      p1.avgTradeDuration || 0,
      p2.avgTradeDuration || 0,
      480, // max difference of 8 hours
    );

    // Risk management similarity
    const slSimilarity = this.normalizedDifference(
      p1.avgStopLoss || 0,
      p2.avgStopLoss || 0,
      50, // max difference of 50 pips
    );
    const tpSimilarity = this.normalizedDifference(
      p1.avgTakeProfit || 0,
      p2.avgTakeProfit || 0,
      100, // max difference of 100 pips
    );
    ...
    const styleScore =
      1 -
      Math.sqrt(
        Math.pow((p1.scalperScore || 0) - (p2.scalperScore || 0), 2) +
          Math.pow((p1.dayTraderScore || 0) - (p2.dayTraderScore || 0), 2) +
          Math.pow((p1.swingScore || 0) - (p2.swingScore || 0), 2),
      ) /
        Math.sqrt(3);

    // Behavioral fingerprint distance
    const fingerprintDistance = this.cosineSimilarity(
      profile1.behavioralFingerprint || Array(32).fill(0),
      profile2.behavioralFingerprint || Array(32).fill(0),
    );
  ...
  private static calculateOverallScore(
    breakdown: ISimilarityBreakdown,
  ): number {
    // Weighted average
    const weights = {
      pairSimilarity: 0.15,
   ...
   */
  static async detectHighSimilarity(
    threshold = 0.7,
  ): Promise<IBehavioralSimilarity[]> {
    await connectToDatabase();

    return BehavioralSimilarity.find({
   ...
   */
  static async runFullAnalysis(): Promise<{
    ...
    console.log(
      `✅ Analysis complete: ${pairsCompared} pairs compared, ${highSimilarityPairs} high similarity pairs found`,
    );
   ...
   */
  static async getSimilarityMatrix(userIds: string[]): Promise<number[][]> {
    await connectToDatabase();
    ...
    for (let i = 0; i < userIds.length; i++) {
      ...
      for (let j = 0; j < userIds.length; j++) {
        if (i === j) {
          ...
          const similarity = await BehavioralSimilarity.findOne({
            userId1: sortedId1,
            userId2: sortedId2,
          });
