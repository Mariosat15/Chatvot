...
export class BehavioralAnalysisService {
  ...
  static async getOrCreateProfile(
    userId: string,
  ): Promise<ITradingBehaviorProfile> {
   ...
   */
  static async updateProfileOnTrade(
    userId: string,
    trade: TradeData,
  ): Promise<void> {
    await connectToDatabase();
    ...
    console.log(
      `📊 Updating trading profile for user ${userId} - Trade: ${trade.pair} ${trade.direction}`,
    );
    ...
    console.log(
      `✅ Profile updated. Total trades analyzed: ${profile.totalTradesAnalyzed}`,
    );
  }
   ...
   */
  private static async recalculatePatterns(
    profile: ITradingBehaviorProfile,
  ): Promise<void> {
    ...
    patterns.tradingHoursDistribution = hourDistribution.map(
      (h) => h / Math.max(totalHours, 1),
    );

    // Calculate averages
    const closedTrades = trades.filter((t) => t.closeTime);

    patterns.avgTradeSize =
      trades.reduce((sum, t) => sum + t.lotSize, 0) / trades.length;

    patterns.avgTradeDuration =
    ...
    patterns.avgStopLoss =
      tradesWithSL.length > 0
        ? tradesWithSL.reduce((sum, t) => sum + (t.stopLoss || 0), 0) /
          tradesWithSL.length
        : 0;

    patterns.avgTakeProfit =
      tradesWithTP.length > 0
        ? tradesWithTP.reduce((sum, t) => sum + (t.takeProfit || 0), 0) /
          tradesWithTP.length
        : 0;
    ...
    const tradesPerDay =
      ...
      Math.max(
        ...
        (new Date(trades[trades.length - 1]?.openTime || Date.now()).getTime() -
          new Date(trades[0]?.openTime || Date.now()).getTime()) /
          (1000 * 60 * 60 * 24),
      );
  ...
  private static calculateSwingScore(trades: IRecentTrade[]): number {
    ...
    const longDurationTrades = trades.filter(
      (t) => (t.duration || 0) > 60 * 24,
    ); // > 1 day
   ...
   */
  private static async generateBehavioralFingerprint(
    profile: ITradingBehaviorProfile,
  ): Promise<void> {
  ...
  static async getAllProfiles(): Promise<ITradingBehaviorProfile[]> {
    await connectToDatabase();
    return TradingBehaviorProfile.find({
