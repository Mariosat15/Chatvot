import PaymentFingerprint, {
  IPaymentFingerprint,
} from "@/database/models/fraud/payment-fingerprint.model";
...
export class PaymentFraudService {
  ...
  static async trackPaymentFingerprint(paymentData: PaymentData): Promise<{
    ...
    console.log(
      `💳 Tracking payment fingerprint for user ${paymentData.userId}`,
    );
    console.log(
      `   Provider: ${paymentData.paymentProvider}, Fingerprint: ${paymentData.paymentFingerprint.substring(0, 12)}...`,
    );
    ...
    if (!userFingerprint) {
      ...
      console.log(
        `✅ Created new payment fingerprint for user ${paymentData.userId}`,
      );
    } else {
      ...
      console.log(
        `📊 Updated payment fingerprint for user ${paymentData.userId} (used ${userFingerprint.timesUsed} times)`,
      );
    }

    // FRAUD DETECTION: Check if payment method is shared
    const linkedUserIds = otherUserFingerprints.map((fp) =>
      fp.userId.toString(),
    );
    ...
    if (isShared) {
      console.log(
        `🚨 SHARED PAYMENT DETECTED! ${linkedUserIds.length + 1} accounts using same payment method`,
      );
      ...
      for (const fingerprint of [userFingerprint, ...otherUserFingerprints]) {
        const otherUsers = allUserIds.filter(
          (id) => id !== fingerprint.userId.toString(),
        );
  ...
  private static async updateFraudScoresForSharedPayment(
    ...
    console.log(
      `📊 Updating fraud scores for ${userIds.length} users with shared payment`,
    );
  ...
  private static async createPaymentFraudAlert(
    ...
    console.log(
      `🚨 Processing payment fraud alert for ${userIds.length} accounts`,
    );
  ...
  static async getPaymentFraudStats(): Promise<{
    ...
    const sharedPaymentMethods = await PaymentFingerprint.countDocuments({
      isShared: true,
    });
    ...
    sharedPayments.forEach((payment) => {
      ...
      payment.linkedUserIds.forEach((id: { toString: () => string }) =>
        affectedUserIds.add(id.toString()),
      );
    });
   ...
   */
  static async getUserPaymentFingerprints(
    userId: string,
  ): Promise<IPaymentFingerprint[]> {
    await connectToDatabase();

    return PaymentFingerprint.find({ userId }).sort({ lastUsed: -1 });
  }
