...
function getMetricValue(
  ...
  switch (rankingMethod) {
    ...
    case "profit_factor":
      const totalWins =
        participant.winningTrades > 0
          ? participant.averageWin * participant.winningTrades
          : 0;
      const totalLosses =
        participant.losingTrades > 0
          ? Math.abs(participant.averageLoss * participant.losingTrades)
          : 0;
...
export function calculateWinProbability(
  ...
  const eligibleParticipants = allParticipants.filter(
    (p) => p.status === "active" && p.totalTrades >= competition.minimumTrades,
  );
  ...
  const currentRank =
    sortedParticipants.findIndex((p) => p.userId === userParticipant.userId) +
    1;
  ...
  const topCompetitorMetric =
    sortedParticipants.length > 0
      ? getMetricValue(sortedParticipants[0], competition.rankingMethod)
      : 0;
  ...
  if (currentRank > 0) {
    if (!isInWinningPosition && currentRank <= sortedParticipants.length) {
      ...
      const winningPositionMetric = getMetricValue(
        sortedParticipants[lastWinningRank - 1],
    ...
    if (isInWinningPosition && currentRank < sortedParticipants.length) {
      ...
      const nextLoserMetric = sortedParticipants[lastWinningRank]
        ? getMetricValue(
            sortedParticipants[lastWinningRank],
            competition.rankingMethod,
          )
        : userMetric;
  ...
  if (!meetsMinimumTrades) {
    ...
    if (distanceFromLosing !== null && Math.abs(userMetric) > 0) {
      ...
      const relativeDistance =
        Math.abs(distanceFromLosing) / Math.abs(userMetric);
    ...
    probabilityScore = Math.min(
      100,
      baseScore + securityBonus + firstPlaceBonus,
    );
