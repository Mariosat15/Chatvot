...
export default function FraudHistorySection() {
  ...
  // Entry detail dialog
  const [selectedEntry, setSelectedEntry] = useState<FraudHistoryEntry | null>(
    null,
  );
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      if (performedByFilter !== "all")
        params.append("performedByType", performedByFilter);
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
            ...
            <Select
              ...
              onValueChange={setPerformedByFilter}
            >
              <SelectTrigger className="bg-zinc-800 border-zinc-700">
            ...
            <div className="space-y-3">
              {history.map((entry) => {
                ...
                const severityConfig =
                  SEVERITY_CONFIG[entry.actionSeverity] ||
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge
                              className={`${severityConfig.color} text-white text-xs`}
                            >
                          ...
                          <div className="text-xs text-zinc-500">
                            {entry.performedBy.type === "admin" &&
                              entry.performedBy.adminEmail && (
                                <span>
                                  By: {entry.performedBy.adminEmail} •{" "}
                                </span>
                              )}
        ...
        <DialogContent className="max-w-2xl bg-zinc-900 border-zinc-800">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {selectedEntry &&
                ACTION_TYPE_CONFIG[selectedEntry.actionType]?.icon}
              {selectedEntry &&
                ACTION_TYPE_CONFIG[selectedEntry.actionType]?.label}
            </DialogTitle>
              ...
              <div className="space-y-4 p-1">
                ...
                <div className="bg-zinc-800 rounded-lg p-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={
                        selectedEntry.performedBy.type === "admin"
                          ? "default"
                          : "secondary"
                      }
                    >
                        ...
                        <>
                          {selectedEntry.duration.startDate && (
                            <p className="text-white">
                              Start:{" "}
                              {formatDate(selectedEntry.duration.startDate)}
                            </p>
                          )}
                          {selectedEntry.duration.endDate && (
                            <p className="text-white">
                              End: {formatDate(selectedEntry.duration.endDate)}
                            </p>
                          )}
                          {selectedEntry.duration.durationDays && (
                            <p className="text-zinc-400">
                              {selectedEntry.duration.durationDays} days
                            </p>
                          )}
                        </>
                      )}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {selectedEntry.previousState && (
                        <div>
                          ...
                          <p className="text-white">
                            {selectedEntry.previousState.accountStatus || "N/A"}
                          </p>
                        </div>
                      )}
                      {selectedEntry.newState && (
                        <div>
                          ...
                          <p className="text-white">
                            {selectedEntry.newState.accountStatus || "N/A"}
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bg-zinc-800 rounded-lg p-4">
                      ...
                      <div className="space-y-2">
                        {selectedEntry.evidence.map((ev, idx) => (
                          <div
                            key={idx}
                            className="bg-zinc-700 rounded p-2 text-sm"
                          >
                ...
                <div className="space-y-6 p-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <Card className="bg-gray-900 border-gray-700">
                      <CardContent className="p-4 text-center">
                        <div className="text-2xl font-bold text-white">
                          {userHistory.stats.totalActions ||
                    ...
                    <Card className="bg-gray-900 border-gray-700">
                      <CardContent className="p-4 text-center">
                        <div className="text-2xl font-bold text-orange-500">
                          {userHistory.stats.suspensionCount ||
                            userHistory.actionCounts.suspended ||
                            0}
                        </div>
                        <div className="text-xs text-zinc-400">Suspensions</div>
                      </CardContent>
                    ...
                    <Card className="bg-gray-900 border-gray-700">
                      <CardContent className="p-4 text-center">
                        <div className="text-2xl font-bold text-red-500">
                          {userHistory.stats.banCount ||
                            userHistory.actionCounts.banned ||
                            0}
                        </div>
                        <div className="text-xs text-zinc-400">Bans</div>
                      </CardContent>
                        ...
                        <Badge
                          className={getStatusBadge(
                            ...
                            userHistory.stats.currentStatus ||
                              ((userHistory.actionCounts.banned || 0) >
                              (userHistory.actionCounts.ban_lifted || 0)
                                ? "banned"
                                : (userHistory.actionCounts.suspended || 0) >
                                    (userHistory.actionCounts
                                      .suspension_lifted || 0)
                                  ? "suspended"
                                  : (userHistory.actionCounts
                                        .investigation_started || 0) > 0
                                    ? "under_investigation"
                                    : (userHistory.actionCounts
                                          .warning_issued || 0) > 0
                                      ? "warned"
                        ...
                        >
                          {(
                            ...
                            (userHistory.actionCounts.ban_lifted || 0)
                              ? "banned"
                              : (userHistory.actionCounts.suspended || 0) >
                                  (userHistory.actionCounts.suspension_lifted ||
                                    0)
                                ? "suspended"
                                : (userHistory.actionCounts
                                      .investigation_started || 0) > 0
                                  ? "under_investigation"
                                  : (userHistory.actionCounts.warning_issued ||
                                        0) > 0
                                    ? "warned"
                                    : "clean")
                          )
                  ...
                  <Card className="bg-gray-900 border-gray-700">
                    ...
                    <CardContent>
                      {(() => {
                        ...
                        const calcScore =
                          userHistory.stats.behaviorScore ||
                          (userHistory.actionCounts.warning_issued || 0) * 10 +
                            (userHistory.actionCounts.suspended || 0) * 25 +
                            (userHistory.actionCounts.banned || 0) * 50 +
                            (userHistory.actionCounts.investigation_started ||
                              0) *
                              5 -
                            (userHistory.actionCounts.suspension_lifted || 0) *
                              5 -
                        ...
                        return (
                          <>
                            ...
                            <p className="text-sm text-zinc-400 mt-2">
                              {score > 70
                                ? "High risk - Multiple serious violations"
                                : score > 40
                                  ? "Medium risk - Has violations on record"
                                  : score > 20
                        ...
                        {Object.entries(userHistory.actionCounts).map(
                          ([type, count]) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className={`p-1 rounded ${config?.color || "bg-gray-500"}`}
                                >
                                  {config?.icon || (
                        ...
                        {userHistory.history.slice(0, 10).map((entry, idx) => {
                          ...
                          return (
                            ...
                            >
                              <div className="relative">
                                <div
                                  className={`p-2 rounded-full ${config?.color || "bg-gray-500"}`}
                                >
                                  {config?.icon || (
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2">
                                  <span className="font-medium text-white">
                                    {config?.label || entry.actionType}
                                  </span>
                                  <Badge
                                    className={`${SEVERITY_CONFIG[entry.actionSeverity]?.color || "bg-gray-500"} text-white text-xs`}
                                  >
