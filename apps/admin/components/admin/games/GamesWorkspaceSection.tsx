...
/**
 * Centralised Games admin workspace (X11 Slice 2, revised).
 *
 * ONE SCREEN FOR EVERY TITLE SETTING that used to live in the wide catalogue table and its
 * five per-row dialogs. The provider list stays reachable through the Providers segment —
 * registration, credentials and the three nested switches are not duplicated here.
 *
 * Reuses the existing per-title routes and the same dialog forms in `inline` mode. No new
 * money path, no new model, no `GameCatalogueEntry` — merchandising stays on `provider_game`
 * until a second title needs an independent page over one provider row.
 */
...
export default function GamesWorkspaceSection() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex min-h-[70vh] overflow-hidden rounded-xl border border-[#1e3a5f]/60 bg-[#020817]">
        {/* Secondary game list */}
        <aside className="flex w-72 shrink-0 flex-col border-r border-[#1e3a5f]/60 bg-[#07152c]/80">
          <div className="border-b border-[#1e3a5f]/50 p-3">
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="h-9 border-[#1e3a5f]/60 bg-[#020817] pl-8 text-sm text-white placeholder:text-white/35"
              />
            </div>
          </div>
                      ...
                      <button
                        ...
                        className={`flex w-full items-center gap-2.5 rounded-lg border px-2.5 py-2 text-left transition ${
                          active
                            ...
                            : "border-transparent hover:border-[#1e3a5f]/50 hover:bg-white/5"
                        }`}
                      >
          ...
          <div className="border-t border-[#1e3a5f]/50 p-3">
            {/*
              Titles arrive from a provider catalogue sync — there is no create-title API.
              The control switches to Providers and names the real next step rather than
              greying out a dead "Add" that teaches an operator the platform is broken.
            */}
            <Button
        ...
        {/* Main editor */}
        <main className="flex min-w-0 flex-1 flex-col bg-[#020817]">
          {!selected ? (
            ...
            <>
              <header className="border-b border-[#1e3a5f]/50 px-5 py-4">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {selected.tagline && (
                        <p className="mt-0.5 text-sm text-cyan-300/80">{selected.tagline}</p>
                      )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      className="border-[#1e3a5f] text-white/80"
                      asChild
                    >
                      <a
                        href={`/games/${selected.gameCode}`}
                        target="_blank"
                        rel="noreferrer"
                      >
              ...
              >
                <div className="overflow-x-auto border-b border-[#1e3a5f]/50 px-3">
...
function ViewSwitch({
  ...
  return (
    <div className="inline-flex rounded-lg border border-[#1e3a5f]/60 bg-[#07152c] p-0.5">
      <button
        type="button"
  ...
  if (url) {
    return (
      ...
      <img
        src={url}
        alt=""
        className={`${dim} shrink-0 rounded-lg border border-[#1e3a5f]/50 object-cover`}
      />
    );
  }
  return (
    <div
      className={`${dim} flex shrink-0 items-center justify-center rounded-lg border border-[#1e3a5f]/50 bg-[#07152c] text-xs font-semibold text-violet-300`}
    >
