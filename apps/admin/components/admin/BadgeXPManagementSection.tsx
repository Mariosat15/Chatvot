...
export default function BadgeXPManagementSection() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // Fetch XP config
        const xpRes = await fetch("/api/badges-xp/manage");
  ...
  const triggerBadgeEvaluation = async (userId?: string) => {
    try {
      setTriggering(true);
      const response = await fetch("/api/trigger-badge-evaluation", {
        method: "POST",
  ...
  const viewUserBadges = async (userId: string) => {
    try {
      const response = await fetch(`/api/badges-xp?userId=${userId}`);
      const data = await response.json();
  ...
  return (
    <div className="space-y-6 max-w-[1920px] mx-auto">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Manually trigger badge evaluation for all users or wait for
          ...
          <TabsContent value="levels" className="space-y-6 mt-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
              {levels.map((level) => (
                <div
                  key={level.level}
                  className="bg-background border-2 rounded-xl p-6 hover:border-primary/50 transition-all"
                >
                  <div className="flex items-center justify-between mb-4">
                  ...
                  {editingLevel ? (
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const newLevels = [...levels];
                          newLevels[level.level - 1].description =
                            e.target.value;
                          setLevels(newLevels);
                        }}
                      />
          ...
          <TabsContent value="xp" className="space-y-6 mt-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Configure how much XP each badge rarity awards
                </p>
              </div>
              {editingBadgeXP ? (
                <div className="flex gap-2">
                  ...
                  <Button
                    onClick={() => setEditingBadgeXP(false)}
                    variant="outline"
                    size="sm"
                  >
                    <X className="h-4 w-4 mr-2" />
                ...
                <Button
                  onClick={() => setEditingBadgeXP(true)}
                  variant="outline"
                  size="sm"
                >
                  <Edit className="h-4 w-4 mr-2" />
                ...
                {editingBadgeXP ? (
                  <Input
                    ...
                    onChange={(e) =>
                      setXpValues({
                        ...xpValues,
                        common: parseInt(e.target.value) || 0,
                      })
                    }
                ...
                {editingBadgeXP ? (
                  <Input
                    ...
                    onChange={(e) =>
                      setXpValues({
                        ...xpValues,
                        rare: parseInt(e.target.value) || 0,
                      })
                    }
                    className="text-2xl font-bold"
                  />
                ...
                {editingBadgeXP ? (
                  <Input
                    ...
                    onChange={(e) =>
                      setXpValues({
                        ...xpValues,
                        epic: parseInt(e.target.value) || 0,
                      })
                    }
                    className="text-2xl font-bold"
                  />
              ...
              {BADGES.filter(
                (badge) =>
                  ...
                  (badgeSearchTerm === "" ||
                    badge.name
                      .toLowerCase()
                      .includes(badgeSearchTerm.toLowerCase()) ||
                    badge.description
                      .toLowerCase()
                      .includes(badgeSearchTerm.toLowerCase()) ||
                    badge.id
          ...
          <Table>
            <TableHeader>
              <TableRow className="text-base">
                ...
                <TableHead className="text-base font-bold min-w-[250px]">
                  User
                </TableHead>
                <TableHead className="text-base font-bold min-w-[200px]">
                  Level & Title
                </TableHead>
                <TableHead className="text-base font-bold">XP</TableHead>
                <TableHead className="text-base font-bold min-w-[180px]">
                  Progress
                </TableHead>
                ...
                return (
                  <TableRow key={user.userId} className="h-20">
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className="text-base px-3 py-2 font-bold"
                      >
      ...
      <Dialog
        open={(!!editingBadge || managingBadges) && !isClosing}
        onOpenChange={(open) => {
          if (!open && !isClosing) {
            setIsClosing(true);
            setTimeout(() => {
      ...
      >
        <DialogContent
          ...
          className="max-w-none w-screen h-screen p-0 m-0 rounded-none border-0 left-0 top-0 translate-x-0 translate-y-0 sm:max-w-none sm:rounded-none data-[state=open]:slide-in-from-left-0 data-[state=open]:slide-in-from-top-0"
        >
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-3">
                  <Label className="text-xl font-semibold">
                    Badge ID {!editingBadge && "(Optional - Auto-generated)"}
                  </Label>
                  <Input
                    value={badgeForm.id}
                    onChange={(e) =>
                      setBadgeForm({ ...badgeForm, id: e.target.value })
                    }
                    placeholder={
                      editingBadge
                        ? badgeForm.id
                ...
                <div className="space-y-3">
                  ...
                  <Select
                    value={badgeForm.category || "Competition"}
                    onValueChange={(value) =>
                      setBadgeForm({ ...badgeForm, category: value })
                    }
                  >
                ...
                <div className="space-y-3">
                  ...
                  <Select
                    value={badgeForm.rarity || "common"}
                    onValueChange={(value) =>
                      setBadgeForm({ ...badgeForm, rarity: value })
                    }
                  >
                    <SelectTrigger className="text-lg h-12">
                      <SelectValue />
                  ...
                  <div>
                    <DialogTitle className="text-4xl font-bold">
                      {(() => {
                        ...
                        if (user?.email)
                          return `${user.email.split("@")[0]}'s Profile`;
                        return `User Profile`;
                      })()}
                    </DialogTitle>
                    <DialogDescription className="text-xl mt-1">
                ...
                <div className="bg-gradient-to-r from-primary/10 to-purple-500/10 rounded-xl p-12 border-2 border-primary/20">
                  <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="flex items-center gap-4 mb-2">
                          ...
                          <div>
                            <p className="text-4xl font-bold">
                              {selectedUser.level.currentTitle}
                            </p>
                            <p className="text-xl text-muted-foreground">
                              Level {selectedUser.level.currentLevel}
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="text-right">
                      <p className="text-5xl font-bold text-primary mb-2">
                        {selectedUser.level.currentXP.toLocaleString()}
                      </p>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-sm text-muted-foreground">
                      <span>
                        Current: {selectedUser.level.currentXP.toLocaleString()}{" "}
                        XP
                      </span>
                      <span>
                        Next:{" "}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {selectedUser.badges.map((badge) => (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-3 pt-3 border-t border-border">
                          <span className="text-sm text-muted-foreground font-semibold">
                            {badge.badgeDetails?.category}
                          </span>
                          <Badge variant="secondary" className="font-bold">
                            <Star className="h-3 w-3 mr-1" />+
                            {
                              BADGE_XP_VALUES[
                                badge.badgeDetails?.rarity || "common"
                              ]
                            }{" "}
                            XP
                          </Badge>
