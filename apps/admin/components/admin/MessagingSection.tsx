...
function useAdminWebSocket(onMessage: (msg: any) => void) {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      const wsUrl =
        process.env.NEXT_PUBLIC_WEBSOCKET_URL || `${wsProtocol}//${host}/ws`;
      const adminId =
        document.cookie
          .split("; ")
      ...
      const ws = new WebSocket(
        `${wsUrl}?token=${encodeURIComponent(adminId)}&type=employee`,
      );
      ...
      ws.onclose = (event) => {
        ...
        if (reconnectCountRef.current < 5) {
          ...
          const delay = Math.min(
            3000 * Math.pow(2, reconnectCountRef.current - 1),
            60000,
          );
  ...
  const setTyping = useCallback((conversationId: string, isTyping: boolean) => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(
        JSON.stringify({ type: "typing", conversationId, isTyping }),
      );
    }
  }, []);
...
export default function MessagingSection() {
  ...
  const [selectedEmployeeForTransfer, setSelectedEmployeeForTransfer] =
    useState<string>("");
  const [transferAllConversations, setTransferAllConversations] =
    useState(false);
  ...
  useEffect(() => {
    const adminIdCookie = document.cookie
      .split("; ")
      .find((c) => c.startsWith("admin_id="))
      ?.split("=")[1];
    ...
    (message: { type: string; data?: any; [key: string]: any }) => {
      switch (message.type) {
        case "new_message":
          ...
          const isCurrentConv =
            msgData?.conversationId === selectedConvRef.current;
                ...
                if (c.id === transferData.conversationId) {
                  return {
                    ...
                    chatTransferredFromName:
                      transferData.chatTransferredFromName,
  ...
  const startConversationWithCustomer = async (
    ...
    try {
      console.log(
        `💬 [UI] Starting conversation with customer: ${customerName} (${customerId})`,
      );
      ...
      const convResponse = await fetch(
        `/api/messaging/conversations/${data.conversation.id}`,
      );
      if (!convResponse.ok) {
        console.error(
      ...
      console.log(
        `💬 [UI] Got conversation details, messages:`,
      ...
      const newConversation = {
        ...
        participants:
          convData.participants || data.conversation.participants || [],
      ...
      setConversations((prev) => {
        ...
        if (exists) {
          return prev.map((c) =>
            c.id === data.conversation.id ? newConversation : c,
          );
        }
  ...
  const fetchConversations = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/messaging/conversations?type=${type}&status=${statusFilter}`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const handleResolve = async (conversationId: string) => {
    if (
      !confirm(
        "Mark this conversation as resolved? AI will handle the next customer message.",
      )
    )
      return;
    ...
    try {
      const response = await fetch(
        `/api/messaging/conversations/${conversationId}/resolve`,
        { method: "POST" },
      );
      if (response.ok) {
        await fetchConversations();
  ...
  useEffect(() => {
    ...
    fetchAssignedCustomers().catch((e) =>
      console.error("Error fetching customers:", e),
    );
    fetchAvailability().catch((e) =>
  ...
  const handleTransfer = async () => {
    ...
    const employee = employees.find(
      (e) => e.id === selectedEmployeeForTransfer,
    );
    if (!employee) return;

    try {
      const response = await fetch(
        `/api/messaging/conversations/${selectedConversation.id}/transfer`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (response.ok) {
        ...
        if (transferAllConversations && result.transferredCount > 1) {
          alert(
            `Successfully transferred ${result.transferredCount} conversations to ${employee.name}`,
          );
        }
      } else {
        const error = await response.json();
  ...
  const handleTransferBack = async (conversationId: string) => {
    const notes = prompt(
      "Optional notes about resolution (leave empty to skip):",
    );

    try {
      const response = await fetch(
        `/api/messaging/conversations/${conversationId}/transfer-back`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    try {
      ...
      if (response.ok) {
        ...
        const conv: Conversation = {
          ...
          lastActivityAt:
            data.conversation.lastActivityAt || new Date().toISOString(),
        };
  ...
  const unassignedCount = conversations.filter(
    (c) => !c.assignedEmployeeId && c.type === "user-to-support",
  ).length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Manage customer support and internal communications
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${wsConnected ? "bg-emerald-500/10 border-emerald-500/30" : "bg-red-500/10 border-red-500/30"}`}
          >
            {wsConnected ? (
              <>
          ...
          >
            <CircleDot
              className={`w-4 h-4 ${isAvailable ? "text-emerald-400" : "text-amber-400"}`}
            />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={stat.label}
            className="bg-[#0d0d14] border border-white/5 rounded-2xl p-4"
          >
                ...
                {assignedCustomers.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="text-gray-600 text-sm mt-1">
                      Customers will appear here when assigned
                    </p>
                  </div>
                ) : (
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {filteredConversations
                        .filter((c) => c.type === "employee-internal")
                        .map((conv) => {
                            ...
                            >
                              <div className="relative">
                                <div className="w-11 h-11 bg-gradient-to-br from-violet-500 to-purple-600 rounded-xl flex items-center justify-center">
                                  <span className="text-white font-bold">
                                    {otherParticipant?.name?.charAt(0) || "E"}
                                  </span>
                                </div>
                      ...
                      {employees.map((employee) => (
                        ...
                        >
                          <div className="relative">
                            ...
                            <div
                              className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#0d0d14] ${
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0 text-left">
                              <div className="flex items-center justify-between mb-0.5">
                                ...
                                <span
                                  className="text-[10px] text-gray-500 ml-2 flex-shrink-0"
          ...
          {selectedConversation ? (
            <>
                    ...
                    <>
                      ...
                      {!selectedConversation.isAIHandled &&
                        (selectedConversation.isResolved ? (
                          <button
                            onClick={() =>
                              handleReopen(selectedConversation.id)
                            }
                          ...
                          <button
                            onClick={() =>
                              handleResolve(selectedConversation.id)
                            }
              ...
              <div
                ...
                style={{
                  background:
                    "radial-gradient(ellipse at top, rgba(16,185,129,0.02) 0%, transparent 50%)",
                }}
              >
                {messages.length === 0 ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <div
                            className={`flex items-center gap-1.5 mt-1.5 text-[11px] ${isOwnMessage ? "justify-end text-white/50" : "text-gray-500"}`}
                          >
                            <span>
              ...
              <div className="p-4 border-t border-white/5 bg-[#0d0d14]">
                  ...
                  <div className="bg-gray-500/10 border border-gray-500/30 rounded-xl p-4 text-center">
                    ...
                    <p className="text-gray-500 text-xs mt-1">
                      ...
                      {selectedConversation.archivedAt &&
                        ` on ${format(new Date(selectedConversation.archivedAt), "MMM d, yyyy")}`}
                    </p>
                    <button
                ...
                ) : selectedConversation.isChatTransferred &&
                  selectedConversation.chatTransferredFrom ===
                    currentAdminId ? (
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 text-center">
                    <p className="text-amber-400 text-sm">
                      ...
                      <span className="font-semibold">
                        {selectedConversation.chatTransferredToName}
                      </span>
                    </p>
                    <p className="text-amber-400/70 text-xs mt-1">
                      You will be notified when the chat is transferred back to
                      you
                    </p>
                  </div>
                ) : selectedConversation.isChatTransferred &&
                  selectedConversation.chatTransferredTo === currentAdminId ? (
                  ...
                  <div className="space-y-3">
                    <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-xl px-4 py-2 flex items-center justify-between">
                      <p className="text-cyan-400 text-sm">
                        ...
                        <span className="font-semibold">
                          {selectedConversation.chatTransferredFromName}
                        </span>
                      </p>
                      <button
                        onClick={() =>
                          handleTransferBack(selectedConversation.id)
                        }
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              <p className="text-gray-500 text-center max-w-md">
                ...
                respond to customers or team members.
              </p>
            </div>
          )}
        </div>
                ...
                {selectedConversation?.type === "user-to-support" && (
                  <label className="flex items-center gap-3 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl cursor-pointer hover:bg-amber-500/15 transition-colors">
                    <input
                      ...
                      onChange={(e) =>
                        setTransferAllConversations(e.target.checked)
                      }
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-400/70 mt-0.5">
                        Transfer all tickets with this customer (including
                        archived)
                      </p>
                    </div>
                  </label>
