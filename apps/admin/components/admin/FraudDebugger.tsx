...
export default function FraudDebugger() {
  ...
  const fetchDebugData = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/debug-fraud");
      if (response.ok) {
          ...
          <Card className="bg-gray-800 border-gray-700">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                <div
                  className={`p-4 rounded-lg ${debugData.settings.enabled.deviceFingerprinting ? "bg-green-500/10 border border-green-500/30" : "bg-red-500/10 border border-red-500/30"}`}
                >
                ...
                <div
                  className={`p-4 rounded-lg ${debugData.settings.enabled.multiAccountDetection ? "bg-green-500/10 border border-green-500/30" : "bg-red-500/10 border border-red-500/30"}`}
                >
                  <div className="flex items-center gap-2 mb-1">
                ...
                <div
                  className={`p-4 rounded-lg ${debugData.settings.enabled.vpnDetection ? "bg-green-500/10 border border-green-500/30" : "bg-red-500/10 border border-red-500/30"}`}
                >
                  <div className="flex items-center gap-2 mb-1">
                        ...
                        <div className="flex-1">
                          ...
                          {fp.linkedAccountsCount > 0 && (
                            <div className="text-xs text-yellow-400 mt-1">
                              🚨 Linked to {fp.linkedAccountsCount} other
                              account(s)
                            </div>
                          )}
                        </div>
            ...
            <Card className="bg-red-500/10 border-red-500/30">
              <CardHeader>
                <CardTitle className="text-red-400">
                  🚨 Diagnosis: Fingerprinting Not Working
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 text-gray-300">
                ...
                <ol className="list-decimal list-inside space-y-2">
                  <li>
                    ...
                    <code>/api/fraud/track-device</code>) is not being called
                    during signup
                  </li>
                  <li>JavaScript errors preventing fingerprint generation</li>
                  <li>
                    Users are signing up but fingerprint tracking fails silently
                  </li>
                ...
                <div className="bg-blue-500/10 border border-blue-500/30 rounded p-3 mt-4">
                  ...
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    ...
                    <li>
                      Check if <code>POST /api/fraud/track-device</code> is
                      called
                    </li>
                    <li>
                      Verify the request completes successfully (200 status)
                    </li>
                  </ol>
                </div>
              </CardContent>
