...
export default function UpcomingPaymentsWidget({ onNavigate, daysAhead = 30 }: UpcomingPaymentsWidgetProps) {
  ...
  const fetchPayments = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/vendors?upcoming=${daysAhead}&isActive=true`);
  ...
  const getStatusBadge = (daysUntil: number) => {
    if (daysUntil < 0) {
      return <Badge className="bg-red-500/20 text-red-400 text-xs">Overdue</Badge>;
    } else if (daysUntil === 0) {
      return <Badge className="bg-orange-500/20 text-orange-400 text-xs">Today</Badge>;
    } else if (daysUntil <= 7) {
      return <Badge className="bg-yellow-500/20 text-yellow-400 text-xs">{daysUntil}d</Badge>;
    } else {
      return <Badge className="bg-gray-500/20 text-gray-400 text-xs">{daysUntil}d</Badge>;
    }
  };

  const overdueCount = payments.filter(p => getDaysUntilPayment(p.nextPaymentDate) < 0).length;
                ...
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className="flex flex-col min-w-0">
                      <span className="text-white text-sm font-medium truncate">{payment.name}</span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {payment.vendorUrl && (
                      <Button
                        ...
                        onClick={() => window.open(payment.vendorUrl, '_blank')}
                      >
