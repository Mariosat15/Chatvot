...
export default function MarketSettingsSection() {
  ...
  const [originalSettings, setOriginalSettings] =
    useState<MarketSettings | null>(null);
  ...
  const [activeSection, setActiveSection] = useState<
    "schedules" | "holidays" | "blocking"
  >("schedules");
  const [automaticHolidays, setAutomaticHolidays] = useState<
    AutomaticHoliday[]
  >([]);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">
                Configure trading hours, holidays & blocking rules
              </p>
            </div>
          </div>
        ...
        <div className="relative grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
          ...
          <div className="bg-gray-800/50 backdrop-blur rounded-xl p-3 border border-gray-700/50">
            ...
            <p className="text-white font-semibold">
              {
                [
                  settings.blockTradingOnHolidays,
                  settings.blockCompetitionsOnHolidays,
                  settings.blockChallengesOnHolidays,
            ...
            <p className="text-white font-semibold">
              {
                ASSET_CLASSES.filter(
                  (a) =>
                    settings.assetSchedules[
                      a.id as keyof typeof settings.assetSchedules
                    ]?.enabled,
                ).length
              }
              /5
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <p className="text-sm text-gray-400">
            Uses Massive.com API for live market status. Your custom holidays
            will still apply.
          </p>
        ...
        >
          ...
          <p className="text-sm text-gray-400">
            Full control over trading hours. Define custom schedules for each
            day and asset class.
          </p>
        </button>
      </div>
      ...
      {activeSection === "schedules" && (
        <div className="space-y-3">
          {settings.mode === "automatic" && (
            <div className="flex items-center gap-3 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
              ...
              <p className="text-sm text-gray-300">
                ...
                These schedules are used as fallback when API is unavailable.
              </p>
            </div>
          )}
          ...
          {ASSET_CLASSES.map((asset) => {
            const schedule =
              settings.assetSchedules[
                asset.id as keyof typeof settings.assetSchedules
              ];
            ...
            return (
              <Collapsible
                key={asset.id}
                open={isExpanded}
                onOpenChange={() => toggleAssetExpanded(asset.id)}
              >
                <div
                ...
                >
                  <CollapsibleTrigger asChild>
                    <button className="w-full p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-12 h-12 rounded-xl bg-gradient-to-br ${asset.color} flex items-center justify-center text-2xl shadow-lg`}
                        >
                          {asset.icon}
                        </div>
                  ...
                  <CollapsibleContent>
                    <div className="px-4 pb-4 pt-2 border-t border-gray-700/30">
                      <div className="grid grid-cols-7 gap-2">
                        {DAYS.map((day) => {
                          const daySchedule =
                            schedule?.[day] || DEFAULT_DAY_SCHEDULE;
                          const isWeekend =
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      Standard market holidays ({templateHolidays.length}{" "}
                      configured)
                    </p>
                  </div>
                </div>
              ...
              {templateHolidays.length === 0 ? (
                <div className="p-6 bg-gray-800/30 rounded-xl border border-dashed border-emerald-500/30 text-center">
                  ...
                  <p className="text-gray-400 text-sm">
                    No template holidays loaded
                  </p>
                  <p className="text-gray-500 text-xs mt-1">
                    Click "Load Template" to add standard market holidays
                  </p>
                </div>
              ) : (
                      ...
                      >
                        <div className="absolute top-2 right-2 flex gap-1">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              holiday._id && handleDeleteHoliday(holiday._id)
                            }
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0">
                            <h4 className="font-semibold text-white text-sm truncate pr-16">
                              {holiday.name}
                            </h4>
                            ...
                            <div className="flex items-center gap-2 mt-2 flex-wrap">
                              ...
                              {holiday.isRecurring && (
                                <Badge
                                  variant="outline"
                                  className="text-[10px] border-gray-600 px-1.5"
                                >
                                  Yearly
                                </Badge>
                              )}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    Your manually added holidays ({customHolidays.length}{" "}
                    configured)
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm text-orange-400 font-medium mb-1">
                  About Blocking Rules
                </p>
                <p className="text-xs text-gray-400">
                  These rules apply when the market is closed - holidays,
                  weekends, or disabled days. Enable to prevent users from
                  joining competitions/challenges during closures.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-3">
            {[
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 rounded-xl bg-gradient-to-br ${rule.color} flex items-center justify-center shadow-lg ${
                      settings[rule.id as keyof MarketSettings]
                        ? "opacity-100"
                        : "opacity-40"
                    }`}
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setSettings({ ...settings, [rule.id]: checked })
                  }
                />
              </div>
            ))}
