...
bezel and `token-1.webp`..`token-8.webp` are the terminals (one image per numeral; the idle,
select, connect and error states are CSS on `data-state`, not separate files - the owner asked on
25 September 2026 for these original numerals back), but every terminal also draws its own
