...
5.1.2 Bankcard payment page 
Bankcard payment page method is used to display to the user the payment page where he will type his bankcard data 
to start the payment. The method requires the following input parameters: - Payment id (required) of the previously created payment by the method 5.2.1; - Cardholder name (optional) could be passed to the method call, otherwise it will be requested on the 
payment page; - Payer email (optional) could be passed to the method call, otherwise it will be requested on the payment 
page 
HTTP request method 
...
https://api.7995-endpoint-b.com/api/v2/payments/cardform 
...
{  
   ...
   cardholder_name: string,   // cardholder name, string of Latin characters, optional, 
    // if not specified the corresponding input field will be displayed on the confirmation page 
   payer_email: string,   // payer email, optional, 
    // if not specified the corresponding input field will be displayed on the confirmation page 
...
https://api.7995-endpoint-b.com/api/v2/payments?offset=<number of items to skip>&limit=<number of items per 
response>&after_date=<date in ISO8601 format>&payment_ids=<id1>,<id2>… 
 
Query string parameters 
 
<number of items to skip> — number of items to skip at the beginning of the list sorted by payment date in 
descending order (latest to earliest); optional, if not passed, the value is 0, i. e. the returned list will contain items 
starting from the latest payment; 
 
<number of items per response> — number of list items to be returned in the response; optional, if not passed the 
value is 10. that is, the response will contain a list of 10 elements; maximum value is 30; 
 
<date in ISO8601 format> — date and time in ISO8601 format, payments starting from which must be returned in 
the list; optional, if not passed all payments will be returned starting from the latest one;  
 
<id1>,<id2>… — comma-separated list of payment identifiers (payment_id) of the payments to be returned in the 
list, optional, if not passed all payments will be returned; maximum 20 payment identifiers can be passed. 
