...
function autoGenerateSecret(): string | null {
  ...
  try {
    ...
    // IMPORTANT: Set in memory FIRST before file operations
    // This ensures the server can function even if .env persistence fails
    process.env.BETTER_AUTH_SECRET = newSecret;

    // Attempt to persist to .env file (best-effort, not required)
    try {
      ...
      if (fs.existsSync(ROOT_ENV_PATH)) {
        ...
        // Use regex that matches only uncommented lines (lines not starting with #)
        // This prevents matching commented-out secrets like "# BETTER_AUTH_SECRET=old"
        const uncommentedSecretRegex = /^BETTER_AUTH_SECRET=.*/m;

        if (uncommentedSecretRegex.test(content)) {
          // Replace only the uncommented line
          content = content.replace(
            uncommentedSecretRegex,
            `BETTER_AUTH_SECRET=${newSecret}`,
          );
        } else {
          // No uncommented secret found - append a new one
...
function validateAuthSecret(): void {
  ...
  if (!secret || isInsecure) {
    if (isProduction) {
      console.error(
        "❌ CRITICAL: Auth secret is missing or insecure for production!",
      );
      ...
      if (generated) {
        ...
        console.warn(
          "⚠️  Auth secret is weak. Use a secure random secret in production.",
        );
        return;
      }
    }
...
async function startServer() {
  ...
  try {
    ...
    if (!poolStats.isReady) {
      ...
      console.error(
        `   Auth operations require the worker pool. Cannot start server.`,
      );
      ...
      console.warn(
        `⚠️ Worker pool partially ready (${poolStats.totalWorkers}/${poolStats.poolSize} workers)`,
      );
      console.warn(
        `   Server will start but auth performance may be degraded.`,
      );
    }
...
async function shutdown(signal: string) {
  ...
  try {
    ...
    // Close database connection (prevents connection leak)
