...
router.post("/register", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existingUser) {
      ...
      console.log(
        `🔧 Found orphaned user ${email}, will clean up in transaction...`,
      );
    }
    ...
    await session.withTransaction(async () => {
      ...
      if (orphanedUserId) {
        ...
        const accountCreatedDuringHash = await Account.findOne(
          { userId: existingUser!.id },
          { session },
        );
    ...
    try {
      ...
      const transporter = nodemailer.createTransport({
        ...
        port: parseInt(
          process.env.SMTP_PORT || process.env.NODEMAILER_PORT || "587",
        ),
        secure:
          (process.env.SMTP_SECURE || process.env.NODEMAILER_SECURE) === "true",
        auth: {
    ...
    try {
      ...
      try {
        ...
        if (autoAssignResponse.ok) {
          const result = (await autoAssignResponse.json()) as {
            assigned?: boolean;
      ...
      if (
        ...
        autoAssignError.name === "AbortError"
      ) {
        console.warn(
    ...
    if (errorMessage === "CONCURRENT_REGISTRATION") {
      res.status(409).json({
        error: "User already exists",
        message:
          "An account with this email address was just created. Please try logging in.",
      });
      return;
    }
...
router.post("/login", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (user.emailVerified !== true) {
      console.log(
        `⚠️ Login blocked for unverified user: ${email} (emailVerified: ${user.emailVerified})`,
      );
      ...
      try {
        ...
        const transporter = nodemailer.createTransport({
          ...
          secure:
            (process.env.SMTP_SECURE || process.env.NODEMAILER_SECURE) ===
            "true",
          auth: {
            user: process.env.SMTP_USER || process.env.NODEMAILER_EMAIL,
            pass: process.env.SMTP_PASS || process.env.NODEMAILER_PASSWORD,
  ...
  try {
    ...
    const authKey =
      typeof authHeader === "string"
        ...
        : undefined;
    const providedKey = Array.isArray(headerKey)
      ? headerKey[0]
      : headerKey || authKey;
    ...
    if (!isValidKey) {
      ...
      res.status(401).json({
        error: "Unauthorized",
        message:
          "Invalid or missing internal API key. Provide via X-Internal-API-Key header.",
      });
      return;
    }
    ...
    for (const userData of hashedUsers) {
      ...
      try {
        ...
        if (existing) {
          ...
          console.log(
            `🔧 Found orphaned user ${userData.email}, will clean up in transaction...`,
          );
        }
    ...
    const totalDuration = Date.now() - startTime;
    console.log(
      `✅ Batch registration: ${successCount}/${users.length} users in ${totalDuration}ms (bcrypt: ${totalBcryptTime}ms parallel)`,
    );
    ...
    res.status(500).json({
      ...
      message:
        "An unexpected error occurred during batch registration. Please try again.",
    });
  }
});
...
router.get("/benchmark", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({
      ...
      message:
        "An error occurred during the benchmark. The worker pool may be unavailable.",
    });
  }
});
