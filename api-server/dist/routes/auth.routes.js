...
router.post("/register", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (autoAssignError instanceof Error &&
...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        const isValid = await (0, worker_pool_1.comparePassword)(password, account.password);
        ...
        if (user.emailVerified !== true) {
            ...
            try {
                ...
                const transporter = nodemailer.createTransport({
                    host: process.env.SMTP_HOST || process.env.NODEMAILER_HOST,
                    port: parseInt(process.env.SMTP_PORT || process.env.NODEMAILER_PORT || "587"),
                    secure: (process.env.SMTP_SECURE || process.env.NODEMAILER_SECURE) ===
                        "true",
                    auth: {
                        user: process.env.SMTP_USER || process.env.NODEMAILER_EMAIL,
                        pass: process.env.SMTP_PASS || process.env.NODEMAILER_PASSWORD,
...
router.post("/register-batch", async (req, res) => {
    ...
    try {
        ...
        const authHeader = Array.isArray(authHeaderRaw)
            ? authHeaderRaw[0]
            : authHeaderRaw;
        const authKey = typeof authHeader === "string"
        ...
        const isValidKey = providedKey &&
            ...
            crypto_1.default.timingSafeEqual(hashKey(providedKey), hashKey(internalApiKey));
        if (!isValidKey) {
