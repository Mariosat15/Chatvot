      ...
      {drips.map((drip) => (
        ...
        >
          <div
            ...
            style={{
              background:
                "linear-gradient(180deg, #8B0000 0%, #DC143C 50%, #FF0000 100%)",
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to top, rgba(100,100,100,0.4), transparent)",
        }}
      />
      {/* Floating icons */}
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[9995]">
      <motion.div
        ...
        style={{
          background:
            "linear-gradient(45deg, rgba(255,0,0,0.03), rgba(0,255,0,0.03), rgba(0,0,255,0.03))",
...
export default function GlobalThemeEffects({
  themeId,
  effects = {},
}: GlobalThemeEffectsProps) {
  const {
  ...
  const renderThemeEffects = () => {
    ...
    switch (themeId) {
      ...
      case "black-friday":
        return (
          <BlackFridayEffects intensity={confettiIntensity || intensity} />
        );
  ...
  return <AnimatePresence>{renderThemeEffects()}</AnimatePresence>;
}
