...
/**
 * Trading branch of the `/play` dispatcher (chapter 13 s1).
 *
 * Rendered from `/competitions/[id]/play` when the contest is not a provider game.
 * `/trade` is a permanent redirect *inwards* onto `/play` — bookmarks and old CTAs still
 * work. Providers stay out of this tree (R18): the play route only mounts this component
 * after `getPlayState` answers `not_provider_contest`.
 *
 * Moved character-for-character from the old trade page (R19) — no trading behaviour change.
 */
export interface CompetitionTradingWorkspaceProps {
  competitionId: string;
  /** Review charts / history after the contest ends. */
  viewOnly?: boolean;
}

export default async function CompetitionTradingWorkspace({
  competitionId,
  viewOnly = false,
}: CompetitionTradingWorkspaceProps) {
  ...
  /**
   * Belt for a caller that mounts this on a provider contest by mistake.
   *
   * The play dispatcher already refuses that path before rendering us; this keeps the
   * workspace safe if something else imports it later. Redirects to `/play`, which then
   * renders the provider host — no loop, because that path never remounts this component.
   */
