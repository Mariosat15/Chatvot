...
const DrawingCanvas = forwardRef<DrawingCanvasHandle, DrawingCanvasProps>(
  (
      ...
      drawings.forEach((item) => {
          ...
          case "text":
            if (screenPoints.length >= 1 && item.text) {
              ...
              if (isSelected || isEditing) {
                ...
                ctx.strokeRect(
                  screenPoints[0].x - 2,
                  screenPoints[0].y - (item.fontSize || 14),
                  metrics.width + 4,
                  (item.fontSize || 14) + 4,
                );
              }
          ...
          case "fibonacci":
            if (screenPoints.length >= 2) {
              ...
              levels.forEach((level, idx) => {
                ...
                const levelPrice =
                  item.points[0].price +
                  (item.points[1].price - item.points[0].price) * level;
        ...
        if (
          ...
          if (item.type === "rectangle" && screenPoints.length >= 2) {
            ...
            handlePoints = [
              { x: canvas.width / dpr / 2, y: screenPoints[0].y },
            ];
          } else if (item.type === "vertical-line") {
            handlePoints = [
              { x: screenPoints[0].x, y: canvas.height / dpr / 2 },
            ];
          }
          ...
          switch (activeTool) {
            ...
            case "arrow":
              ...
              if (activeTool === "arrow") {
                ...
                ctx.lineTo(
                  drawCurrent.x - 15 * Math.cos(angle - Math.PI / 6),
                  drawCurrent.y - 15 * Math.sin(angle - Math.PI / 6),
                );
                ctx.lineTo(
                  drawCurrent.x - 15 * Math.cos(angle + Math.PI / 6),
                  drawCurrent.y - 15 * Math.sin(angle + Math.PI / 6),
    ...
    const hitTest = useCallback(
      (point: { x: number; y: number }): DrawingItem | null => {
        const threshold = 12;
          ...
          switch (d.type) {
            case "freehand":
              for (let j = 0; j < pts.length - 1; j++) {
                if (distToSegment(point, pts[j], pts[j + 1]) < threshold)
                  return d;
              }
              break;
            case "trend-line":
            ...
            case "arrow":
              if (
                pts.length >= 2 &&
                distToSegment(point, pts[0], pts[1]) < threshold
              )
                return d;
              break;
            case "horizontal-line":
            ...
            case "fibonacci":
              if (pts.length >= 2) {
                ...
                if (point.y >= minY - threshold && point.y <= maxY + threshold)
                  return d;
              }
              break;
          }
      ...
      (
        ...
        if (drawing.type === "rectangle" && pts.length >= 2) {
          ...
          for (let i = 0; i < corners.length; i++) {
            if (
              Math.hypot(point.x - corners[i].x, point.y - corners[i].y) <
              threshold
            ) {
              return i;
            }
      ...
      (e: React.MouseEvent) => {
        ...
        if (canvas) {
          if (editingId) {
            ...
            if (
              editing &&
              hitTestHandle({ x: pt.x, y: pt.y }, editing) !== null
            ) {
      ...
      (e: React.MouseEvent) => {
        ...
        if (editingPointIndex !== null && editingId && drawCurrent) {
          ...
          if (drawing) {
            ...
            onDrawingsChange(
              drawings.map((d) =>
                d.id === editingId ? { ...d, points: newPoints } : d,
              ),
            );
          }
        ...
        if (isDrawing && activeTool && drawStart) {
          ...
          if (activeTool === "freehand" && freehandPath.length > 3) {
            ...
            points = freehandPath.filter(
              (_, i) => i % 2 === 0 || i === freehandPath.length - 1,
            );
          } else if (
            ...
            pt &&
            Math.hypot(pt.x - drawStart.x, pt.y - drawStart.y) > 5
          ) {
            points = [drawStart, pt];
          }
