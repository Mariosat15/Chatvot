...
/**
 * Trading branch of the challenge `/play` dispatcher — sibling of
 * `CompetitionTradingWorkspace`. Mounted from `/challenges/[id]/play` when the challenge is
 * not a provider game. `/trade` permanently redirects inwards.
 */
export interface ChallengeTradingWorkspaceProps {
  challengeId: string;
  viewOnly?: boolean;
}

export default async function ChallengeTradingWorkspace({
  challengeId,
  viewOnly = false,
}: ChallengeTradingWorkspaceProps) {
  ...
  /**
   * Belt for a mistaken mount on a provider challenge. The play dispatcher already routes
   * those to the provider host; redirecting to `/play` cannot loop.
   */
