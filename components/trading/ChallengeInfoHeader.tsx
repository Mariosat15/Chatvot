...
export function ChallengeInfoHeader({
  ...
  const [myStats, setMyStats] = useState<ParticipantStats>({
    ...defaultStats,
    ...initialMyStats,
  });
  const [opponentStats, setOpponentStats] = useState<ParticipantStats>({
    ...defaultStats,
    ...initialOpponentStats,
  });
  ...
  const winningStatus = determineWinningStatus(
    myStats,
    opponentStats,
    rankingMethod,
  );
  ...
  const opponentFormattedValue = formatRankingValue(
