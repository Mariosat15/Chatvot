...
import { ChevronDown, ChevronRight, Search, X, Zap, Loader2, Swords, Crown, Gem, Globe } from 'lucide-react';
      ...
      try {
        ...
        if (res.ok) {
          ...
          const grouped: Record<string, { label: string; icon: JSX.Element; pairs: ForexSymbol[] }> = {
            major: { label: '⚔️ Major Pairs', icon: <Swords className="w-4 h-4" />, pairs: [] },
            cross: { label: '💎 Cross Pairs', icon: <Gem className="w-4 h-4" />, pairs: [] },
            exotic: { label: '🌍 Exotic Pairs', icon: <Globe className="w-4 h-4" />, pairs: [] },
            custom: { label: '✨ Custom', icon: <Crown className="w-4 h-4" />, pairs: [] },
          };
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-white/60 text-[10px]">
                {isLoadingSymbols ? 'Loading...' : `${totalPairs} pairs available`}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoadingSymbols ? (
            ...
            Object.entries(filteredCategories).map(([categoryKey, category]) => {
              ...
              return (
                <div key={categoryKey} className="border-b border-purple-500/10">
                  {/* Category Header */}
                  <button
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-1.5 min-w-0">
                              <span className="text-sm">{getFlag(pair as ForexSymbol)}</span>
                              <span className={cn(
                              ...
                              {isSelected && <Zap className="w-3 h-3 text-yellow-400" />}
                            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {prices.get(selectedSymbol) && (
              <span className="text-purple-300 font-mono font-bold text-sm"
                ...
                {prices.get(selectedSymbol)!.bid.toFixed(selectedSymbol.includes('JPY') ? 3 : 5)}
              </span>
            )}
          </div>
        </div>
