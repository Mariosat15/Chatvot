...
import { 
  ...
  Filter, Search, BellOff, Settings, History, Clock
} from 'lucide-react';
...
const CATEGORY_INFO: Record<string, { label: string; icon: string; color: string }> = {
  ...
  challenge: { label: 'Challenges', icon: '⚔️', color: 'bg-orange-500/20 text-orange-400' },
  ...
  admin: { label: 'Admin', icon: '📢', color: 'bg-amber-500/20 text-amber-400' },
  security: { label: 'Security', icon: '🔐', color: 'bg-red-500/20 text-red-400' },
  social: { label: 'Social', icon: '👥', color: 'bg-cyan-500/20 text-cyan-400' },
  messaging: { label: 'Messaging', icon: '💬', color: 'bg-indigo-500/20 text-indigo-400' },
};
...
export default function NotificationCenter() {
  ...
  }, [fetchNotifications, activeSubTab]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 p-1 bg-gray-800/50 rounded-lg w-fit">
        ...
        >
          ...
          {unreadCount > 0 && (
            <Badge className="bg-yellow-500 text-black text-xs px-1.5 py-0 ml-1">
              {unreadCount}
            </Badge>
          )}
        </button>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {unreadCount > 0 && (
                <Button onClick={handleMarkAllAsRead} variant="outline" size="sm" className="border-gray-700 text-gray-300">
                  <CheckCheck className="h-4 w-4 mr-2" />
                  Mark All Read
                </Button>
              )}
              {notifications.length > 0 && (
                <Button onClick={handleClearAll} variant="outline" size="sm" className="border-red-700 text-red-400 hover:bg-red-950">
