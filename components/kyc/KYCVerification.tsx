...
export default function KYCVerification({
  ...
  const fetchStatus = async (showToast = false) => {
    ...
    try {
      ...
      if (data.enabled !== undefined) {
        ...
        if (showToast) {
          if (
            data.userStatus.status !== oldStatus &&
            data.userStatus.status !== "pending"
          ) {
            ...
            toast.info(
              "Still processing... Please check again in a few minutes.",
            );
          } else {
  ...
  useEffect(() => {
    if (
      ...
      status?.latestSession?.createdAt
    ) {
      ...
      if (data.sessionUrl) {
        ...
        try {
          ...
          if (url.protocol !== "https:" && url.hostname !== "localhost") {
            throw new Error("Invalid session URL protocol");
          }
          ...
          toast.error("Invalid verification URL received");
        }
      } else {
        ...
        {(() => {
          const canRetry =
            ...
            (userStatus.status === "pending" &&
              ...
              isSessionStale(status.latestSession.createdAt));
          ...
          return (
            <Button
              ...
              disabled={
                starting || userStatus.attempts >= status.userStatus.maxAttempts
              }
            >
              {starting ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                ...
                {status.requiredForWithdrawal &&
                  status.requiredForDeposit &&
                  " and "}
      ...
      <CardContent className="space-y-4">
        ...
        >
          <div className="flex items-start gap-3">
            <StatusIcon
              className={`h-5 w-5 ${statusConfig.textColor} mt-0.5`}
            />
            <div>
              <p className={`font-medium ${statusConfig.textColor}`}>
                {userStatus.status === "approved"
                  ? status.messages.approved
                  : userStatus.status === "declined"
                    ? status.messages.declined
                    : userStatus.status === "pending"
              ...
              {userStatus.status === "approved" && userStatus.verifiedAt && (
                <p className="text-sm text-gray-400 mt-1">
                  Verified on{" "}
                  {new Date(userStatus.verifiedAt).toLocaleDateString()}
                  {userStatus.expiresAt &&
                    ` • Expires ${new Date(userStatus.expiresAt).toLocaleDateString()}`}
                </p>
              )}
            </div>
          </div>
        ...
        {userStatus.status === "approved" &&
          status.latestSession?.dataRetentionExpiresAt && (
            <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
              <div className="flex items-start gap-3">
                <Info className="h-5 w-5 text-blue-400 mt-0.5 flex-shrink-0" />
                <div className="space-y-2">
                  <p className="text-sm font-medium text-blue-400">
                    Important: Re-verification Notice
                  </p>
                  <div className="text-sm text-gray-400 space-y-1">
                    <p>
                      ...
                      {userStatus.expiresAt
                        ? new Date(userStatus.expiresAt).toLocaleDateString(
                            "en-US",
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Our verification provider retains your identity verification
                    records for 2 years. After{" "}
                    ...
                    , you will need to re-verify your identity to continue using
                    withdrawal features. We'll notify you before this date.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            {userStatus.attempts > 0 && (
              <div className="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span>
                  {userStatus.attempts} of {userStatus.maxAttempts} verification
                  attempts used
                </span>
              </div>
            )}
            ...
            {(() => {
              ...
              return (
                <Button
                  ...
                  onClick={startVerification}
                  disabled={
                    starting || userStatus.attempts >= userStatus.maxAttempts
                  }
                >
                  {starting ? (
                    <>
              ...
              <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      Please contact support for assistance with identity
                      verification.
                    </p>
                  </div>
                </div>
                ...
                {status.latestSession &&
                  isSessionStale(status.latestSession.createdAt) && (
                    <p className="text-xs text-orange-400 text-center">
                      Your previous verification session has expired. You can
                      retry above.
                    </p>
                  )}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-gray-400">
                            If you cancelled or closed the verification window,
                            you can either wait for the automatic reset
        ...
        {userStatus.status === "none" && (
          <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
            ...
            <ul className="text-sm text-gray-400 space-y-1">
              <li>
                • A valid government-issued ID (passport, ID card, or driver's
                license)
              </li>
