...
// Reason: Extract the correct metric value for a performer based on the competition's ranking method
/* eslint-disable @typescript-eslint/no-explicit-any */
function getPerformerMetricValue(participation: any, method: string): number {
  switch (method) {
    case "pnl":
      return participation.pnl || 0;
    case "roi":
      return participation.pnlPercentage || 0;
    case "total_capital":
      return participation.currentCapital || 0;
    case "win_rate":
      return participation.winRate || 0;
    case "total_wins":
      return participation.winningTrades || 0;
    case "profit_factor": {
      const wins = participation.winningTrades || 0;
      const losses = participation.losingTrades || 0;
...
export default function GlobalRiskMetrics({
  competitions,
  overallStats,
}: GlobalRiskMetricsProps) {
  if (competitions.length === 0) return null;

  // Calculate global risk metrics
  const totalExposure = competitions.reduce(
    (sum, comp) => sum + comp.participation.usedMargin,
    0,
  );

  const totalAvailableCapital = competitions.reduce(
    (sum, comp) => sum + comp.participation.availableCapital,
    0,
  );

  const globalMarginUsage =
    overallStats.totalCapital > 0
      ? (totalExposure / overallStats.totalCapital) * 100
      : 0;

  const totalUnrealizedPnL = competitions.reduce(
    (sum, comp) => sum + (comp.participation.unrealizedPnl || 0),
    0,
  );

  const totalRealizedPnL = competitions.reduce(
    (sum, comp) => sum + comp.participation.realizedPnl,
    0,
  );

  // Risk score calculation (0-100, higher = more risk)
  const avgCapitalHealth =
    competitions.reduce(
      (sum, comp) =>
        sum +
  ...
  const criticalCompetitions = competitions.filter(
    (comp) =>
      (comp.participation.currentCapital / comp.participation.startingCapital) *
        100 <
        60 ||
      (comp.participation.usedMargin / comp.participation.currentCapital) *
        100 >
        70,
  ).length;
  ...
  const riskScore =
    100 -
    avgCapitalHealth +
    globalMarginUsage * 0.5 +
    criticalCompetitions * 10;
  const finalRiskScore = Math.max(0, Math.min(100, riskScore));

  // Reason: Sort by rank position (lower rank = better) since each competition
  // may use a different ranking method (pnl, roi, win_rate, etc.)
  const sortedByRank = [...competitions].sort(
    (a, b) =>
      (a.participation.currentRank || 9999) -
      (b.participation.currentRank || 9999),
  );
  const bestPerformer = sortedByRank[0];
  const worstPerformer = sortedByRank[sortedByRank.length - 1];
  ...
  return (
    <div className="bg-gradient-to-br from-gray-800 via-gray-800 to-gray-900 border border-gray-700 rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              Aggregate metrics across all {competitions.length} competition
      ...
      <div className="p-6 border-b border-gray-700">
        <h4 className="text-sm font-bold text-gray-100 mb-4">
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className={`p-4 rounded-xl border ${riskBg} ${riskBorder}`}>
          ...
          <p className={`text-2xl font-bold ${riskColor} mb-1`}>{riskLevel}</p>
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span
              className={`font-semibold ${
                globalMarginUsage > 70
                  ? "text-red-500"
                  : globalMarginUsage > 50
                    ? "text-orange-500"
                    : "text-green-500"
              }`}
            >
      ...
      <div className="p-6 border-t border-gray-700 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {(() => {
          const method: RankingMethod =
            bestPerformer.competition?.rules?.rankingMethod || "pnl";
          const config = getRankingConfig(method);
          const metricValue = getPerformerMetricValue(
            bestPerformer.participation,
            method,
          );
          return (
        ...
        {(() => {
          const method: RankingMethod =
            worstPerformer.competition?.rules?.rankingMethod || "pnl";
          const config = getRankingConfig(method);
          const metricValue = getPerformerMetricValue(
            worstPerformer.participation,
            method,
          );
          return (
