      ...
      <section className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-3 md:gap-4">
        ...
        <div className="col-span-2 bg-gradient-to-br from-emerald-500/20 to-emerald-600/10 border border-emerald-500/30 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-5">
          ...
          <p className="text-xs sm:text-sm text-gray-400">
            Across {data.overview.activeContests} active contest
            {data.overview.activeContests !== 1 ? "s" : ""}
          </p>
        </div>
        ...
        >
          ...
          <p
            className={`text-xl sm:text-2xl md:text-3xl font-black mb-0.5 sm:mb-1 tabular-nums ${data.overview.totalPnL >= 0 ? "text-green-400" : "text-red-400"}`}
          >
            {data.overview.totalPnL >= 0 ? "+" : ""}
        ...
        <section className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 border border-gray-700/50 rounded-xl sm:rounded-2xl overflow-hidden">
          <div className="p-3 sm:p-4 md:p-5 border-b border-gray-700/50">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0">
              ...
              <Link
                href="/competitions"
                className="text-xs sm:text-sm text-yellow-400 hover:text-yellow-300 flex items-center gap-1 self-end sm:self-auto"
              >
                All Competitions{" "}
            ...
            <section className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 border border-gray-700/50 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-5">
              ...
              <div className="space-y-1.5 sm:space-y-2">
                {data.charts.tradesBySymbol.slice(0, 5).map((item, i) => (
                  <div
                    key={item.symbol}
                    className="flex items-center justify-between p-1.5 sm:p-2 bg-gray-800/50 rounded-lg"
                  >
                    <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                      <span className="text-[10px] sm:text-xs font-bold text-gray-500 w-3 sm:w-4 flex-shrink-0">
                        {i + 1}
                      </span>
                    ...
                    <div className="text-right flex-shrink-0 ml-2">
                      ...
                      <p
                        className={`text-xs sm:text-sm font-bold tabular-nums ${item.pnl >= 0 ? "text-green-400" : "text-red-400"}`}
                      >
                        {item.pnl >= 0 ? "+" : ""}
        ...
        <div className="space-y-4 sm:space-y-6 order-1 lg:order-2">
          ...
          <section className="bg-gradient-to-br from-yellow-500/10 to-orange-500/10 border border-yellow-500/30 rounded-xl sm:rounded-2xl overflow-hidden">
            <div className="p-3 sm:p-4 border-b border-yellow-500/20 flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] sm:text-xs text-gray-400">
                    {data.competitions.active.length} active
                  </p>
                </div>
              </div>
              <Link
                href="/competitions"
                className="text-[10px] sm:text-xs text-yellow-400 hover:text-yellow-300 flex items-center gap-0.5 sm:gap-1"
              >
            ...
            <div className="p-2 sm:p-3 border-t border-yellow-500/20 grid grid-cols-3 gap-2">
              ...
              <MiniStat
                label="Won"
                value={data.competitions.stats.won}
                color="yellow"
              />
              <MiniStat
                label="Top 3"
                value={data.competitions.stats.topThreeFinishes}
                color="orange"
              />
            </div>
          </section>
          ...
          <section className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border border-purple-500/30 rounded-xl sm:rounded-2xl overflow-hidden">
            <div className="p-3 sm:p-4 border-b border-purple-500/20 flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] sm:text-xs text-gray-400">
                    {data.challenges.active.length} active
                  </p>
                </div>
              </div>
              <Link
                href="/challenges"
                className="text-[10px] sm:text-xs text-purple-400 hover:text-purple-300 flex items-center gap-0.5 sm:gap-1"
              >
                View All <ChevronRight className="h-3 w-3" />
              </Link>
            </div>
            ...
            <div className="p-2 sm:p-3 border-t border-purple-500/20 grid grid-cols-3 gap-2">
              ...
              <MiniStat
                label="Wins"
                value={data.challenges.stats.wins}
                color="green"
              />
              <MiniStat
                label="Win Rate"
                value={`${data.challenges.stats.winRate.toFixed(0)}%`}
                color="purple"
              />
            </div>
          </section>
              ...
              <div className="p-3 sm:p-4 border-b border-gray-700/50 flex items-center justify-between">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] sm:text-xs text-gray-400">
                      {data.recentActivity.positions.length} active
                    </p>
                  </div>
                </div>
              </div>
      ...
      <section className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 border border-gray-700/50 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6">
        <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs text-gray-400">
              Your current performance streaks
            </p>
          </div>
        </div>
          ...
          <GameStreakCard
            ...
            value={
              data.competitions.stats.bestRank > 0
                ? `#${data.competitions.stats.bestRank}`
                : "-"
            }
            subtitle="All time"
            color="yellow"
...
function MiniStat({
  ...
  return (
    <div className="text-center">
      <p
        className={`text-sm sm:text-base md:text-lg font-bold tabular-nums ${color === "yellow" ? "text-yellow-400" : color === "orange" ? "text-orange-400" : color === "green" ? "text-green-400" : color === "purple" ? "text-purple-400" : "text-white"}`}
      >
        {value}
      </p>
...
function GameStreakCard({
  ...
  return (
    ...
    >
      ...
      <p className="text-[9px] sm:text-[10px] md:text-xs text-white font-medium truncate">
        {label}
      </p>
      <p className="text-[8px] sm:text-[9px] md:text-xs text-gray-500 mt-0.5 sm:mt-1 truncate">
        {subtitle}
      </p>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between text-[10px] sm:text-xs">
        ...
        <span
          className={`font-bold tabular-nums ${competition.pnl >= 0 ? "text-green-400" : "text-red-400"}`}
        >
...
function TradeCard({ trade }: { trade: any }) {
  return (
    <div className="flex items-center justify-between p-1.5 sm:p-2 bg-gray-800/50 rounded-lg">
      ...
      <div className="text-right flex-shrink-0 ml-2">
        <p
          className={`text-xs sm:text-sm font-bold tabular-nums ${trade.pnl >= 0 ? "text-green-400" : "text-red-400"}`}
        >
...
function PositionCard({ position }: { position: any }) {
  return (
    <div className="flex items-center justify-between p-1.5 sm:p-2 bg-gray-800/50 rounded-lg">
      ...
      <div className="text-right flex-shrink-0 ml-2">
        <p
          className={`text-xs sm:text-sm font-bold tabular-nums ${position.unrealizedPnL >= 0 ? "text-green-400" : "text-red-400"}`}
        >
  ...
  return (
    <div className="text-center py-4 sm:py-6">
      ...
      {action && href && (
        <Link
          href={href}
          className="text-[10px] sm:text-xs text-yellow-400 hover:text-yellow-300"
        >
          {action} →
        </Link>
      )}
...
function EquityCurveChart({
  ...
  if (data.length < 2) {
    return (
      <div className="h-32 sm:h-40 md:h-48 flex items-center justify-center text-gray-500 text-xs sm:text-sm">
        Not enough data
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      <div className="absolute top-0 right-0 text-right">
        <p
          className={`text-lg sm:text-xl md:text-2xl font-bold tabular-nums ${isPositive ? "text-green-400" : "text-red-400"}`}
        >
          ${lastEquity.toFixed(0)}
        </p>
        <p
          className={`text-xs sm:text-sm tabular-nums ${isPositive ? "text-green-400" : "text-red-400"}`}
        >
          {isPositive ? "+" : ""}
          {change.toFixed(2)} (
          {((change / (firstEquity || 1)) * 100).toFixed(2)}%)
        </p>
      </div>
      <svg
...
function WinLossDonut({
  ...
  if (total === 0) {
    return (
      <div className="h-24 sm:h-28 md:h-32 flex items-center justify-center text-gray-500 text-xs sm:text-sm">
        No data
      </div>
    );
  }
    ...
    <div className="flex items-center justify-center gap-3 sm:gap-4">
      <div className="relative w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-sm sm:text-base md:text-lg font-bold text-white tabular-nums">
            {total}
          </span>
        </div>
      </div>
