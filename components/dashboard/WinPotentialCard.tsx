...
import {
  getRankingConfig,
  getMetricIcon,
} from "@/lib/services/ranking-config.service";
...
export default function WinPotentialCard({
  ...
  const winData = calculateWinProbability(userParticipation, allParticipants, {
  ...
  const methodColorSchemes: Record<
    string,
    {
      gradient: string;
      border: string;
  ...
  const getStatusConfig = () => {
    ...
    return (
      methodColorSchemes[rankingConfig.color] || methodColorSchemes.emerald
    );
  };
  ...
  const isInPrizePosition =
    winData.currentRank <= winData.totalWinners && winData.currentRank > 0;
        ...
        {winData.currentRank > 1 && winData.status !== "disqualified" && (
          <div className="mb-4 p-3 rounded-lg bg-gray-800/50 border border-gray-700/50">
            <div className="flex items-center justify-between">
              ...
              >
                {gap >= 0 ? "+" : ""}
                {formatMetricValue(gap, competition.rankingMethod)}
              </span>
            </div>
              ...
              <div className="flex-1 h-1.5 bg-gray-700 rounded-full overflow-hidden">
                <div
                  className={cn(
                    ...
                    winData.percentOfLeader >= 90
                      ? "bg-emerald-500"
                      : winData.percentOfLeader >= 70
                        ? "bg-amber-500"
                        : "bg-red-500",
                  )}
                  style={{
                    width: `${Math.min(100, Math.max(5, winData.percentOfLeader))}%`,
                  }}
                />
              </div>
          ...
          <div className="mb-4 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30">
            ...
            {winData.gapToNextRank !== null && allParticipants.length > 1 && (
              <p className="text-xs text-emerald-400/70 text-center mt-1">
                +
                {formatMetricValue(
                  Math.abs(winData.gapToNextRank),
          ...
          <div className="mt-3 p-2.5 rounded-lg bg-red-500/10 border border-red-500/30">
            ...
            <div className="mt-2 h-1.5 bg-red-900/50 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min(100, (userParticipation.totalTrades / competition.minimumTrades) * 100)}%`,
                }}
              />
            </div>
          </div>
