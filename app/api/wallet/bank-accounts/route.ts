  ...
  try {
    ...
    const maskedAccounts = accounts.map((account: any) => ({
      ...
      nuveiConnected:
        ...
        (!!account.nuveiUpoId && account.nuveiStatus === "active"),
  ...
  try {
    ...
    if (!country || country.length !== 2) {
      return NextResponse.json(
        {
          success: false,
          error: "Valid country code is required (e.g., DE, NL, FR)",
        },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Maximum 5 bank accounts allowed. Please remove an existing account first.",
        },
        { status: 400 },
      );
    }
    ...
    const automaticWithdrawalsEnabled =
      withdrawalSettings?.nuveiWithdrawalEnabled ?? false;
    ...
    if (cleanIban && automaticWithdrawalsEnabled) {
      ...
      nuveiStatus = "ready";
      console.log(
        "🏦 Bank account ready for automatic withdrawals (will use refundTransaction with bank details)",
      );
    }
    ...
    const bankAccount = await UserBankAccount.create({
      ...
      nickname:
        nickname?.trim() ||
        `${bankName || "Bank"} ****${cleanIban?.slice(-4) || accountNumber?.slice(-4)}`,
    });
