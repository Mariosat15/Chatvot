...
export async function GET() {
  try {
    ...
    const totalPrizesFromCompetitions = completedCompetitions.reduce((sum, comp) => {
      return sum + (comp.prizePool || 0);
    }, 0);

    // Calculate total prizes from platform financials (more accurate)
    const totalPrizesPaid = platformFinancials?.totalPrizesPaid || totalPrizesFromCompetitions;

    // Calculate active traders (users who traded in last 30 days)
    const activeTraders = await db.collection('positions').aggregate([
    ...
    const activePrizePool = await db.collection('competitions').aggregate([
    ...
    const stats = {
      ...
      formatted: {
        ...
        totalCompetitions: formatNumber(completedCompetitions.length + activeCompetitions),
