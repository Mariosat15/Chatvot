...
export async function GET() {
  try {
    ...
    const topCompetitionWinners = await db.collection('competitionparticipants').aggregate([
      {
        $match: {
          prizeWon: { $gt: 0 }
        }
    ...
    const challengeWins = await db.collection('challenges').aggregate([
      {
        $match: {
          status: 'completed',
    ...
    const challengeWinsMap = new Map(
      challengeWins.map(c => [c._id, { won: c.challengesWon, winnings: c.totalChallengeWinnings }])
    );
    ...
    const leaderboard: LeaderboardEntry[] = topCompetitionWinners.map((winner, index) => {
      const user = userMap.get(winner._id);
      const challengeData = challengeWinsMap.get(winner._id) || { won: 0, winnings: 0 };
      const totalWinnings = (winner.totalPrizeWon || 0) + (challengeData.winnings || 0);
      
      // Calculate win rate (competitions where they placed top 3)
      const competitionsTop3 = winner.avgRank <= 3 ? winner.competitionsParticipated : Math.floor(winner.competitionsParticipated * 0.3);
      const winRate = winner.competitionsParticipated > 0 
        ? Math.round((competitionsTop3 / winner.competitionsParticipated) * 100)
        : 0;
