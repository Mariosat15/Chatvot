...
export async function GET() {
  try {
    ...
    const formattedCompetitions = competitions.map((comp) => {
      ...
      return {
        ...
        participantsPercentage: Math.round(
          ((comp.currentParticipants || 0) / (comp.maxParticipants || 100)) *
            100,
        ),
        status: comp.status,
        statusBadge,
    ...
    const activeCompetitions = formattedCompetitions.filter(
      (c) => c.status === "active",
    );
    const upcomingCompetitions = formattedCompetitions.filter(
      (c) => c.status === "upcoming",
    );
