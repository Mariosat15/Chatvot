  ...
  try {
    ...
    switch (action) {
      ...
      case "profile": {
        ...
        const summary =
          await BehavioralAnalysisService.getProfileSummary(userId);
        const style = await BehavioralAnalysisService.getTradingStyle(userId);
        const stats =
          await BehavioralAnalysisService.getAverageTradeStats(userId);
      ...
      case "similar": {
        ...
        const similarities =
          await SimilarityDetectionService.detectHighSimilarity(threshold);
      ...
      case "mirror-trading": {
        ...
        const mirrorPairs =
          await MirrorTradingService.getAllMirrorTradingPairs();
      ...
      case "matrix": {
        ...
        if (userIds.length < 2) {
          ...
          const matrix =
            await SimilarityDetectionService.getSimilarityMatrix(ids);
        ...
        const matrix =
          await SimilarityDetectionService.getSimilarityMatrix(userIds);
  ...
  try {
    ...
    switch (action) {
      case "calculate-similarity": {
        ...
        const result = await SimilarityDetectionService.calculateSimilarity(
          userId1,
          userId2,
        );
      ...
      case "check-mirror-trading": {
        ...
        return NextResponse.json({
          ...
          mirrorTrading: {
            ...
            timingCorrelation:
              (result.timingCorrelation * 100).toFixed(1) + "%",
      ...
      case "recalculate-profile": {
        ...
        const profile =
          await BehavioralAnalysisService.getOrCreateProfile(userId);
        ...
        return NextResponse.json({
          ...
          debug: {
            profiles: {
              ...
              withTrades: profiles.filter((p) => p.totalTradesAnalyzed > 0)
                .length,
              list: profileSummary,
            },
            similarities: {
              total: similarities.length,
              highSimilarity: similarities.filter(
                (s) => s.similarityScore >= 0.7,
              ).length,
              mirrorTrading: similarities.filter((s) => s.mirrorTradingDetected)
                .length,
