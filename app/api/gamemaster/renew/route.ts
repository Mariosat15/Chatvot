...
export async function POST() {
  ...
  try {
    ...
    const subscription = await GameMasterSubscription.findOne({ userId }).session(mongoSession);
    ...
    if (subscription.packageId) {
      const currentPackage = await MarketplaceItem.findById(subscription.packageId).session(mongoSession);
      if (currentPackage && currentPackage.gameMasterConfig) {
        renewalPrice = currentPackage.price;
        durationDays = currentPackage.gameMasterConfig.subscriptionDurationDays || 30;
        packageConfig = {
