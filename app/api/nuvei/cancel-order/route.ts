...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!transaction) {
      console.log(
        `Transaction not found for clientUniqueId: ${clientUniqueId}`,
      );
      return NextResponse.json({
        success: true,
    ...
    if (transaction.userId !== userId) {
      console.error(
        `User ${userId} tried to cancel transaction belonging to ${transaction.userId}`,
      );
    ...
    if (transaction.status !== "pending") {
      console.log(
        `Transaction ${transaction._id} is already ${transaction.status}, not updating`,
      );
      return NextResponse.json({
        success: true,
    ...
    transaction.failureReason =
      sanitizedReason ||
      sanitizedErrorDesc ||
      (newStatus === "cancelled" ? "User cancelled" : "Payment failed");
    transaction.processedAt = new Date();
    transaction.metadata = {
      ...transaction.metadata,
      clientErrorCode:
        typeof errorCode === "string" ? errorCode.substring(0, 50) : errorCode,
    ...
    console.log(
      `Transaction ${transaction._id} marked as ${newStatus}: ${reason || errorDescription || "No reason provided"}`,
    );
