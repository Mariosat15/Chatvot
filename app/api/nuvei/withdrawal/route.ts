...
export async function POST(req: NextRequest) {
  try {
    ...
    if (withdrawalMethod === "bank_transfer") {
      ...
      if (!bankAccount) {
        return NextResponse.json(
          {
            error:
              "Bank account not found. Please add a bank account in your wallet settings.",
          },
          { status: 400 },
        );
      }
      ...
      if (!actualUpoId) {
        return NextResponse.json(
          {
            error:
              "No card available for refund. You need to make a deposit first, or use bank transfer instead.",
    ...
    const withdrawalParams: Parameters<
      typeof nuveiService.submitWithdrawal
    >[0] = {
      userTokenId,
    ...
    const withdrawalRequest = await WithdrawalRequest.create({
      ...
      // Payout details
      payoutMethod:
        withdrawalMethod === "card_refund" || withdrawalMethod === "card_payout"
          ? "nuvei_card_payout"
          : "nuvei_bank_transfer",
    ...
    if (isBankTransfer && bankAccount) {
      ...
      if (!bankUpoId) {
        ...
        return NextResponse.json(
          {
            error:
              'This bank account is not set up for automatic withdrawals. Please click "Enable Auto" on your bank account in wallet settings first.',
    ...
    if ("error" in nuveiResult && nuveiResult.error) {
      ...
      if (isTechnicalError) {
        ...
        console.log(
          "💸 Technical error detected - falling back to manual processing",
        );
        ...
        const userMessage =
          "Automatic processing is temporarily unavailable. Your withdrawal has been queued for manual processing by our team within 24-48 hours.";
      ...
      try {
        ...
        console.log(
          `📧 Withdrawal failed notification sent for user ${userId}`,
        );
      } catch (notifError) {
        console.error(
    ...
    const successResult = nuveiResult as Exclude<
      typeof nuveiResult,
      { error: string }
    >;
    ...
    const nuveiStatus = (
      successResult.wdRequestStatus ||
      successResult.transactionStatus ||
      ""
    ).toUpperCase();
    ...
    if (nuveiStatus === "APPROVED") {
      ...
      statusMessage =
        "🎉 Withdrawal approved! Funds will be sent to your account within 1-3 business days.";
    } else if (nuveiStatus === "PENDING") {
      // Bank payouts typically start as PENDING and become APPROVED via DMN
      finalStatus = "processing";
      statusMessage =
        "✅ Withdrawal submitted! Your bank transfer is being processed and will arrive within 1-3 business days.";
    ...
    await WithdrawalRequest.findByIdAndUpdate(withdrawalRequest._id, {
      $set: {
        ...
        ...(finalStatus === "failed"
          ? {
              failedAt: new Date(),
              failedReason: successResult.reason || "Declined by processor",
            }
          : {}),
        "metadata.nuveiTransactionId":
          successResult.wdRequestId || successResult.transactionId,
    ...
    if (finalStatus === "completed") {
      ...
      if (platformFee > 0) {
        try {
          ...
          console.log(
            `💸 Recorded platform fee: €${platformFee} (${platformFeeCredits} credits)`,
          );
        } catch (feeError) {
          console.error(
            "💸 Error recording platform fee (non-blocking):",
            feeError,
          );
    ...
    if (finalStatus === "failed") {
      ...
      await WalletTransaction.findByIdAndUpdate(walletTx._id, {
        ...
        "metadata.failureReason":
          successResult.reason || "Declined by processor",
      });
      ...
      try {
        ...
        await sendWithdrawalFailedNotification(
          ...
          successResult.reason ||
            "Withdrawal was declined by the payment processor. Your credits have been returned.",
        );
        console.log(
          `📧 Withdrawal declined notification sent for user ${userId}`,
        );
      } catch (notifError) {
        console.error(
          "❌ Error sending withdrawal declined notification:",
          notifError,
        );
      }
    ...
    const nameToUse =
      ...
      (emailToUse ? emailToUse.split("@")[0] : "User");

    console.log(
      `📧 Checking email notification: status=${finalStatus}, email=${emailToUse || "NONE"}`,
    );
    ...
    if (finalStatus === "completed" || finalStatus === "processing") {
      if (emailToUse) {
        ...
        console.warn(
          "⚠️ Cannot send withdrawal email - no email address available",
        );
      }
    } else {
      console.log(
        `📧 Skipping email - status is ${finalStatus} (not completed/processing)`,
      );
    }
    ...
    return NextResponse.json({
      ...
      estimatedProcessingTime:
        finalStatus === "completed"
          ? "1-3 business days for funds to arrive"
          : "1-3 business days",
    });
  } catch (error) {
...
export async function GET() {
  try {
    ...
    const nuveiRequests = await nuveiService.getWithdrawalRequests({
      userTokenId,
    });
    ...
    return NextResponse.json({
      ...
      nuveiRequests:
        "withdrawalRequests" in nuveiRequests
          ? nuveiRequests.withdrawalRequests
          : [],
      localRequests,
    });
  } catch (error) {
