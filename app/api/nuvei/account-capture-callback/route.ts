...
export async function GET(request: NextRequest) {
  ...
  if (hasNoParams || isExplicitSuccess) {
    console.log(
      "🏦 Account capture callback - assuming success (DMN handles processing)",
    );
    console.log(
      "🏦 hasNoParams:",
      hasNoParams,
  ...
  console.log(
    "🏦 Account capture callback - unknown status:",
    status,
    pppStatus,
  );
  return NextResponse.redirect(
...
export async function POST(request: NextRequest) {
  ...
  const status =
    params.get("status") || params.get("Status") || params.get("ppp_status");
  ...
  if (status === "SUCCESS" || status === "OK" || status === "APPROVED") {
    console.log(
      "🏦 Account capture (POST) successful, UPO:",
      userPaymentOptionId,
    );
    return NextResponse.redirect(
