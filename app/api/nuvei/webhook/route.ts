...
export async function POST(req: NextRequest) {
  try {
    ...
    // ==============================
    const isWithdrawalDmn = !!(
      params.wdRequestId ||
      params.wdRequestStatus ||
      params.merchantWDRequestId ||
      params.merchant_wd_request_id
    );
    ...
    const isAccountCaptureDmn =
      params.type === "ACCOUNT_CAPTURE" ||
      (params.payment_method === "apmgw_BankPayouts" &&
    ...
    if (isCardTokenizationDmn) {
      ...
      return NextResponse.json({
        status: "OK",
        message: "Card tokenization acknowledged",
      });
    }
    ...
    if (provider) {
      secretKey = provider.credentials.find(
        (c: { key: string }) => c.key === "secret_key",
      )?.value;
    }
    ...
    const nuveiTransactionId =
      params.TransactionID || params.transactionId || params.PPP_TransactionID;
    const status = params.Status || params.ppp_status;
    ...
    console.log(
      `Processing DMN: nuveiTransactionId=${nuveiTransactionId}, status=${status}, clientUniqueId=${clientUniqueId}`,
    );
    ...
    if (status === "APPROVED" && errCode === 0) {
      ...
      if (!claimed) {
        ...
        if (currentTxn?.status === "completed") {
          console.log(
            `✅ Transaction ${transaction._id} already completed, skipping (duplicate DMN)`,
          );
          return NextResponse.json({
            status: "OK",
        ...
        if (currentTxn?.status === "processing") {
          console.log(
            `⏳ Transaction ${transaction._id} already being processed, skipping (concurrent DMN)`,
          );
          return NextResponse.json({
            status: "OK",
        ...
        console.log(
          `⚠️ Transaction ${transaction._id} status: ${currentTxn?.status}, cannot process`,
        );
        return NextResponse.json({
          status: "OK",
      ...
      console.log(
        `🔒 Claimed transaction ${transaction._id} for processing (user: ${transaction.userId})`,
      );
      ...
      if (params.userPaymentOptionId && transaction.userId) {
        try {
          ...
          console.log(
            `💳 Stored UPO ${params.userPaymentOptionId} for user ${transaction.userId}`,
          );
        } catch (upoError) {
      ...
      try {
        const { completeDeposit } =
          await import("@/lib/actions/trading/wallet.actions");
        ...
        if (cardFingerprint && transaction.userId) {
          try {
            console.log(
              `🔍 Tracking Nuvei payment fingerprint: ${cardFingerprint.substring(0, 12)}...`,
            );
            ...
            if (fraudResult.fraudDetected) {
              console.log(
                `🚨 FRAUD DETECTED: Payment method shared across ${fraudResult.linkedUsers.length + 1} accounts!`,
              );
              console.log(
                `   Linked users: ${fraudResult.linkedUsers.join(", ")}`,
              );
            }
          } catch (fraudError) {
        ...
        console.error(
          "❌ CRITICAL: completeDeposit failed for Nuvei:",
          completeError,
        );
        console.error(
          `❌ Transaction ${transaction._id} for user ${transaction.userId} needs manual review`,
        );
        console.error(
          `❌ Amount: ${transaction.amount} credits, Nuvei TX: ${nuveiTransactionId}`,
        );
        ...
        console.error(
          `🚨 ALERT: Deposit ${transaction._id} approved by Nuvei but failed to process. Manual review required!`,
        );
      }
 ...
 */
async function handleAccountCaptureDmn(
  params: NuveiDmnParams,
  ...
  try {
    ...
    if (!userId) {
      console.error(
        "🏦 Could not extract userId from userTokenId:",
        userTokenId,
      );
      return NextResponse.json({
        status: "OK",
        message: "Invalid userTokenId format",
      });
    }
    ...
    if (existingUPO) {
      console.log(
        "🏦 UPO already exists, updating lastUsed:",
        userPaymentOptionId,
      );
    ...
    try {
      const UserBankAccount = (
        await import("@/database/models/user-bank-account.model")
      ).default;
...
async function handlePayoutDmn(params: NuveiDmnParams): Promise<NextResponse> {
  try {
    ...
    if (!withdrawalRequest) {
      console.log("💸 Payout DMN: Withdrawal request not found for:", {
        transactionId,
        clientUniqueId,
      });
      ...
      return NextResponse.json({
        status: "OK",
        message: "Withdrawal request not found (likely already processed)",
      });
    }
    ...
    if (
      ...
      console.log(
        `💸 Payout DMN: Withdrawal ${withdrawalRequest._id} already ${withdrawalRequest.status}, skipping`,
      );
    ...
    console.log(
      `💸 Payout DMN: Updating withdrawal ${withdrawalRequest._id} from ${withdrawalRequest.status} to ${newStatus}`,
    );
    ...
    if (newStatus === "completed") {
      const CreditWalletModel = (
        await import("@/database/models/trading/credit-wallet.model")
      ).default;
      const wallet = await CreditWalletModel.findOne({
      ...
      if (wallet) {
        ...
        if (!alreadyCounted) {
          ...
          wallet.totalWithdrawn =
            (wallet.totalWithdrawn || 0) + creditsWithdrawn;
          await wallet.save();
          ...
          console.log(
            `💸 Payout DMN: Updated totalWithdrawn: +${creditsWithdrawn} credits for user ${withdrawalRequest.userId}`,
          );
        } else {
          console.log(
            `💸 Payout DMN: totalWithdrawn already updated for withdrawal ${withdrawalRequest._id}, skipping`,
          );
        }
      }
      ...
      if (platformFee > 0) {
        try {
          ...
          if (!existingFee) {
            ...
            console.log(
              `💸 Payout DMN: Recorded platform fee: €${platformFee}`,
            );
          } else {
            console.log(
              `💸 Payout DMN: Platform fee already recorded for withdrawal ${withdrawalRequest._id}`,
            );
          }
        } catch (feeError) {
          console.error(
            "💸 Payout DMN: Error recording platform fee (non-blocking):",
            feeError,
          );
        }
      }
      ...
      console.log(
        `📧 Payout DMN: Checking email notification, userEmail=${emailToUse || "NOT SET"}`,
      );
      ...
      if (emailToUse) {
        try {
          ...
          if (withdrawalRequest.payoutMethod?.includes("card")) {
            ...
            withdrawalRequest.payoutMethod?.includes("bank") ||
            withdrawalRequest.payoutMethod?.includes("sepa")
          ) {
          ...
          console.log(
            `✅ Payout DMN: Email sent successfully to ${emailToUse}`,
          );
        } catch (emailError) {
        ...
        console.warn(
          "⚠️ Payout DMN: Cannot send email - no email address on withdrawal request",
        );
      }
    }
    ...
    if (shouldRefund) {
      ...
      if (wallet) {
        ...
        console.log(
          `💸 Payout DMN: Refunded ${creditsToRefund} credits to user ${withdrawalRequest.userId}`,
        );
    ...
    console.log(
      `💸 Payout DMN processed successfully: ${withdrawalRequest._id} -> ${newStatus}`,
    );
    return NextResponse.json({
      status: "OK",
      message: `Payout DMN processed: ${newStatus}`,
    });
  } catch (error) {
 ...
 */
async function handleWithdrawalDmn(
  params: NuveiDmnParams,
): Promise<NextResponse> {
  try {
    const wdRequestId = params.wdRequestId;
    const wdRequestStatus = params.wdRequestStatus;
    const merchantWDRequestId =
      params.merchantWDRequestId || params.merchant_wd_request_id;
    const status = params.Status || params.ppp_status;
    const errCode = parseInt(params.ErrCode || params.errCode || "0");
    const reason = params.Reason || "";
    const transactionId =
    ...
    if (!merchantWDRequestId) {
      ...
      return NextResponse.json({
        status: "OK",
        message: "Missing merchantWDRequestId",
      });
    }
    ...
    console.log(
      `💸 Updating withdrawal ${merchantWDRequestId}: ${withdrawalRequest.status} -> ${newStatus}`,
    );
    ...
    if (newStatus === "completed") {
      ...
      updateData.failedReason =
        reason || "Withdrawal declined by payment provider";
      ...
      if (wallet) {
        ...
        if (!alreadyCounted) {
          const creditsWithdrawn = Math.abs(
            withdrawalRequest.amountCredits || 0,
          );
          wallet.totalWithdrawn =
            (wallet.totalWithdrawn || 0) + creditsWithdrawn;
          await wallet.save();
          ...
          console.log(
            `💸 Withdrawal DMN: Updated totalWithdrawn: +${creditsWithdrawn} credits for user ${withdrawalRequest.userId}`,
          );
        } else {
          console.log(
            `💸 Withdrawal DMN: totalWithdrawn already updated for withdrawal ${withdrawalRequest._id}, skipping`,
          );
        }
      }
      ...
      if (platformFee > 0) {
        try {
          ...
          if (!existingFee) {
            ...
            console.log(
              `💸 Withdrawal DMN: Recorded platform fee: €${platformFee}`,
            );
          } else {
            console.log(
              `💸 Withdrawal DMN: Platform fee already recorded for withdrawal ${withdrawalRequest._id}`,
            );
          }
        } catch (feeError) {
          console.error(
            "💸 Withdrawal DMN: Error recording platform fee (non-blocking):",
            feeError,
          );
        }
      }
    ...
    if (shouldRefund) {
      ...
      if (wallet) {
        ...
        if (creditsToRefund > 0) {
          ...
          console.log(
            `💸 Refunded ${creditsToRefund} credits to user ${withdrawalRequest.userId}`,
          );
          ...
          // Send notification
          try {
            const { sendWithdrawalFailedNotification } =
            ...
            console.error(
              "💸 Error sending withdrawal failed notification:",
              notifError,
            );
          }
        } else {
          console.warn(
            "💸 No credits to refund (amountCredits is 0 or missing)",
          );
        }
      }
    }
    ...
    if (newStatus === "completed") {
      // In-app notification
      try {
        const { sendWithdrawalCompletedNotification } =
          await import("@/lib/services/notification.service");
        ...
        console.error(
          "💸 Error sending withdrawal completed notification:",
          notifError,
        );
      }
      ...
      console.log(
        `📧 Withdrawal DMN: Checking email notification, userEmail=${emailToSend || "NOT SET"}`,
      );
      ...
      if (emailToSend) {
        try {
          ...
          console.log(
            `✅ Withdrawal DMN: Email sent successfully to ${emailToSend}`,
          );
        } catch (emailError) {
        ...
        console.warn(
          "⚠️ Withdrawal DMN: Cannot send email - no email address on withdrawal request",
        );
      }
    }
...
export async function GET() {
  return NextResponse.json({
    status: "OK",
    message:
      "Nuvei webhook endpoint active (handles both payments and withdrawals)",
  });
}
