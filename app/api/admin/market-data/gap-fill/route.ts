...
import { fetchCandlesForRange, getRecentCandles, Timeframe } from '@/lib/services/forex-historical.service';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const timeframesToFill = timeframe ? [timeframe] : ['1m', '5m', '15m', '30m', '1h', '4h', '1d'];
    ...
    for (const sym of symbols) {
      ...
      if (timeframesToFill.includes('1m')) {
        ...
        if (candles.length >= 2) {
          ...
          for (let i = 1; i < candles.length; i++) {
            ...
            if (missingMinutes > 0 && missingMinutes < 10000) { // Skip huge gaps
              try {
          ...
          if (symbolGapsFilled > 0 || symbolCandlesFilled > 0) {
            ...
            console.log(`✅ [Gap Fill] ${sym} 1m: ${symbolCandlesFilled} candles filled`);
          }
        }
      }
      ...
      for (const tf of timeframesToFill) {
        ...
        const candles = await historicalModel.find({ symbol: sym })
          .sort({ timestamp: 1 })
          .lean() as Array<{ timestamp: Date; open: number; high: number; low: number; close: number }>;
        ...
        if (candles.length < 50) {
          ...
          try {
            const apiCandles = await getRecentCandles(sym as ForexSymbol, config.apiTf, 500);
          ...
          for (let i = 1; i < candles.length; i++) {
            ...
            if (timeDiff > expectedGapSeconds * 2) {
              ...
              // Skip weekend gaps
              const isWeekendGap = (prevDay === 5 || prevDay === 6) && missingMinutes >= 2880 && missingMinutes <= 4500;
              if (isWeekendGap) continue;
              
              console.log(`🔧 [Gap Fill] ${sym} ${tf}: Filling gap ${new Date(prevTime).toISOString()} → ${new Date(currTime).toISOString()}`);
          ...
          const newestTime = new Date(candles[candles.length - 1].timestamp).getTime();
          const nowTime = Date.now();
          const trailingGapSeconds = (nowTime - newestTime) / 1000;
          
          if (trailingGapSeconds > expectedGapSeconds * 2) {
            console.log(`🔧 [Gap Fill] ${sym} ${tf}: Filling trailing gap from ${new Date(newestTime).toISOString()} to now`);
            
            try {
              const trailingCandles = await getRecentCandles(sym as ForexSymbol, config.apiTf, 100);
              ...
              console.error(`❌ [Gap Fill] ${sym} ${tf} trailing gap error:`, err);
            }
          }
        }
        ...
        if (tfGapsFilled > 0 || tfCandlesFilled > 0) {
          ...
          console.log(`✅ [Gap Fill] ${sym} ${tf}: ${tfCandlesFilled} candles filled`);
        }
      }
    }
    ...
    console.log(`🔧 [Gap Fill] COMPLETE: ${totalCandlesFilled} candles across ${totalGapsFilled} gaps`);
