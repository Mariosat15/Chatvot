  ...
  try {
    ...
    if (daysToKeep === undefined || daysToKeep < 0) {
      return NextResponse.json(
        { error: "daysToKeep must be 0 or greater" },
        { status: 400 },
      );
    }
    ...
    const { count: countBefore, size: sizeBefore } =
    ...
    const { count: countAfter, size: sizeAfter } =
      await getCollectionStats(collection);
    ...
    console.log(
      `🧹 [Cleanup] Deleted ${result.deletedCount} candles older than ${cutoffDate.toISOString()}`,
    );
    console.log(
      `📊 [Cleanup] Freed ${(freedSpace / 1024 / 1024).toFixed(2)} MB`,
    );
