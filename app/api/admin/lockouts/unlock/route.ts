...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(
      `🔐 [Main App] API key present: ${!!apiKey}, Expected key set: ${!!expectedKey}`,
    );
    ...
    console.log(
      `📋 [Main App] Unlock request for email: ${email}, userId: ${userId}`,
    );
    ...
    if (!emailToUnlock && userId) {
      ...
      if (db) {
        ...
        console.log(
          `📋 [Main App] Looked up email from userId: ${emailToUnlock}`,
        );
      }
    }
    ...
    console.log(
      `🔓 [Main App] Calling adminUnlockAccount for: ${emailToUnlock}`,
    );
