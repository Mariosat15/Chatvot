...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `📬 Result:`,
      result ? `Created with ID ${result._id}` : "NOT CREATED",
    );
    ...
    return NextResponse.json({
      ...
      message: result
        ...
        : "Notification not created (check logs)",
      templateCount,
...
export async function GET() {
  try {
    ...
    const allTemplates = await NotificationTemplate.find({})
      .select("templateId name category isEnabled")
      .lean();

    const [enabledTemplates, userNotifications, unreadCount] =
      await Promise.all([
