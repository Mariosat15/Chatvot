...
export async function POST(req: NextRequest) {
  try {
    ...
    const isFirstAppeal = !restriction.appealSubmittedAt;
    restriction.appealSubmittedAt = new Date();
    restriction.appealConversationId = conversationId;
    await restriction.save();

    // Add an admin-visible note on the user's profile so support staff
    // browsing the "Notes" tab immediately see that an appeal was filed.
    // Only write the note on the FIRST appeal submission to avoid noise
    // if the user clicks the button again.
    if (isFirstAppeal) {
      try {
        const excerpt =
          userMessageRaw.length > 0
            ? `\n\nUser message: ${userMessageRaw.slice(0, 500)}${
                userMessageRaw.length > 500 ? "…" : ""
              }`
            : "";
        const noteContent = [
          `🛡️ Appeal submitted for case ${caseId}`,
          `Restriction: ${restriction.restrictionType} • Reason: ${restriction.reason}`,
        ...
        await UserNote.create({
          userId,
          // Reason: user-originated system note. There is no admin session here;
          // we tag it clearly so admins know it was generated by the platform.
          adminId: "system",
          adminName: "System",
          content: noteContent,
          category: "support",
        ...
        // Don't fail the appeal just because the note couldn't be written.
        console.warn("⚠️ [Appeal] Failed to create admin note:", err);
      }
    }
