...
async function getAuthPageSettings() {
  try {
    ...
    return {
      testimonialText:
        heroSettings?.authPageTestimonialText ||
      ...
      testimonialRole:
        heroSettings?.authPageTestimonialRole || "Retail Investor",
      ...
      dashboardImage:
        heroSettings?.authPageDashboardImage || "/assets/images/dashboard.png",
          ...
          <div className="flex items-center justify-between">
            <div>
              <cite className="auth-testimonial-author">
                - {authSettings.testimonialAuthor}
              </cite>
              <p className="max-md:text-xs text-gray-500">
                {authSettings.testimonialRole}
              </p>
            </div>
            <div className="flex items-center gap-0.5">
              {Array.from(
                { length: authSettings.testimonialRating },
                (_, i) => i + 1,
              ).map((star) => (
                <Image
