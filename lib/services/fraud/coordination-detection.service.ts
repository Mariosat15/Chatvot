...
export class CoordinationDetectionService {
  ...
  static async detectCoordinatedEntry(
    ...
    console.log(
      `🎯 Checking for coordinated entries in competition ${competitionId}`,
    );
    console.log(`   Entries to analyze: ${recentEntries.length}`);

    recentEntries.forEach((entry, i) => {
      console.log(
        `   Entry ${i + 1}: User ${entry.userId.substring(0, 8)}... at ${entry.entryTime}`,
      );
    });

    if (recentEntries.length < this.MIN_COORDINATED_ACCOUNTS) {
      console.log(
        `   ❌ Not enough entries (need ${this.MIN_COORDINATED_ACCOUNTS}+)`,
      );
      return null;
    }
    ...
    let currentGroup: { userId: string; entryTime: Date }[] = [
      sortedEntries[0],
    ];

    for (let i = 1; i < sortedEntries.length; i++) {
      const timeDiff =
        new Date(sortedEntries[i].entryTime).getTime() -
    ...
    const lastEntry = new Date(
      largestGroup[largestGroup.length - 1].entryTime,
    ).getTime();
          ...
          data: {
            ...
            entrySequence: largestGroup.map((e, i) => ({
              ...
              deltaFromFirst:
                i === 0
                  ? "0s"
                  : `+${Math.round((new Date(e.entryTime).getTime() - firstEntry) / 1000)}s`,
            })),
  ...
  static async findNearbyEntries(
    ...
    return {
      ...
      timeDiffs: nearbyEntries.map(
        (e) => (new Date(e.joinedAt).getTime() - entryTimeMs) / 1000,
      ),
    };
  }
  ...
  static async trackCompetitionEntryPattern(
    ...
    console.log(
      `📝 Tracking competition entry for user ${userId.substring(0, 8)}...`,
    );
  ...
  static async analyzeEntryTimingPatterns(
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      const timeDiff =
        (new Date(sorted[i + 1].entryTime).getTime() -
          new Date(sorted[i].entryTime).getTime()) /
        1000;
      ...
      if (timeDiff <= rapidFireThreshold) {
        ...
        const existingGroupIndex = coordinatedGroups.findIndex((g) =>
          g.includes(sorted[i].userId),
        );

        if (existingGroupIndex >= 0) {
          if (
            !coordinatedGroups[existingGroupIndex].includes(
              sorted[i + 1].userId,
            )
          ) {
    ...
    for (let i = 0; i < sorted.length; i++) {
      const burstEnd =
        new Date(sorted[i].entryTime).getTime() + burstWindow * 1000;
      const burstEntries = sorted.filter((e) => {
        const time = new Date(e.entryTime).getTime();
        return (
          time >= new Date(sorted[i].entryTime).getTime() && time <= burstEnd
        );
      });
    ...
    const uniquePatterns = suspiciousPatterns.filter(
      (pattern, index, self) =>
        index ===
        self.findIndex(
          (p) =>
