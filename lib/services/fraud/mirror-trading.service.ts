...
export class MirrorTradingService {
  ...
  static async detectMirrorTrading(
    ...
    console.log(
      `🪞 Checking for mirror trading: ${userId1.substring(0, 8)}... vs ${userId2.substring(0, 8)}...`,
    );
    ...
    console.log(
      `   User1 trades: ${trades1.length}, User2 trades: ${trades2.length}`,
    );
        ...
        evidence: [
          {
            ...
            data: {
              ...
              timingCorrelation:
                (analysis.timingCorrelation * 100).toFixed(1) + "%",
              directionCorrelation: analysis.directionCorrelation.toFixed(2),
              tradingPattern: isOppositeDirection
                ? "Opposite Direction"
                : "Same Direction",
  ...
  static analyzeTradeSequence(
    ...
    const timingCorrelation = Math.max(
      0,
      1 - avgTimeDelta / this.SYNC_WINDOW_SECONDS,
    );

    // Direction correlation: -1 (all opposite) to 1 (all same)
    const directionCorrelation =
      (sameDirection - oppositeDirection) / matchingTrades;
    ...
    const matchRatio = Math.min(
      1,
      matchingTrades / Math.min(trades1.length, trades2.length),
    );
    const consistentDirection = Math.abs(directionCorrelation); // Both extremes are suspicious

    const score =
      matchRatio * 0.3 + timingCorrelation * 0.4 + consistentDirection * 0.3;
  ...
  static async checkRealTimeMirrorTrading(
    ...
    console.log(
      `🔄 Real-time mirror trading check for ${trade.pair} by ${userId.substring(0, 8)}...`,
    );
    ...
    console.log(
      `   Found ${recentProfiles.length} profiles that have traded ${trade.pair}`,
    );

    let checksRun = 0;
    for (const otherProfile of recentProfiles) {
      // Filter to recent trades
      const recentTrades = otherProfile.recentTradeSequence.filter(
        (t: { pair: string; openTime: Date }) =>
          t.pair === trade.pair &&
      ...
      if (recentTrades.length > 0) {
        console.log(
          `   👀 User ${otherProfile.userId.toString().substring(0, 8)}... has ${recentTrades.length} recent ${trade.pair} trades`,
        );
        ...
        const matchingTrade = recentTrades.find(
          (t: { openTime: Date }) =>
            Math.abs(
              new Date(t.openTime).getTime() -
                new Date(trade.openTime).getTime(),
            ) <=
            this.SYNC_WINDOW_SECONDS * 1000,
        );

        if (matchingTrade) {
          console.log(
            `   ⚠️ POTENTIAL MIRROR TRADE with ${otherProfile.userId}`,
          );
          console.log(
            `      Their trade: ${matchingTrade.direction} @ ${matchingTrade.openTime}`,
          );
          console.log(
            `      Your trade: ${trade.direction} @ ${trade.openTime}`,
          );

          checksRun++;
          // Run full analysis
          await this.detectMirrorTrading(
            userId,
            otherProfile.userId.toString(),
          );
        }
      }
    }
    ...
    console.log(
      `   ✅ Mirror trading check completed. ${checksRun} potential matches analyzed.`,
    );
  }
  ...
  > {
    ...
    return similarities.map((s) => ({
      ...
      directionPattern:
        s.mirrorTradingEvidence.filter(
          (e: { isOpposite: boolean }) => e.isOpposite,
        ).length >
        s.mirrorTradingEvidence.filter(
          (e: { isOpposite: boolean }) => !e.isOpposite,
        ).length
