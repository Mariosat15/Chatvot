...
class AlertManagerService {
     ...
     */
    static async createOrUpdateAlert(params) {
        await (0, mongoose_1.connectToDatabase)();
        const { alertType, userIds, title, description, severity, confidence, evidence, competitionId } = params;
        ...
        const userObjectIds = userIds.map(id => {
            try {
                return new mongoose_2.default.Types.ObjectId(id.toString());
            }
            catch (e) {
        ...
        const resolvedAlertOfSameType = await fraud_alert_model_1.default.findOne({
            ...userQuery,
            ...evidenceTypeCheck,
        ...
        const existingAlert = await fraud_alert_model_1.default.findOne({
            ...userQuery,
        ...
        if (existingAlert) {
            ...
            const allAlertsForUsers = await fraud_alert_model_1.default.find(userQuery).select('_id status alertType title').lean();
            if (allAlertsForUsers.length > 0) {
                console.log(`   📊 All alerts for these users:`);
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                allAlertsForUsers.forEach((a, i) => {
     ...
     */
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    static async updateExistingAlert(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    existingAlert, alertType, evidence, severity, confidence, userIds, competitionId) {
        ...
        const isDuplicateEvidence = (newEvidence) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return existingAlert.evidence.some((existing) => {
                if (existing.type !== newEvidence.type)
                    return false;
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        existingAlert.evidence.forEach((e) => {
            if (e.type.includes('device') || e.type.includes('fingerprint'))
                detectionMethods.add('same_device');
            if (e.type.includes('payment'))
                detectionMethods.add('same_payment');
            if (e.type.includes('ip'))
                detectionMethods.add('same_ip');
            if (e.type.includes('mirror'))
                detectionMethods.add('mirror_trading');
            ...
            if (e.type.includes('rapid'))
                detectionMethods.add('rapid_creation');
            if (e.type.includes('vpn') || e.type.includes('proxy') || e.type.includes('tor'))
        ...
        if (severityLevels[severity] > severityLevels[existingAlert.severity]) {
     ...
     * Create a new alert
     */
    static async createNewAlert(params) {
        const { alertType, userIds, title, description, severity, confidence, evidence, competitionId } = params;
        ...
        await fraud_alert_model_1.default.create({
            alertType,
            severity,
            status: 'pending',
            primaryUserId: new mongoose_2.default.Types.ObjectId(userIds[0]),
            suspiciousUserIds: userIds.map(id => new mongoose_2.default.Types.ObjectId(id)),
            confidence,
            title,
            description,
     ...
     */
    static async canCreateAlert(userIds, alertType, competitionId) {
        await (0, mongoose_1.connectToDatabase)();
        const userQuery = {
            $or: [
                { suspiciousUserIds: { $in: userIds.map(id => new mongoose_2.default.Types.ObjectId(id)) } },
                { primaryUserId: { $in: userIds.map(id => new mongoose_2.default.Types.ObjectId(id)) } }
            ]
        };
     ...
     */
    static formatMethodName(method) {
        return method
            .replace(/_/g, ' ')
