...
interface WebSocketGlobalState {
  ...
  completedCandlesToBroadcast: Array<{
    symbol: string;
    timeframe: string;
    time: number;
    open: number;
 ...
 */
function checkTPSLOnPriceUpdate(
  symbol: ForexSymbol,
  bid: number,
  ask: number,
): void {
 ...
 */
function queuePriceForMongoCache(
  symbol: string,
  bid: number,
  ask: number,
  timestamp: number,
  ...
  if (!mongoPriceWriteTimer) {
    mongoPriceWriteTimer = setTimeout(
      flushPricesToMongo,
      MONGO_PRICE_WRITE_INTERVAL_MS,
    );
  }
}
...
async function flushPricesToMongo(): Promise<void> {
  ...
  const updates = Array.from(pendingPriceUpdates.entries()).map(
    ([symbol, data]) => ({
      symbol,
      bid: data.bid,
      ask: data.ask,
  ...
  try {
    ...
    const PriceCache = (await import("@/database/models/price-cache.model"))
      .default;
...
async function loadSymbolSpreadSettings(): Promise<void> {
  ...
  if (
    ...
    now - symbolSettingsLoadTime < SYMBOL_SETTINGS_CACHE_MS
  ) {
    return;
  }
 ...
 */
function updateHigherTimeframeCaches(
  symbol: string,
  price: number,
  currentTime: number,
): void {
 ...
 */
async function saveCompletedCandleToMongoDB(
  ...
  try {
    ...
    console.error(
      `❌ [Candle Save Error] ${candle.symbol}:`,
...
async function saveCompletedHigherTimeframeCandle(
  ...
  try {
    ...
    if (!historicalModel) {
      console.error(
        `❌ [Auto-Save] No historical model for timeframe: ${timeframe}`,
      );
      return;
    }
    ...
    try {
      ...
      console.warn(
        `⚠️ [Auto-Save ${timeframe}] Failed to augment with 1m:`,
    ...
    console.error(
      `❌ [Auto-Save ${timeframe}] ${candle.symbol}:`,
      error instanceof Error ? error.message : error,
    );
  }
}
 ...
 */
function calculateFormingCandleForTimeframe(
  symbol: string,
  ...
  const currentPeriodStart =
    Math.floor(forming1m.time / periodSeconds) * periodSeconds;
  ...
  for (const candle of buffer) {
    if (
      ...
      candle.time < currentPeriodStart + periodSeconds
    ) {
 ...
 */
function handleAggregateMessage(
  msg: {
    ev: string;
    pair?: string;
    p?: string;
  ...
  isMinuteAggregate: boolean = false,
): void {
  ...
  if (!candleSaveTimer) {
    candleSaveTimer = setTimeout(async () => {
      ...
      try {
        ...
        console.error(
          "❌ [Candle DB] Failed to save candles:",
          error instanceof Error ? error.message : error,
        );
...
export function getCachedPrices(
  symbols: ForexSymbol[],
): Map<ForexSymbol, StreamingPriceQuote> {
  const result = new Map<ForexSymbol, StreamingPriceQuote>();
 ...
 */
export function updateCacheFromRest(
  symbol: ForexSymbol,
  quote: Omit<StreamingPriceQuote, "source">,
): void {
  // Only update if WebSocket hasn't provided a more recent price
  const existing = priceCache.get(symbol);
  if (
    !existing ||
    existing.source !== "websocket" ||
...
export async function broadcastDataUpdated(
  ...
  try {
    ...
    console.warn(
      `⚠️ [Broadcast] Failed to notify data update: ${error instanceof Error ? error.message : error}`,
    );
  }
}
...
async function loadBroadcastInterval(): Promise<number> {
  try {
    ...
    if (!db) {
      console.warn(
        "⚠️ [Broadcast] Database not connected, using default interval",
      );
      return 200;
    }
    ...
    console.warn(
      "⚠️ [Broadcast] Failed to load interval setting, using default:",
      error instanceof Error ? error.message : error,
    );
    return 200;
  }
...
async function checkAndUpdateBroadcastInterval(): Promise<void> {
  const now = Date.now();
  if (now - lastBroadcastSettingsCheck < BROADCAST_SETTINGS_CHECK_INTERVAL)
    return;
...
async function broadcastFormingCandles(): Promise<void> {
  ...
  const formingCandles = Array.from(state.formingCandles.values()).filter((c) =>
    changedSymbols.has(c.symbol),
  );

  const formingCandles5m = Array.from(state.formingCandles5m.values()).filter(
    (c) => changedSymbols.has(c.symbol),
  );

  const formingCandles15m = Array.from(state.formingCandles15m.values()).filter(
    (c) => changedSymbols.has(c.symbol),
  );

  const formingCandles30m = Array.from(state.formingCandles30m.values()).filter(
    (c) => changedSymbols.has(c.symbol),
  );

  const formingCandles1h = Array.from(state.formingCandles1h.values()).filter(
    (c) => changedSymbols.has(c.symbol),
  );

  const formingCandles4h = Array.from(state.formingCandles4h.values()).filter(
    (c) => changedSymbols.has(c.symbol),
  );

  const formingCandlesD = Array.from(state.formingCandlesD.values()).filter(
    (c) => changedSymbols.has(c.symbol),
  );

  const formingCandlesW = Array.from(state.formingCandlesW.values()).filter(
    (c) => changedSymbols.has(c.symbol),
  );

  const formingCandlesM = Array.from(state.formingCandlesM.values()).filter(
    (c) => changedSymbols.has(c.symbol),
  );

  const prices = Array.from(state.priceCache.values()).filter((p) =>
    changedSymbols.has(p.symbol),
  );
  ...
  try {
    ...
    if (Math.random() < 0.01) {
      console.warn(
        "⚠️ [Broadcast] Failed to send to WebSocket server:",
        error instanceof Error ? error.message : error,
      );
    }
  }
    ...
    for (const symbol of symbols) {
      try {
        ...
        const aggregateForPeriod = (periodStart: number, periodEnd: number) => {
          const periodCandles = candles1m.filter(
            (c) => c.t >= periodStart && c.t < periodEnd,
          );
          if (periodCandles.length === 0) return null;
