...
export async function generateInvoicePDF(
  ...
  try {
    ...
    const invoiceNumWidth = helvetica.widthOfTextAtSize(
    ...
    if (invoice.companyAddress) {
      ...
      const cityPostal =
        `${invoice.companyAddress.city || ""} ${invoice.companyAddress.postalCode || ""}`.trim();
      if (cityPostal) {
    ...
    if (invoice.customerAddress) {
      ...
      const customerCityPostal =
        `${invoice.customerAddress.city || ""} ${invoice.customerAddress.postalCode || ""}`.trim();
    ...
    if (invoice.thankYouMessage) {
      const thankYouWidth = helveticaBold.widthOfTextAtSize(
