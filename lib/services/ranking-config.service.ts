...
export const RANKING_CONFIGS: Record<RankingMethod, RankingConfig> = {
  ...
  profit_factor: {
    ...
    getMetricValue: (p) => {
      const totalWins =
        p.winningTrades > 0 ? p.averageWin * p.winningTrades : 0;
      const totalLosses =
        p.losingTrades > 0 ? Math.abs(p.averageLoss * p.losingTrades) : 0;
...
export function validateRankingConfigs(): boolean {
  const requiredMethods: RankingMethod[] = [
    "pnl",
    "roi",
    "total_capital",
    "win_rate",
  ...
  return requiredMethods.every((method) => {
    ...
    return (
      config &&
      config.name &&
      typeof config.getMetricValue === "function" &&
