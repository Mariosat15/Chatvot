/**
 * Payout Provider Registry (single source of truth)
 *
 ...
 *   2. Implement a payout adapter (see lib/services/payout/payout-adapter.ts
 *      in the admin app) and register it.
 *
 * No withdrawal route, settings UI or model has to be rewritten — they all
 * read from this registry. This module is intentionally free of any
 * server-only imports so it can also be imported by client components.
 *
 * Reason: previously "Nuvei" was hard-coded across the withdrawal routes,
 * making it impossible to add a second payout rail without editing every
 * file. Centralising capabilities here removes that coupling.
 */
...
export interface PayoutProviderDefinition {
   ...
   * the enable signal. Reason: keeps backward compatibility with the existing
   * `nuveiWithdrawalEnabled` toggle.
   */
  enableFlag?: string;
}
