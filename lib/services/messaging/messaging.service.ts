...
async function broadcastToWebSocket(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[WS Broadcast] Failed to broadcast ${endpoint}:`,
        response.status,
      );
    }
  } catch (error) {
    ...
    console.warn(
      `[WS Broadcast] Error broadcasting ${endpoint}:`,
...
export class MessagingService {
  ...
  static async createConversation(params: {
    ...
    const conversation = await Conversation.create({
      ...
      assignedEmployeeId: params.assignedEmployeeId
        ? new Types.ObjectId(params.assignedEmployeeId)
        : undefined,
  ...
  static async getOrCreateSupportConversation(
    ...
    try {
      ...
      console.error(
        "❌ [MessagingService] Error checking customer assignment:",
        error,
      );
    }
    ...
    let conversation = await Conversation.findOne({
      ...
      $or: [{ isResolved: { $ne: true } }, { isResolved: { $exists: false } }],
    });
  ...
  static async sendMessage(params: {
    ...
    if (moderatedContent.length > settings.maxMessageLength) {
      throw new Error(
        `Message too long. Maximum ${settings.maxMessageLength} characters allowed.`,
      );
    }
    ...
    try {
      ...
      console.warn(
        "[MessagingService] Failed to stop typing indicator:",
  ...
  static async sendFriendRequest(
    ...
    const hasPending = await (FriendRequest as any).hasPendingRequest(
      fromUser.id,
      toUser.id,
    );
    if (hasPending) {
    ...
    if (requestsToday >= settings.friendRequestsPerDayLimit) {
      throw new Error(
        "Daily friend request limit reached. Please try again tomorrow.",
      );
    }
  ...
  static async blockUser(
    ...
    let friendship = await (Friendship as any).getFriendship(
      userId,
      blockedUserId,
    );
  ...
  static async unblockUser(
    ...
    const friendship = await (Friendship as any).getFriendship(
      userId,
      blockedUserId,
    );
  ...
  static async transferConversation(
    ...
    reason?: string,
    _transferredBy?: { id: string; name: string },
  ): Promise<IConversation> {
    await connectToDatabase();
    ...
    if (previousEmployeeId) {
      const oldParticipant = conversation.participants.find(
        (p: { id: string; type: string }) =>
          p.id === previousEmployeeId && p.type === "employee",
      );
      if (oldParticipant) {
    ...
    if (!conversation.metadata.transferHistory)
  ...
  static async escalateFromAI(
    ...
    console.log(
      `🤖→👤 [escalateFromAI] Conversation: ${conversationId}, Reason: ${reason}`,
    );
    ...
    if (assignedEmployee) {
      ...
      const isParticipant = conversation.participants.some(
        (p: { id: string; type: string }) =>
          p.id === assignedEmployee!.id && p.type === "employee",
      );
    ...
    const aiParticipant = conversation.participants.find(
      (p: { id: string }) => p.id === "ai-assistant",
    );
    if (aiParticipant) {
  ...
  static async setPresence(
    ...
    if (status === "offline") {
      ...
      await (UserPresence as any).setOnline(
        participantId,
        participantType,
        deviceInfo,
      );
  ...
  static async setTyping(
    ...
    try {
      ...
      console.warn(
        "[MessagingService] setTyping error:",
        err instanceof Error ? err.message : err,
      );
    }
  }
  ...
  static async stopTyping(participantId: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "[MessagingService] stopTyping error:",
        err instanceof Error ? err.message : err,
      );
    }
  }
  ...
  static async heartbeat(participantId: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "[MessagingService] heartbeat error:",
        err instanceof Error ? err.message : err,
      );
    }
  }
  ...
  static async searchMessages(
    ...
    await connectToDatabase();
    return (Message as any).searchMessages(participantId, searchTerm, {
      limit,
    });
  }
