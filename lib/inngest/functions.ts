  ...
  async ({ event, step }) => {
    ...
    const settings = await step.run("get-settings", async () => {
      ...
      return {
        ...
        openaiApiKey:
          whitelabel?.openaiApiKey || process.env.OPENAI_API_KEY || "",
    ...
    if (shouldUseAI) {
      ...
      const prompt = settings.aiPersonalizationPrompt.replace(
        "{{userProfile}}",
        userProfile,
      );
      ...
      try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a friendly email copywriter. Write engaging, personalized welcome email content. Keep it concise (2-3 sentences max).",
              },
              {
                role: "user",
                content: prompt,
        ...
        console.error(
          "⚠️ AI personalization failed, using default intro:",
          error,
        );
...
export const updateCompetitionStatuses = inngest.createFunction(
  ...
  async ({ step }) => {
    return await step.run("update-statuses", async () => {
      const { checkAndFinalizeCompetitions } =
      ...
      const upcomingCompetitions = await Competition.find({
        status: "upcoming",
      }).select("name startTime participants");
      ...
      const _fifteenMinutesFromNow = new Date(now.getTime() + 15 * 60 * 1000);
      ...
      if (startingSoonCompetitions.length > 0) {
        try {
          ...
          for (const comp of startingSoonCompetitions) {
            ...
            console.log(
              `🔔 Sent competition_starting_soon to ${participants.length} participants for ${comp.name}`,
            );
          }
        } catch (notifError) {
          console.error(
            "Error sending starting soon notifications:",
            notifError,
          );
        }
      }
      ...
      const activeCompetitions = await Competition.find({
        status: "active",
      }).select("_id name endTime participants");
      ...
      if (endingSoonCompetitions.length > 0) {
        try {
          ...
          for (const comp of endingSoonCompetitions) {
            ...
            console.log(
              `🔔 Sent competition_ending_soon to ${participants.length} participants for ${comp.name}`,
            );
          }
        } catch (notifError) {
      ...
      for (const comp of competitionsToStart) {
        const participantCount =
          comp.currentParticipants || comp.participants?.length || 0;
        ...
        if (participantCount < minRequired) {
          ...
          startedCount++;
          console.log(
            `✅ Started "${comp.name}" with ${participantCount} participants`,
          );
        }
      }

      _updatedCount += startedCount;
      ...
      try {
        const { finalizeEndedChallenges, expirePendingChallenges } =
        ...
        if (challengesExpired > 0 || challengesFinalized > 0) {
          console.log(
            `⚔️ Challenge status update: ${challengesExpired} expired, ${challengesFinalized} finalized`,
          );
        }
      ...
      console.log(
        `✅ Competition status update: ${startedCount} started, ${cancelledCount} cancelled, finalization result:`,
        finalizationResult,
      );
...
export const monitorMarginLevels = inngest.createFunction(
  ...
  async ({ step }) => {
    return await step.run("check-margin-levels", async () => {
      ...
      console.log(
        `🔄 Starting margin monitoring with ${checkIntervalSeconds}s interval`,
      );

      // Calculate how many checks to perform in this minute
      const checksPerMinute = Math.max(
        1,
        Math.floor(60 / checkIntervalSeconds),
      );
      ...
      const Challenge = (
        await import("@/database/models/trading/challenge.model")
      ).default;
      ...
      for (let i = 0; i < checksPerMinute; i++) {
        ...
        if (i < checksPerMinute - 1) {
          await new Promise((resolve) =>
            setTimeout(resolve, actualInterval * 1000),
          );
        }
      }
...
export const evaluateUserBadges = inngest.createFunction(
  ...
  { cron: "0 * * * *" }, // Every hour
  async () => {
    const { evaluateUserBadges: evaluateBadges } =
      await import("@/lib/services/badge-evaluation.service");
    const { default: CompetitionParticipant } =
    ...
    try {
      ...
      console.log(
        `✅ Badge evaluation complete: ${totalNewBadges} new badges awarded to ${usersWithNewBadges} users`,
      );
...
export const updatePriceCache = inngest.createFunction(
  ...
  async () => {
    const { getAllCachedPrices } =
      await import("@/lib/services/websocket-price-streamer");
    const { fetchRealForexPrices } =
      await import("@/lib/services/real-forex-prices.service");
    const { setPrices, getRedis } =
      await import("@/lib/services/redis.service");
    type ForexSymbolType =
      import("@/lib/services/pnl-calculator.service").ForexSymbol;
    ...
    try {
      ...
      console.log(
        `📊 WebSocket: Connected=${status.connected}, Auth=${status.authenticated}, Subscribed=${status.subscribed}, Pairs=${status.cachedPairs}`,
      );
      ...
      if (status.connected && status.authenticated) {
        ...
        const { WhiteLabel } =
          await import("@/database/models/whitelabel.model");
        await connectToDatabase();
        const settings = await WhiteLabel.findOne();
        const redisSyncEnabled =
          ...
          if (redis) {
            ...
            if (wsPrices.size > 0) {
              const redisPrices = new Map<
                string,
                { bid: number; ask: number; mid: number; timestamp: number }
              >();
              wsPrices.forEach((quote, symbol) => {
                redisPrices.set(symbol, {
                  bid: quote.bid,
                  ask: quote.ask,
                  mid: quote.mid,
              ...
              console.log(
                `✅ WebSocket prices synced to Redis: ${status.cachedPairs} pairs (Multi-server mode)`,
              );
            }
          }
        } else {
          console.log(
            `✅ WebSocket prices live: ${status.cachedPairs} pairs (Single-server mode)`,
          );
        }
      ...
      console.log(
        `💰 REST API fallback: ${prices.size} pairs loaded to memory (Redis sync DISABLED)`,
      );
...
export const processTradeQueue = inngest.createFunction(
  ...
  async () => {
    ...
    if (!redis) {
      return {
        success: true,
        message: "Redis not enabled, skipping queue processing",
      };
    }
    ...
    for (let i = 0; i < maxBatchSize; i++) {
      ...
      try {
        if (trade.action === "close") {
          ...
          const closeReason = trade.data.closeReason as
            | "stop_loss"
            | "take_profit"
            | "margin_call";
    ...
    if (processed > 0 || failed > 0) {
      console.log(
        `📋 Trade queue: ${processed} processed, ${failed} failed, ${stats.pending} pending`,
      );
    }
...
export const sendInvoiceEmailJob = inngest.createFunction(
  ...
  async ({ event, step }) => {
    const { invoiceId, customerEmail, customerName, invoiceNumber } =
      event.data;

    console.log(
      `📧 [INVOICE] Sending invoice email for ${invoiceNumber} to ${customerEmail}`,
    );
    ...
    await step.run("send-invoice-email", async () => {
      try {
        ...
        console.log(
          `✅ [INVOICE] Email sent successfully for ${invoiceNumber}`,
        );
