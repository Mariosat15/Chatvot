...
exports.TITLE_LEVELS = exports.BADGE_XP_VALUES = void 0;
...
function getTitleByXP(xp) {
    for (let i = exports.TITLE_LEVELS.length - 1; i >= 0; i--) {
        if (xp >= exports.TITLE_LEVELS[i].minXP) {
            return exports.TITLE_LEVELS[i];
        }
    }
    return exports.TITLE_LEVELS[0];
}
/**
 * Get next title level
 */
function getNextTitle(currentLevel) {
    if (currentLevel >= 10)
        return null;
    return exports.TITLE_LEVELS[currentLevel]; // currentLevel is 1-based, array is 0-based, so [currentLevel] gives next
}
/**
 ...
 */
function getXPForBadge(rarity) {
    return exports.BADGE_XP_VALUES[rarity];
}
