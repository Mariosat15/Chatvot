...
async function checkCompetitionRiskLimits(
  ...
  console.log(
    `📊 [RISK] Checking limits - Starting: $${startingCapital}, Current: $${currentCapital}`,
  );
  ...
  const currentDrawdownPercent =
    ((startingCapital - currentCapital) / startingCapital) * 100;

  console.log(
    `   Max Drawdown: ${maxDrawdownPercent}% | Current Drawdown: ${currentDrawdownPercent.toFixed(2)}%`,
  );
  console.log(
    `   Balance Threshold: $${maxDrawdownThreshold.toFixed(2)} | Current: $${currentCapital.toFixed(2)}`,
  );

  if (currentCapital <= maxDrawdownThreshold) {
    console.log(
      `🛑 [RISK] Max drawdown exceeded: Balance $${currentCapital.toFixed(2)} <= Threshold $${maxDrawdownThreshold.toFixed(2)} (${maxDrawdownPercent}% limit)`,
    );
    return {
      allowed: false,
  ...
  const todaysRealizedPnL = todaysClosedPositions.reduce(
    (sum, pos) => sum + (pos.pnl || 0),
    0,
  );
  const dailyLossPercent =
    todaysRealizedPnL < 0
      ? (Math.abs(todaysRealizedPnL) / startingCapital) * 100
      : 0;

  // Daily loss threshold amount (as positive number for comparison)
  const dailyLossThresholdAmount =
    (startingCapital * dailyLossLimitPercent) / 100;

  console.log(
    `   Daily Loss Limit: ${dailyLossLimitPercent}% ($${dailyLossThresholdAmount.toFixed(2)})`,
  );
  console.log(
    `   Today's Realized PnL: $${todaysRealizedPnL.toFixed(2)} (${dailyLossPercent.toFixed(2)}% loss)`,
  );

  if (
    todaysRealizedPnL < 0 &&
    Math.abs(todaysRealizedPnL) >= dailyLossThresholdAmount
  ) {
    console.log(
      `🛑 [RISK] Daily loss limit exceeded: Today's loss $${Math.abs(todaysRealizedPnL).toFixed(2)} >= Limit $${dailyLossThresholdAmount.toFixed(2)}`,
    );
    return {
      allowed: false,
  ...
  if (competition.riskLimits?.equityCheckEnabled) {
    ...
    if (openPositions.length > 0) {
      ...
      for (const position of openPositions) {
        try {
          ...
          if (currentPrice) {
            const exitPrice =
              position.side === "buy" ? currentPrice.bid : currentPrice.ask;
            const priceDiff =
              position.side === "buy"
          ...
          console.warn(
            `Could not get price for ${position.symbol}, using last known PnL`,
          );
    ...
    console.log(
      `📊 [RISK] Equity check: Balance=${currentCapital.toFixed(2)}, Unrealized=${totalUnrealizedPnL.toFixed(2)}, Equity=${currentEquity.toFixed(2)}, Threshold=${equityThreshold.toFixed(2)}`,
    );

    if (currentEquity <= equityThreshold) {
      const equityLossPercent = (
        (1 - currentEquity / startingCapital) *
        100
      ).toFixed(1);
      console.log(
        `🛑 [RISK] Equity drawdown exceeded: Equity ${currentEquity.toFixed(2)} <= Threshold ${equityThreshold.toFixed(2)} (${equityDrawdownPercent}% of starting ${startingCapital})`,
      );
      return {
        allowed: false,
  ...
  console.log(
    `✅ [RISK] Risk limits check passed - Drawdown: ${(((startingCapital - currentCapital) / startingCapital) * 100).toFixed(1)}%/${maxDrawdownPercent}%, Daily Loss: $${Math.abs(todaysRealizedPnL).toFixed(2)}/$${dailyLossThresholdAmount.toFixed(2)}`,
  );
...
export const placeOrder = async (params: {
  ...
  try {
    ...
    console.log(
      `⏰ Checking market status for placing ${params.orderType} order...`,
    );
    ...
    if (!restrictionCheck.allowed) {
      ...
      throw new Error(
        restrictionCheck.reason || "You are not allowed to trade",
      );
    }
    ...
    const contestData = await getContestAndParticipant(
      competitionId,
      session.user.id,
    );
    if (!contestData) {
    ...
    if (contestData.status !== "active") {
      const contestName =
        contestType === "competition" ? "Competition" : "Challenge";
      if (contestData.status === "completed") {
        throw new Error(
          `${contestName} has ended. You can no longer place trades.`,
        );
      } else if (contestData.status === "cancelled") {
        throw new Error(
          `${contestName} was cancelled. Trading is not available.`,
        );
    ...
    if (contestType === "competition" && contest.isPaused) {
      ...
      throw new Error(
        `Competition is paused (${pauseReason}). Trading is temporarily suspended. Please wait for the competition to resume.`,
      );
    }
    ...
    if (participant.status !== "active") {
      ...
      if (participant.status === "liquidated") {
        ...
        throw new Error(
          `⛔ You have been DISQUALIFIED from this ${contestName.toLowerCase()} due to liquidation (${reason}). You can no longer trade.`,
        );
      } else if (participant.status === "disqualified") {
        const reason = participant.disqualificationReason || "rule violation";
        throw new Error(
          `⛔ You have been DISQUALIFIED from this ${contestName.toLowerCase()}. Reason: ${reason}. You can no longer trade.`,
        );
      } else if (participant.status === "completed") {
        throw new Error(
          `This ${contestName.toLowerCase()} has ended for you. You can no longer place trades.`,
        );
      } else if (participant.status === "refunded") {
        throw new Error(
          `Your entry fee was refunded. You are no longer participating in this ${contestName.toLowerCase()}.`,
        );
      }

      throw new Error(
        `Your participation status is "${participant.status}". You cannot trade in this ${contestName.toLowerCase()}.`,
      );
    }
    ...
    if (contestData.riskLimits?.enabled) {
      const riskLimitCheck = await checkCompetitionRiskLimits(
        contest,
        participant,
      );
      if (!riskLimitCheck.allowed) {
        console.log(
          `   ❌ Trade blocked due to risk limits: ${riskLimitCheck.reason}`,
        );
    ...
    if (
      orderType === "market" &&
      lockedPrice &&
      Date.now() - lockedPrice.timestamp < MAX_PRICE_AGE_MS
    ) {
      ...
      console.log(
        `   Locked BID: ${lockedPrice.bid.toFixed(5)} (${side === "sell" ? "← WILL USE" : ""})`,
      );
      console.log(
        `   Locked ASK: ${lockedPrice.ask.toFixed(5)} (${side === "buy" ? "← WILL USE" : ""})`,
      );
      ...
      console.log(
        `   ✅ Execution price: ${executionPrice.toFixed(5)} (${side === "buy" ? "ASK" : "BID"}) 🔒 LOCKED`,
      );
    } else {
      ...
      if (lockedPrice) {
        console.log(
          `   ⚠️ Locked price expired (age: ${Date.now() - lockedPrice.timestamp}ms)`,
        );
      }
      ...
      console.log(
        `   BID: ${freshPrice.bid.toFixed(5)} (${side === "sell" ? "← WILL USE" : ""})`,
      );
      console.log(
        `   ASK: ${freshPrice.ask.toFixed(5)} (${side === "buy" ? "← WILL USE" : ""})`,
      );
      console.log(`   Spread: ${spreadPips} pips`);
      console.log(
        `   Timestamp: ${new Date(freshPrice.timestamp).toISOString()}`,
      );
      ...
      console.log(
        `✅ [ORDER] Execution price: ${executionPrice.toFixed(5)} (${side === "buy" ? "ASK" : "BID"})`,
      );
    }
    ...
    try {
      ...
      if (orderType === "market") {
        ...
        PriceLog.create({
          ...
          console.warn(
            "⚠️ Failed to create price log (non-critical):",
            logError,
          );
        });
        ...
        console.log(
          `   Entry Price: ${executionPrice.toFixed(5)} (${side === "buy" ? "ASK" : "BID"})`,
        );
        console.log(
          `   Bid/Ask at Entry: ${currentPriceQuote.bid.toFixed(5)} / ${currentPriceQuote.ask.toFixed(5)}`,
        );
        console.log(
          `   Spread: ${(currentPriceQuote.spread / pipSize).toFixed(2)} pips`,
        );
        ...
        console.log(
          `   Previous Available: $${participant.availableCapital.toFixed(2)}`,
        );
        console.log(
          `   New Available: $${(participant.availableCapital - marginRequired).toFixed(2)} (Margin Locked 🔒)`,
        );
        ...
        console.log(
          `✅ Limit order placed: ${side} ${quantity} ${symbol} @ ${limitPrice}`,
        );
      }
        ...
        import("@/lib/services/notification.service")
          .then(({ notificationService }) => {
            notificationService
              ...
              .catch((notifError) => {
                console.error(
                  "Error sending order filled notification:",
                  notifError,
                );
              });
          })
    ...
    // Re-throw Next.js redirect/not-found signals — they must propagate
    if (
      typeof error === "object" &&
      error !== null &&
      "digest" in error &&
...
export const cancelOrder = async (orderId: string) => {
  try {
    ...
    console.log(
      `🔐 Checking trading restrictions for user ${session.user.id} (cancel order)`,
    );
    const { canUserPerformAction } =
      await import("@/lib/services/user-restriction.service");
    const restrictionCheck = await canUserPerformAction(
    ...
    if (!restrictionCheck.allowed) {
      ...
      throw new Error(
        restrictionCheck.reason || "You are not allowed to cancel orders",
      );
    }
    ...
    console.log(
      `   💡 Note: No margin was locked (limit orders don't lock margin until execution)`,
    );
    ...
    console.error("Error cancelling order:", msg);
    return { success: false as const, error: msg };
  }
};
        ...
        try {
          ...
          await CompetitionParticipant.findByIdAndUpdate(
            participant._id,
            {
              ...
              $set: {
                availableCapital:
                  participant.availableCapital - order.marginRequired,
          ...
          console.log(
            `✅ Limit order executed: ${order.symbol} @ ${marketPrice}`,
          );
        } catch (error) {
          await mongoSession.abortTransaction();
