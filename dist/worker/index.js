...
const margin_check_job_1 = require("./jobs/margin-check.job");
const competition_end_job_1 = require("./jobs/competition-end.job");
const challenge_finalize_job_1 = require("./jobs/challenge-finalize.job");
const trade_queue_job_1 = require("./jobs/trade-queue.job");
const price_cache_job_1 = require("./jobs/price-cache.job");
const evaluate_badges_job_1 = require("./jobs/evaluate-badges.job");
...
agenda.define('margin-check', async () => {
    ...
    try {
        const result = await (0, margin_check_job_1.runMarginCheck)();
...
agenda.define('competition-end', async () => {
    ...
    try {
        const result = await (0, competition_end_job_1.runCompetitionEndCheck)();
        const duration = Date.now() - startTime;
...
agenda.define('challenge-finalize', async () => {
    ...
    try {
        const result = await (0, challenge_finalize_job_1.runChallengeFinalizeCheck)();
        const duration = Date.now() - startTime;
...
agenda.define('trade-queue', async () => {
    const startTime = Date.now();
    try {
        const result = await (0, trade_queue_job_1.runTradeQueueProcessor)();
        const duration = Date.now() - startTime;
...
agenda.define('price-cache', async () => {
    try {
        const result = await (0, price_cache_job_1.runPriceCacheUpdate)();
...
agenda.define('evaluate-badges', async () => {
    ...
    try {
        const result = await (0, evaluate_badges_job_1.runBadgeEvaluation)();
        const duration = Date.now() - startTime;
