...
exports.getMatchableTraders = getMatchableTraders;
exports.findBestMatch = findBestMatch;
exports.getRankedMatches = getRankedMatches;
const mongoose_1 = require("@/database/mongoose");
const user_presence_model_1 = __importDefault(require("@/database/models/user-presence.model"));
const global_leaderboard_actions_1 = require("@/lib/actions/leaderboard/global-leaderboard.actions");
/**
...
function calculateTraderLevel(stats) {
    ...
    const activityScore = (totalTrades * 1) +
 ...
 */
function calculateMatchScore(trader1, trader2) {
    const reasons = [];
 ...
 */
function leaderboardEntryToMatchableTrader(entry, presence) {
 ...
 */
async function getMatchableTraders(currentUserId) {
    await (0, mongoose_1.connectToDatabase)();
    // Get leaderboard data - this has all the pre-calculated stats and scores
    const leaderboardData = await (0, global_leaderboard_actions_1.getGlobalLeaderboard)();
    // Get online status for all users
    const onlineStatuses = await user_presence_model_1.default.find({}).lean();
 ...
 */
async function findBestMatch(currentUserId) {
    await (0, mongoose_1.connectToDatabase)();
    ...
    const matches = tradersToMatch.map(trader => {
 ...
 */
async function getRankedMatches(currentUserId, limit = 50) {
    await (0, mongoose_1.connectToDatabase)();
    // Get leaderboard data
    const leaderboardData = await (0, global_leaderboard_actions_1.getGlobalLeaderboard)();
    // Find current user in leaderboard
    ...
    const matches = traders.map(trader => {
        const { score, reasons } = calculateMatchScore(currentUser, trader);
        return {
            trader: { ...trader, matchScore: score },
            matchScore: score,
