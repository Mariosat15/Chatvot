...
exports.generateStrategySignals = generateStrategySignals;
exports.getSignalColor = getSignalColor;
exports.getSignalLabel = getSignalLabel;
...
const getIndicatorValue = (indicator, params, candles, index, calculatedIndicators) => {
...
const evaluateCondition = (condition, candles, index, calculatedIndicators) => {
    const value = getIndicatorValue(condition.indicator, condition.indicatorParams, candles, index, calculatedIndicators);
    if (isNaN(value))
        return false;
    let compareValue;
    if (condition.compareWith === 'value') {
        ...
        compareValue = getIndicatorValue(condition.compareIndicator || 'sma', condition.compareIndicatorParams, candles, index, calculatedIndicators);
    }
    ...
    switch (condition.operator) {
        ...
        case 'crosses_above':
            if (index === 0)
                return false;
            const prevValue = getIndicatorValue(condition.indicator, condition.indicatorParams, candles, index - 1, calculatedIndicators);
            const prevCompare = condition.compareWith === 'value'
                ? (condition.compareValue || 0)
                : getIndicatorValue(condition.compareIndicator || 'sma', condition.compareIndicatorParams, candles, index - 1, calculatedIndicators);
            return prevValue <= prevCompare && value > compareValue;
        case 'crosses_below':
            if (index === 0)
                return false;
            const prevVal = getIndicatorValue(condition.indicator, condition.indicatorParams, candles, index - 1, calculatedIndicators);
...
const evaluateRule = (rule, candles, index, calculatedIndicators) => {
    if (rule.conditions.length === 0)
        return false;
    if (rule.logic === 'AND') {
        return rule.conditions.every(condition => evaluateCondition(condition, candles, index, calculatedIndicators));
    }
    else {
        return rule.conditions.some(condition => evaluateCondition(condition, candles, index, calculatedIndicators));
    }
};
/**
 * Generate signals for a strategy configuration
 */
function generateStrategySignals(config, candles) {
    const signals = [];
    const calculatedIndicators = new Map();
