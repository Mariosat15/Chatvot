...
exports.distributePrizesWithTies = distributePrizesWithTies;
...
function getRankingValue(participant, method) {
    switch (method) {
        ...
        case 'profit_factor':
            ...
            if (totalLosses === 0)
                return totalWins > 0 ? 9999 : 0; // Infinity if all wins
 ...
 */
function getTieBreakerValue(participant, tieBreaker) {
    switch (tieBreaker) {
        case 'trades_count':
 ...
 */
function areParticipantsTied(a, b, rules) {
    ...
    if (rules.tieBreaker1 && rules.tieBreaker1 !== 'split_prize') {
        ...
        if (Math.abs(aTie1 - bTie1) >= epsilon)
            return false;
    }
    ...
    if (rules.tieBreaker2 && rules.tieBreaker2 !== 'split_prize') {
        ...
        if (Math.abs(aTie2 - bTie2) >= epsilon)
            return false;
    }
 ...
 */
function checkQualification(participant, rules, options) {
 ...
 */
function calculateRankings(participants, rules, options) {
    ...
    const qualifiedParticipants = participants.map((p) => {
        ...
        return {
            ...
            isTied: false,
            tiedWith: [], // Track tied participants
 ...
 */
function distributePrizesWithTies(rankedParticipants, prizeDistribution, grossPrizePool, // CHANGED: Now receives GROSS prize pool
rules, platformFeePercentage = 0 // NEW: Platform fee to deduct from each prize
) {
    const distributions = [];
    ...
    filledPrizePositions.forEach((pos) => {
        ...
        if (winnersCount === 1) {
            ...
            if (rules.tiePrizeDistribution === 'split_equally') {
                ...
                const sorted = winnersAtRank.sort((a, b) => new Date(a.enteredAt).getTime() - new Date(b.enteredAt).getTime());
 ...
 */
function getTieBreakerDescription(tieBreaker) {
    const descriptions = {
