...
exports.CoordinationDetectionService = void 0;
...
class CoordinationDetectionService {
     ...
     */
    static async detectCoordinatedEntry(competitionId, recentEntries) {
        ...
        // Sort by entry time
        const sortedEntries = [...recentEntries].sort((a, b) => new Date(a.entryTime).getTime() - new Date(b.entryTime).getTime());
        // Group entries within the coordination window
        const coordinatedGroups = [];
        let currentGroup = [sortedEntries[0]];
        ...
        const largestGroup = coordinatedGroups.reduce((max, group) => group.length > max.length ? group : max, coordinatedGroups[0]);
     ...
     */
    static async findNearbyEntries(userId, competitionId, entryTime, allEntries) {
        await (0, mongoose_1.connectToDatabase)();
        ...
        return {
            ...
            timeDiffs: nearbyEntries.map(e => (new Date(e.joinedAt).getTime() - entryTimeMs) / 1000)
        };
    }
    /**
     * Track competition entry pattern for a user
     */
    static async trackCompetitionEntryPattern(userId, competitionId, entryTime) {
        await (0, mongoose_1.connectToDatabase)();
        ...
        if (!profile) {
            ...
            await trading_behavior_profile_model_1.default.create({
                userId,
     ...
     */
    static async detectRapidAccountCreation(accounts) {
        await (0, mongoose_1.connectToDatabase)();
        const ONE_HOUR_MS = 60 * 60 * 1000;
        // Sort by creation time
        const sorted = [...accounts].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
        const groups = [];
     ...
     */
    static async analyzeEntryTimingPatterns(competitionId, entries) {
        await (0, mongoose_1.connectToDatabase)();
        const coordinatedGroups = [];
        ...
        for (let i = 0; i < sorted.length - 1; i++) {
            ...
            if (timeDiff <= rapidFireThreshold) {
                ...
                const existingGroupIndex = coordinatedGroups.findIndex(g => g.includes(sorted[i].userId));
        ...
        const uniquePatterns = suspiciousPatterns.filter((pattern, index, self) => index === self.findIndex(p => p.type === pattern.type &&
...
exports.CoordinationDetectionService = CoordinationDetectionService;
// Time window for coordinated entry detection (5 minutes)
CoordinationDetectionService.COORDINATION_WINDOW_MINUTES = 5;
// Minimum accounts for coordinated entry alert
CoordinationDetectionService.MIN_COORDINATED_ACCOUNTS = 2;
