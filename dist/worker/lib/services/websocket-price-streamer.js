...
exports.updateCacheFromRest = updateCacheFromRest;
exports.closeWebSocket = closeWebSocket;
exports.resetWebSocket = resetWebSocket;
 ...
 */
function checkTPSLOnPriceUpdate(symbol, bid, ask) {
    const now = Date.now();
    const lastCheck = lastTPSLCheck.get(symbol) || 0;
    // Throttle checks per symbol (500ms minimum between checks)
    if (now - lastCheck < TPSL_CHECK_THROTTLE_MS)
        return;
    lastTPSLCheck.set(symbol, now);
    // Fire and forget - don't block price updates
    Promise.resolve().then(() => __importStar(require('./tpsl-realtime.service'))).then(({ checkTPSLForSymbol }) => checkTPSLForSymbol(symbol, bid, ask))
        .catch(() => {
 ...
 */
function queuePriceForMongoCache(symbol, bid, ask, timestamp) {
 ...
 */
async function flushPricesToMongo() {
    mongoPriceWriteTimer = null;
    if (pendingPriceUpdates.size === 0)
        return;
    ...
    try {
        ...
        const PriceCache = (await Promise.resolve().then(() => __importStar(require('@/database/models/price-cache.model')))).default;
...
async function initializeWebSocket() {
    ...
    try {
        const { initializeTPSLCache } = await Promise.resolve().then(() => __importStar(require('./tpsl-realtime.service')));
...
function subscribeToFeeds() {
    const state = getState();
    const ws = state.ws;
    if (!ws || ws.readyState !== 1 || !state.isAuthenticated)
        return;
 ...
 */
function updateCacheFromRest(symbol, quote) {
...
function isAdminProcess() {
    ...
    const args = process.argv.join(' ').toLowerCase();
    if (args.includes('apps/admin') || args.includes('apps\\admin'))
        return true;
