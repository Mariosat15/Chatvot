...
exports.generateTradingPatterns = generateTradingPatterns;
exports.analyzeSimulationResults = analyzeSimulationResults;
exports.generateTestScenario = generateTestScenario;
exports.analyzeFailureRootCause = analyzeFailureRootCause;
 ...
 */
async function generateTradingPatterns(type) {
 ...
 */
async function analyzeSimulationResults(run) {
...
function generateBasicAnalysis(data) {
    ...
    const avgResponse = parseFloat(data.avgResponseTime) || 0;
    const p95Response = parseFloat(data.p95ResponseTime) || 0;
    const p99Response = parseFloat(data.p99ResponseTime) || 0;
    const maxResponse = parseFloat(data.maxResponseTime) || 0;
    const rps = parseFloat(data.requestsPerSecond) || 0;
    const cpuUsage = parseFloat(data.peakCpuUsage) || 0;
    const memUsage = parseFloat(data.peakMemoryUsage) || 0;
    const totalRequests = data.totalRequests || 0;
    const duration = (data.duration || 0) / 1000;
    const testCases = data.testCases || [];
    const usersCreated = data.usersCreated || 0;
    const tradesExecuted = data.tradesExecuted || 0;
    const competitionsCreated = data.competitionsCreated || 0;
    const challengesCreated = data.challengesCreated || 0;
    // ===== ERROR RATE ANALYSIS =====
    if (errorRate > 10) {
        findings.push({
            type: 'error',
 ...
 */
async function generateTestScenario(description) {
    const openai = await getOpenAIClient();
    if (!openai) {
 ...
 */
async function analyzeFailureRootCause(errorLogs) {
    const openai = await getOpenAIClient();
    if (!openai) {
        return {
