...
exports.detectVPNProxy = detectVPNProxy;
exports.isHighRiskIP = isHighRiskIP;
 ...
 */
function isSuspiciousProvider(isp, org, asn) {
    const combinedText = `${isp} ${org} ${asn}`.toLowerCase();
    return SUSPICIOUS_ISPS.some(suspicious => combinedText.includes(suspicious.toLowerCase())) || HOSTING_ASNS.some(hostingAsn => asn.includes(hostingAsn));
}
/**
 * Calculate risk score based on detection results
 */
function calculateRiskScore(isVPN, isProxy, isTor, isHosting) {
    let score = 0;
    if (isTor)
        score += 50; // Tor is highest risk
    if (isVPN)
        score += 30; // VPN is medium-high risk
    if (isProxy)
        score += 25; // Proxy is medium risk
    if (isHosting)
        score += 20; // Datacenter IP is medium-low risk
 ...
 */
async function detectVPNProxy(ipAddress) {
    ...
    try {
        ...
        const response = await fetch(`http://ip-api.com/json/${ipAddress}?fields=status,message,country,countryCode,region,city,timezone,isp,org,as`, {
            method: 'GET',
            headers: {
        ...
        const isVPN = isSuspicious && (isp.toLowerCase().includes('vpn') ||
            org.toLowerCase().includes('vpn'));
        const isProxy = isSuspicious && (isp.toLowerCase().includes('proxy') ||
            org.toLowerCase().includes('proxy') ||
            isp.toLowerCase().includes('anonymizer'));
        const isTor = isSuspicious && (isp.toLowerCase().includes('tor') ||
 ...
 */
function isHighRiskIP(detection) {
    // Tor is always high risk
    if (detection.isTor)
        return true;
    // VPN + Hosting is suspicious (common cheater setup)
    if (detection.isVPN && detection.isHosting)
        return true;
    // Multiple indicators
    const indicators = [
        detection.isVPN,
