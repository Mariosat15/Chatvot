...
exports.updateCachedPrice = updateCachedPrice;
exports.initializeMarketData = initializeMarketData;
exports.updateCachedSpread = updateCachedSpread;
exports.getCurrentPrice = getCurrentPrice;
exports.getCurrentPrices = getCurrentPrices;
exports.subscribeToPriceUpdates = subscribeToPriceUpdates;
...
function getBasePrice(symbol) {
    ...
    if (base === 'USD') {
        // USD/XXX pairs
        if (['CAD', 'CHF', 'SGD'].includes(quote))
            return 1.3;
        if (['MXN', 'ZAR', 'TRY', 'SEK', 'NOK'].includes(quote))
            return 15; // Exotics
        return 1.0;
    }
    if (quote === 'USD') {
...
function initializeMarketData() {
    ...
    Object.keys(pnl_calculator_service_1.FOREX_PAIRS).forEach((symbol) => {
...
const dynamicSpreadCache = new Map();
/**
...
async function fetchRealPrices() {
    ...
    const symbols = Object.keys(pnl_calculator_service_1.FOREX_PAIRS);
 ...
 */
function subscribeToPriceUpdates(symbol, callback) {
 ...
 */
async function getHistoricalCandles(symbol, timeframe, count = 100) {
...
function generateSimulatedCandles(symbol, timeframe, count) {
    ...
    const pairConfig = pnl_calculator_service_1.FOREX_PAIRS[symbol];
    if (!pairConfig)
        return []; // Return empty if pair not configured
    const pip = pairConfig.pip;
    const timeframeMs = {
        '1m': 60 * 1000,
        '5m': 5 * 60 * 1000,
...
function getAvailableSymbols() {
    return Object.entries(pnl_calculator_service_1.FOREX_PAIRS).map(([symbol, config]) => {
        const price = currentPrices.get(symbol);
        return {
            symbol: symbol,
            name: config.name,
