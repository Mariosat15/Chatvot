...
exports.finalizeEndedChallenges = finalizeEndedChallenges;
exports.expirePendingChallenges = expirePendingChallenges;
...
const challenge_settings_model_1 = __importDefault(require("@/database/models/trading/challenge-settings.model"));
...
async function finalizeChallenge(challengeId) {
    ...
    try {
        ...
        const challenge = await challenge_model_1.default.findById(challengeId).session(session);
        ...
        // Get participants
        const participants = await challenge_participant_model_1.default.find({
        ...
        for (const [userId, stats] of participantStats.entries()) {
            ...
            await challenge_participant_model_1.default.findByIdAndUpdate(participant._id, {
                $set: {
        ...
        const settings = await challenge_settings_model_1.default.getSingleton();
        ...
        const getRankingValue = (participant) => {
        ...
        const getTieBreakerValue = (participant, tieBreaker) => {
            switch (tieBreaker) {
                case 'trades_count':
        ...
        if (winnerId && !isTie) {
            // Winner takes all
            const winnerWallet = await credit_wallet_model_1.default.findOne({ userId: winnerId }).session(session);
            if (winnerWallet) {
                ...
                const winnerParticipant = winnerId === challenger.userId ? challenger : challenged;
            ...
            const loserParticipant = loserId === challenger.userId ? challenger : challenged;
            ...
            if (settings.tiePrizeDistribution === 'split_equally') {
                ...
                for (const participant of [challenger, challenged]) {
                    const wallet = await credit_wallet_model_1.default.findOne({
                ...
                const chalWallet = await credit_wallet_model_1.default.findOne({
        ...
        try {
            ...
            if (winnerId && !isTie) {
                ...
                const tieResolution = settings.tiePrizeDistribution === 'split_equally'
        ...
        console.log(`✅ Challenge ${challengeId} finalized: Winner: ${winnerName || 'TIE'}`);
...
async function finalizeEndedChallenges() {
    try {
        ...
        const endedChallenges = await challenge_model_1.default.find({
            status: 'active',
            endTime: { $lte: now },
        }).select('_id');
        ...
        for (const challenge of endedChallenges) {
            try {
                ...
                results.push({ id: challenge._id, error: error.message });
            }
        }
...
async function expirePendingChallenges() {
    try {
        await (0, mongoose_1.connectToDatabase)();
        const now = new Date();
        const result = await challenge_model_1.default.updateMany({
            status: 'pending',
