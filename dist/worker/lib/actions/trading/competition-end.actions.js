...
exports.finalizeCompetition = finalizeCompetition;
exports.checkAndFinalizeCompetitions = checkAndFinalizeCompetitions;
...
async function finalizeCompetition(competitionId) {
    ...
    try {
        ...
        const allParticipants = await competition_participant_model_1.default.find({
        ...
        const allPositions = await trading_position_model_1.default.find({
            competitionId: competition._id,
        }).session(session);
        ...
        const TradingOrder = (await Promise.resolve().then(() => __importStar(require('@/database/models/trading/trading-order.model')))).default;
        ...
        for (const position of openPositions) {
            try {
                ...
                await trading_position_model_1.default.findByIdAndUpdate(position._id, {
                    $set: {
                        status: 'closed',
                        exitPrice: exitPrice,
        ...
        for (const [userId, stats] of participantStats.entries()) {
            ...
            const averageLoss = stats.losingTrades > 0
                ...
                : 0;
            await competition_participant_model_1.default.findByIdAndUpdate(stats.participant._id, {
                $set: {
        ...
        const { calculateRankings, distributePrizesWithTies } = await Promise.resolve().then(() => __importStar(require('@/lib/services/competition-ranking.service')));
        ...
        const rules = competition.rules || {
            ...
            tieBreaker2: 'join_time', // Secondary tiebreaker to ensure ranking
            minimumTrades: 0,
        ...
        const prizeDistributions = distributePrizesWithTies(rankedParticipants, competition.prizeDistribution || [], prizePool, // Pass GROSS prize pool, not net
        rules, platformFeePercentage // Pass platform fee to deduct from each prize
        );
        ...
        for (const dist of prizeDistributions) {
            ...
            if (winner) {
                ...
                let winnerWallet = await credit_wallet_model_1.default.findOne({ userId: winner.userId }).session(session);
                if (!winnerWallet) {
                    const newWallets = await credit_wallet_model_1.default.create([
                        {
                ...
                await credit_wallet_model_1.default.findOneAndUpdate({ userId: winner.userId }, {
                    $inc: {
        ...
        const { PlatformFinancialsService } = await Promise.resolve().then(() => __importStar(require('@/lib/services/platform-financials.service')));
        ...
        try {
            ...
            for (const dist of prizeDistributions) {
                ...
                if (winner) {
                    if (dist.rank === 1) {
                        // Winner notification
                        notificationService.notifyCompetitionWon(winner.userId, competition.name, dist.prizeAmount).catch(e => console.error('Failed to send winner notification:', e));
                    }
                    else if (dist.rank <= 3) {
                        // Podium notification
                        notificationService.notifyPodiumFinish(winner.userId, competition.name, dist.rank, dist.prizeAmount).catch(e => console.error('Failed to send podium notification:', e));
                    }
                    // Send prize received notification to all winners
                    notificationService.notifyPrizeReceived(winner.userId, competition.name, dist.prizeAmount, dist.rank).catch(e => console.error('Failed to send prize notification:', e));
                }
            }
            ...
            for (const participant of disqualifiedParticipants) {
                notificationService.notifyDisqualified(participant.userId, competition._id.toString(), competition.name, participant.disqualificationReason || 'Did not meet competition requirements').catch(e => console.error('Failed to send disqualification notification:', e));
            }
            ...
            for (const participant of leaderboard) {
                ...
                notificationService.notifyCompetitionEnded(participant.userId, competition._id.toString(), competition.name, participant.rank || 0, pnl).catch(e => console.error('Failed to send competition end notification:', e));
            }
