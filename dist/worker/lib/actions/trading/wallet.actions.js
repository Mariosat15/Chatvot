...
exports.getWalletStats = exports.cancelDeposit = exports.initiateWithdrawal = exports.completeDeposit = exports.initiateDeposit = exports.getWalletTransactions = exports.getWalletBalance = exports.getOrCreateWallet = void 0;
...
const getOrCreateWallet = async () => {
    try {
        ...
        let wallet = await credit_wallet_model_1.default.findOne({ userId: session.user.id });
        // Create wallet if doesn't exist
        if (!wallet) {
            wallet = await credit_wallet_model_1.default.create({
...
const getWalletBalance = async () => {
    try {
        ...
        if (!session?.user)
            (0, navigation_1.redirect)('/sign-in');
        await (0, mongoose_1.connectToDatabase)();
        const wallet = await credit_wallet_model_1.default.findOne({ userId: session.user.id });
        if (!wallet) {
...
const getWalletTransactions = async (limit = 50) => {
    try {
        const session = await auth_1.auth.api.getSession({ headers: await (0, headers_1.headers)() });
        if (!session?.user)
            (0, navigation_1.redirect)('/sign-in');
        await (0, mongoose_1.connectToDatabase)();
        const transactions = await wallet_transaction_model_1.default.find({
            userId: session.user.id,
        })
            .sort({ createdAt: -1 })
            .limit(limit)
...
const initiateDeposit = async (amount, currency = 'EUR') => {
    try {
        const session = await auth_1.auth.api.getSession({ headers: await (0, headers_1.headers)() });
        if (!session?.user)
            (0, navigation_1.redirect)('/sign-in');
        ...
        const wallet = await (0, exports.getOrCreateWallet)();
        // Get centralized fee settings (for tracking platform earnings)
        const CreditConversionSettings = (await Promise.resolve().then(() => __importStar(require('@/database/models/credit-conversion-settings.model')))).default;
...
const completeDeposit = async (transactionId, paymentId, paymentMethod) => {
    try {
        await (0, mongoose_1.connectToDatabase)();
        ...
        try {
            // Get transaction
            const transaction = await wallet_transaction_model_1.default.findById(transactionId).session(mongoSession);
            if (!transaction) {
            ...
            await credit_wallet_model_1.default.findOneAndUpdate({ userId: transaction.userId }, {
                $inc: {
            ...
            try {
                const InvoiceSettings = (await Promise.resolve().then(() => __importStar(require('@/database/models/invoice-settings.model')))).default;
                ...
                if (invoiceSettings.sendInvoiceOnPurchase) {
                    const { InvoiceService } = await Promise.resolve().then(() => __importStar(require('@/lib/services/invoice.service')));
                    const { sendInvoiceEmail } = await Promise.resolve().then(() => __importStar(require('@/lib/nodemailer')));
                    const { getUserById } = await Promise.resolve().then(() => __importStar(require('@/lib/utils/user-lookup')));
            ...
            try {
                ...
                const wallet = await credit_wallet_model_1.default.findOne({ userId: transaction.userId });
                const newBalance = wallet?.creditBalance || creditsToAdd;
                await notificationService.notifyDepositCompleted(transaction.userId, eurAmount, newBalance);
...
const initiateWithdrawal = async (creditsAmount) => {
    try {
        const session = await auth_1.auth.api.getSession({ headers: await (0, headers_1.headers)() });
        if (!session?.user)
            (0, navigation_1.redirect)('/sign-in');
        ...
        const { eurToCreditsRate, minimumWithdrawal, platformWithdrawalFeePercentage, bankWithdrawalFeePercentage, bankWithdrawalFeeFixed, withdrawalFeePercentage // fallback
         } = conversionSettings;
        ...
        try {
            ...
            const withdrawalTransaction = await wallet_transaction_model_1.default.create([
                {
                    userId: session.user.id,
...
const cancelDeposit = async (transactionId, status = 'cancelled', reason) => {
    try {
        await (0, mongoose_1.connectToDatabase)();
        // Find and update the transaction
        const transaction = await wallet_transaction_model_1.default.findById(transactionId);
        if (!transaction) {
