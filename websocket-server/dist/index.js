...
exports.getOnlineParticipants = getOnlineParticipants;
...
const conversationSubscribers = new Map(); // conversationId -> connectionIds
const participantConnections = new Map(); // participantId -> connectionIds
// ==========================================
// MongoDB Connection
// ==========================================
...
function handleDisconnect(connectionId) {
    ...
    for (const [targetId, subs] of presenceSubscribers) {
        ...
        if (subs.size === 0)
            presenceSubscribers.delete(targetId);
    }
...
function addSubscriber(conversationId, connectionId) {
...
function removeSubscriber(conversationId, connectionId) {
...
function broadcastToConversation(conversationId, event, excludeParticipantId) {
    const subscribers = conversationSubscribers.get(conversationId);
    if (!subscribers)
        return;
...
function broadcastToParticipant(participantId, event) {
...
const presenceSubscribers = new Map(); // participantId -> Set of connectionIds watching them
function subscribeToPresence(watcherConnectionId, targetParticipantId) {
...
function unsubscribeFromPresence(watcherConnectionId, targetParticipantId) {
    const subs = presenceSubscribers.get(targetParticipantId);
    if (subs) {
        subs.delete(watcherConnectionId);
        if (subs.size === 0)
            presenceSubscribers.delete(targetParticipantId);
    }
}
