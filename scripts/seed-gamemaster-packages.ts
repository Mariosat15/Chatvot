...
const gameMasterPackages = [
  {
    ...
    description:
      "Perfect for getting started as a Game Master. Create 1 competition per day and earn 5% from your referrals' entry fees.",
    category: "gamemaster",
    price: 299,
    currency: "credits",
    isActive: true,
  ...
  },
  {
    ...
    description:
      "For serious Game Masters. Create up to 3 competitions per day with larger participant limits and earn 7.5% from referrals.",
    category: "gamemaster",
    price: 599,
    currency: "credits",
    isActive: true,
  ...
  },
  {
    ...
    description:
      "The ultimate Game Master experience. Unlimited daily competitions, massive participant limits, and earn 10% from all your referrals.",
    category: "gamemaster",
    price: 999,
    currency: "credits",
    isActive: true,
  ...
  try {
    ...
    for (const pkg of gameMasterPackages) {
      ...
      const existing = await collection.findOne({
        $or: [{ name: pkg.name, category: "gamemaster" }, { slug: pkg.slug }],
      });
      ...
      if (existing) {
        ...
        console.log(
          `      • ${pkg.gameMasterConfig.maxCompetitionsPerDay} competitions/day`,
        );
        console.log(
          `      • ${pkg.gameMasterConfig.maxUsersPerCompetition} max users/competition`,
        );
        console.log(
          `      • ${pkg.gameMasterConfig.referralFeePercentage}% referral fee`,
        );
        console.log(
          `      • ${pkg.gameMasterConfig.subscriptionDurationDays} days duration`,
        );
      }
    }
    ...
    console.log(
      "   • Game Master Pro - 599 credits (7.5% referral) ⭐ Featured",
    );
    console.log(
      "   • Game Master Elite - 999 credits (10% referral) ⭐ Featured",
    );
  } catch (error) {
