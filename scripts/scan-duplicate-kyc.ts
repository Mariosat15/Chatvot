...
async function scanForDuplicateKYC() {
  ...
  try {
    ...
    console.log(
      `   Auto-suspend enabled: ${fraudSettings?.duplicateKYCAutoSuspend ?? false}`,
    );
    console.log(
      `   Block deposits: ${fraudSettings?.duplicateKYCBlockDeposits ?? true}`,
    );
    console.log(
      `   Block trading: ${fraudSettings?.duplicateKYCBlockTrading ?? true}`,
    );
    console.log(
      `   Block competitions: ${fraudSettings?.duplicateKYCBlockCompetitions ?? true}`,
    );
    console.log(
      `   Block challenges: ${fraudSettings?.duplicateKYCBlockChallenges ?? true}`,
    );
    console.log(
      `   Allow withdrawals: ${fraudSettings?.duplicateKYCAllowWithdrawals ?? true}`,
    );
    console.log("");
    ...
    console.log(
      `📊 Found ${approvedSessions.length} approved KYC session(s)\n`,
    );
    ...
    for (const session of approvedSessions) {
      if (session.personData?.idNumber) {
        const key = session.personData.idNumber
          .toUpperCase()
          .replace(/\s/g, "");
    ...
    for (const group of duplicateGroups) {
      ...
      for (const userId of userIds) {
        try {
          ...
          await score.save();
          console.log(
            `   📊 Updated score for ${userId}: ${score.totalScore}% (${score.riskLevel})`,
          );
          scoresUpdated++;
        } catch {
