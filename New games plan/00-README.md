...
2. **Forgetting the admin mirror.** 75 duplicated model files. A schema field added in one app and not the other makes the field invisible to the other app and strippable on whole-document writes; a missing *enum value* fails the write outright. **This has already happened** - 10 mirrored pairs are confirmed out of sync today. **Handled in Stage 0.**
