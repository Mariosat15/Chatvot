...
| **The admin app pays no GM earnings at all** | A live defect. `lib/actions/trading/competition-end.actions.ts` held ~500 lines of GM earnings logic at lines 931-1459, **extracted to `lib/services/settlement/game-master-fees/` by X5 on 4 Sep 2026 and mirrored into `apps/admin`**; `apps/admin/lib/actions/trading/competition-end.actions.ts` still has **none of it** - only `isGmCreated` at line 709. A competition finalized through the admin app pays the Game Master nothing, and records no `retained_gm_fee` explaining why |
