...
async function getSettings(): Promise<MarketDataSettings | null> {
  try {
    ...
    const settings = await MarketDataSettingsModel.findOne({
...
function shouldRunCleanup(
  ...
  const _scheduleMinute = schedule.minute ?? 0;
...
interface CleanupResult {
  ...
  collections: Record<
    string,
    { deleted: number; before: number; after: number }
  >;
}
...
async function runCleanup(
  ...
  console.log(
    `   Delete Oldest: ${deleteOldest.enabled ? `${deleteOldest.days} days` : "OFF"}`,
  );
  console.log(
    `   Keep Recent: ${keepRecent.enabled ? `${keepRecent.days} days` : "OFF"}`,
  );
  ...
  for (const collectionName of collectionsToClean) {
    try {
      ...
      if (deleteOldest.enabled && deleteOldest.days > 0) {
        ...
        console.log(
          `   ${collectionName}: Deleted oldest ${deleteOldest.days} days = ${result.deletedCount} records`,
        );
      }
      ...
      if (keepRecent.enabled && keepRecent.days >= 0) {
        ...
        collectionDeleted += result.deletedCount;
        console.log(
          `   ${collectionName}: Keep recent ${keepRecent.days} days = ${result.deletedCount} records removed`,
        );
      }
  ...
  console.log(
    `🧹 [Auto Cleanup] Complete: Deleted ${totalDeleted} total records`,
  );
...
export async function runMarketDataMaintenance(): Promise<void> {
  try {
    ...
    if (settings.cleanup.enabled && settings.cleanup.mode === "auto") {
      const lastRun = settings.cleanup.lastRun
        ? new Date(settings.cleanup.lastRun)
        : null;

      if (shouldRunCleanup(settings.cleanup.schedule, lastRun)) {
        console.log(
          "🧹 [Market Data Maintenance] Schedule triggered, running auto cleanup...",
        );
        console.log(
          `   Schedule: ${settings.cleanup.schedule.type} at ${settings.cleanup.schedule.hour}:00 UTC`,
        );
        if (settings.cleanup.schedule.type !== "daily") {
          console.log(
            `   Days: ${settings.cleanup.schedule.weekDays.map((d) => ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][d]).join(", ")}`,
          );
        }
