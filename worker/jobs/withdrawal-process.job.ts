...
export function defineWithdrawalProcessJob(agenda: Agenda) {
  ...
  agenda.define("check-pending-withdrawals", async (_job: Job) => {
    ...
    try {
      ...
      const processingCount = await WithdrawalRequest.countDocuments({
        status: "processing",
      });
      ...
      if (pendingCount > 0 || approvedCount > 0) {
        ...
        if (settings.processingMode === "manual") {
          console.log(
            `   ℹ️  Manual mode: Admin must transfer via company bank`,
          );
        }
      } else {
  ...
  agenda.define("check-stuck-withdrawals", async (_job: Job) => {
    ...
    try {
      ...
      if (stuckWithdrawals.length > 0) {
        console.warn(
          `⚠️ Found ${stuckWithdrawals.length} withdrawal(s) in processing for >24 hours:`,
        );

        for (const withdrawal of stuckWithdrawals) {
          const hoursStuck = Math.round(
            (Date.now() - new Date(withdrawal.processedAt!).getTime()) /
              (1000 * 60 * 60),
          );
  ...
  agenda.define("check-old-pending-withdrawals", async (_job: Job) => {
    ...
    try {
      ...
      if (oldPending.length > 0) {
        console.warn(
          `⚠️ Found ${oldPending.length} withdrawal(s) pending for >48 hours:`,
        );

        for (const withdrawal of oldPending) {
          const hoursWaiting = Math.round(
            (Date.now() - new Date(withdrawal.requestedAt).getTime()) /
              (1000 * 60 * 60),
          );
          console.warn(`  - ID: ${withdrawal._id}`);
          console.warn(`    User: ${withdrawal.userEmail}`);
