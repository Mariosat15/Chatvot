...
export interface IMarketDataSettingsDocument
  extends IMarketDataSettings, Document {}
...
export const MarketDataSettings =
  (mongoose.models
    .market_data_settings as mongoose.Model<IMarketDataSettingsDocument>) ||
  mongoose.model<IMarketDataSettingsDocument>(
...
export async function getMarketDataSettings(): Promise<IMarketDataSettings> {
  const existingSettings = await MarketDataSettings.findOne().lean();

  if (existingSettings) {
    return existingSettings as IMarketDataSettings;
  }

  // Create default settings
  const newSettings = await MarketDataSettings.create(DEFAULT_SETTINGS);
  return newSettings.toObject() as IMarketDataSettings;
}
...
export async function getChartHistoryLimit(): Promise<{
  enabled: boolean;
  days: number;
}> {
  const settings = await getMarketDataSettings();
...
export async function getLazyLoadSettings(): Promise<{
  initial: number;
  batch: number;
}> {
  const settings = await getMarketDataSettings();
