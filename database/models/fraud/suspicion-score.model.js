...
const ScoreHistorySchema = new mongoose_1.Schema({
    timestamp: {
        type: Date,
        default: Date.now,
        index: true
...
const SuspicionScoreSchema = new mongoose_1.Schema({
    userId: {
        type: mongoose_1.Schema.Types.ObjectId,
        ref: 'user',
        required: true,
...
SuspicionScoreSchema.methods.addPercentage = function (method, percentage, evidence) {
    ...
    if (breakdown) {
        ...
        Object.values(this.scoreBreakdown).reduce((sum, item) => {
...
SuspicionScoreSchema.methods.addLinkedAccount = function (linkedUserId, matchType, confidence) {
    // Check if already linked
    const exists = this.linkedAccounts.some((acc) => acc.userId.toString() === linkedUserId.toString());
    if (!exists) {
...
SuspicionScoreSchema.methods.resetScore = function () {
    this.totalScore = 0;
    this.riskLevel = 'low';
    this.lastUpdated = new Date();
    // Reset all breakdown percentages
    Object.keys(this.scoreBreakdown).forEach(key => {
        const breakdown = this.scoreBreakdown[key];
        if (breakdown) {
...
SuspicionScoreSchema.statics.findByRiskLevel = function (level) {
    return this.find({ riskLevel: level }).sort({ totalScore: -1 });
};
const SuspicionScore = mongoose_1.models.SuspicionScore || (0, mongoose_1.model)('SuspicionScore', SuspicionScoreSchema);
exports.default = SuspicionScore;
//# sourceMappingURL=suspicion-score.model.js.map