...
GameMasterSubscriptionSchema.virtual("canEarnFees").get(function () {
  return (
    this.status === "active" && this.endDate > new Date() && !this.isPaused
  );
});
...
GameMasterSubscriptionSchema.statics.generateReferralCode =
  function (): string {
...
const GameMasterSubscription =
  ...
  model<IGameMasterSubscription>(
    "GameMasterSubscription",
    GameMasterSubscriptionSchema,
  );
