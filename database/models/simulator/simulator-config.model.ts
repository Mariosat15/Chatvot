...
export interface ISimulatorConfig extends Document {
  name: string;
  description?: string;
  ...
  // User Simulation
  virtualUsers: number;
  userRegistrationRate: number; // users per second
  ...
  tpSlPercentage: number; // % of trades with TP/SL
  
  // Admin Actions
  simulateAdminActions: boolean;
  paymentApprovalDelay: number; // seconds
  
  // Fraud Simulation
  simulateFraud: boolean;
  fraudPercentage: number; // % of users that are "fraudulent"
  ...
  cpuStressLevel: number; // 1-10
  memoryStressLevel: number; // 1-10
...
const SimulatorConfigSchema = new Schema<ISimulatorConfig>({
...
const SimulatorConfig = models.SimulatorConfig || model<ISimulatorConfig>('SimulatorConfig', SimulatorConfigSchema);
