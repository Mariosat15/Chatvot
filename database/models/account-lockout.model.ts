...
AccountLockoutSchema.statics.isAccountLocked = async function (
  email: string,
): Promise<{ locked: boolean; lockout?: IAccountLockout }> {
  const now = new Date();
...
AccountLockoutSchema.statics.unlockExpired =
...
const AccountLockout =
  ...
  model<IAccountLockout>("AccountLockout", AccountLockoutSchema);
