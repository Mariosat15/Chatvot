...
function validateTrade(
  ...
  if (entryLog) {
    ...
    const expectedEntry =
      position.side === "long" ? entryLog.ask : entryLog.bid;
    validation.expectedEntryPrice = expectedEntry;
    validation.entrySlippagePips = getPipDifference(
      position.entryPrice,
      expectedEntry,
      position.symbol,
    );
    validation.entryPriceCorrect =
      validation.entrySlippagePips <= PRICE_TOLERANCE_PIPS;
  ...
  if (position.status === "closed" || position.status === "liquidated") {
    ...
    if (exitLog) {
      ...
      validation.exitSlippagePips = getPipDifference(
        actualExitPrice,
        expectedExit,
        position.symbol,
      );
      validation.exitPriceCorrect =
        validation.exitSlippagePips <= PRICE_TOLERANCE_PIPS;
...
function printTradeValidation(
  ...
  console.log(
    `   Symbol: ${v.symbol} | Side: ${v.side.toUpperCase()} | Status: ${v.status}`,
  );

  // Entry details
  console.log(
    `   📥 ENTRY: ${v.entryPrice.toFixed(5)} @ ${new Date(v.openedAt).toLocaleString()}`,
  );
  if (v.entryHasLog && showDetails) {
    const entryIcon = v.entryPriceCorrect ? "✓" : "✗";
    const entryColor = v.entryPriceCorrect ? "green" : "red";
    console.log(
      `      Bid/Ask: ${v.entryBid?.toFixed(5)} / ${v.entryAsk?.toFixed(5)} | Spread: ${v.spreadAtEntry?.toFixed(2)} pips`,
    );
    log(
      `      ${entryIcon} Expected: ${v.expectedEntryPrice?.toFixed(5)} (${v.side === "long" ? "ASK" : "BID"}) | Slip: ${v.entrySlippagePips?.toFixed(2)} pips`,
      entryColor,
    );
  ...
  if (v.status === "closed" || v.status === "liquidated") {
    console.log(
      `   📤 EXIT: ${v.exitPrice?.toFixed(5)} @ ${v.closedAt ? new Date(v.closedAt).toLocaleString() : "N/A"}`,
    );
    if (v.exitHasLog && showDetails) {
      const exitIcon = v.exitPriceCorrect ? "✓" : "✗";
      const exitColor = v.exitPriceCorrect ? "green" : "red";
      console.log(
        `      Bid/Ask: ${v.exitBid?.toFixed(5)} / ${v.exitAsk?.toFixed(5)} | Spread: ${v.spreadAtExit?.toFixed(2)} pips`,
      );
      log(
        `      ${exitIcon} Expected: ${v.expectedExitPrice?.toFixed(5)} (${v.side === "long" ? "BID" : "ASK"}) | Slip: ${v.exitSlippagePips?.toFixed(2)} pips`,
        exitColor,
      );
    ...
    if (v.pnlPips !== undefined) {
      ...
      log(
        `   💰 P&L: ${v.pnlPips >= 0 ? "+" : ""}${v.pnlPips.toFixed(2)} pips`,
        pnlColor,
      );
    }
  }
...
function printSummary(
  ...
  if (summary.entriesWithLog > 0) {
    ...
    const entryAcc =
      summary.entriesWithLog > 0
        ? (summary.entriesPassed / summary.entriesWithLog) * 100
        : 0;
    const entryColor =
      entryAcc >= 95 ? "green" : entryAcc >= 80 ? "yellow" : "red";
  ...
  if (summary.exitsWithLog > 0) {
    ...
    const exitAcc =
      summary.exitsWithLog > 0
        ? (summary.exitsPassed / summary.exitsWithLog) * 100
        : 0;
    const exitColor =
      exitAcc >= 95 ? "green" : exitAcc >= 80 ? "yellow" : "red";
  ...
  // Show warnings
  const warningTrades = validations.filter(
  ...
  if (summary.failedValidations === 0) {
    ...
    log(
      `❌ ${summary.failedValidations} TRADE(S) HAVE EXECUTION ISSUES`,
      "red",
    );
  }
  console.log("═".repeat(70) + "\n");
...
async function validateAllTrades(options: {
  ...
  if (!mongoUri) {
    ...
    log(
      "Make sure you have a .env or .env.local file with MONGODB_URI",
      "yellow",
    );
    process.exit(1);
  }
  ...
  try {
    const positionsCollection =
      mongoose.connection.collection("tradingpositions");
    ...
    // Fetch positions
    let positionsCursor = positionsCollection
      .find(query)
      .sort({ openedAt: -1 });
    if (options.limit) {
    ...
    for (const position of positions) {
      ...
      if (processedCount % 100 === 0) {
        log(
          `Processing trade ${processedCount}/${positions.length}...`,
          "cyan",
        );
      }

      // Look for price logs for this trade
      const priceLogs = await priceLogsCollection
        .find({
          tradeId: position._id.toString(),
        })
        .toArray();
    ...
    // Calculate summary
    const closedTrades = validations.filter(
      (v) => v.status === "closed" || v.status === "liquidated",
    );
    ...
    const summary: ValidationSummary = {
      ...
      entriesPassed: validations.filter((v) => v.entryPriceCorrect === true)
        .length,
      entriesFailed: validations.filter((v) => v.entryPriceCorrect === false)
        .length,

      // Exit stats
      exitsWithLog: closedTrades.filter((v) => v.exitHasLog).length,
      exitsWithoutLog: closedTrades.filter((v) => !v.exitHasLog).length,
      exitsPassed: validations.filter((v) => v.exitPriceCorrect === true)
        .length,
      exitsFailed: validations.filter((v) => v.exitPriceCorrect === false)
        .length,

      // Overall
      passedValidations: validations.filter(
        (v) => v.validationStatus === "PASS",
      ).length,
      failedValidations: validations.filter(
        (v) => v.validationStatus === "FAIL",
      ).length,
      warningValidations: validations.filter(
        (v) => v.validationStatus === "WARNING",
      ).length,
      noDataValidations: validations.filter(
        (v) => v.validationStatus === "NO_DATA",
      ).length,
    ...
    // Calculate averages
    const entrySlipage = validations.filter(
      (v) => v.entrySlippagePips !== undefined,
    );
    if (entrySlipage.length > 0) {
      summary.avgEntrySlippage =
        entrySlipage.reduce((a, v) => a + (v.entrySlippagePips || 0), 0) /
        entrySlipage.length;
    }

    const exitSlippage = validations.filter(
      (v) => v.exitSlippagePips !== undefined,
    );
    if (exitSlippage.length > 0) {
      summary.avgExitSlippage =
        exitSlippage.reduce((a, v) => a + (v.exitSlippagePips || 0), 0) /
        exitSlippage.length;
    }
    ...
    if (withSpread.length > 0) {
      summary.avgSpread =
        withSpread.reduce((a, v) => a + (v.spreadAtEntry || 0), 0) /
        withSpread.length;
    }
    ...
    const displayValidations = options.failedOnly
      ? validations.filter(
          (v) =>
            v.validationStatus === "FAIL" || v.validationStatus === "WARNING",
        )
      : validations;
...
async function main(): Promise<void> {
  ...
  console.log("");
  console.log(
    "Validates ALL trades in the database for correct bid/ask execution.",
  );
