...
async function validateNewTrade(trade: TradeEvent): Promise<void> {
  ...
  if (currentPrice) {
    ...
    console.log(
      `    BID: ${currentPrice.bid.toFixed(5)} (Blue line - Sell here)`,
    );
    console.log(
      `    ASK: ${currentPrice.ask.toFixed(5)} (Red line - Buy here)`,
    );
    console.log(`    MID: ${currentPrice.mid.toFixed(5)}`);
    console.log(
      `    Spread: ${(currentPrice.spread / getPipSize(trade.symbol)).toFixed(2)} pips`,
    );

    // Expected entry price
    const expectedEntry =
      trade.side === "long" ? currentPrice.ask : currentPrice.bid;
    const pipDiff = getPipDifference(
      trade.entryPrice,
      expectedEntry,
      trade.symbol,
    );
    ...
    console.log(
      `    Expected Entry: ${expectedEntry.toFixed(5)} (${trade.side === "long" ? "ASK" : "BID"})`,
    );
    ...
    if (pipDiff < 0.5) {
      ...
      log(
        `  ⚠️ WARNING: Entry differs by ${pipDiff.toFixed(2)} pips (market may have moved)`,
        "yellow",
      );
    } else {
      log(
        `  ❌ ERROR: Large price difference (${pipDiff.toFixed(2)} pips)`,
        "red",
      );
    }
...
async function validateClosedTrade(trade: TradeEvent): Promise<void> {
  ...
  if (currentPrice && trade.exitPrice) {
    // Expected exit price
    const expectedExit =
      trade.side === "long" ? currentPrice.bid : currentPrice.ask;
    const pipDiff = getPipDifference(
      trade.exitPrice,
      expectedExit,
      trade.symbol,
    ...
    console.log(
      `    Expected Exit: ${expectedExit.toFixed(5)} (${trade.side === "long" ? "BID" : "ASK"})`,
    );
    ...
    console.log(
      `    P&L:           ${pnlPips >= 0 ? "+" : ""}${pnlPips.toFixed(2)} pips`,
    );
...
async function startMonitoring(): Promise<void> {
  ...
  const knownPositions = new Map<
    string,
    { status: string; exitPrice?: number }
  >();
  ...
  log(
    `Loaded ${existingPositions.length} existing positions from database`,
    "cyan",
  );
  console.log("");
  console.log("═".repeat(70));
...
async function main(): Promise<void> {
  ...
  console.log("");
  console.log(
    "Monitors ALL trades in real-time and validates execution prices.",
  );
  console.log(
    "No contest ID required - watches globally across all competitions/challenges.",
  );
  console.log("");
